...
- `src/cli/` — `hamuna` CLI（同步到 `~/.hamuna/bin/`）
- `src/shared/` — 前后端共享类型
- `src-tauri/` — Tauri Rust 层
- `specs/` — 设计文档（ARCHITECTURE.md / DESIGN.md / tech_docs/ / guides/）
- `bundled-agents/hamuna_helper/` — 内置 MA 小助理
- `scripts/` — 构建辅助脚本（download_nodejs、esbuild-bundle、verify-theme 等）

> **路径别名：** tsconfig 与 Vite 中 `@/*` 映射为 `src/renderer/*`，`src/shared/` 无别名，直接 import `../../shared/...` 或从 server 端 import `../shared/...`。
...
| `hamuna` CLI / Admin API | `tech_docs/cli_architecture.md` |
...
### 定时任务系统
Rust `TaskStore` 是所有新定时自动化的唯一权威，`TaskSchedulerController` 直接从 Running Task 重建 timer；Chat/CLI/IM 的 Cron 命令只是兼容 surface，禁止写 `cron_tasks.json`。旧文件只在 backend startup 迁移，Loop 不迁移。AI 统一通过 `hamuna cron ...` CLI 使用定时任务能力；历史 `im-cron` MCP 已退役。详见 ARCHITECTURE「定时任务系统」。
...
| `~/.hamuna/config.json` 裸 `tmp + rename` | 多写者 race，密钥静默丢失 | Node `withConfigLock` / Rust `with_config_lock` / renderer `withConfigLock` | — (路径作用域，banning all `fs::rename` 噪音过大) |
...
| Rust 日志用 `log::info!` / `warn!` / `error!` / `debug!` / `trace!` | 不进统一日志（`~/.hamuna/logs/unified-{date}.log`），renderer 日志面板和"读 unified log"的红线全失效 | `ulog_info!` / `ulog_warn!` / `ulog_error!` / `ulog_debug!` | clippy |
| 前端 `@tauri-apps/plugin-fs` 读写工作区 | Tauri fs scope 仅覆盖 `~/.hamuna/**`，工作区路径会失败 | `invoke('cmd_read_workspace_file')` / `cmd_write_workspace_file` | — (路径作用域，import 维度判不准) |
...
# 运行单个测试文件
npx vitest run --project unit -- src/shared/workspacePath.test.ts
npx vitest run --project dom -- src/renderer/components/chat-input/SimpleChatInput.test.tsx
# 按名称过滤
...
# Windows 构建缓存/CSP 问题
powershell -ExecutionPolicy Bypass -File .\rebuild_clean.ps1
```
...
日志来自三层（React / Node.js Sidecar / Rust），汇入统一日志 `~/.hamuna/logs/unified-{YYYY-MM-DD}.log`。**用户报告问题时 MUST 主动读日志，不等用户粘贴。**
...
- **Rust 层**：额外查 `~/Library/Logs/com.hamuna.app/HamunaAgent.log`
...
应用内置 AI 助手运行在 `~/.hamuna/`，通过 `/hamuna-cli` system skill 调用 `hamuna` CLI **直接执行**用户管理操作（不是输出操作步骤）。该 skill 是全局的——所有 session（Chat / IM Bot / Cron / Helper）都能用它驱动 HamunaAgent 的产品能力。
...
| 改动类型 | 先看 |
|---------|------|
| UI 组件/页面 | `src/renderer/` → `specs/DESIGN.md` → `tech_docs/react_stability_rules.md` |
| Sidecar 生命周期 | `ARCHITECTURE.md` §1 → `tech_docs/sidecar_cold_start.md` → `tech_docs/session_architecture.md` |
| Agent Runtime | `ARCHITECTURE.md` §9 → `tech_docs/multi_agent_runtime.md` |
| MCP / Tools | `ARCHITECTURE.md` §2-3 → `tech_docs/pit_of_success.md` builtin-MCP 节 → `tech_docs/sdk_custom_tools_guide.md` |
| IM Bot / Channel | `ARCHITECTURE.md` §6 → `tech_docs/im_integration_architecture.md` |
| Task Center / Cron | `ARCHITECTURE.md` §5 → `tech_docs/task_center.md` → `tech_docs/task_provider_routing.md` |
| Plugin Bridge | `ARCHITECTURE.md` §7 → `tech_docs/plugin_bridge_architecture.md` |
| 工作区文件 IO | `ARCHITECTURE.md` §17 → `tech_docs/pit_of_success.md` workspace_files 节 |
| 全文搜索 | `ARCHITECTURE.md` §14 → `tech_docs/search_architecture.md` |
| Theme / 视觉 | `specs/DESIGN.md` → `tech_docs/theme_system.md` |
| Rust 命令/进程 | `src-tauri/` → clippy toc → `pit_of_success.md` local_http / process_cmd 节 |
| Config 持久化 | `ARCHITECTURE.md` 「Config 持久化」→ `pit_of_success.md` withConfigLock 节 |
| 添加新语言 | `tech_docs/i18n_architecture.md` |
