...
Write-Host "  HamunaAgent 版本回滚" -ForegroundColor Yellow
Write-Host "=========================================" -ForegroundColor Cyan
Write-Host ""
...
$WorkDir = Join-Path ([System.IO.Path]::GetTempPath()) ("hamuna-rollback-" + [System.Guid]::NewGuid().ToString("N"))
...
if ($RollbackMac -and $TargetVersion -ne $CurrentMacVersion) {
    ...
    # darwin-aarch64.json
    $armSigPath = Join-Path $SigDir "HamunaAgent_${TargetVersion}_aarch64.app.tar.gz.sig"
    $armTarName = "HamunaAgent_${TargetVersion}_aarch64.app.tar.gz"
    if (-not (Test-Path $armSigPath)) {
        $armSigPath = Join-Path $SigDir "HamunaAgent_aarch64.app.tar.gz.sig"
        $armTarName = "HamunaAgent_aarch64.app.tar.gz"
    }

    if (Test-Path $armSigPath) {
        $armSig = (Get-Content $armSigPath -Raw).Trim()
        $armManifest = @{ version = $TargetVersion; notes = "HamunaAgent v$TargetVersion"; pub_date = $PubDate; signature = $armSig; url = "$DownloadBaseUrl/releases/v$TargetVersion/$armTarName" }
    ...
    # darwin-x86_64.json
    $intelSigPath = Join-Path $SigDir "HamunaAgent_${TargetVersion}_x86_64.app.tar.gz.sig"
    $intelTarName = "HamunaAgent_${TargetVersion}_x86_64.app.tar.gz"
    if (-not (Test-Path $intelSigPath)) {
        $intelSigPath = Join-Path $SigDir "HamunaAgent_x86_64.app.tar.gz.sig"
        $intelTarName = "HamunaAgent_x86_64.app.tar.gz"
    }
    if (-not (Test-Path $intelSigPath)) {
        $intelSigPath = Join-Path $SigDir "HamunaAgent_${TargetVersion}_x64.app.tar.gz.sig"
        $intelTarName = "HamunaAgent_${TargetVersion}_x64.app.tar.gz"
    }

    if (Test-Path $intelSigPath) {
        $intelSig = (Get-Content $intelSigPath -Raw).Trim()
        $intelManifest = @{ version = $TargetVersion; notes = "HamunaAgent v$TargetVersion"; pub_date = $PubDate; signature = $intelSig; url = "$DownloadBaseUrl/releases/v$TargetVersion/$intelTarName" }
    ...
    if ($armDmg -or $intelDmg) {
        ...
        $latestManifest = @{ version = $TargetVersion; pub_date = $PubDate; release_notes = "HamunaAgent v$TargetVersion"; downloads = $downloads }
...
if ($RollbackWin -and $TargetVersion -ne $CurrentWinVersion) {
    ...
    # windows-x86_64.json
    $winSigPath = Join-Path $SigDir "HamunaAgent_${TargetVersion}_x86_64.nsis.zip.sig"
    $winZipName = "HamunaAgent_${TargetVersion}_x86_64.nsis.zip"
    ...
    if (Test-Path $winSigPath) {
        ...
        $winManifest = @{ version = $TargetVersion; notes = "HamunaAgent v$TargetVersion"; pub_date = $PubDate; signature = $winSig; url = "$DownloadBaseUrl/releases/v$TargetVersion/$winZipName" }
    ...
    if ($winExe) {
        $latestWin = @{
            version = $TargetVersion; pub_date = $PubDate; release_notes = "HamunaAgent v$TargetVersion"
