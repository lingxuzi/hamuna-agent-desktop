...
HamunaAgent 在 Linux 上通过 AppImage（便携）+ deb（apt 源）分发。
...
**架构说明**：HamunaAgent Linux 默认构建 `x86_64-unknown-linux-gnu`，对应 glibc 发行版。arm64（`aarch64-unknown-linux-gnu`）需在对应 Linux arm64 主机上构建 —— Tauri 不支持从 macOS 交叉编译 Linux。
...
产物路径：
- AppImage：`src-tauri/target/<target>/release/bundle/appimage/HamunaAgent_<ver>_<arch>.AppImage`
- deb：`src-tauri/target/<target>/release/bundle/deb/HamunaAgent_<ver>_<arch>.deb`

### AppImage 用法

```bash
chmod +x HamunaAgent_0.2.0_amd64.AppImage
./HamunaAgent_0.2.0_amd64.AppImage
```
...
```bash
sudo dpkg -i HamunaAgent_0.2.0_amd64.deb
sudo apt-get install -f  # 补齐缺失的系统依赖（正常情况下 deb 元数据已声明）
```

安装后启动：
```bash
hamuna  # deb 把可执行文件链接到 /usr/bin/
# 或从桌面菜单启动（Applications > Development > HamunaAgent）
```
...
| 组件 | 路径（app 内） |
|------|--------------|
| Sidecar / Bridge / CLI | `resources/server-dist.js` / `resources/plugin-bridge-dist.mjs` / `resources/cli/hamuna.js` |
...
```bash
./HamunaAgent_0.2.0_amd64.AppImage --appimage-extract-and-run
```

## 发布 / CI

[`publish_linux.sh`]（待建 —— 当前手动上传）：产物上传到 R2 的 `https://releases.hamuna.io/linux/` 路径；Tauri updater manifest 自动包含 Linux 条目。
