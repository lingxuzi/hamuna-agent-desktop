...
本文档描述 HamunaAgent 的构建流程、发布流程以及分发渠道的完整信息。

---

## 概览

HamunaAgent 支持 **macOS** 和 **Windows** 平台：
...
所有发布文件存储在 **Cloudflare R2**，通过自定义域名 `download.hamuna.io` 提供访问。
...
└── releases/
    └── v{VERSION}/
        ├── HamunaAgent_{VERSION}_aarch64.dmg         # ARM DMG
        ├── HamunaAgent_{VERSION}_x64.dmg             # Intel DMG
        ├── HamunaAgent_{VERSION}_aarch64.app.tar.gz  # ARM 更新包
        ├── HamunaAgent_{VERSION}_aarch64.app.tar.gz.sig  # ARM 签名
        ├── HamunaAgent_{VERSION}_x64.app.tar.gz      # Intel 更新包
        └── HamunaAgent_{VERSION}_x64.app.tar.gz.sig  # Intel 签名
```
...
```
  ...
  ├─────────────────────────────────────────────────────────┤
  │  Apple Silicon (ARM64)                                  │
  │    ✓ DMG:    HamunaAgent_0.1.0_aarch64.dmg              │
  │    ✓ tar.gz: HamunaAgent.app.tar.gz                      │
  │    ✓ 签名:   HamunaAgent.app.tar.gz.sig                  │
  │                                                         │
  │  Intel (x86_64)                                         │
  │    ✓ DMG:    HamunaAgent_0.1.0_x64.dmg                   │
  │    ✓ tar.gz: HamunaAgent.app.tar.gz                      │
  │    ✓ 签名:   HamunaAgent.app.tar.gz.sig                  │
  └─────────────────────────────────────────────────────────┘
```
...
**用途**：单独打包并上传 HamunaAgent 托管的 Codex Runtime 到 R2。它是开发 / 发版准备阶段的资源发布入口，不属于桌面 App 的 `publish_release.sh` / `publish_windows.ps1` 流程。
...
```json
{
  ...
  "release_notes": "HamunaAgent v0.1.0",
  "downloads": {
    "mac_arm64": {
      "name": "Apple Silicon",
      "url": "https://download.hamuna.io/releases/v0.1.0/HamunaAgent_0.1.0_aarch64.dmg"
    },
    "mac_intel": {
      "name": "Intel Mac",
      "url": "https://download.hamuna.io/releases/v0.1.0/HamunaAgent_0.1.0_x64.dmg"
    }
  }
}
```
...
{
  "version": "0.1.0",
  "notes": "HamunaAgent v0.1.0",
  "pub_date": "2026-01-24T10:00:00Z",
  "platforms": {
    "darwin-aarch64": {
      "signature": "dW50cnVzdGVkIGNvbW1lbnQ6...",
      "url": "https://download.hamuna.io/releases/v0.1.0/HamunaAgent.app.tar.gz"
    }
  }
}
```
...
```json
{
  "plugins": {
    "updater": {
      ...
      "endpoints": [
        "https://download.hamuna.io/update/{{target}}.json"
      ]
    }
  }
}
...
```bash
# 检查官网 API
curl -s https://download.hamuna.io/update/latest.json | jq .

# 检查自动更新清单
curl -s https://download.hamuna.io/update/darwin-aarch64.json | jq .
```
...
**更新检查失败**
- 检查 CSP 配置是否允许 `download.hamuna.io`
