# HamunaAgent Windows 构建与测试指南

本文档描述 HamunaAgent Windows 版本的构建流程、发布流程以及测试注意事项。

---

## 概览

HamunaAgent Windows 版本支持 **x86_64 (64位)** 架构，提供两种分发格式：

| 格式 | 文件 | 用途 |
|------|------|------|
| **NSIS 安装包** | `HamunaAgent_x.x.x_x64-setup.exe` | 标准安装，有向导界面 |
| **便携版** | `HamunaAgent_x.x.x_x86_64-portable.zip` | 解压即用，无需安装 |
...
└── releases/
    └── v{VERSION}/
        ├── HamunaAgent_{VERSION}_x64-setup.exe       # NSIS 安装包
        ├── HamunaAgent_{VERSION}_x86_64-portable.zip # 便携版
        ├── HamunaAgent_{VERSION}_x86_64.nsis.zip     # 自动更新包
        └── HamunaAgent_{VERSION}_x86_64.nsis.zip.sig # 更新签名
```
...
```
src-tauri/target/x86_64-pc-windows-msvc/debug/hamuna.exe
```
...
├── HamunaAgent_x.x.x_x64-setup.exe       # NSIS 安装包
├── HamunaAgent_x.x.x_x86_64-portable.zip # 便携版
├── HamunaAgent_x.x.x_x64-setup.nsis.zip  # 自动更新包
└── HamunaAgent_x.x.x_x64-setup.nsis.zip.sig  # 更新签名
```
...
- [ ] 解压后直接运行 `hamuna.exe`（Cargo 包名是小写，主二进制即 `hamuna.exe`）
...
- [ ] 配置保存在 `%APPDATA%\HamunaAgent\` 目录

**验证数据目录**：
```powershell
ls $env:APPDATA\HamunaAgent
```
...
curl -s https://download.hamuna.io/update/windows-x86_64.json

# 网站下载 API
curl -s https://download.hamuna.io/update/latest_win.json
```
...
**注意事项**：
- 覆盖安装后，用户数据（Projects、Providers）应该保留在 `%APPDATA%\HamunaAgent`
- 如遇到 WebView 缓存问题，可清理 `%LOCALAPPDATA%\HamunaAgent\EBWebView`
