...
- 普通跨 session send/watch 事件仍走 `session_architecture.md` 的
  `<hamuna-session-event>` 协议。协议整体位于隐藏 envelope 内；renderer 只对
...
<task-meta>
  Task id: {taskId}
  Source session id: {fromSessionId} (use `hamuna session send {fromSessionId} -p "..."` to follow up)
  Current time: {now}
</task-meta>
<task-result>
...
| `hamuna-space-issue` | Space issue | Space IssueDelivery |
...
| Space IssueDelivery（0.3.2 v2） | `src-tauri/src/space_cloud.rs::build_space_issue_delivery_message_for_locale` | `<system-reminder><hamuna-space-issue><registered-agent-context>…</registered-agent-context><registered-agent-instruction>…</registered-agent-instruction><operating-guidance>…</operating-guidance><deliveries>…</deliveries></hamuna-space-issue></system-reminder>` + 本地化可见提示 |
...
Space v2 的内部 trust boundary 不改变通用 reminder wire protocol：Cloud 提供权威 Registered Agent instruction/revision、transport 索引与轻量因果导航；Desktop 描述当前 CLI/workspace/Task 执行方法并组装 Prompt；Issue 用户文本必须有界 XML escape，不能伪造结构。Renderer 仍只消费外层 `hamuna-space-issue` badge 与 reminder 后的 visible tail。已发布旧 Desktop 的 v1 结构继续由 Cloud 版本化 projection 支持，但不进入 0.3.2 builder。
