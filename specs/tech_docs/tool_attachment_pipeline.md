...
| Positive allow-list | `tool-attachments.ts::isAllowedExternalAttachmentPrefix` | 拒绝引用 `~/Documents/secrets.docx` 等"既不在黑名单也不该读"的路径；只允许 `~/.codex/` `~/.hamuna/` `~/Documents/` `~/Desktop/` `~/Downloads/` 及子目录 |
...
| `src/renderer/utils/hamunaProtocol.ts` | Tauri custom-protocol URL 形态：macOS/Linux = `hamuna://...`；Windows/WebView2 = `http://hamuna.localhost/...` |
...
- **P5（部分完成，PRD 0.2.30）— builtin 媒体接入**：内置 `edge-tts`（音频）/ `gemini-image`（图片）
  ...
  - **P5 完成（#293 / 0.2.33）— SDK 通用 `content[]` image ContentBlock 归一化**：任意 builtin 工具
    ...
    **存储位置**：base64 图片写进统一的工作区目录 `<workspace>/hamuna_files/<工具名>/`（不同工具各自
    一个文件夹，沿用 edge-tts / gemini-image 的 `hamuna_files/` 约定，首写自动 gitignore），作为
    `sourcePath`（工具卡「在 Finder 显示 / 打开」指向它）；同时保留一份 trusted-root 副本
    （`~/.hamuna/generated/tool-attachments/<sid>/<tid>/`）作 `savedPath` 供重启后稳定渲染——
    即「工作区原件 + trusted 服务副本」双写，与 gemini-image 同构。无工作区（IM/cron）回退
    `~/.hamuna/generated/<工具名>/`。`externalPath` / `url` 源保留各自 allow-list / SSRF 守卫的原位置。
...
- **GC / size limits**：session 软删除时清理 `~/.hamuna/generated/tool-attachments/<sid>/`；
