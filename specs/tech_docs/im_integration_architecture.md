...
Goal Mode 是 current-session 状态，因此 IM / Agent Channel 里由 AI 调 `hamuna goal create --objective-file ...` 创建的 Goal，仍属于当前 peer session：
...
pub struct HealthManager {
 ...
 persist_path: PathBuf, // ~/.hamuna/im_bots/{bot_id}/state.json
}
...
- 健康状态：`~/.hamuna/im_bots/{bot_id}/state.json`
- 消息缓冲：`~/.hamuna/im_bots/{bot_id}/buffer.json`
- 去重缓存：`~/.hamuna/im_bots/{bot_id}/dedup.json`（仅飞书）
...
interface ImBotConfig {
 ...
 // OpenClaw 社区插件专属
 openclawPluginId?: string; // 安装 ID / pluginId，用于定位 ~/.hamuna/openclaw-plugins/<pluginId>
...
**存储位置**：`~/.hamuna/config.json` → `imBotConfigs: ImBotConfig[]`
...
| 文档 | 说明 |
|------|------|
| [架构总览](../ARCHITECTURE.md) | HamunaAgent 整体架构 |
...
Mino 默认工作区的"文件内容模板"和 HamunaAgent 的"产品级 Agent 默认策略"是两层：
