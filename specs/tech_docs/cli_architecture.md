...
HamunaAgent 内置了一个自配置 CLI 工具（`hamuna`），让 AI 和用户都能通过命令行管理应用配置。CLI 是一个轻量 TypeScript 脚本，解析命令行参数后转发为 HTTP 请求到 Sidecar 的 Admin API，所有业务逻辑都在 Sidecar 侧。
...
Goal Mode 是 CLI 的特殊 current-session 控制能力：`hamuna goal create` 与 UI `/goal` 创建同一个 session-owned Goal，`hamuna goal update` 是模型把 Goal 标记为 complete / blocked 的受限出口。它不是普通 Cron command 的别名。
...
│                                                                     │
│ 用户: "帮我配个 MCP"                                                 │
│   → AI Bash 工具 → `hamuna mcp add --id xxx ...`                  │
│   → PATH 查找 ~/.hamuna/bin/hamuna                              │
│   → Node 执行 hamuna.ts                                            │
│   → fetch(127.0.0.1:${HAMUNA_PORT}/api/admin/mcp/add)             │
...
│ 场景 2：用户终端调用（次要用途）                                       │
│                                                                     │
│ 终端: `HamunaAgent mcp list` 或 `hamuna mcp list`                    │
│   → cli.rs:is_cli_mode() 检测 CLI 参数                               │
│   → 不启动 GUI / 不杀 sidecar / 不触发单实例焦点                      │
│   → 找到 bundled Node +  ~/.hamuna/bin/hamuna                      │
│   → 读 ~/.hamuna/sidecar.port 找到 Global Sidecar 端口             │
│   → 注入 HAMUNA_PORT → 转发到 Admin API                            │
└─────────────────────────────────────────────────────────────────────┘
```
...
| **CLI 脚本** | `src/cli/hamuna.ts` | 参数解析、命令路由、HTTP 调用、输出格式化（含 `recoveryHint` 渲染） |
...
| **PATH 注入** | `src/server/agent-session.ts` (`buildClaudeSessionEnv`) | 将 `~/.hamuna/bin` / `~/.hamuna/npm-global/bin` 加入 SDK 子进程 PATH |
...
─────────────────                          ─────────────────
src/cli/                                   ~/.hamuna/
├── hamuna.ts   ──── cmd_sync_cli ────►  ├── bin/
└── hamuna.cmd                           │   ├── hamuna       (chmod 755, 去掉 .ts 后缀)
                                           │   └── hamuna.cmd   (Windows)
...
```bash
#!/usr/bin/env bun    ← hamuna.ts 第一行 shebang
```

CLI 脚本有两种执行方式：
1. **AI Bash 工具调用**：SDK 子进程的 PATH 包含 `~/.hamuna/bin`，直接 `hamuna mcp list`，shebang 找到 PATH 中的 bun 执行
2. **Rust CLI 入口调用**：`cli.rs` 显式调用 `bun ~/.hamuna/bin/hamuna <args>`

### 端口发现

```
优先级：--port 标志 > HAMUNA_PORT 环境变量
```

- **AI 调用场景**：`buildClaudeSessionEnv()` 注入 `HAMUNA_PORT` 环境变量（当前 Session Sidecar 端口）
- **终端调用场景**：`cli.rs` 从 `~/.hamuna/sidecar.port` 文件读取 Global Sidecar 端口，注入 `HAMUNA_PORT`

### 命令体系

```
hamuna <group> <action> [args] [flags]
...
// 失败
{
  ...
  "recoveryHint": {                                 // 结构化恢复建议
    "recoveryCommand": "hamuna runtime list",     //   下一步可运行的命令
...
**`recoveryHint` 设计**：CLI 在人类可读模式下渲染为 `→ Run: <command>   <message>` 追加在错误行下方，JSON 模式保留完整字段。目的是让 AI 调用者在验证失败时能一步恢复 —— "想知道哪些 runtime 可用？按提示跑 `hamuna runtime list`" —— 不需要读源码或反复试错。
...
```bash
hamuna runtime list                             # 看哪些 runtime 装了、未装的给出安装提示
hamuna runtime describe <runtime>               # 看某 runtime 的 model + permissionMode 枚举
hamuna agent show <agent-id>                    # 看某 Agent 的 effective 默认（按 runtime 正确解析）
hamuna agent list --active|--archived           # 按工作区归档状态筛选 Agent
```
...
`hamuna goal --help` 是 Goal Mode 的内置 skill 文档。系统提示词只告诉模型在明确 User 要求“Goal Mode / Goal Loop / 目标模式 / 设立目标 / 持续执行直到完成”时先运行 help，再按 help 使用子命令；不要把 help 全量塞进主 system prompt。

命令语义：

| 命令 | 何时调用 | 效果 |
|------|----------|------|
| `hamuna goal get` / `list` | 查看当前 session 是否已有 Goal，或状态更新前确认 | 返回当前 session Goal，或 `goal: null` |
| `hamuna goal create --objective-file <path> [--deadline <ISO-with-offset>] [--max-executions <n>] [--ai-can-exit <bool>]` | 仅当 User 明确要求进入 Goal/目标模式 | 从本地普通文本文件读取 objective，创建 current-session Goal，启动自动续跑，广播 `goal:changed`；可为新 Goal 设置结束条件 |
| `hamuna goal update --status complete` | 当前证据证明 objective 全部完成且无剩余工作 | 停止自动续跑，标记 complete，终态通知 |
| `hamuna goal update --status blocked` | 同一 blocker 连续至少 3 个 Goal turn 仍无法推进 | 停止自动续跑，标记 blocked，终态通知 |
...
- 普通 Cron surface 不创建或管理 Goal。`hamuna cron add --schedule '{"kind":"loop"}'` 会被拒绝；Goal 创建统一走 `hamuna goal create --objective-file ...`。objective/reason 是 file-only 输入，不接受 inline 或 positional 文本。
...
`hamuna cron` 保留既有用户命令名和 JSON shape，但不再创建 `CronTask`。所有 add/list/update/start/stop/remove/run-now 都由 Rust compatibility facade 直接读写 `TaskStore`，时间触发由 `TaskSchedulerController` 管理；`cron_tasks.json` 只作为启动迁移的只读历史格式。
...
```bash
hamuna runtime diagnose codex [--workspace=<path>] [--json]
hamuna diagnose runtime codex [--workspace=<path>] [--json]    # 别名糖
```
...
- `--json` 输出可直接贴 issue（issue #194 是这个能力的原始来源——用户终端能调 `@oai/artifact-tool`、HamunaAgent Codex Runtime 里调不到，诊断面板 + CLI 双入口让差异可见）
...
3. 文件名需从 `hamuna.ts` → `hamuna`（去掉 .ts 后缀，shebang 才能直接跑）

### 方案

```
app 启动 → ConfigProvider → invoke('cmd_sync_cli')
  → 读 ~/.hamuna/.cli-version
  → 内容 == CLI_VERSION 常量 → 跳过（return Ok(false)）
  → 不等 → 拷贝 Resources/cli/hamuna.ts → ~/.hamuna/bin/hamuna
        → chmod 755（Unix）
        → 拷贝 hamuna.cmd（Windows）
        → 写 .cli-version = CLI_VERSION
```

**开发约束**：修改 `src/cli/hamuna.ts` 或 `src/cli/hamuna.cmd` 后，MUST bump `CLI_VERSION`（`src-tauri/src/commands.rs`），否则用户端 CLI 不会更新。

### 与 ADMIN_AGENT_VERSION 的关系

| 门控 | 控制内容 | 文件 | 版本文件 |
|------|---------|------|---------|
| `CLI_VERSION` | CLI 脚本 (`hamuna.ts`, `hamuna.cmd`) | `~/.hamuna/.cli-version` | `src-tauri/src/commands.rs` |
| `ADMIN_AGENT_VERSION` | 小助理 CLAUDE.md + Skills | `~/.hamuna/.admin-agent-version` | `src-tauri/src/commands.rs` |
| `SYSTEM_SKILLS_VERSION` | `src-tauri/src/commands.rs::SYSTEM_SKILLS` 列出的版本化系统级 Skills；其中 Required 子集由 `src/shared/systemSkills.ts` 统一定义 | `~/.hamuna/.system-skills-version` | `src-tauri/src/commands.rs` |

三个版本门控**独立运作**，修改各自内容只需 bump 对应版本即可。

`SYSTEM_SKILLS` 是版本化安装集合，`REQUIRED_SYSTEM_SKILLS` 是其中始终可用的产品契约子集，二者不能混为一谈。canonical 名单在 `src/shared/systemSkills.ts`，Rust workspace/slash 路径在 `src-tauri/src/workspace_files/skills_config.rs` 维护必要镜像，并由 cross-language test 锁定；改名单必须同步这两处，禁止 UI、CLI 或其它模块再复制第三份。当前 Required 包括 `hamuna-memory-update`、`hamuna-memory-gardener`、`hamuna-memory-molt`、`hamuna-cli`、`hamuna-docs`。读取旧 `skills-config.json` 和每次写回都会移除这些名称的 stale disabled 项；Skills API 以 `required:true, enabled:true` 投影，disable 请求返回 409。其它版本化或用户 Skill 仍可正常 enable/disable。
...
```bash
# macOS — 直接调用 app 二进制
/Applications/HamunaAgent.app/Contents/MacOS/HamunaAgent mcp list

# 或者创建 alias
alias hamuna='/Applications/HamunaAgent.app/Contents/MacOS/HamunaAgent'
hamuna status
```
...
**开发约束**：在 `src/cli/hamuna.ts` 中新增 `hamuna <group>` 顶层命令时，MUST 把 `<group>` 加入 `CLI_COMMANDS`，否则 `HamunaAgent <group> ...` 会进入 GUI 模式（无反馈）。
...
fn discover_sidecar_port() -> Option<String> {
    // 读取 ~/.hamuna/sidecar.port（Global Sidecar 启动时写入）
    // 校验是合法端口号（防止陈旧/损坏文件）
}
```

**前提**：HamunaAgent GUI 必须已经运行（Global Sidecar 存活），CLI 才能连接。如果 app 未运行，CLI 脚本会报 `ECONNREFUSED` 并提示用户。
...
- `hamuna <exact leaf> --help` 是 Agent 的工具说明。每个 Space leaf 独立描述 WHEN TO CALL、EFFECT、REQUIRED CONTEXT、OPTIONS、ACTOR AND PERMISSIONS、FILE SAFETY、OUTPUT、EXAMPLES、RECOVERY，不能回落到泛化 group help。
- `hamuna space issue --help` 是 Issue 动作面的统一 discovery 入口；具体参数继续以下一级 leaf help 为权威。0.3.2 不再暴露 `space issue delivery ignore`：不行动是合法模型决策，不需要修改 Delivery；transport ACK 由 connector 自动维护。
- Goal discovery 走 `space goal list --space <slug> [--include-archived]`，只把 active `data.items[].id` 用作 create/list/update 的 `--goal`。`hamuna goal` 是本地 Session Goal Mode，`hamuna space goal` 是 Cloud 组织 Goal，help 必须保持命名空间消歧。
...
| 子命令 | 事件 | 关键不变量 |
|--------|------|------------|
| `hamuna session send` | `send.request` / 可选 `send.result` | 目标 session 收到 `<hamuna-session-event type="send.request">`；若需要回执，目标 turn terminal 后自动把 `send.result` 推回源 session |
| `hamuna session watch` | `watch.already_idle` / `watch.completed` / `watch.error` | Rust Management API 先确认目标 live state；目标忙时在目标 Sidecar 注册 pending watch，完成事件确认送达后才 ack 清理；目标已 idle 时调用方立即收到最近结果 |

事件 prompt 统一由 `src/server/inbox/session-event.ts` 渲染，标签形态为
`<hamuna-session-event ...>`，payload 内部会 neutralize 协议结构标签。新增
...
- 失败响应原样透传 `recoveryHint`（例如 Management API 不可达时 Admin handler 注入 `→ Run: hamuna status` 指引）

### 官方 CLI 工具与用户 CLI 工具

HamunaAgent CLI 同时承载两类“工具”：

- 官方 CLI 工具：产品内置、稳定可用，由 HamunaAgent 自己实现和审核，例如 `hamuna vision analyze`。它们可以出现在设置页「工具箱」和对话工具菜单中，但不属于 MCP，也不受用户 CLI 工具注册表实验开关影响。
- 用户注册 CLI 工具：用户通过 `hamuna tool add` 注册的自定义 Agent-CLI 工具，受实验室开关控制，并通过 registry 注入新 session prompt。
...
用户注册 CLI 工具注册表（`hamuna tool ...`、设置页「工具箱 / CLI 工具」、`tool-creator` skill、用户工具 prompt 注入）受 `config.cliToolRegistryEnabled` 控制。该开关位于「设置 → 关于&反馈 → 实验室」，默认关闭，且不能通过通用 `hamuna config set cliToolRegistryEnabled ...` 修改，避免 AI 自行绕过人类可见的实验开关。

关闭时：
- Settings 不渲染工具箱里的 CLI 工具模块。
- `/api/admin/tool/*` 全部返回门控错误；`hamuna tool --help` 只显示开启指引。
- `buildSystemPromptAppend(..., { userCliToolsEnabled: false })` 不读取 `~/.hamuna/tools/registry.json`，因此新会话不会自动发现用户注册工具。
...
不受影响：
- 稳定内置 `hamuna` CLI 能力（cron / task / thought / im / widget / runtime 等）仍然注入并可用。
- 已经存在于 `~/.hamuna/bin` 的工具 shim 不会被删除；门控的是 HamunaAgent 的注册、管理、自动发现和 `tool-creator` 注入，不是用户磁盘上可执行文件的生命周期。

由于系统提示词和 SDK skill 集合只在 session 启动 / pre-warm 时固化，开关变化对已有会话的提示内容不会 retroactive 改写；但实际 `hamuna tool ...` 调用会立即被 Admin API 门控。
...
PATH 优先级（agent-session.ts::buildClaudeSessionEnv）：
  ...
  ~/.hamuna/npm-global/bin  → HamunaAgent-localized npm installs / legacy AI 自装 CLI 落点
  ~/.hamuna/bin             → HamunaAgent 自己的 CLI（hamuna）+ 升级残留
  系统 PATH                    → 用户其他工具
```

`~/.hamuna/bin` 当前只放 `hamuna` CLI。早期版本曾在这里写 `agent-browser` 等 wrapper —— 升级用户磁盘上可能仍残留这些文件，但被 `~/.hamuna/npm-global/bin` 在 PATH 上抢先匹配，自然失效，无需主动清理。

`~/.hamuna/npm-global/` 是 HamunaAgent 建议的 AI 自装 CLI 落点。`buildClaudeSessionEnv()` 只注入 `HAMUNA_NPM_GLOBAL_PREFIX` 和 PATH，不再给整个 SDK shell env 设置 `npm_config_prefix` / `NPM_CONFIG_PREFIX` / `PREFIX`，否则 nvm 会在每次 zsh/bash 初始化时吐兼容性警告。需要固定安装落点的 skill 用命令级 env：`npm_config_prefix="$HAMUNA_NPM_GLOBAL_PREFIX" npm install -g <pkg>`。
...
| **权限控制** | 脚本权限 755（owner rwx），`~/.hamuna/` 目录权限遵循用户 HOME 策略 |
...
| 问题 | 排查方法 |
|------|---------|
| `ECONNREFUSED` | HamunaAgent GUI 未运行，先启动应用 |
| `HAMUNA_PORT not set` | 在 AI Bash 环境外直接运行了脚本（缺少环境变量注入） |
| CLI 脚本不存在 | 应用未初始化过（`cmd_sync_cli` 未执行），启动一次 GUI |
| CLI 版本过旧 | `~/.hamuna/.cli-version` 与 `commands.rs` 的 `CLI_VERSION` 不匹配，重启应用触发同步 |
| 终端 `hamuna` 找不到 | 场景 2 需要用完整路径或创建 alias，`~/.hamuna/bin` 默认不在 shell PATH |
| `Management API not available` | Node.js Sidecar 起来了但 Rust Management API 没起 — CLI 会附带 `→ Run: hamuna status` 指引 |
| `HamunaAgent task list` 进了 GUI | 新命令组忘了加进 `CLI_COMMANDS`（`src-tauri/src/cli.rs`） |
