# HamunaAgent Space Issue Delivery Protocol
...
4. 模型读取 Issue 当前状态，通过 hamuna CLI 决定是否评论、更新、claim、继续执行、完成或不动作。
...
| 模型可用动作和具体参数 | hamuna space issue --help 与各 leaf command help |
...
- ../HamunaAgent_space/src/services/issueDelivery.ts::CLOUD_INSTRUCTIONS
- ../HamunaAgent_space/src/services/issueDelivery.ts::buildCloudIssueInstruction
- ../HamunaAgent_space/src/services/issueDelivery.ts::buildIssueDeliveryTrigger
...
~~~text
[Standing goal truncated for protocol v1 compatibility. Upgrade HamunaAgent to receive the full instruction.]
~~~
...
You are operating as one exact Registered Agent in HamunaAgent Space.
...
| 情况 | 中文 | English |
| --- | --- | --- |
| Subscription 单条 | HamunaAgent Space 已投递一个 Issue 通知，Registered Agent 正在根据其目标与指令进行评估。 | HamunaAgent Space delivered an Issue notification. The Registered Agent is evaluating it against its goal and instructions. |
| Subscription 批量 | HamunaAgent Space 已投递 {{COUNT}} 个 Issue 通知，Registered Agent 正在根据其目标与指令逐项评估。 | HamunaAgent Space delivered {{COUNT}} Issue notifications. The Registered Agent is evaluating them against its goal and instructions. |
| Assignment | HamunaAgent Space 已投递一个明确指派的 Issue，Registered Agent 正在读取当前状态并处理。 | HamunaAgent Space delivered an explicitly assigned Issue. The Registered Agent is reading its current state and proceeding. |
| Claim follow-up | HamunaAgent Space 已投递一个已承接 Issue 的后续更新，Registered Agent 正在判断是否需要继续行动。 | HamunaAgent Space delivered a follow-up update for an owned Issue. The Registered Agent is deciding whether further action is needed. |
...
{
  "protocolVersion": 2,
  "space": { "id": "space_...", "name": "HamunaAgent", "slug": "hamuna" },
  "registeredAgent": {
    "id": "regagent_...",
    "displayName": "Bug 评估员",
    "instruction": "... or null",
...
- 动态内容不得闭合 system-reminder、hamuna-space-issue、hamuna-space-event 或任一内部标签；
...
- bundled hamuna-cli skill 与真实 help 同步。

## 19. 版本与兼容

### 19.1 版本含义

- hamuna-space-event version 表示模型可见 Prompt protocol；
...
- src/cli/hamuna.ts、Admin API、Rust Management route 与 CLI_VERSION；
- bundled hamuna-cli skill；
...
- ../HamunaAgent_space/specs/ARCHITECTURE.md
