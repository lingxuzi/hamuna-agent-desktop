...
Production registry 按产品顺序注册八套完整 Theme：`hamuna-default`、`default-black`、`sage`、
...
src/renderer/theme/themes/hamuna-default.* canonical manifest + CSS
...
3. `themeSelectionExplicit:true` 永久尊重有效的非空 `themeId`。兼容旧配置时，非 canonical ID 推断为显式选择；历史自动物化的 `hamuna-default` 推断为未选择。用户在新选择器中明确选择 `hamuna-default` 后会连同显式标记一起保存，不再被迁移。
...
html[data-theme-id='hamuna-default'][data-color-scheme='light'] { ... }
html[data-color-scheme='dark'],
html[data-theme-id='hamuna-default'][data-color-scheme='dark'] { ... }
```
...
`index.html` 只读取 `hamuna:theme-bootstrap`：
