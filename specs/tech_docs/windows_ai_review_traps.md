...
> Purpose: a living adversarial-review checklist for HamunaAgent changes that are developed mostly on macOS but must work on Windows.
>
...
Review rule: do not ask "does this work on my Mac?". Ask "which Windows boundary did this cross, and what existing HamunaAgent owner already normalizes that boundary?" If no owner/helper exists, add one close to the boundary instead of sprinkling call-site guards.
...
| `\\?\` long-path prefix breaks non-Rust consumers | `pit_of_success.md::normalize_external_path`; #229 stripped `\\?\` from bundled Node path in `hamuna.cmd` |
| Windows shell quoting drops CLI content | issue #149; `src/cli/hamuna.ts` requires `--content-file` / `--prompt-file` for fragile payloads |
...
**Why AI misses it.** macOS paths are usually POSIX, case-sensitive enough for developer mental models, and use `/`. Windows paths can arrive as `C:\...`, `C:/...`, `c:/...`, UNC paths, paths with trailing separators, or extended-length `\\?\` paths. Different stores in HamunaAgent legitimately persist different forms.
...
**Why AI misses it.** AI-generated commands often work in zsh/bash and look escaped enough. On Windows, HamunaAgent may pass through Git Bash, `cmd.exe`, `.cmd` shims, npm wrappers, PowerShell, and Node argv parsing. Each layer has different quoting and newline behavior.

**Real failures.**

- issue #149: `hamuna thought create '<text>'` sometimes arrived without content on Windows, producing a server-side 422. The durable workaround is `--content-file`.
- `hamuna session send -p` rejects multiline or large inline prompts because `cmd.exe` can treat newline as a command boundary and drop subsequent flags.
...
- User-installed or HamunaAgent-managed CLIs were not detected because the app over-relied on shell PATH.
...
- Tool attachments initially used loopback HTTP URLs that did not match `img-src`; Windows needed app-owned custom protocol URL shape `http://hamuna.localhost/...`.
...
- Use `hamunaProtocol` / attachment URL helpers for app-owned resources. Do not invent `file://`, `asset://`, raw loopback, or platform-specific URL literals in components.
...
- Runtime env policy had to distinguish HamunaAgent proxy from terminal proxy.
- HamunaAgent-managed Codex provider must resolve as `runtime='codex'`, `runtimeSource='managed-provider'`, and use managed `CODEX_HOME`; mixing it with user-managed system CLI state creates confusing success/failure.
