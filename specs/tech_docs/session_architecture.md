...
message.uuid`），HamunaAgent 必须清掉该 stale anchor 并降级为裸 `resume`。这类恢复
是内部一致性降级：保留日志用于排障，但不向用户 toast / Agent Error。

### Session 间事件协议（send / watch）

`hamuna session send` / `watch` 不是普通文本拼接，而是结构化的 session event
协议。CLI 经 `/api/admin/session/*` 进入当前 Sidecar，事件统一渲染为
`<hamuna-session-event ...>` prompt，正文 payload 先经过结构标签 neutralize，
...
prompt 不走通用 `<hamuna-session-event>` 外包。Rust Space owner 会直接渲染
`<system-reminder><hamuna-space-issue>…</hamuna-space-issue></system-reminder>` user message，
让前端隐藏内部处理指令并显示 `Space issue` badge。这个特例只适用于 Space Issue
delivery，不改变 `hamuna session send/watch` 的通用事件协议。`system-reminder`
...
Goal 是 current Session 的独立持久状态，物理存储为 `~/.hamuna/session_goals.json`，不嵌入 `SessionMetadata`，也不复用 Task/Cron：
...
- UI `/goal`、当前 Session 内的 `hamuna goal create`、私聊 IM/Agent Channel 都写同一 Goal。创建前必须 materialize 真实 Session id；Rust 拒绝 `pending-*`，没有 post-hoc rebind。
...
- CLI/Admin：`hamuna goal get|list|create|update`。
...
**身份边界**：`sdkUuid` 的值就是 SDK stream / transcript 中的 message UUID；HamunaAgent
不另造这个 ID。但 HamunaAgent JSONL 里“曾经记录过该 `sdkUuid`”不等于 SDK 当前
...
`currentSessionUuids` 是 HamunaAgent 侧的 freshness cache，不是 SDK transcript 权威。
从磁盘 seed 的 UUID 只能说明 HamunaAgent store 里存在过该身份；它可用于避免明显
...
Rewind 会立即截断 HamunaAgent store；SDK 侧只能等下一次 `query({ resume,
...
**文件锁**：`sessions.json` 多 Sidecar 共享需锁。HamunaAgent 走 `withFileLock` / `with_file_lock`（详见 `pit_of_success.md` 的「withFileLock」节）。
...
## 双重存储：HamunaAgent 与 SDK

### 背景

Claude Agent SDK 内置了独立的 session 持久化机制（`persistSession` 选项默认 `true`）。HamunaAgent 调用 SDK 时，**两端各自独立写入会话数据**，形成双重存储。

### 存储位置对比

```
~/.hamuna/sessions/                ← HamunaAgent 写入
...
### 为什么不能去掉 HamunaAgent 存储

HamunaAgent 自身的存储服务于不同的业务场景：
...
| 维度 | SDK 存储 | HamunaAgent 存储 |
|------|----------|---------------|
| 写入者 | SDK 内部自动写入 | HamunaAgent `agent-session.ts` |
| 读取者 | SDK resume / `/insights` | HamunaAgent 前端 UI |
| 格式 | 消息树 + 操作记录 | 扁平消息列表 + 业务指标 |
| 索引 | 无（按文件遍历） | `sessions.json` 全局索引 |
| 生命周期 | 跟随 SDK 项目目录 | 跟随 HamunaAgent 数据目录 |

体积参考：SDK 存储约 1.7× HamunaAgent 存储（SDK 携带完整上下文元数据 + queue-operation 内部记录）。可定期清理过期的 SDK session 数据（例如 >30 天的已关闭 session）。
...
- 诊断"恢复只显示一部分"：读磁盘 `~/.hamuna/refs/<id>` 的 spilled body（后端实发的 JSON，可直接 `node` 解析）对比前端显示，先把"后端发了什么 vs 前端显示什么"一刀切开。
