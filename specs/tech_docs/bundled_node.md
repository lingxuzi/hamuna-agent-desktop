...
HamunaAgent 将 Node.js v24 运行时打包到应用内，实现**单一 runtime、零外部依赖**分发。用户无需安装 Node.js 即可运行所有功能（Sidecar、Plugin Bridge、MCP Server、社区 npm 包、`hamuna` CLI）。
...
|---|---|
| macOS ARM (M1/M2/...) | `HamunaAgent.app/Contents/Resources/nodejs/bin/node` |
...
└── Contents/
    ...
    └── Resources/
        ...
        └── cli/hamuna.js            # hamuna CLI（esbuild bundle）

注：v0.2.0+ 起 `agent-browser` 不再 bundle —— 改由 bundled-skills/agent-browser/SKILL.md
教 AI 在首次使用时通过命令级 `npm_config_prefix="$HAMUNA_NPM_GLOBAL_PREFIX" npm install -g agent-browser@<pinned>`
自装到 `~/.hamuna/npm-global/bin/`。`buildClaudeSessionEnv` 只暴露
`HAMUNA_NPM_GLOBAL_PREFIX`，不把 `npm_config_prefix` 泄漏到整个 SDK shell env。
```
...
3. `~/.hamuna/npm-global/bin`（HamunaAgent-localized npm installs / legacy AI-installed CLIs）
4. `~/.hamuna/bin`（`hamuna` CLI 所在）
5. 系统 PATH
...
skill 必须用命令级 env（例如 `npm_config_prefix="$HAMUNA_NPM_GLOBAL_PREFIX" npm install -g ...`）。
...
- HamunaAgent-owned preset 使用 `shared/mcpPackages.ts` 的精确 package spec；旧配置里的已知 `@latest` 在 runtime boundary 归一化，避免每次进程启动重新查询 registry
...
4. **CLI 打包**：esbuild bundle `src/cli/hamuna.ts` → `resources/cli/hamuna.js`
