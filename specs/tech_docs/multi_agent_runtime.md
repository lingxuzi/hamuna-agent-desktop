...
| 权限 | `permission_request` | 委托 HamunaAgent UI 审批 |
...
外部 runtime 还带有 `RuntimeConfig.source` / `HAMUNA_RUNTIME_SOURCE`，用于区分同一个 runtime 协议由谁管理：

| Source | 含义 | 典型入口 |
|---|---|---|
| `system-cli` | 用户自行安装并登录的本机 CLI | 实验室「更多 Agent Runtime」里选择 Codex / Claude Code / Gemini |
| `managed-provider` | HamunaAgent 管理 runtime 二进制、安装状态与登录状态 | Provider 列表里的 `codex-sub`（Codex 订阅） |
...
**IM native-card 例外**：当 `InteractionScenario` 是 IM / Agent Channel 且 `hostInteraction.askUserQuestion === 'native-card'` 时，`fullAgency` 不能直接传给 Claude Code 的 `bypassPermissions`。`AskUserQuestion` 通过 CC `control_request/can_use_tool` + `--permission-prompt-tool stdio` 回到 HamunaAgent；bypass 会跳过这条交互通道。`external-session.ts` 在 runtime 边界把启动态权限降为 `auto/acceptEdits`，同时对非 `AskUserQuestion` 的 permission request 做 fullAgency fast-path 自动允许，保持“普通工具自治、结构化提问可交互”的语义。
...
| `developerInstructions` | string? | ✅ `systemPromptAppend` | HamunaAgent 三层系统提示词 |
...
**MCP owner 边界**：Codex 的 `thread/start` / `thread/resume` schema 不接受 MCP 配置，但这不等于所有 Codex 会话都只能读取 `~/.codex/`。`runtimeSource:'managed-provider'` 由 HamunaAgent 持有 app-server 进程，因此在 spawn 时用 `-c mcp_servers.<name>.*=...` 注入当前 workspace 的有效 MCP；`runtimeSource:'system-cli'` 仍由用户自己的 Codex 配置持有 MCP，HamunaAgent 不覆盖。Managed 注入前必须复用 `utils/mcp-command.ts` 解析绝对 npx 路径、`-y` 与 HamunaAgent preset 的精确版本，不能和 builtin SDK 路径各自解释同一份 MCP definition。

### Skills 加载

Codex 原生扫描 `.agents/skills`，而 HamunaAgent/Claude Agent SDK 的工作区协议使用 `.claude/skills`。为保持产品层一致性，Codex adapter 在 `startSession()` 中做两步桥接：

1. 调 `syncProjectUserConfigFiles(workspacePath)`，把 `~/.hamuna/skills` 中启用的用户级 skills 同步为工作区 `.claude/skills/*` symlink（与 builtin Claude SDK 共用同一套磁盘桥接逻辑，不另建 Codex 专用目录）。
...
`app-server` 还会通过 JSON-RPC Server → Client request 向 HamunaAgent 要结果。`src/server/runtimes/codex.ts::KNOWN_CODEX_SERVER_REQUEST_METHODS` 是显式 allowlist，升级 Codex CLI 时必须先用 `codex app-server generate-ts --out <dir>` 对照 `v2/ServerRequest.ts`，再决定映射或 fail-closed。当前对接约束：

| Server request | HamunaAgent 映射 |
|---|---|
...
| `item/tool/call` / token refresh / attestation | HamunaAgent 不托管，显式 error |
...
| HamunaAgent | Codex approvalPolicy | sandbox |
|----------|---------------------|---------|
...
JSON-RPC 2.0 持久进程,与 Codex `app-server` 形态同构。HamunaAgent 作为 ACP Client,
...
| RPC 方法 | 处理 |
|---------|------|
| `session/request_permission` | 派生 `permission_request` UnifiedEvent(同时若未发 `tool_use_start` 则 late-bind)。HamunaAgent 返回 `{outcome:{outcome:'selected',optionId:...}}`;选项 `optionId` 基于 ACP 回传的 `options[].kind`(`allow_once` / `allow_always` / `reject_once`)健壮匹配。`default` 模式下 Gemini 跳过 `tool_call` notification 直接发 permission request,runtime 在此路径补发 `tool_use_start`,保证前端显示一致 |
...
1. **基底提取(一次性,按版本缓存)**:`extractGeminiBasePrompt(version)` 启动一个
   ...
   缓存路径:`~/.hamuna/tmp/gemini-prompts/base-<version>.md`,v0.37.2 约 25KB。

2. **per-session 合并**:`writeSessionSystemPrompt(sessionId, myAgentsPrompt, version)` 把
   HamunaAgent 的三层 prompt(base-identity + channel + scenario)前置,基底附在
   `---` 分隔符后并包上 "以 HamunaAgent 指令为优先" 的说明。写入:
   `~/.hamuna/tmp/gemini-prompts/session-<sessionId>.md`。
...
| HamunaAgent 内部值 | Gemini ACP modeId |
|----------------|-------------------|
| `default`       | `default`  |
| `autoEdit`      | `autoEdit` |
| `yolo`          | `yolo`     |
...
**完全不由 HamunaAgent 管理**。Gemini CLI 支持 OAuth、`GEMINI_API_KEY`、Vertex AI 三种方式,
用户自行在本机完成登录(`gemini` 交互式向导或 shell rc 导出环境变量),HamunaAgent 子进程
...
HamunaAgent session 身份下，造成历史会话和新会话串写。
...
| `turn` | busy 时进入 turn-boundary queue | busy 时进入 HamunaAgent turn-boundary queue，当前 turn 完成后再 `turn/start` | turn-boundary queue |

实现边界：
- `AgentRuntime.steerMessage?()` 是可选能力；只有 Codex adapter 实现。`external-session` 只看 capability，不硬编码 runtime 名。
- `turn/steer` 必须带 `expectedTurnId`（来自 Codex 当前 active turn）和 HamunaAgent user message id 作为 `clientUserMessageId`。
...
外部 Runtime 在 HamunaAgent 容器内的行为不一定等同于用户终端里直接跑——env、proxy、shell 探测、PATH 都可能差异化。诊断面板把这些差异显式 surface 出来，env policy 让用户在三种 env 注入策略间切换。
...
| 字面量 | 行为 | 适用场景 |
|--------|------|---------|
| `'hamuna'`（默认） | 继承 Sidecar 的 `process.env` proxy var——Rust 侧 `proxy_config::apply_to_subprocess` 已在 Sidecar 启动时注入了用户在 HamunaAgent 设置里配的 proxy | 绝大多数用户；HamunaAgent 提供一站式 proxy 管理 |
| `'terminal'` | 剥掉继承的 proxy var，恢复用户 interactive shell 在 `~/.zshrc` / `~/.bashrc` 里 export 的（warmup 时 `shell.ts::warmupShellPath` 抓的 8 个 var：`HTTP_PROXY`/`HTTPS_PROXY`/`ALL_PROXY`/`NO_PROXY` × 大小写）；语义上等同于"用户在自己电脑的终端里手动启动这个 CLI 时看到的 env" | 用户终端能调某个 endpoint 但 HamunaAgent 里调不到（issue #194 原始场景）；用 Clash TUN / VPN 等系统层路由的用户也走这档（shell 通常没 export proxy → 等同于无 proxy） |

**未知字面量 → fallback 到 `'hamuna'`**（`env-utils.ts::augmentedProcessEnv` 防御纵深；Codex review #5 catch）。

**0.2.16 dev 历史**：曾短暂存在第三个 `'direct'` 字面量（剥掉所有 proxy var），dogfooding 反馈"三个选项太复杂"后在 0.2.16 release 前移除。已存盘的 `'direct'` 在 `resolveAgentEnvPolicy` 校验白名单里 fallback 到 `'hamuna'`（UI 会显示选中"HamunaAgent 代理"，依赖 strip 行为的用户可手动改成"跟随终端"——shell 里没 export proxy 时效果与原 `'direct'` 一致）。

**校验入口统一**：disk 上的 `agent.runtimeConfig.envPolicy.proxy` MUST 通过 `env-utils.resolveAgentEnvPolicy(workspacePath)` 读，**禁止**裸 `raw as RuntimeEnvPolicy` cast——后者会让 `proxy: 'evil_value'` 这种 typo 在诊断面板上显示成 `'hamuna'`，对用户隐藏 misconfig。两个调用点（`external-session.ts` 会话启动 + `admin-api.ts` CLI diagnose handler）现已统一走这个 helper。
...
```typescript
{
  proxyPolicy: 'hamuna' | 'terminal' | 'direct',
  httpProxy?: string,
  httpsProxy?: string,
  allProxy?: string,
  noProxy?: string,
...
```bash
hamuna runtime diagnose codex --workspace=<path>   # 主形式
hamuna diagnose runtime codex --workspace=<path>   # 别名糖
```
...
config.multiAgentRuntime (磁盘/React state)
  ...
  │     → sidecar/session_lifecycle.rs 或 sidecar/instances.rs 在 spawn 时注入 HAMUNA_RUNTIME
  │
  ├── Node factory.ts: getCurrentRuntimeType()
  │     → 读取 process.env.HAMUNA_RUNTIME
  │     → 未设置 → 'builtin'
...
| `src/server/runtimes/claude-code.ts` → `FORWARDER_SCRIPT` | CC SessionStart hook 转发脚本（运行时生成至 `~/.hamuna/.cc-hooks/forwarder.cjs`） |
