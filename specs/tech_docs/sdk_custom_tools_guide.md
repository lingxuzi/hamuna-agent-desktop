...
> 技术文档：如何在 HamunaAgent 中使用 Claude Agent SDK 创建自定义 MCP 工具
...
`im-media` 已在 0.2.11 退役，能力统一由 `hamuna` CLI 提供，不能重新注册成
SDK MCP。
...
`hamuna cron exit --reason ...`。该命令只记录当前 Turn 的退出请求，不直接修改
Task 状态。
...
`hamuna cron` CLI，最终写入 TaskStore。
...
5. **内置工具调试**：查看 `~/.hamuna/logs/unified-{today}.log`，搜索对应 MCP server id
