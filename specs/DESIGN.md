...
HamunaAgent 是一款 AI Agent 桌面客户端，采用**温暖纸张质感**的设计风格，营造舒适、专业的使用体验。
...
HamunaAgent 的视觉由完整 `Theme` 管理；light / dark / system 是 `AppearanceMode`，不是三套 Theme。一套 Theme 必须同时交付并验收 light 与 dark，system 只跟随 OS 解析其中一套。

Production catalog 当前包含八套完整 Theme：HamunaAgent Default、Default Black、Sage、Claude、
Linear、Proof、Codex、Raycast。`hamuna-default` 仍是 canonical
fallback；它的物理 owner 是：

- `src/renderer/theme/themes/hamuna-default.css`：通用首帧 fallback + 精确 Theme root / light / dark root 下的字体角色、颜色、材质、圆角、阴影、动画和 Floating Ball 运行时 Token；同一文件既静态保护 canonical 首帧，也由 manifest 提供实际 source 给注册校验与 runtime 激活；
- `src/renderer/theme/themes/hamuna-default.ts`：Launcher Hero 与 xterm / Monaco / Mermaid / Prism / Widget adapters；
...
| Theme | 主要视觉角色 |
|---|---|
| HamunaAgent Default / Default Black | 暖纸张、陶土橙；Default Black 是当前产品默认，仅将 light 主按钮改为中性黑，本章色值表仍只描述 canonical Theme |
...
下列值描述 canonical `hamuna-default` 的 light scheme；真实定义以
`src/renderer/theme/themes/hamuna-default.css` 为准。dark scheme 也在同一文件中完整定义。
...
```css
/* hamuna-default.css — Theme 只拥有运行时视觉值 */
:root,
html[data-theme-id='hamuna-default'] {
...
/* Windows: renderer 全局 scroll capture 给正在滚动的元素加
   .hamuna-scrollbar-active。默认 thumb 透明，滚动停止后恢复透明，
   保留 6px 几何以避免内容列重排。 */
...
html.platform-windows.platform-windows.hamuna-scrollbar-active,
html.platform-windows.platform-windows .hamuna-scrollbar-active {
  scrollbar-color: var(--ink-subtle) transparent;
}
```
...
品牌区 JSX 只消费 `ResolvedTheme.hero`。产品名、zh-CN/en-US slogan、文字视觉参数和每个 scheme 的可选 bundled 背景槽都由 Theme 拥有；`BrandSection` 不硬编码 `HamunaAgent` 或 slogan source。canonical Theme 当前没有独立背景图，因此与迁移前视觉一致。
...
| 版本 | 日期 | 变更 |
|------|------|------|
| 2.7.9 | 2026-07-23 | **产品默认 Theme 与显式选择解耦**：Default Black 成为未选择用户当前跟随的产品默认，`hamuna-default` 继续仅承担 canonical fallback；新增显式选择状态，未来调整产品默认不覆盖用户选择；Absolutely 用户可见名改为 Claude；Theme 菜单选中标记移到名称之后、light/dark 色块之前 |
...
| 2.6.0 | 2026-07-20 | **Theme System 架构收口（PRD 0.3.2）**：区分 Theme / AppearanceMode / ResolvedColorScheme；现有 light/dark 视觉完整迁入 canonical `hamuna-default`；Launcher Hero、CSS Token、xterm、Monaco、Mermaid、Prism、Widget 与 Floating Ball 统一消费 `ResolvedTheme`；Space `space-mono` 明确保持独立；本次无有意视觉优化 |
