...
Write-Host "  HamunaAgent Windows 发布到 R2" -ForegroundColor Green
...
$ManifestDir = [System.IO.Path]::GetTempPath() + "hamuna-manifest-" + [System.Guid]::NewGuid().ToString("N")
...
if ($SigFile) {
    Write-Host "    - HamunaAgent_${Version}_x86_64.nsis.zip.sig"
    $uploadFiles += $SigFile
}
...
if ($SigFile) {
    ...
    $sigUploadName = "HamunaAgent_${Version}_x86_64.nsis.zip.sig"
