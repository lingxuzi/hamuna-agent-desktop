...
echo ""
echo -e "${CYAN}╔═══════════════════════════════════════════════════════╗${NC}"
echo -e "${CYAN}║${NC}  ${GREEN}🤖 HamunaAgent macOS Dev 构建${NC}                           ${CYAN}║${NC}"
...
# 杀死残留 HamunaAgent 实例（避免生产版和 debug 版同时运行互相打架）
# 优先使用 PID lock file 精确杀——只杀 HamunaAgent 主进程，不误杀其他 node 进程。
# SIGKILL(-9) 防止 macOS Automatic Termination 自动重启被杀的 .app。
echo -e "${BLUE}[准备] 杀死残留进程...${NC}"
LOCK_FILE="$HOME/.hamuna/app.lock"
if [ -f "$LOCK_FILE" ]; then
    OLD_PID=$(cat "$LOCK_FILE" 2>/dev/null)
    # Validate PID is a positive integer before using it with kill
    if [[ "$OLD_PID" =~ ^[1-9][0-9]*$ ]] && kill -0 "$OLD_PID" 2>/dev/null; then
        echo -e "${YELLOW}  杀死运行中的 HamunaAgent (PID $OLD_PID)...${NC}"
        kill -9 "$OLD_PID" 2>/dev/null || true
    fi
    rm -f "$LOCK_FILE"
fi
# Fallback: 杀死任何漏网的 HamunaAgent 进程（lock file 可能不存在或 PID 已过期）
pkill -9 -f "HamunaAgent.app" 2>/dev/null || true
...
# / plugin-bridge-dist.mjs / cli/hamuna.js）则由 tauri:build 的
...
rm -rf "${PROJECT_DIR}/src-tauri/target/debug/HamunaAgent.app"
...
# hamuna CLI 的打包不在这里——`npm run tauri:build` 的 beforeBuildCommand
# (tauri.conf.json) 已包含 `npm run build:cli`，由 `scripts/esbuild-bundle.mjs`
# 的 post-build hook 同步把 hamuna.cmd 拷贝到 resources/cli/。dev 脚本只需
