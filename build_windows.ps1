...
try {
    ...
    Write-Host "  HamunaAgent Windows 发布构建" -ForegroundColor Green
    ...
    # 杀死残留进程（避免文件锁定）
    $appProcesses = Get-Process | Where-Object { $_.ProcessName -eq "HamunaAgent" }

    if ($appProcesses) {
        $appProcesses | Stop-Process -Force -ErrorAction SilentlyContinue
        Write-Host "  清理了 $($appProcesses.Count) 个 HamunaAgent 进程" -ForegroundColor Gray
    }
    ...
    while ($waited -lt $maxWait) {
        $remainingApp = Get-Process -Name "HamunaAgent" -ErrorAction SilentlyContinue
        if (-not $remainingApp) {
            break
        }
    ...
    & npm run build:cli
    if ($LASTEXITCODE -ne 0) { throw "hamuna CLI 打包失败" }
...
"@
    ...
    if (-not $SkipPortable) {
        ...
        # Cargo package name is `hamuna` (lowercase), so Tauri's main binary is
        # hamuna.exe — NOT HamunaAgent.exe (that's only the productName / shortcut).
        # The old HamunaAgent.exe path worked by luck on case-insensitive NTFS; use the
        ...
        if (Test-Path $exePath) {
            ...
            $zipName = "HamunaAgent_${Version}_x86_64-portable.zip"
            ...
            Write-Host "  警告: 未找到 hamuna.exe，跳过便携版创建" -ForegroundColor Yellow
        }
        Write-Host ""
    }
