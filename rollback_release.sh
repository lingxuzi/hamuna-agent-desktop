...
echo ""
echo -e "${CYAN}╔═══════════════════════════════════════════════════════╗${NC}"
echo -e "${CYAN}║${NC}  ${YELLOW}⏪ HamunaAgent 版本回滚${NC}                                  ${CYAN}║${NC}"
echo -e "${CYAN}╚═══════════════════════════════════════════════════════╝${NC}"
echo ""
...
if [ $ROLLBACK_MAC -eq 1 ] && [ "$TARGET_VERSION" != "$CURRENT_MAC_VERSION" ]; then
    ...
    ARM_SIG_FILE=""
    ARM_TAR_NAME=""
    if [ -f "${SIG_DIR}/HamunaAgent_${TARGET_VERSION}_aarch64.app.tar.gz.sig" ]; then
        ARM_SIG_FILE="${SIG_DIR}/HamunaAgent_${TARGET_VERSION}_aarch64.app.tar.gz.sig"
        ARM_TAR_NAME="HamunaAgent_${TARGET_VERSION}_aarch64.app.tar.gz"
    elif [ -f "${SIG_DIR}/HamunaAgent_aarch64.app.tar.gz.sig" ]; then
        ARM_SIG_FILE="${SIG_DIR}/HamunaAgent_aarch64.app.tar.gz.sig"
        ARM_TAR_NAME="HamunaAgent_aarch64.app.tar.gz"
    fi
        ...
        cat > "${MANIFEST_DIR}/darwin-aarch64.json" << EOFMANIFEST
{
  ...
  "notes": "HamunaAgent v${TARGET_VERSION}",
  "pub_date": "${PUB_DATE}",
...
EOFMANIFEST
    ...
    INTEL_SIG_FILE=""
    INTEL_TAR_NAME=""
    if [ -f "${SIG_DIR}/HamunaAgent_${TARGET_VERSION}_x86_64.app.tar.gz.sig" ]; then
        INTEL_SIG_FILE="${SIG_DIR}/HamunaAgent_${TARGET_VERSION}_x86_64.app.tar.gz.sig"
        INTEL_TAR_NAME="HamunaAgent_${TARGET_VERSION}_x86_64.app.tar.gz"
    elif [ -f "${SIG_DIR}/HamunaAgent_x86_64.app.tar.gz.sig" ]; then
        INTEL_SIG_FILE="${SIG_DIR}/HamunaAgent_x86_64.app.tar.gz.sig"
        INTEL_TAR_NAME="HamunaAgent_x86_64.app.tar.gz"
    elif [ -f "${SIG_DIR}/HamunaAgent_${TARGET_VERSION}_x64.app.tar.gz.sig" ]; then
        INTEL_SIG_FILE="${SIG_DIR}/HamunaAgent_${TARGET_VERSION}_x64.app.tar.gz.sig"
        INTEL_TAR_NAME="HamunaAgent_${TARGET_VERSION}_x64.app.tar.gz"
    fi
...
EOFMANIFEST
    ...
    if [ -n "$ARM_DMG_NAME" ] || [ -n "$INTEL_DMG_NAME" ]; then
        LATEST_JSON="{\n  \"version\": \"${TARGET_VERSION}\",\n  \"pub_date\": \"${PUB_DATE}\",\n  \"release_notes\": \"HamunaAgent v${TARGET_VERSION}\",\n  \"downloads\": {"
...
if [ $ROLLBACK_WIN -eq 1 ] && [ "$TARGET_VERSION" != "$CURRENT_WIN_VERSION" ]; then
    ...
    WIN_SIG_FILE=""
    WIN_ZIP_NAME=""
    if [ -f "${SIG_DIR}/HamunaAgent_${TARGET_VERSION}_x86_64.nsis.zip.sig" ]; then
        WIN_SIG_FILE="${SIG_DIR}/HamunaAgent_${TARGET_VERSION}_x86_64.nsis.zip.sig"
        WIN_ZIP_NAME="HamunaAgent_${TARGET_VERSION}_x86_64.nsis.zip"
    else
