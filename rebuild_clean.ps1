...
Write-Host "║  🔧 HamunaAgent 彻底清理重建                              ║" -ForegroundColor Cyan
Write-Host "╚═══════════════════════════════════════════════════════╝" -ForegroundColor Cyan
Write-Host ""
...
if (-not $SkipUninstall) {
    ...
    # 检查是否安装
    $app = Get-WmiObject -Class Win32_Product | Where-Object { $_.Name -like "HamunaAgent*" } | Select-Object -First 1
...
$webviewCache = "$env:LOCALAPPDATA\HamunaAgent\EBWebView"
...
$appData = "$env:APPDATA\HamunaAgent"
...
$killed = 0
Get-Process | Where-Object { $_.ProcessName -eq "HamunaAgent" } | ForEach-Object {
...
$cleanDirs = @(
    ...
    "src-tauri\target\x86_64-pc-windows-msvc\release\build\hamuna-*",
