...
正常 Agent / Channel / Plugin 关系先读 `/hamuna-docs/references/agents-channels.md` 与 `tools-skills-plugins.md`。
...
hamuna agent channel list <agent-id> --json
hamuna agent runtime-status --json
hamuna plugin list --json
...
- 登录成功但消息不进 HamunaAgent：查 Bridge message route 到 Rust 的日志，再查 Agent Channel 是否 enabled。
