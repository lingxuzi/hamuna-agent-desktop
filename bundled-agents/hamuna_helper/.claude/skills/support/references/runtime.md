...
使用场景：Codex / Gemini / Claude Code 不工作；终端能用但 HamunaAgent 里不行；外部 Runtime 的模型、权限、MCP、登录或代理异常。

先读 `/hamuna-docs/references/models-providers-runtimes.md` 确认 Provider / Model / Runtime 与 `runtimeSource` 的产品边界。
...
- system-cli Codex 的 MCP 由 Codex 自己管理，HamunaAgent 不把 Workspace MCP 注入其中。
- managed-provider Codex 启动 app-server 时，会尝试注入当前 Workspace 中安全且兼容的 HamunaAgent MCP；builtin/in-process、不支持的传输或不安全配置会被跳过。因此“Codex MCP 都与 HamunaAgent 无关”只适用于 system-cli。
...
hamuna runtime describe codex --json
hamuna runtime describe gemini --json
hamuna agent list --json
hamuna agent show <agent-id> --json
rg -n "HAMUNA_RUNTIME|external-session|external-runtime|runtime_diagnostics|RuntimeDiagnostics|runtimeSource|managed-provider|managed-codex|codex-sub|Codex|Gemini|ACP|app-server|envPolicy|mcp" ./logs/unified-*.log | node .claude/skills/support/scripts/redact-log-output.mjs | tail -200
```
...
```bash
hamuna runtime diagnose codex --workspacePath <absolute-workspace-path> --json
```
...
- system-cli auth 不健康：让用户用 Runtime 自己的登录入口恢复；HamunaAgent 不伪造其登录态。
...
- `effectiveEnv.proxyPolicy=terminal` 但 proxy 为空：说明探测到的终端环境没有这些变量，不足以证明 HamunaAgent 漏注入。
