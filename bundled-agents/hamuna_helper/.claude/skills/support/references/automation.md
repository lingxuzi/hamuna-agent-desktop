...
正确产品语义先读 `/hamuna-docs/references/automation.md`。这里仅处理实际行为偏离预期的现场。
...
```bash
hamuna cron status --json
hamuna cron list --json
hamuna cron list --workspace <absolute-workspace-path> --json
hamuna cron runs <task-id> --limit 20 --json
hamuna task list --json
hamuna task get <task-id> --json
...
```bash
hamuna cron run-now <task-id>
# 或按 Task 当前状态和精确 help 选择：
hamuna task run <task-id>
hamuna task rerun <task-id>
```
...
```bash
hamuna session send <session-id> -p "..."
```
