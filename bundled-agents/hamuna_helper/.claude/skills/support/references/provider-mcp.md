...
正确概念与生效时机先读 `/hamuna-docs/references/models-providers-runtimes.md` 和 `tools-skills-plugins.md`。
...
```bash
hamuna model verify <provider-id> --json
hamuna model verify <provider-id> --model <model-id> --json
```
...
- `codex/system-cli` 使用 Codex 自有 MCP；`codex/managed-provider` 可能注入安全兼容的 HamunaAgent Workspace MCP。先确认 runtimeSource 再选证据。

## MCP 取证

```bash
hamuna mcp list --json
hamuna mcp show <mcp-id> --json
hamuna mcp oauth status <mcp-id> --json
...
```bash
hamuna mcp test <mcp-id> --json
```

判断要点：

- `command not found`：核对 command 与实际子进程 PATH；HamunaAgent 自带 Node/npx 不代表所有用户 CLI 都存在。
