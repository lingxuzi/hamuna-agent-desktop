...
TVC 标准时长规划（**基线 12s = 1 张 grid**——agnes 单段上限；不在每张 grid 内硬塞 15s）：

| TVC 时长 | 多宫格数量 | 视频提示词段数 | 说明 |
|---------|-----------|-------------|------|
| 12s | 1 张 3x3 | 1 段 | **新增基线**（取代旧 15s）；每格≈1.3s |
| 15s（legacy brief） | 1 张 3x3 | 1 段 + 3s keyframe 收尾 | 推荐 Phase 1 沟通时把 brief 改 12s；若坚持 15s，3s 收尾走 keyframe 兜底（不计入多宫格） |
| 30s | 2 张 3x3 | 2 段 + 6s keyframe 收尾 | 标准 TVC，最常见；2 × 12 = 24s grid + 6s 收尾 |
| 60s | 5 张 3x3 | 5 段 | 完整叙事；5 × 12 = 60s 正好填满（不再 4 × 15s=60s，那超出 agnes 上限） |
...
|------|------|---------|------|---------|---------|------|
| G1 | 多宫格 3x3 | 0-12s  | 16:9 | 品牌世界 | 7/9 | 极限运动开场 |
| G2 | 多宫格 3x3 | 12-24s | 16:9 | 产品世界 | 9/9 | 产品电影化拆解 |
| G3 | keyframe 收尾 | 24-30s | 16:9 | 产品世界 | 1/1 | 6s Hero Shot + End Frame 留白 |
```
...
**30s TVC（2 段 × 12s + 6s 收尾）— 品牌世界穿梭型**：

| 时段 | 世界 | 功能 | 情绪 |
|------|------|------|------|
| 0-4s | 品牌世界 | Hook：极限场景/生活瞬间 | 肾上腺素/共鸣 |
| 4-8s | 交叉 | 品牌世界 ↔ 产品特写交替（匹配剪辑衔接） | 惊叹/好奇 |
| 8-16s | 产品世界 | 产品电影化拆解/功能可视化 | 专注/震撼 |
| 16-20s | 品牌世界 | 回到使用场景，产品融入其中 | 向往/认同 |
| 20-24s | 产品世界 | 产品 Hero Shot | 记忆锚定 |
| 24-30s | keyframe 收尾 | End Frame + Logo + Slogan 留白 | 收束 |

**30s TVC（2 段 × 12s + 6s 收尾）— 纯产品电影化型**：

| 时段 | 功能 | 产品状态 |
|------|------|---------|
| 0-3s | 产品从黑暗中觉醒 | 光线唤醒 + 材质微距 |
| 3-7s | Phase-by-Phase 功能拆解 | 零件悬浮拆解、传感器发光 |
| 7-12s | 组装回弹 + 功能可视化 | 屏幕亮起、追踪框、数字跳动 |
| 12-18s | 爆发旋转 + 多角度展示 | 旋转中光影流动 |
| 18-23s | 材质微距高潮 | 极近距离材质质感 |
| 23-30s | keyframe 收尾 | Hero Pose + Logo 留白 + End Frame |

**12s TVC（基线 — 取代旧 15s TVC）**：

| 时段 | 功能 |
|------|------|
| 0-3s | Hook（品牌世界一闪 or 产品爆发登场） |
| 3-8s | 核心卖点视觉化（1-2 个功能的电影化呈现） |
| 8-10s | 产品 Hero Shot |
| 10-12s | End Frame 留白 |
...
多宫格是从一条 12 秒视频中冻结出来的 9 个关键帧（**12s = agnes 单段上限 = 1 张 grid 的最大叙事空间**）。**写逐格描述之前，先用 1-2 句话勾勒视频脉络**——镜头语言怎么连续、产品状态怎么变、画面之间怎么衔接。视频脉络不等于一镜到底——它可以包含硬切、匹配剪辑、溶解等各种转场，关键是每格在时间轴上有明确的位置和因果。视频提示词是同一条脉络的展开，多宫格是同一条脉络的冻结——两者从同一源头生长。
...
6. **视频脉络先行**：每张多宫格写逐格描述前，先勾勒 12 秒段落的视频脉络（12s = agnes 单段上限；不再 15s，那是历史遗留，已超出 agnes 能力）。多宫格是脉络的冻结，视频提示词是脉络的展开
