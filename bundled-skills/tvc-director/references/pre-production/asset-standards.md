...
> **规范速查**：本文件的提示词 + 参考图规范权威来源为 [`asset-prompting-cheatsheet.md`](../asset-prompting-cheatsheet.md)（4 类资产 / 6 段 schema / Failure Recovery）。本文是展开版，cheatsheet 是速查表。

> **产品宣传视频必须上传产品参考图，无例外。**
>
> 参考图决定提示词的**写法路径**——有参考图时 prompt 直接引用"图中产品/图中人物"，**不描述外观细节**（外观由参考图锁定，多余描述反而干扰还原）。
>
> 角色图和场景图可以无参考图（用文字描述），但产品图必须有参考图。产品参考图应在创意简报阶段（Phase 1）主动追问，不得等到 Phase 5.5 才补问。

---

## Part 1: 资产规划 — 从分镜到资产清单
...
**参考图交互**：
- **产品参考图必须上传**（Phase 1 主动追问，不得以文字描述替代）
- 用户未提供时，提示用户上传，不得进入下一阶段
- 提供后直接生成 prompt，不额外索要

**prompt 写法**（有参考图）：
- 直接引用"参考图片，为这个产品生成xxx"——**不描述产品外观细节**（外观由参考图锁定）
- 示例："参考图片，为这个产品生成多视图，白底影棚，统一侧光+轮廓光..."
...
**产品多视图生成**（agnes_image_edit，本地路径作参考图）：
...
商业摄影风格，8K分辨率，干净背景。
```
- 工具：`agnes_image_edit`，`image_paths` 传入本地参考图路径
- 上传 img.remit.ee 仅在后续视频生成时需要
...
当产品穿在身上/戴在手上/拿在手里时（跑鞋、手表、首饰、外套、眼镜、手袋等），角色资产图提示词**必须引用前期已生成的产品多视图**，让角色穿着/佩戴产品出镜。写法：用 `(图N)` 引用——`穿着图1的跑鞋` / `手腕佩戴图1的手表` / `身穿图1的外套`。不引用产品图 = 角色脚上/手上/身上的产品外观不可控 = 后续分镜一致性崩塌。
...
| 路径 | prompt 写法 | 示例 |
|------|-----------|------|
| **无产品场景（默认）** | `参考图片，基于图中场景氛围生成xxx` — 不描述场景细节 | "参考图片，基于图中场景氛围生成 16:9 宽幅空镜，无人物无产品，保持相同色调和光影基调" |
| **有产品场景** | 同时传入场景参考图 + 产品参考图，产品以合适比例融入环境 | "参考图片A为场景氛围，参考图片B为产品外观。生成 16:9 场景图：现代家居浴室角落，暖黄窗光，白色圆形空气净化设备置于墙角，产品比例适中，不抢空间氛围。保持色调一致，浅景深，手持纪实感。无人物。" |
...
| 类型 | 要求 |
|------|------|
| **无产品场景图** | 无人物、无产品；只锁定"空间本身"；人和产品在分镜帧中再加入 |
| **有产品场景图** | 必须引用产品参考图；产品以自然比例融入环境（占比 10%-30%），不得抢走空间氛围；色调与场景一致 |
| **通用要求** | 时间和天气必须明确；色调是核心交付；前中远景分层；宽幅 16:9 |
