...
#### agnes-image-2.5-flash / agnes-video-2.5-flash 多宫格人物约束

**3x3 grid 多宫格物理约束**：每格只占屏幕约 1/9（≈ 27% 宽度），AI 模型在如此小的空间内难以稳定生成完整人脸——面部细节不足、特征易失真、跨格一致性崩塌。这一限制**与具体模型无关**（适用于 agnes-image-2.5-flash、agnes-video-2.5-flash、未来其他图/视频模型），是 3x3 多宫格拼接的物理约束，不是模型能力限制。人物出镜方式限于：
*   手部 / 手腕特写（珠宝/手表最常用）
*   下半脸（嘴唇、下颌线）
*   身体局部特写（肩颈、锁骨、腰线）
*   全身远景 / 背影 / 剪影
...
**资产图不受此限制**：角色三视图在分镜之前生成，单图占比 100%，人脸细节充分——角色三视图可包含完整人脸，用于锁定人物设定。限制仅在多宫格分镜阶段生效。
...
### 5. 叙事切分 (Pacing)
将总时长划分为合乎逻辑的节拍幕次（通常以 12 秒为一个叙事单元——**agnes 单段上限**；不再 15s，那超出 agnes 能力）：
*   **0-12s (起 - Hook)**：建立世界观与情绪基调，抓住眼球。
*   **12-24s (承 - Develop)**：场景展开，产品核心特征、氛围或冲突显现。
*   **24-36s (转 - Climax)**：视听高潮，最快节奏的快剪或最具视觉张力的特写。
*   **36-48s (合 - Resolution)**：情绪落地，产品 Hero Shot 亮相，品牌理念收束。
*   **48-60s (收 - End Frame)**：第二收束段 / Hero 留白 / Logo + Slogan（60s 拆 5 段 × 12s 时启用）。
...
**【分幕剧本】**（按时长分块，通常每 12 秒一幕）

#### ⏱️ 第一幕：[幕标题] (00:00 - 00:12)
