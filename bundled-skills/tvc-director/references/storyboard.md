...
定义整张图的视觉基调（色调 + 氛围关键词）和物理格式。**image_edit 模式**：仅写「场景 + 色调 + 比例」三件套（模型自带参考图锁定产品外观一致性，不要重复约束）。**image_generate 模式（纯文生图）**：额外写「产品/角色外观关键词」。

**必须包含**：

- **网格声明**：`生成一张包含N个分镜的组合图，按RxC网格排列`
- **色调 + 氛围**：如「卧室晨光场景，米白床品、薄纱窗帘、暖金木色基调」——只写色调和氛围，**不写**"3D 奇幻动画风格" / "ARRI Alexa 电影摄影机"等风格/设备标签（image_edit 模式下这些标签会触发每 panel 独立渲染、丢失多宫格无缝感）
- **画面比例**：整张多宫格图的比例（通常 16:9 或 1:1）

**image_edit 模式简洁示例**（推荐）：
> 生成一张包含9个分镜的组合图，按3x3网格排列。卧室晨光场景，米白床品、薄纱窗帘、暖金木色基调，16:9横屏。

**image_generate 模式（纯文生图）补充示例**：
> 生成一张包含9个分镜的组合图，按3x3网格排列。卧室晨光场景，米白床品、薄纱窗帘、暖金木色基调。沿<Picture 1>还原产品外观——白色椭圆盒、顶部黑色同心圆散热孔、USB-C接口。16:9横屏。

### 第二层：参考图映射

当使用 `edit` 模式引用 Phase 4 资产图时，在此层建立映射关系。

格式：`<Picture 1>[资产类型/名称], <Picture 2>[资产类型/名称]...`

如果无参考图（纯文生图），跳过此层，在第一层中用文字描述角色/环境的关键外观特征。

### 第三层：叙事主线 + 逐格描述

这是多宫格提示词的核心。**每 panel 描述保持 1 句话**（image_edit 模式下，多句描述会触发每 panel 独立精细渲染、丢失多宫格无缝感；端到端测试 v3 vs v5：多句描述 grid panel 接缝明显，1 句描述 grid panel 大小一致）。写法随**剧情密度**变化——详见第四节。

### 第四层：一致性锚（可选）

**仅在 image_generate 模式（纯文生图，无 asset 参考）时使用**——image_edit 模式下 `image_edit` 模型自带多宫格一致性，**不要重复约束**（重复"保持整体风格统一 / 角色一致"反而触发每 panel 独立渲染，v3 端到端踩坑）。

**image_generate 模式追加项**（在全部逐格描述之后）：
...
### image_edit 模式禁忌清单（2026-09 v3 端到端踩坑）

🔴 **image_edit 模式生成 grid 时，禁止**：

1. 写"电影级静帧摄影质感" / "ARRI Alexa 拍摄" / "3D 奇幻动画风格" / "虚幻引擎 5 实时渲染" 等**风格/设备标签**——会触发每 panel 独立渲染，多宫格接缝明显
2. 写"保持整体风格统一" / "9 个分镜保持产品外观严格一致" / "整体保持 XX 氛围" 等**显式一致性约束**——`image_edit` 自带多宫格一致性，重复约束会干扰模型
3. 写"色调从...过渡至..." / "P9 末段色温微转冷" 等**色调连续性描述**——会触发每 panel 按指定色调独立渲染，9 宫接缝明显
4. 写每 panel 多句详细描述（"卧室全景：薄纱窗帘透晨光，床头柜在画面右侧，床上枕头整齐，盒子在床头柜中央，晨光氛围"）—— 每 panel 1 句即可（"卧室全景，薄纱窗帘透晨光，盒子在床头柜"），多句会触发每 panel 独立精细渲染

**推荐写法**：第一层「场景 + 色调 + 比例」+ 第三层 9 panel 各 1 句简短描述。**不写第四层**。

---

## 二、视频脉络先行

多宫格不是 9 张美图的拼盘——它是从一条 12 秒视频中冻结出来的 9 个关键帧（**12s = agnes 单段上限 = 1 张 grid 的最大叙事空间**）。**先有流，再有帧。**

### 核心原则

写逐格描述之前，先用 1-2 句话勾勒这 12 秒的**视频脉络**：镜头语言怎么连续、产品状态怎么变、画面之间怎么衔接。视频脉络是时间轴上的因果链——每格承接什么、过渡到什么——而不只是"9 格放在一起好看"。
...
### 视频脉络的衔接：bridge 段（多段拼接必备）

多段视频拼接时，每段的脉络**不是独立闭环**——它必须为下一段留出 0.8-1.5s 的"视觉走廊"，让观众感觉不到切场。**bridge 段**就是这段走廊在多宫格里的体现。

**单段 12s 的脉络分块**：

```
[主叙事 10.5-11s] → [bridge 0.8-1.5s]
         ↓                    ↓
   完成本段故事      为下一段铺垫视觉走廊
```

**bridge 段在多宫格的位置**：每张 grid 的**最后一格**（P9）应是 bridge 镜头，承接下一段的 P1。bridge 段的写法三选一（详见 Part 六）：

- **同构图推进**：P9 与下段 P1 景别/视角/产品位置一致，只换光色
- **时间流逝法**：P9 主体静止、光线缓慢过渡
- **尾帧延续法**：P9 = 产品 hero shot 定格，下段从这一位置起运动

**单段脉络写法（含 bridge）示例**：

```
视频脉络（30s TVC 第 2 段 / 12s）：
缓推入产品主体特写→环绕镜头展示材质→微距潜入内部结构→
功能激活瞬间→拉远揭示使用场景→[bridge 1s] 同构图定格收束，色温向冷蓝过渡
                  ↑                                          ↑
              主叙事 11s                              bridge 1s（视觉走廊给下段）
```

**没有 bridge 的代价**：30s TVC = 12s + 12s 直接拼接，段交界处观众会感到"跳"。加入 bridge 后，过渡段（拼接处）画面构图一致 + 光色自然过渡 = "感觉不到切场"。**bridge 是拼接成本的保险，不是装饰**。
...
| 规格 | 格数 | 时长覆盖 | 适用场景 |
|------|------|---------|---------|
| **3x3** | 9格 | ≈12秒 | **标准叙事段落（agnes 上限）**、产品宣传完整流程、最常用；**9 panel 必须是同一场景不同机位**（详见下方铁律） |
...
### 铁律：12s 9 panel = 同一场景不同机位（2026-09 端到端踩坑）

🔴 **1 张 3x3 grid 的 9 个 panel 不是 9 个分镜故事板**（9 个跳切场景），而是**1 段 12s 视频的 9 个关键帧**（同一场景的不同机位/景别/视角）。

**正确做法**：9 panel 在同一空间内通过运镜展开——
...
| P5 | 特写 | 材质 / 肌理 / 细节 |
| P6 | 微距 | 微观结构 / 内部 / 功能点 |
| P7 | 微距或拉远起点 | 收束前的最后一个细节 / 转场准备 |
| P8 | 拉远中 | 回到主体 + 露出环境 |
| P9 | bridge 定格 | 桥接到下一段（同构图 / 时间流逝 / 尾帧延续） |

**反模式（video 会跳过次要 panel）**：9 panel 塞 4-5 个完全跳切的场景（卧室 → 街景 → 门口 → 厨房 → 沙发）→ video 模型消化 grid 后只复刻"主旋律"（主体 + 关键道具），自动省略次要场景里的"户外人物"等 panel，输出视频跟 grid 看起来"对不上"。

**端到端踩坑**（30s TVC G1）：grid 9 panel 跨 4-5 个不同场景（卧室/街景/门口/厨房/沙发），video 模型在 reference 模式下输出 12s 全部居家消毒场景，**完全跳过街景/门口/沙发的人物 panel**。修复：grid 改为"卧室 + 厨房 + 桌面"3 个连贯空间 + 9 panel 全部在该空间内不同机位；或在 30s TVC 拆段时把"户外人物"放到独立的 G2/G3（每段仍保持单场景多机位）。

**横跨场景的 TVC 拆段建议**：如果 brief 真的需要 4-5 个不同场景（卧室 + 街景 + 厨房 + 沙发），**不要塞进 1 张 grid**——按场景拆 2-3 张 grid（每张 9 panel = 1 段 12s 单场景多机位），30s TVC 拆 2-3 段视频拼接（详见 Part 六 桥接镜头法）。
...
在 agnes-image-2.5-flash 中使用 edit 模式并上传资产图：
- 第一层正常写全局风格
- 第二层映射资产图：`<Picture 1>女战士角色三视图, <Picture 2>雪山环境概念`
...
将视频按 12 秒为单位切割，每段对应一张 3x3 网格（**12s = agnes 单段上限**；30s TVC 拆 2×12s + 6s 收尾，60s 拆 5×12s，详见 SKILL.md TVC 标准时长规划表）。**每张 grid 独立选择剧情密度**。
...
在 agnes-image-2.5-flash 中使用 edit 模式，上传多宫格原图作为参考图。按阅读顺序逐格拆分。
...
| 网格 | 时段 | 类型 | 剧情密度 | 叙事功能 |
|------|------|------|---------|---------|
| G1 | 0-12s  | 品牌世界 Grid | 低密度 | 极限运动场景蒙太奇 |
| G2 | 12-24s | 产品世界 Grid | 低密度 | 产品多角度影棚展示 |
| G3 | 24-30s | keyframe 收尾（不计入 grid） | — | Hero Shot + End Frame 留白 |
...
**必须覆盖**：视觉风格、色彩基调、约束条件、**"无背景音乐"（必写）**、**沿<Picture 1>多宫格分镜设定构图与色彩，按<Picture 2>产品多视图还原主体外观**（grid + product 双图都必须在 prompt 显式引用，缺一个都会偏离还原）。

> **视频模型双图输入**：视频模型接收两张图片——多宫格分镜图（构图 / 色彩 / 镜头节奏锚点）+ 产品多视图（产品外观锚点）。**prompt 必须同时引用两张图**，典型结尾句式：`沿<Picture 1>多宫格分镜设定构图与色彩，按<Picture 2>产品多视图还原主体外观`。**`<Picture N>` 引用是图片与视频通用语法**——视频提示词同样适用，不是「图片生成阶段专用」。**reference 模式不锁首帧**，多张图作为视觉参考同时喂给视频生成模型。
...
**输出示例**：
> 风格：暗黑童话告别场景 / 红黑高对比森林色调 / 定格动画质感 / 强烈光源对冲构图（门内白光 vs 门外赤红森林）/ 细腻表情特写 / 情绪克制的离别氛围 / 无字幕 / 无背景音乐 沿<Picture 1>多宫格分镜设定构图与色彩，按<Picture 2>产品多视图还原主体外观
...
### 段间过渡手法（30s/60s TVC 多段拼接必读，2026-09 mooko.cn/article/52 实战）

短视频拼接最大的视觉事故是"突然切场"——前一段在桌面静物，下一段已经跳到户外，观众的视觉惯性被打断。**段间过渡手法**是为了让观众**感觉不到切场**，而不是为了"有视觉过渡"——过渡的最终目标是自然（不要刻意炫技）。

#### 1. 桥接镜头法（bridge shot，最常用）

**做法**：在每段（除最后一段）的**末尾预留 0.8-1.5s**（从 12s 中挤出），写一个**纯视觉过渡镜头**——不交代新信息，只承担「视觉交接」职能。

**三种子类型**：

| 子类型 | 写法 | 适用 |
|--------|------|------|
| **同构图推进** | 后段开头的构图 = 前段 bridge 的构图（景别、视角、产品位置一致），只换光线/背景 | 同一产品跨场景（如桌面静物→户外使用）|
| **时间流逝法** | 前段 bridge = 主体静止、光线缓慢变化（晨光→暮光 / 暖金→冷蓝）| 情绪短片、慢节奏叙事 |
| **尾帧延续法** | 前段最后 0.8s 固定一帧（产品 hero shot / 主体居中 / 浅景深），后段从这一帧位置开始运动 | 产品 TVC、强调品牌定格 |

**bridge 段在多宫格里的位置**：每张 grid 的 P9（右下角或最后一格）应是 bridge 镜头，承接下一段的 P1。例如 30s TVC 第二张 grid 的 P9 应与第三段（keyframe 收尾）的 first_frame 构图一致。

**bridge 段在视频提示词里的写法**：直接写在 Multi-Phase 模板的**最后一个 Phase**，时长 `0.8s-1.5s`，用 `[桥接·同构图推进]` / `[桥接·时间流逝]` / `[桥接·尾帧延续]` 标注。prompt 内容只描述视觉交接，不描述新事件。

#### 2. 风格锁定三句（每段必写，跨段一致）

每段视频提示词的「风格声明」区域 MUST 写**完全相同的 3 句**（不仅概念一致，而是字面复制粘贴）：

```
风格锁定三句（30s TVC 全段统一，照抄）：
1. 统一光色：暖金侧光为主，冷蓝阴影过渡，整体色温 5500K
2. 统一材质：哑光金属质感，浅景深柔焦背景，颗粒感高光
3. 统一运动：镜头运动速度 0.5x 慢速，主体微动，背景静谧
```

**为什么是三句不是一句**：单句（如"电影感"）AI 理解随机；三句各管一个维度（光色/材质/运动），跨段复制粘贴后才可能逼近"同一台摄影机拍出来"的效果。

**禁忌**：
- 不要写"延续前段风格"——AI 不读前段
- 不要写"整体电影感、高级感"——不可量化的形容词
- 不要漏写任何一句——三句全到位才锁定

#### 3. 负面提示词清单（每段必加，独立段落）

每段视频提示词末尾 MUST 加一个 `Negative Prompt` 段落，统一使用同一份清单。例：

```
Negative Prompt（30s TVC 全段统一）：
no text overlay, no watermark, no logo overlay, no UI elements,
no distortion, no morphing artifacts, no flicker, no jitter,
no extra fingers, no deformed hands, no uncanny valley face,
no BGM, no background music, no narration
```

**为什么必须有**：agnes-video-2.5-flash 默认倾向生成 UI/字幕/logo（水印），不主动排除就会出戏。"no BGM / no background music" 必加——后期铺 BGM 不能让 video 自己先有。

**清单最小集**（8 条，按重要度）：`text overlay, watermark, UI elements, distortion, flicker, extra fingers, BGM, background music`。最少 6 条，少于 6 条容易被模型"漏防"。

#### 4. 节奏比例（30s/60s 跨段节奏分配）

**三段比例 30 / 45 / 25**（建立 / 推进 / 收束）：

| 段 | 占比 | 秒数（30s TVC）| 秒数（60s TVC）| 作用 |
|----|------|---------------|---------------|------|
| **建立** | 30% | 0-9s | 0-18s | 品牌世界入场，氛围铺底 |
| **推进** | 45% | 9-22s | 18-45s | 产品世界展开，核心特征 |
| **收束** | 25% | 22-30s | 45-60s | Hero Shot + End Frame 留白 |

**不要平均用力**：30s TVC 用平均每段 10s = 12s 容量浪费 + 收束仓促。**推进段必须最长**（产品故事讲透），收束段次之（留白 + Logo），建立段最短（够建立氛围即可）。

**与桥接镜头法的关系**：bridge 不计入节奏比例——bridge 是"过渡设备"，叙事内容在建立/推进/收束三段里，bridge 是连接它们的 0.8-1.5s 视觉走廊。实际段时长 = 节奏段时长 - bridge 时长（如推进段 13s 中有 1s bridge）。
...
```
风格：科技极简 / 纯黑+珠光白+冰蓝呼吸光 / Low-key 棚拍 / 无背景音乐 沿<Picture 1>多宫格分镜设定构图与色彩，按<Picture 2>产品多视图还原主体外观
