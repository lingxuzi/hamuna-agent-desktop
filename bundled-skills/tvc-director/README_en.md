...
- **Copy-paste ready** — Output prompts are invoked directly through `mcp__multimedia-creator__agnes25_*` tools, no manual rework needed
...
### Step 2 — AI generates multiview via agnes-image-2.5-flash

The AI outputs product multiview prompts and invokes `mcp__multimedia-creator__agnes25_image_edit` (pass the reference image in `image_paths`) / `__image_generate` (text-only); local paths are auto-converted to base64, producing standardized multi-angle product shots:

<img src="https://github.com/user-attachments/assets/9d97df38-79f1-4a6a-a6bb-0fdbd731faca" width="600" alt="Product multiview" />

### Step 3 — AI generates 3×3 storyboard via agnes-image-2.5-flash

The AI outputs a 3×3 multi-grid storyboard prompt (with per-panel composition, lighting, and camera direction) and invokes `mcp__multimedia-creator__agnes25_image_generate`, passing the product multiview + environment images in `image_paths` as `<Picture 1>` `<Picture 2>` references:
...
|-------------|------|---------|
| Product multiview | `agnes25_image_edit` (with reference) / `__image_generate` (text-only) | Product anchor for subsequent steps |
| 3×3 storyboard grid | `agnes25_image_generate` (`image_paths` passes asset images) | Video first frame + visual QA |
| Multi-Phase video script | `agnes25_video_generate` (`mode="reference"`, `images[]` references assets) | Generate 15s final video |
...
│
├── assets/                         # Product asset prompts (agnes-image-2.5-flash)
│   └── prompts/
...
│       └── ...
│
├── keyframes/                      # Storyboard keyframe prompts (agnes-image-2.5-flash)
│   └── prompts/
...
1. **Product multiview** — AI invokes `mcp__multimedia-creator__agnes25_image_edit` (`image_paths` for reference image) or `__image_generate` (text-only); local paths auto-convert to base64
2. **Storyboard keyframes** — AI invokes `mcp__multimedia-creator__agnes25_image_generate`, passing product multiview + environment images in `image_paths` as `<Picture N>` references
3. **Video scripts** — AI invokes `mcp__multimedia-creator__agnes25_video_generate`, `mode="reference"` + `images=["<grid path>","<multiview path>"]`, referencing via `<Picture N>` in the prompt
