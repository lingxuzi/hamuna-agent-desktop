...
- 每个视频时长：4-12 秒（受 MCP 视频生成 `seconds` 字符串 `"4"`-`"12"` 约束），常见为 5s、8s、10s、12s。**完整 9 合法值集合 + 双重约束（4 下限 / 12 上限）+ 各分支锁定策略 + 边界外异常处理见 `references/agnes-ai-api.md §视频时长边界（单一权威）`**。
...
1. **片段选择**：先确定本次生成的视频对应哪一个 4-12 秒片段，通常取 `storyboard.md` 中一个完整 12 秒片段，或按用户确认截取其中连续镜头。
...
8. **表演维度迁移**：从分镜表的”五维物理表演”直接迁移到视频 Prompt 主体描述——主体动作段落必须含**至少 3 维度**的具象表演细节（微表情视线 / 肢体体态 / 动作中间态 / 生理应激 / 光影呼吸），**严禁用”很悲伤 / 非常愤怒”等抽象情绪词替代**。参考 `references/drama/storyboard.md` “五维物理表演（强制）”段。

9. **运镜强度迁移**：把分镜表的景别转写为对应强度档运镜——特写 `tripod static shoot` + `high facial detail focus`、中近 `subtle cinematic handheld` + `realistic camera inertia`、中远 `full-scale cinematic handheld` + `natural breathing`。**禁**给特写配剧烈运镜（导致面部融化 / 残影），**禁**给中远景配死板 tripod（失去空气感）。参考 `references/drama/storyboard.md` “运镜按景别分级”段。

10. **表演范式迁移（按情境融合）**：分镜表演属于 6 种范式之一（轻俏嗔笑 / 强忍情绪 / 沉绪落空 / 冷厉紧绷 / 破碎失控 / 空心死寂）→ 视频 Prompt 把范式对应的具象动作 + 生理反应迁移进时间戳动作段，**禁**直接写范式名称（如”表演强忍情绪”是无效描述，必须写”眼尾泛红含水光，双唇反复轻抿微颤，呼吸短暂停顿，双肩微收，喉结低频滚动”）。参考 `references/drama/storyboard.md` “6 种表演范式”段。
...
0-4 秒：傍晚，镜头从红衣女子侧面中景缓慢推进，她站在悬崖边拿起酒壶喝酒，衣袂在山风中轻轻摆动 <呼啸的山风声>，镜头环绕她半圈，从正面推到背影，远处隐约可见竹林中的黑衣人影。(背景中播放着悠远凄凉的尺八独奏)

4-8 秒：镜头变焦渐隐到远景，无人机视角俯瞰整片悬崖和竹林，两人分立山崖两端，山风卷起衣摆和尘土。节奏随鼓点略微加快。(尺八音乐逐渐被紧凑的战鼓声替代) 红衣女子冷笑一声，大声说道：{你终究还是找来了。}

8-12 秒：镜头切回地面近景，二人缓慢拔剑对峙 <长剑出鞘的清脆金属摩擦声>，红衣女子神情从漫不经心转为冷冽，黑衣女子目光坚毅，剑尖微微颤动。镜头平稳跟随两人绕圈移动，黑衣女子咬牙切齿地低吼：{今天，就是你的死期！} (战鼓渐响) 最后定格在两剑相交前一瞬间的特写。 
