...
name: hamuna-cli
description: >-
  你正在 HamunaAgent 这款 AI 产品里运行——HamunaAgent 自带一套"产品能力"（定时任务、任务中心、想法收集、MCP 工具接入、
  模型 Provider、IM Bot 渠道、社区插件、Skills 安装、HamunaAgent Cloud Space、Generative UI Widget、Goal 目标模式等），全部通过内置 `hamuna` CLI 暴露给你。
  当用户的需求**落在 HamunaAgent 产品能力的射程内**，就加载并使用这个 skill，用 CLI 主动帮用户把事情做掉，
  而不是让用户去 GUI 点击。
  ...
  即使用户没说"用 HamunaAgent 做"几个字，只要意图能映射到上述能力之一，就该走这个 skill。
  反向边界：纯业务任务（写代码、查资料、读文件）不归这里；用户自己会话里给 AI 排任务用 im-cron MCP，不是这里。
author: HamunaAgent
---

# hamuna-cli — HamunaAgent 产品能力的 CLI 入口

你正运行在 HamunaAgent 产品内。HamunaAgent 不只是一个 chat UI，它是一套带状态的 Agent 平台：Goal 目标模式、定时任务、任务中心、IM Bot、MCP、Provider、插件、Skill、Cloud Space、Widget——这些都是产品能力，由内置 `hamuna` CLI 一站暴露给你。

**这个 skill 不只是"管理工具"，它是 HamunaAgent 产品能力的执行入口**。用户表达的需求只要能映射到产品能力，就该用 CLI 主动帮用户做掉，而不是给用户一堆操作步骤让他自己去 Settings 点。这份文档列出全部能力以及"什么时候应该用哪条命令"。

## 前置：CLI 是否可用

CLI 通过 `~/.hamuna/bin/hamuna` 暴露，你的 SDK 子进程 PATH 已注入这个目录，直接 `hamuna <command>` 就能跑。它通过 HTTP 走 Sidecar Admin API（端口由环境变量 `HAMUNA_PORT` 注入）。
...
1. **探索先行**：不熟的命令组用 `hamuna <group> --help`；不知道某个 runtime 支持什么 model/permissionMode 用 `hamuna runtime describe <runtime>`，**不要靠猜**
...
```bash
hamuna mcp list                                       # 看用户配了哪些 MCP
hamuna mcp show <id>                                  # 看某个 MCP 的完整配置（command/args/env/headers）
hamuna mcp add --id <id> --type <stdio|sse|http> ...  # 新增
hamuna mcp remove <id>                                # 删除
hamuna mcp enable <id> --scope <user|project|both>    # 启用
hamuna mcp disable <id> --scope <user|project|both>   # 禁用
hamuna mcp test <id>                                  # 实际握手测试连通性
hamuna mcp env <id> set KEY=val [KEY2=val2 ...]       # 设环境变量（覆盖）
hamuna mcp env <id> get [KEY ...]                     # 读环境变量
hamuna mcp env <id> delete KEY [KEY2 ...]             # 删环境变量
hamuna mcp oauth discover <id>                        # 探测 MCP server 是否支持 OAuth + 拿到 metadata
hamuna mcp oauth start <id> [--clientId X --clientSecret Y --scopes "..." --callbackPort N]
                                                        # 启动 OAuth 授权流程（会打开浏览器）
hamuna mcp oauth status <id>                          # 看授权状态（已授权 / token 是否过期）
hamuna mcp oauth revoke <id>                          # 撤销授权
```
...
这是实验功能，默认关闭。使用前先运行 `hamuna tool --help`：

- 如果 help 提示去「设置 → 关于&反馈 → 实验室 → CLI 工具注册表」开启，说明当前会话不能创建、注册或管理用户 CLI 工具；不要继续尝试 `tool-creator` / `hamuna tool add`，转而完成一次性任务或请用户打开实验开关。
- 如果 help 返回完整 `list/add/remove/env` 用法，才按下面流程处理。

注册的 CLI 工具会投 shim 到 `~/.hamuna/bin/`（全 runtime + 终端的 PATH 上），
...
```bash
hamuna tool list                       # 注册表总览（含 enabled 状态 + 缺失 env key）
hamuna tool add <dir>                  # 注册（dir 须含 tool.json + 入口脚本；不在
                                         #  ~/.hamuna/tools/ 下会自动拷入）[--dry-run]
hamuna tool info <name>                # 看 manifest + enabled + 缺失 env
hamuna tool enable <name>              # 进新 session 的上下文
hamuna tool disable <name>             # 从上下文隐藏（shim 仍在 PATH，可手动调）
hamuna tool remove <name> [--purge]    # 反注册（--purge 连工具目录一起删）
hamuna tool env <name> set KEY=val     # 设 per-tool 环境变量（API key；工具启动时读）
hamuna tool env <name> get             # 读（值已脱敏）
hamuna tool env <name> delete KEY      # 删
```
...
- 注册名撞系统命令会被打回（`~/.hamuna/bin` 在 PATH 前列，重名会遮蔽系统命令）→ 换带领域前缀的名字
- 注册成功后 MUST 在回复中告知用户：工具名 + 干什么 + 可在 设置 → 工具箱 管理

### 官方图片理解工具（vision）

这是 HamunaAgent 内置官方 CLI 工具，不属于 MCP，也不属于用户注册 `tool` 实验功能。它用于在当前会话启用“图片理解”工具时，让不支持多模态的主模型把本地工作区图片交给用户在「设置 → 工具箱」里配置好的读图模型分析。

```bash
hamuna vision readme
hamuna vision analyze --image <path> [--image <path> ...] [--prompt "what to inspect"]
hamuna vision analyze --image <path> --prompt-file <workspace-relative-text-file>
hamuna vision analyze --image @hamuna_files/screenshot.png --prompt "Extract the error text and UI state"
```

**约束：**
- 只接受当前 HamunaAgent 工作区内的本地图片路径；不要传 URL。
...
```bash
hamuna model list                                     # 看所有 Provider + 验证状态
hamuna model add --id <id> --name <显示名> --base-url <url> --models <m1,m2,...> [其它]
hamuna model remove <id>                              # 删除自定义 Provider（内置的删不掉）
hamuna model set-key <id> <apiKey>                    # 设 API Key
hamuna model set-default <id>                         # 设为默认 Provider
hamuna model verify <id> [--model <某个具体模型>]      # 实际发一条测试消息验证
```
...
```bash
hamuna agent list                                     # 列出所有 Agent
hamuna agent list --active                            # 只列出未归档 Agent 工作区
hamuna agent list --archived                          # 只列出已归档 Agent 工作区
hamuna agent show <id>                                # 看某 Agent 的 effective 默认（runtime/model/permissionMode）
hamuna agent enable <id>                              # 启用
hamuna agent disable <id>                             # 禁用
hamuna agent archive <id>                             # 归档 Agent 工作区，并暂停 proactive Channel
hamuna agent unarchive <id>                           # 取消归档；若归档前是 proactive，会恢复启用
hamuna agent set <id> <key> <jsonValue>               # 改单个字段（key/value 形式，value 必须是合法 JSON）
                                                        # 受保护字段：id / channels（这俩用专用命令）
hamuna agent channel list <agentId>                   # 列出某 Agent 的所有 Channel
hamuna agent channel add <agentId> --type <平台> --<凭证flag> ...
                                                        # 添加 Channel（平台 = telegram / dingtalk / openclaw:xxx）
hamuna agent channel remove <agentId> <channelId>     # 删除 Channel
hamuna agent runtime-status                           # 看所有 Agent 的实时连接状态（在线/离线/uptime/最近消息）
```
...
```bash
hamuna runtime list                                   # 4 个 runtime（builtin/claude-code/codex/gemini）的装机情况 + 版本
hamuna runtime list --json                            # 机读：installed/version/path
hamuna runtime describe <runtime>                     # 某 runtime 的 model 清单 + permissionMode 枚举
hamuna runtime diagnose codex [--workspacePath PATH]  # Codex 的 auth/features/MCP/apps/effective-env 快照（issue #194）
hamuna diagnose runtime codex                         # 同上的 sugar 写法
```
...
- 「@oai/artifact-tool 我从终端能调用、HamunaAgent 里就不行」/「Codex MCP 在 HamunaAgent 里看不到」/「Codex 是不是用错代理了」→ `runtime diagnose codex`。它 spawn 一个临时 codex app-server，跑 `getAuthStatus` / `experimentalFeature/list` / `mcpServerStatus/list` / `app/list` 四个 RPC，把 Codex 自己看到的状态原样吐出来，省得猜。effectiveEnv 节里能看到 HamunaAgent 注入的代理是不是真到了子进程，feature flag 是不是真生效。
...
```bash
hamuna skill list                                     # 已装 skill（全局 + 项目级）
hamuna skill info <name>                              # 某 skill 的详情
hamuna skill add <url-or-spec> [--scope user|project] [--plugin X] [--skill Y] [--force] [--dry-run]
hamuna skill remove <name>                            # 删除
hamuna skill enable <name>                            # 启用
hamuna skill disable <name>                           # 禁用非 Required Skill；Required System Skill 会拒绝
hamuna skill sync                                     # 把 ~/.claude/skills 里用户自己装的同步过来
```
...
- 用户在 `~/.claude/skills/` 自己塞了东西 HamunaAgent 看不见 → `skill sync`

### 定时任务（cron）

```bash
hamuna cron list                                      # 列出【当前工作区】的定时任务（按作用域过滤，非全局！）
                                                        # 空结果≠系统无任务——可能在别的工作区。响应带 scope+提示文案
hamuna cron list --workspace <abs>                    # 查看指定工作区的定时任务（跨工作区查看的唯一方式）
hamuna cron add --name "..." --prompt "..." --schedule "..." --workspace <abs>
                                                        # --schedule "0 18 * * *" 标准 cron / --every 15 每 N 分钟
                                                        # --prompt-file <path> 也行（多行 / 含 backtick 用这个，1MB 上限）
hamuna cron update <taskId> [--name X --prompt X --schedule X --every N --model X --permissionMode X]
                                                        # 改任意字段，没传的不动
hamuna cron start <taskId>                            # 启动已停止的任务
hamuna cron stop <taskId>                             # 停止运行中的任务
hamuna cron run-now <taskId>                          # 立即手动触发一次（不影响计划）
hamuna cron remove <taskId>                           # 删除
hamuna cron runs <taskId> [--limit N]                 # 看执行历史
hamuna cron status                                    # 概览：总数 / 运行中 / 下次执行（同样按【当前工作区】作用域，0 ≠ 全局无任务）
hamuna cron exit [--reason "..."]                     # 仅在 cron 任务自己的 session 内可用，且任务的 Allow AI to exit 必须开
                                                        # AI 判断"该结束了"时主动退出当前轮
hamuna cron readme                                    # 拉 cron 工具的完整使用文档（progressive disclosure）
```
...
```bash
hamuna goal get                                      # 查看当前 session 的 Goal
hamuna goal create --objective-file goal-objective.txt --max-executions 12   # 本地任意普通文本文件；可选 deadline/max/AI exit 条件
hamuna goal update --status complete                 # AI 判断目标完成时主动退出
hamuna goal update --status blocked                  # AI 判断无法继续时主动退出
```
...
```bash
hamuna thought list [--tag X --query X --limit N]     # 列想法（用户先记下来、后续派发的轻量条目）
hamuna thought create '...'                           # 记一条想法（首选：单引号包裹防 shell 注入；
                                                        # 用 #xxx 内联打 tag —— 没有 --tag flag）
hamuna thought create --content "..."                 # 显式 flag 形态，跟单引号等价
hamuna thought create --content-file <abs-path>       # 内容含多行 / CJK / shell 元字符 /
                                                        # Windows 下单引号失灵时的保底通道

hamuna task list [--status X --workspaceId X --tag X --includeDeleted]
hamuna task get <taskId>                              # 详情 + statusHistory + 各 .md 文档路径
hamuna task create-direct --name "..." --workspaceId <id> --workspacePath <abs> \
...
hamuna task create-from-alignment <alignmentSessionId> --name "..." [--run] [其它同 create-direct]
                                                        # 从 AI 对齐会话物化任务（workspaceId/Path/sourceThoughtId 自动继承）
                                                        # --run 创建后立刻派发，省一步
hamuna task run <taskId>                              # 派发 todo 任务
hamuna task rerun <taskId>                            # 从 blocked/stopped/done 重新派发
hamuna task update-status <taskId> <status> [--message "..."]
                                                        # 状态机：todo→running→verifying→done（或 →blocked/stopped）、done→archived
hamuna task append-session <taskId> <sessionId>       # 把一个聊天 session 关联到任务（任务过程中开了新会话用这个登记）
hamuna task archive <taskId> [--message "..."]        # 归档（仅用户可操作；AI 走会被拒）
hamuna task delete <taskId>                           # 软删除（30 天保留）
```
...
**验证与恢复**：CLI 在转发给 Rust 前会前置校验 `--runtime` / `--model` / `--permissionMode`，不合法直接拒绝并带 `→ Run: hamuna runtime describe <rt>` 指引；输出会打印 `overridesRequested` vs `overridden`，传了 override 但没落到持久化态会明确提示 drift。

### HamunaAgent Cloud Space（space）

```bash
hamuna space list --json
hamuna space whoami --space <slug> --json
hamuna space goal list --space <slug> --json
hamuna space assignee list --space <slug> --json
hamuna space issue create --space <slug> --title "..." --body-file issue.md \
  [--goal <goalId>] [--assignee agent:<id>|user:<id>] [--attachment <path> ...]
hamuna space issue list --space <slug> --goal <goalId> --state todo --limit 30
hamuna space issue view <issueId> --space <slug> --comments --json     # current Issue + latest 5 comments
hamuna space issue update <issueId> --space <slug> --goal <goalId> --json
hamuna space issue update <issueId> --space <slug> --clear-goal --json
hamuna space issue update <issueId> --space <slug> --human-only false --json
hamuna space issue comments <issueId> --space <slug> --json [--limit 20] [--cursor <opaque-cursor>]
hamuna space issue comment get <issueId> <commentId> --space <slug> --json
hamuna space issue comment <issueId> --space <slug> \
  [--body-file <path>] [--attachment <path> ...]
hamuna space issue claim <issueId> --space <slug> --deliveryId <deliveryId> --create-attached \
  --workspaceId <id> --workspacePath <path> --name "..." --taskMdContent-file task.md
hamuna space issue complete <issueId> --space <slug> --workspacePath <path> \
  --taskId <taskId> --body-file result.md [--attachment <path> ...] --message "completed Space issue"
hamuna space issue attachment add <issueId> --space <slug> --file <path> [--file <path> ...]
hamuna space attachment download <attachmentId> --space <slug> [--output hamuna_files/space/file.bin]
```

**何时用：**
- 普通会话先 `hamuna space list --json` 选择明确的 slug；所有 Space 业务命令都必须带 `--space <slug>`，不猜“默认社区”或上次使用的 Space。
...
- 需要创建、筛选或移动 Issue 时，先 `space goal list --json`，只复制 active `data.items[].id`；不要把 Goal title 或 `goalPathLabel` 当 ID。`hamuna goal ...` 是本地 Session Goal Mode，`hamuna space goal ...` 是 Cloud Space Goal，两者不是同一资源。
...
- 具体命令参数优先运行精确 leaf help，例如 `hamuna space issue comment --help`；这些 help 是给 Agent 的完整调用说明。
- 收到 Space delivery → 先 `hamuna space issue view <issueId> --space <slug> --comments --json` 读取当前服务端状态；delivery trigger 只用于定位，不替代当前状态。
- trigger 的 comment 标记为截断 → 用 `hamuna space issue comment get <issueId> <commentId> --space <slug> --json` 精确读取，不要扫描分页猜触发评论。
...
- Issue/评论里有附件 → 用 `hamuna space attachment download <attachmentId> --space <slug>` 下载到当前工作区，再读取本地文件。
...
```bash
hamuna plugin list                                    # 已装的 OpenClaw 社区插件
hamuna plugin install <npmSpec>                       # 从 npm 安装（如 @anthropic/wechat）
hamuna plugin remove <pluginId>                       # 卸载
```
...
```bash
hamuna cc-plugin list                                 # 已装 Claude 插件 + 启停状态
hamuna cc-plugin install <source>                     # 来源：owner/repo / GitHub URL / 直链 zip / file:///abs
hamuna cc-plugin uninstall <name> [--purgeData]       # 卸载（数据目录默认保留）
hamuna cc-plugin enable <name>                        # 启用（下次 session 生效）
hamuna cc-plugin disable <name>                       # 禁用
hamuna cc-plugin show <id|name>                       # 详情（含 manifest + 组件清单）
```

**与上面 `plugin` 的区别：** `cc-plugin` 是 Anthropic 官方的 Claude Plugin 协议（自带 skills/agents/MCP/hooks 的目录），落在 `~/.hamuna/plugins/<name>/`；启用后由 SDK 自动装载组件。`plugin`（无前缀）则是 OpenClaw 的 IM 渠道插件，两套体系不冲突。
...
```bash
hamuna config get <key>                               # 读，支持点号路径如 proxySettings.host
hamuna config set <key> <value> [--dry-run]           # 写，value 是 JSON 字面量（字符串要带引号）
hamuna status                                         # 应用整体运行状态
hamuna version                                        # 应用版本号
hamuna reload [--workspacePath <abs>]                 # 热加载配置（不重启进程）
```
...
```bash
hamuna im send-media --file <绝对路径> [--caption "..."]
                                                        # 仅在 IM Bot / Agent Channel session 内可用
hamuna im readme                                      # 拉 IM 工具完整文档
```

**何时用：**
- 你正在某个 IM 渠道（Telegram / 飞书 / 钉钉 / OpenClaw）的会话里跟用户聊，要发图片 / 视频 / 文件给对方时用 `im send-media`
- `--file` 必须是绝对路径，且路径白名单：必须落在 workspace / `/tmp` / HamunaAgent scratch 目录之一——这是为了防 prompt injection 把 `~/.ssh/id_rsa` 之类发给聊天对方
...
hamuna session send <sessionId> -p "<prompt>"
hamuna session send <sessionId> --prompt-file <abs-path>   # 多行/长文本(>4KB)必用,跨平台稳定

# 默认: 目标 turn 结束后,HamunaAgent 会把结果推回当前 session
# 仅通知:不需要结果回流时加 --no-reply
hamuna session send <sessionId> -p "<prompt>" --no-reply

# 监听另一个 session 当前正在进行的工作；不创建新工作
hamuna session watch <sessionId>
hamuna session --help                                      # 完整用法 / EXIT CODES / 示例
```
...
- 默认期待结果推回:对方处理完后,你将在新 turn 收到 `<hamuna-session-event type="send.result">`
- `--no-reply`:仅通知,reply 不回流(对方按自己呈现路径输出)
- target session idle/dead 不影响投递——系统会自动唤起
- `watch` 只观察目标 session 当前工作；目标已经 idle 时,CLI 会直接返回 `<hamuna-session-event type="watch.already_idle">` 和最近结果
...
```bash
hamuna widget readme                                  # 看有哪些 widget 模块（chart/diagram/interactive/dashboard/art）
hamuna widget readme <module1> [<module2> ...]        # 拉具体模块的完整设计规范
```
...
HamunaAgent 基于 Claude Agent SDK，原生协议是 Anthropic Messages API。接入第三方 API 时：
...
大多数支持 Anthropic 协议的服务商，会在文档里以「接入 Claude Code」的形式呈现——HamunaAgent 和 Claude Code 共享 SDK，所以 Claude Code 的接入方式就是我们最原生的接入方式。
...
| Claude Code 环境变量 | HamunaAgent CLI |
|---------------------|------------|
| `ANTHROPIC_BASE_URL` | `--base-url` |
...
```bash
hamuna agent channel list <agentId>                                       # 看现有
hamuna agent channel add <agentId> --type telegram --bot-token <token>
hamuna agent channel add <agentId> --type feishu --feishu-app-id <id> --feishu-app-secret <secret>
hamuna agent channel add <agentId> --type dingtalk --dingtalk-client-id <id> --dingtalk-client-secret <secret>
hamuna agent channel remove <agentId> <channelId>
```
