...
name: hamuna-docs
description: >-
  HamunaAgent 产品使用知识库与用户可见行为说明。用户或 Agent 只要需要了解 HamunaAgent 是什么、能做什么、某项功能怎么用、
  功能入口或前置条件、功能之间的区别与关系、正确情况下应该出现什么结果、有哪些限制或常见误解，就使用这个 skill；
  也适用于在任意工作区规划怎样借助 HamunaAgent 的 Workspace、Session、Task、Goal、Provider、Runtime、MCP、Skill、Agent Channel、
  Cloud Space 等能力完成工作。它面向软件使用而非源码开发。需要查询或修改当前实例状态时再加载 `/hamuna-cli`；
  已经出现报错、崩溃或实际行为偏离预期时，在内置小助理中转入 `/support`。
---

# HamunaAgent 产品使用知识库

这个 Skill 是 HamunaAgent 产品行为的知识入口。它帮助用户理解和使用软件，也帮助 Agent 建立正确的产品心智模型；它不是源码架构文档、命令参数大全或故障日志手册。
...
| 设计一套 HamunaAgent 使用流程 | 读取涉及的 references，先说明各能力的职责，再组合工作流 |
| 希望你查询现场状态或代为操作 | 用本 Skill 确认语义；随后加载 `/hamuna-cli`，以 CLI 的实时 help 和返回值为准 |
| 已按正确方式操作，但报错、崩溃、无响应或结果不符合预期 | 在内置小助理里加载 `/support`；先用本 Skill 确认预期，再用本地证据诊断 |
| 要修改 HamunaAgent 源码或评审实现架构 | 如果当前工作区是 HamunaAgent 仓库，改读仓库 `CLAUDE.md`、`specs/ARCHITECTURE.md` 与相关 `tech_docs/`；本 Skill 不是开发者文档 |
...
| 用户在问什么 | 读取 |
|---|---|
| HamunaAgent 的定位、能力地图、主要入口、应该选哪类功能 | `references/product-overview.md` |
...
5. 把稳定的产品契约和当前实例状态分开。功能是否存在、当前有哪些模型、某开关是否开启等现场值，使用 `/hamuna-cli` 或 UI 查询，不凭静态文档猜。
