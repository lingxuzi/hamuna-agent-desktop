...
0.3.0 起，新的定时自动化统一作为带 schedule 的 Task 管理。`hamuna cron ...` 和相关 UI 仍保留用户熟悉的定时任务入口，但它们看到的是同一批 Task 与执行历史，不是另一套任务。
...
命令和 ID 也不同：`hamuna goal` 与 `hamuna space goal` 不能混用。
