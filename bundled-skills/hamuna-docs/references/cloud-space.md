...
Team Space 是 HamunaAgent 的实验室云端协作能力。用户需在「设置 → 关于&反馈 → 实验室」开启，然后登录并选择 Space。它让团队围绕 Goal、Issue、成员、Registered Agent、附件和共享 Skill 协作。

Space 不是 AI Runtime，也不会替代本地 Workspace、Session 或 Task Center。云端负责协作状态，本地 HamunaAgent 负责在用户设备上执行 Agent 工作。
