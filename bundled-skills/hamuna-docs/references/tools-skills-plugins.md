...
| 官方 CLI Tool | HamunaAgent 内置、跨 Runtime 可调用的产品工具，例如图片理解 |
...
HamunaAgent 支持 STDIO、HTTP、SSE 类型的 MCP，也带有若干预置或内置能力。MCP 可以在全局启用，也可以针对 Workspace 启用。
...
外部 Runtime 不一定消费 HamunaAgent MCP，具体见 `models-providers-runtimes.md`。

## Skills

Skill 是给 Agent 的可复用工作方法，核心是 `SKILL.md`，还可以带 references、scripts 和 assets。HamunaAgent 支持：
...
`hamuna` 是产品管理入口，用于查询和操作 Provider、MCP、Task、Agent、Space 等能力。它不是用户 CLI Tool 注册表的一部分。
...
用户可以把符合 Agent-CLI 契约的非交互工具注册到 HamunaAgent。启用后，新 Session 能看到工具描述并从 PATH 调用。该能力受「CLI 工具注册表」实验开关控制；关闭时不会自动发现或管理用户工具，但官方 `hamuna` CLI 不受影响。
...
遵循 Anthropic Claude Plugin 目录协议，可以包含 Skills、Agents、MCP 和 Hooks。HamunaAgent 在 builtin Runtime 中提供这套集成；启停后通常在下一次 Session 重启或发消息时生效。外部 Claude Code/Codex/Gemini Runtime 各自管理自己的插件体系，不自动读取 HamunaAgent Claude Plugin。
...
- 媒体画廊直接出现在工具结果后
- 产物保存到 Workspace 的 `hamuna_files/<tool>/`
...
- “CLI 工具注册表关了，`hamuna` 也不能用”：官方 CLI 始终可用。
