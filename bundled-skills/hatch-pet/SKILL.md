---
name: hatch-pet
description: Create, repair, validate, visually QA, and package HamunaAgent/Codex-compatible animated pets and pet spritesheets from character art, generated images, company or prospect brand cues, or visual references. Use when a user wants a lightweight-worker desktop pet workflow, a non-pixel custom pet style, a prospect or company mascot pet, or a full 8x9 animated pet atlas with transparent unused cells, QA contact sheets, and pet.json packaging. This skill composes the installed $imagegen system skill for visual generation and uses bundled scripts for deterministic spritesheet assembly.
---
...
The built-in `$imagegen` path stores generated PNG bytes in the rollout that invokes it, even when it also writes a file under `${CODEX_HOME:-$HOME/.codex}/generated_images` or `${HAMUNA_HOME:-$HOME/.hamuna}/generated_images`. Deleting files later reduces filesystem use, but it does not shrink an already-written rollout. Keep image generation isolated and bounded:
...
- After copying the selected generated output into `decoded/`, remove the selected original from `${CODEX_HOME:-$HOME/.codex}/generated_images` or `${HAMUNA_HOME:-$HOME/.hamuna}/generated_images` when it lives there, then remove its now-empty generation directory if possible.
...
For a normal pet run, expect up to 10 visual generation jobs: 1 base pet plus 9 row-strip jobs. The HamunaAgent/Codex pet contract currently uses all 9 states: `idle`, `running-right`, `running-left`, `waving`, `jumping`, `failed`, `waiting`, `running`, and `review`. The only deterministic visual derivation is `running-left`, which may be produced by mirroring `running-right` only after `running-right` has been generated, visually inspected, and explicitly approved as safe to mirror. If mirroring is not appropriate, generate `running-left` as a normal grounded `$imagegen` row.
...
```bash
HAMUNA_HOME="${HAMUNA_HOME:-$HOME/.hamuna}"
SKILL_DIR="${HATCH_PET_SKILL_DIR:-$HAMUNA_HOME/skills/hatch-pet}"
...
```bash
for GENERATED_ROOT in "${HAMUNA_HOME:-$HOME/.hamuna}/generated_images" "${CODEX_HOME:-$HOME/.codex}/generated_images"; do
  case "$SOURCE" in
    "$GENERATED_ROOT"/*)
      rm -f "$SOURCE"
...
Package output is written outside the run directory by default. In HamunaAgent, write custom pets to `${HAMUNA_HOME:-$HOME/.hamuna}/pets` so the desktop pet picker can load them directly. The same package shape is compatible with Codex if copied under `${CODEX_HOME:-$HOME/.codex}/pets`.
...
HAMUNA_HOME="${HAMUNA_HOME:-$HOME/.hamuna}"
PET_DIR="$HAMUNA_HOME/pets/$PET_ID"
mkdir -p "$PET_DIR"
...
- verify all 9 rows match the HamunaAgent/Codex pet state contract and the same pet identity
...
- Row-by-row review confirms the animation cycles are complete enough for the HamunaAgent/Codex pet contract.
...
- `${HAMUNA_HOME:-$HOME/.hamuna}/pets/<pet-name>/pet.json` and `${HAMUNA_HOME:-$HOME/.hamuna}/pets/<pet-name>/spritesheet.webp` are staged together for custom pets.
