#!/usr/bin/env python3
"""Create a HamunaAgent/Codex-compatible pet run folder, prompts, and imagegen job manifest."""
...
def infer_description(args: argparse.Namespace, reference_paths: list[Path]) -> str:
    ...
    if args.pet_notes.strip():
        return sentence(f"A compact HamunaAgent/Codex-compatible pet: {args.pet_notes}")
    if args.brand_name.strip():
        return sentence(f"A compact HamunaAgent/Codex-compatible pet inspired by {args.brand_name}")
    if reference_paths:
        return "A compact HamunaAgent/Codex-compatible pet based on the provided reference image."
    return "A compact original HamunaAgent/Codex-compatible pet ready for animation."
...
def infer_pet_notes(args: argparse.Namespace, reference_paths: list[Path]) -> str:
    ...
    return "a compact original HamunaAgent/Codex-compatible pet"
...
def base_pet_prompt(args: argparse.Namespace) -> str:
    ...
    return f"""Create one clean full-body reference sprite for HamunaAgent/Codex-compatible pet {args.display_name}.
...
def row_prompt(
    ...
    return f"""Create one horizontal animation strip for HamunaAgent/Codex-compatible pet `{args.pet_id}`, state `{state}`.
...
def retry_row_prompt(
    ...
    state_prompt = STATE_PROMPTS[state]
    state_requirements = "\n".join(f"- {line}" for line in STATE_REQUIREMENTS[state])
    return f"""Create HamunaAgent/Codex-compatible pet row `{state}` for `{args.pet_id}`: exactly {frames} full-body frames in one horizontal strip on flat pure {chroma_name} {chroma_key}.
