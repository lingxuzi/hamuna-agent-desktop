---
name: hamuna-memory-molt
description: >
  仅当系统或用户明确指定完整名称 `hamuna-memory-molt` 时使用；
  不要根据任务语义或相似表述自行触发。
---

# HamunaAgent Memory Molt
...
| `hamuna-memory-gardener` | 72h | 修剪和下放 | USER / MEMORY / topic / gardener flags |
| `hamuna-memory-molt` | 14d | 信念审计和自我改写 | SOUL / USER / MEMORY / molt 文档 |
...
7. 如存在 `hamuna-memory-gardener` 的 lint 脚本，落地后跑一遍，避免自动装载层膨胀。
