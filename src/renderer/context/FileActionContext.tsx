...
interface FileActionProviderProps {
  ...
  /** Floating-ball only: raise HamunaAgent, focus the session tab, and open the
   *  given workspace-relative file in the main preview surface. */
  onOpenHamunaAgentPreview?: (
    path: string,
...
export function FileActionProvider({ children, workspacePath, onInsertReference, refreshTrigger, onFilePreviewExternal, onQuoteFile, onQuoteSelection, onRevealInTree, menuProfile = 'default', onOpenHamunaAgentPreview }: FileActionProviderProps) {
  ...
  const onOpenHamunaAgentPreviewRef = useRef(onOpenHamunaAgentPreview);
  onOpenHamunaAgentPreviewRef.current = onOpenHamunaAgentPreview;
  ...
  const openFileLink = useCallback((href: string): boolean => {
    ...
    void (async () => {
      ...
      if (menuProfile === 'floatingBall' && target.scope === 'workspace' && onOpenHamunaAgentPreviewRef.current) {
        const fileName = targetFileName(target.path);
        if (pathInfo.type === 'file' && (isPreviewable(fileName) || !!getRichDocKind(fileName))) {
          onOpenHamunaAgentPreviewRef.current(target.path, {
            displayPath: href,
  ...
  const handleOpenHamunaAgentPreview = useCallback((path: string, displayPath?: string, initialLineNumber?: number): void => {
    onOpenHamunaAgentPreviewRef.current?.(path, initialLineNumber
  ...
  const menuItems = useMemo((): ContextMenuItem[] => {
    ...
    if (menuProfile === 'floatingBall') {
      const canOpenHamunaAgentPreview =
        scope === 'workspace' &&
        pathType === 'file' &&
        !!onOpenHamunaAgentPreviewRef.current &&
      ...
      return [
        ...
        {
          label: t('fileActions.openHamunaAgentPreview'),
          icon: <PanelRightOpen className="h-4 w-4" />,
          disabled: !canOpenHamunaAgentPreview,
          onClick: () => handleOpenHamunaAgentPreview(path, displayPath, initialLineNumber),
        },
      ];
    }
    ...
    return items;
  }, [menuState, menuProfile, t, handlePreview, handleCopyPath, handleReference, handleOpenWithDefault, handleOpenInFinder, handleRevealInTree, handleOpenHamunaAgentPreview]);
