...
export async function ensureManagedCodexProviderDevGateDefault(): Promise<void> {
    if (isBrowserDevMode()) {
        ...
        try {
            const stored = localStorage.getItem('hamuna:config');
        ...
        localStorage.setItem('hamuna:config', JSON.stringify({
            ...normalized,
 ...
 */
export const CONFIG_CHANGED_EVENT = 'hamuna:config-changed';
...
/**
 * Ensure ~/.hamuna is registered as an internal project. Called on-demand when user triggers bug report.
 *
...
export async function ensureSelfAwarenessWorkspace(
    ...
    try {
        ...
        if (project && !project.internal) {
            await patchProject(project.id, { internal: true, name: 'HamunaAgent 诊断' });
            // patchProject updates both disk and React state; use the patched fields locally
            project = { ...project, internal: true, name: 'HamunaAgent 诊断' };
        }
        return project ?? null;
    } catch (err) {
