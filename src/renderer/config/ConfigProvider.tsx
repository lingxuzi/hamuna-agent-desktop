...
import { getNxgdAuthState } from './services/nxgdSubscriptionService';
import {
    getAllProviders,
...
export function ConfigProvider({ children }: { children: React.ReactNode }) {
    ...
    // Stamp `verifyStatus[nxgd] = 'valid'` once server-side nxgd auth is
    // registered. Chat's subscription provider filter (`isProviderAvailable`)
    // gates on `verifyStatus[nxgd].status === 'valid'` for built-in
    ...
    // The cheapest reliable signal is `/api/nxgd/auth/state` (server holds
    // module-level preload state from `preloadNxgdAuth`). We don't auto-
    // populate `provider.models` here — `models: []` is the preset's natural
    // empty default; the user picks a model via Settings → Manage Models or
    // the first-run wizard's step 2 picker, both of which write to
    // `presetCustomModels[nxgd]` and stamp valid on the way out.
    useEffect(() => {
        const nxgdSource = rawProviders.find(provider => provider.id === 'nxgd');
        if (!nxgdSource) return; // nxgd removed by user → nothing to verify
        let cancelled = false;
        getNxgdAuthState()
            .then(async (state) => {
                if (cancelled) return;
            ...
            .catch((err) => {
                if (cancelled) return;
                console.warn('[nxgd] failed to read auth state', err);
            });
        return () => { cancelled = true; };
    }, [rawProviders]);
    ...
    useEffect(() => {
        ...
        window.addEventListener('hamuna:config-changed', handler);
        return () => window.removeEventListener('hamuna:config-changed', handler);
    }, [refreshConfigFromDisk]);
