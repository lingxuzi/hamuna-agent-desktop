...
const floatingMocks = vi.hoisted(() => ({
  ...
  loadAppConfig: vi.fn(async () => ({ themeId: 'hamuna-default', appearanceMode: 'dark' })),
}));
...
describe('FloatingThemeRuntime', () => {
  ...
  it('uses the bootstrap first frame, hydrates durable config, then accepts live main-window events', async () => {
    ...
    expect(screen.getByTestId('floating-theme')).toHaveTextContent('hamuna-default:light');
    await waitFor(() => expect(screen.getByTestId('floating-theme')).toHaveTextContent('hamuna-default:dark'));
    await waitFor(() => expect(floatingMocks.listener).not.toBeNull());

    act(() => floatingMocks.listener?.({
      payload: { themeId: 'hamuna-default', appearanceMode: 'light' },
    }));
    expect(screen.getByTestId('floating-theme')).toHaveTextContent('hamuna-default:light');
  ...
  it('registers live sync before hydration and never lets an older hydration overwrite a newer event', async () => {
    ...
    await act(async () => {
      resolveConfig({ themeId: 'hamuna-default', appearanceMode: 'dark' });
