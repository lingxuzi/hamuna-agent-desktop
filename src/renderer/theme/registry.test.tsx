...
import { myAgentsDefaultTheme } from './themes/hamuna-default';
...
describe('ThemeRegistry', () => {
  ...
  it('derives embedded adapters and Hero from each accepted package', () => {
    for (const definition of themeRegistry.getAcceptedDefinitions()) {
      for (const scheme of ['light', 'dark'] as const) {
        ...
        expect(resolved.hero.productName).toBe('HamunaAgent');
  ...
  it('rejects duplicate IDs and incomplete Theme packages', () => {
    ...
    const lateCanonicalOverride = {
      ...myAgentsDefaultTheme,
      stylesheetText: `${myAgentsDefaultTheme.stylesheetText}\nhtml[data-theme-id='hamuna-default'][data-color-scheme='light'] { --ink: initial; }`,
    } as ThemeDefinition;
    ...
    const escapedCanonicalOverride = {
      ...myAgentsDefaultTheme,
      stylesheetText: `${myAgentsDefaultTheme.stylesheetText}\nhtml[data-theme-id='hamuna-def\\61ult'][data-color-scheme='light'] { --ink: initial; }`,
    } as ThemeDefinition;
    ...
    const splitCanonicalSchemeFallback = {
      ...myAgentsDefaultTheme,
      stylesheetText: `${myAgentsDefaultTheme.stylesheetText.replace(/\s*--ink\s*:[^;]+;/, '')}\nhtml[data-theme-id='hamuna-default'][data-color-scheme='light'] { --ink: #1c1612; }`,
    } as ThemeDefinition;
  ...
  it('falls back as one complete default Theme and diagnoses an unknown ID once', () => {
    ...
    expect(first.themeId).toBe('hamuna-default');
  ...
  it('rejects an invalid optional package without making the canonical registry unbootable', () => {
    ...
    expect(registry.getProductionIds()).toEqual(['hamuna-default']);
    expect(registry.resolve(invalidTheme.id, 'dark', false).themeId).toBe('hamuna-default');
  ...
  it('contains optional construction failures inside the Registry boundary', () => {
    ...
    expect(registry.resolve('broken-construction', 'light', false).themeId).toBe('hamuna-default');
  ...
  it.each([
    ...
    expect(registry.getProductionIds()).toEqual(['hamuna-default', validAfterFailure.id]);
    expect(registry.resolve(invalid.id, 'dark', false).themeId).toBe('hamuna-default');
    expect(warn).toHaveBeenCalled();
  });
});
