...
describe('ThemeRuntimeProvider', () => {
  ...
  it('primes a validated optional package before the first React paint', () => {
    ...
    expect(document.getElementById('hamuna-active-theme-stylesheet')).toHaveAttribute(
      'data-theme-id',
      'linear',
    );
  });
  ...
  it('atomically activates every production Theme package in both schemes', () => {
    const view = render(
      <ThemeRuntimeProvider
        ...
        selection={{ themeId: 'hamuna-default', appearanceMode: 'light' }}
      >
        <Probe />
      </ThemeRuntimeProvider>,
    );
  ...
  it('removes synthetic root state on fallback and snapshots the resolved default ID', () => {
    ...
    expect(document.documentElement.dataset.themeId).toBe('hamuna-default');
    expect(document.documentElement.dataset.colorScheme).toBe('light');
    expect(document.documentElement).not.toHaveClass('dark');
    expect(screen.getByTestId('theme-probe').textContent).not.toContain('synthetic-');
    const activeStylesheet = document.getElementById('hamuna-active-theme-stylesheet');
    expect(activeStylesheet).toHaveAttribute('data-theme-id', 'hamuna-default');
