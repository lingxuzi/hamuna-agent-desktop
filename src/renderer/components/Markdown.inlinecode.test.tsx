...
describe('Markdown inline-code file paths', () => {
  ...
  it('uses the floating-ball four-action menu profile', async () => {
    ...
    const labels = screen.getAllByRole('button').map((b) => b.textContent);
    expect(labels).toEqual(['复制', '引用', '打开所在文件夹', '打开 HamunaAgent 预览']);

    fireEvent.click(screen.getByText('打开 HamunaAgent 预览'));
    expect(mocks.onOpenHamunaAgentPreview).toHaveBeenCalledWith(REL, { displayPath: REL });
  });
});
