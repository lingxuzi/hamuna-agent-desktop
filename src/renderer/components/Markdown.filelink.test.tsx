...
const mocks = vi.hoisted(() => ({
  ...
  onOpenHamunaAgentPreview: vi.fn(),
}));
...
function renderFloatingMarkdown(markdown: string) {
  render(
    <FileActionProvider
      ...
      onOpenHamunaAgentPreview={mocks.onOpenHamunaAgentPreview}
    >
      <Markdown>{markdown}</Markdown>
    </FileActionProvider>,
  );
...
describe('Markdown local file links', () => {
  ...
  it('opens workspace absolute path links in the HamunaAgent file preview instead of the system default app', async () => {
  ...
  it('opens previewable workspace links through the floating-ball HamunaAgent preview bridge', async () => {
    ...
    await waitFor(() => {
      expect(mocks.onOpenHamunaAgentPreview).toHaveBeenCalledWith(
