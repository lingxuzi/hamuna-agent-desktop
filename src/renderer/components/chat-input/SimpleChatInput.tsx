...
const SimpleChatInput = memo(forwardRef<SimpleChatInputHandle, SimpleChatInputProps>(function SimpleChatInput({
  ...
  // External runtimes (Claude Code / Codex) authenticate via their own CLI — no HamunaAgent provider required.
  ...
  useImperativeHandle(ref, () => ({
    ...
    clearWorkspaceBoundDraft: () => {
      // Match `@<path>` tokens that target the workspace-managed `hamuna_files/`
      ...
      // sequence like "@hamuna_files/foo.pdf  " collapses fully — the
      // earlier `\s?` left a stray space behind in the multi-space case.
      const current = inputValueRef.current;
      const pattern = /@hamuna_files\/[^\s]+\s*/g;
