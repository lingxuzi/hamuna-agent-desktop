...
//
// Production (Tauri): `resolveHamunaAgentProtocolUrl('/attachment/<rel>')` hits
// the async URI scheme handler in `src-tauri/src/attachment_protocol.rs`
// (`hamuna://...` on macOS/Linux, `http://hamuna.localhost/...` on
// Windows). The handler serves bytes from `~/.hamuna/attachments/<rel>`
...
export function resolveAttachmentUrl(att: {
  ...
  if (isTauri()) {
    return resolveHamunaAgentProtocolUrl(`/attachment/${encoded}`);
  }
