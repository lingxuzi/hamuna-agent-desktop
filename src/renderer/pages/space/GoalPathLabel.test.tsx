...
describe("GoalPathLabel", () => {
  it("renders a root goal as black text with normal weight", () => {
    render(<GoalPathLabel label="HamunaAgent社区" leafLabel="HamunaAgent社区" />);

    const label = screen.getByTitle("HamunaAgent社区");
  ...
  it("mutes the ancestor and keeps the leaf black without bolding it", () => {
    render(
      <GoalPathLabel
        label="HamunaAgent社区 / HamunaAgent BUGFIX"
        leafLabel="HamunaAgent BUGFIX"
      />,
    );

    const label = screen.getByTitle("HamunaAgent社区 / HamunaAgent BUGFIX");
    expect(within(label).getByText("HamunaAgent社区")).toHaveClass(
      "text-[var(--ink-muted)]/75",
    );
    const leaf = within(label).getByText("HamunaAgent BUGFIX");
  ...
  it("applies the same hierarchy to Cloud compact deep paths", () => {
    const fullLabel = "../HamunaAgent BUGFIX/Windows 系统兼容性优化";
    render(
      <GoalPathLabel
        label={fullLabel}
    ...
    expect(within(label).getByText("../HamunaAgent BUGFIX")).toHaveClass(
      "text-[var(--ink-muted)]/75",
    );
