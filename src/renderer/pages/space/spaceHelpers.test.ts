...
describe("space issue helpers", () => {
  ...
  it("builds a concise read-first Issue handoff prompt", () => {
    const prompt = buildIssueCommandPrompt({
      spaceName: "HamunaAgent社区",
      spaceSlug: "official",
      issueId: "iss_123",
    });

    expect(prompt).toBe([
      "Instruction: 这是一个来自 HamunaAgent Space「HamunaAgent社区」的 Issue。请先通过下方只读命令获取当前上下文（标题、正文、附件元数据和最新评论）；如附件与判断有关，按需下载并读取。读取后，先向用户概括你的理解并提出下一步建议，等待用户确认后再修改代码、执行处理动作或变更 Issue 状态。",
      "",
      "- Issue ID: iss_123",
      "- Space slug: official",
      "",
      "阅读 Issue：",
      "`hamuna space issue view iss_123 --space official --comments --json`",
      "",
      "查看其他可用操作：",
      "`hamuna space issue --help`",
    ].join("\n"));
    expect(prompt).not.toContain("hamuna space issue claim");
    expect(prompt).not.toContain("hamuna space issue complete");
    expect(prompt).not.toContain("hamuna issue iss_123");
  });
