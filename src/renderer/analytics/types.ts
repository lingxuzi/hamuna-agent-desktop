 ...
 * 的"+ 新建"按钮，也可以通过 `hamuna task create-direct` CLI），就必须
 ...
 *   - `cli`         用户在终端手动跑 `hamuna` 命令
 *   - `cli_agent`   AI 子进程（agent）通过 CLI 调用（`HAMUNA_PORT` 在环境里）
 *   - `cron`        定时任务调度器
 ...
 * 用 HamunaAgent 的"。详见 `specs/tech_docs/analytics_design.md`。
 *
 * 取值约定：
...
export interface SessionNewParams {
  /** SDK Session ID（与 ~/.hamuna/sessions/*.jsonl 文件名一致） */
  session_id: string;
 ...
 *   - CLI（`hamuna agent channel add/remove`）→ source: cliSource()
 */
