...
// `X-HamunaAgent-Tab-Id` whenever B mounted later — breaking PRD §6.4
...
export async function proxyFetch(
    ...
    if (correlation.tabId && !headers['X-HamunaAgent-Tab-Id'] && !headers['x-hamuna-tab-id']) {
        headers['X-HamunaAgent-Tab-Id'] = correlation.tabId;
    }
    if (correlation.sessionId && !headers['X-HamunaAgent-Session-Id'] && !headers['x-hamuna-session-id']) {
        headers['X-HamunaAgent-Session-Id'] = correlation.sessionId;
    }
