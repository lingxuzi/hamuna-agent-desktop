...
describe('spaceCloud API errors', () => {
  it('normalizes issue comment transport errors without leaking the raw URL', async () => {
    ...
    mocks.invoke.mockRejectedValueOnce(
      new Error(
        'Space API request failed: error sending request for url (https://space.hamuna.io/api/issues/iss_123/comments)',
      ),
    );
    ...
    expect(message).not.toContain('https://space.hamuna.io');
  });
  ...
  it('redacts URLs, bearer tokens, and local paths from debug details', async () => {
    ...
    const normalized = normalizeSpaceError(
      new Error(
        'Space API request failed: Bearer secret.token /Users/ethan/.hamuna/space/session.json https://space.hamuna.io/api/issues',
      ),
    ...
    expect(normalized.debugMessage).not.toContain('https://space.hamuna.io');
  });
  ...
  it('uses structured Space error envelopes when available', async () => {
    ...
    const envelope = {
      ...
      requestId: 'req_123',
      recoveryHint: { message: 'Login with Google from HamunaAgent Cloud Space.' },
    };
    mocks.invoke.mockRejectedValueOnce(envelope);

    await expect(spaceCommentIssue('iss_123', 'hello')).rejects.toThrow('评论发送失败：请重新登录 HamunaAgent 社区');

    const normalized = normalizeSpaceError(envelope, { method: 'POST', path: '/api/issues/iss_123/comments' });
    expect(normalized.userMessage).toBe('评论发送失败：请重新登录 HamunaAgent 社区');
