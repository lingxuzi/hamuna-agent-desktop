...
export default function App() {
 ...
 // Agent) — same path as opening ~/.hamuna from the Launcher.
 ...
 const handleRestartAndUpdate = useCallback(async () => {
  ...
  // write. persistOpenTabsDurable() additionally fsyncs a ~/.hamuna/
 ...
 // `hamuna task create-from-alignment` at the end.
 useEffect(() => {
  const handler = async (raw: Event) => {
   ...
   try {
    ...
    // `~/.hamuna/tasks/<alignmentSessionId>/metadata.json` so that
    // when the AI later calls `hamuna task create-from-alignment`
 ...
 useEffect(() => {
  const handleLaunchBugReport = async (event: CustomEvent<{
   ...
   try {
    ...
    // Ensure ~/.hamuna registered as internal project
