/**
 * hamuna — Self-Configuration CLI for HamunaAgent
 *
 ...
 *
 * Environment:
 *   HAMUNA_PORT — Sidecar port (injected by buildClaudeSessionEnv)
 *   HAMUNA_SESSION_ID — current HamunaAgent session id for attached-session tasks
 *
 * No shebang here. `npm run build:cli` (esbuild) injects `#!/usr/bin/env node`
 * through `--banner:js` so the *built* `hamuna.js` artifact is what carries
...
let PORT = process.env.HAMUNA_PORT ?? '';
let BASE = '';
...
export function parseArgs(args: string[]): { positional: string[]; flags: Record<string, unknown> } {
  ...
  // so `hamuna session send <sid> -p "..."` works as documented in PRD §3.1
 ...
 * exits 1 with a `hamuna <command>` usage line for the AI to follow.
 *
...
function requirePositional(
  ...
  console.error(`  Usage: hamuna ${command} <${argName}>${flagAlternative ? ` (or --${flagAlternative} <${argName}>)` : ''}`);
  process.exit(1);
}
...
export const TOP_HELP = `hamuna — HamunaAgent Self-Configuration CLI

Usage: hamuna <command> [options]
...
Global flags:
  ...
  --port NUM  Override Sidecar port (default: $HAMUNA_PORT)

Examples:
  hamuna mcp list
  hamuna mcp show playwright
  hamuna mcp add --id playwright --type stdio --command npx --args @playwright/mcp@latest
  hamuna mcp enable playwright --scope both
  hamuna mcp oauth discover notion-mcp
  hamuna mcp oauth start notion-mcp
  hamuna vision readme
  hamuna vision analyze --image hamuna_files/screenshot.png --prompt "Summarize the UI state"
  hamuna model list
  hamuna model set-key deepseek sk-xxx
  hamuna skill list
  hamuna skill add vercel-labs/skills --skill react-best-practices
  hamuna skill add https://github.com/anthropics/skills --plugin document-skills
  hamuna skill add "npx skills add foo/bar --skill baz" --force
  hamuna skill remove my-skill
  hamuna skill sync
  hamuna cron list
  hamuna goal get
  hamuna goal create --objective-file goal-objective.txt --max-executions 12
  hamuna goal update --status complete
  hamuna runtime list                       # see installed runtimes + install hints
  hamuna runtime describe codex             # models + permission modes
  hamuna runtime diagnose codex             # auth / features / MCP / apps / env snapshot (issue #194)
  hamuna diagnose runtime codex             # alias for runtime diagnose
  hamuna agent list --archived              # archived Agent workspaces
  hamuna agent show <agent-id>              # effective defaults for a workspace
  hamuna agent archive <agent-id>
  hamuna agent unarchive <agent-id>
  hamuna task list
  hamuna task get <taskId>            # returns metadata + docs paths
                                        # (task.md / verify.md / progress.md /
                                        #  alignment.md — read/edit them with
                                        #  standard Read/Edit/Write tools)
  hamuna task update-status <taskId> running --message "starting work"
  hamuna task update-status <taskId> verifying
  hamuna task update-status <taskId> done --message "bundle size dropped 40%"
  hamuna task append-session <taskId> <sessionId>
  hamuna task run <taskId>
  hamuna task rerun <taskId>
  hamuna task create-direct --name "review PR" \\
    ...
    #   hamuna runtime list  +  hamuna runtime describe <runtime>
    # before choosing values. Omit any flag to inherit the agent workspace default.
  hamuna task create-from-alignment <alignmentSessionId> --name "新任务"
  ...
  hamuna space issue view <issueId> --space <slug> --comments --json
  hamuna space issue claim <issueId> --space <slug> --deliveryId <deliveryId>
  ...
  hamuna space issue comment <issueId> --space <slug> --body-file result.md --attachment report.pdf
  hamuna space issue comment get <issueId> <commentId> --space <slug> --json
  hamuna space issue complete <issueId> --space <slug> --taskId <taskId> --body-file result.md \\
      --message "completed Space issue"
  hamuna space issue attachment add <issueId> --space <slug> --file report.pdf
  hamuna space issue close <issueId> --space <slug>
  hamuna space attachment download <attachmentId> --space <slug> --output hamuna_files/space/file.bin
  hamuna thought list
  hamuna plugin list
  hamuna cc-plugin list
  hamuna cc-plugin install anthropics/example-plugin
  hamuna cc-plugin install file:///path/to/dev-plugin
  hamuna cc-plugin enable my-plugin
  hamuna version
  hamuna reload

Run 'hamuna <command> --help' for details on a specific command.`;
    ...
    if (msg.includes('ECONNREFUSED') || msg.includes('fetch failed')) {
      if (isSpaceJsonInvocation()) {
        console.log(JSON.stringify({
          success: false,
          code: 'HAMUNA_UNAVAILABLE',
          error: 'Cannot connect to the HamunaAgent app.',
          suggestion: 'Start HamunaAgent and retry the same Space command.',
        }, null, 2));
        process.exit(3);
      }
      console.error('Error: Cannot connect to HamunaAgent. Is the app running?');
      if (process.env.CODEX_SANDBOX || process.env.CODEX_SANDBOX_NETWORK_DISABLED === '1') {
        ...
        console.error('  If HamunaAgent is running on localhost, switch Codex to no-restrictions or run the command from your normal terminal.');
      }
      process.exit(3);
    }
    throw err;
...
export function printResult(
  ...
  if (group === 'tool' && action === 'env') {
    ...
    if (missing && missing.length > 0) {
      ...
      console.log(`  → Run: hamuna tool env ${String(data.name)} set ${missing[0]}=<value>`);
    }
    if (result.hint) console.log(`\n${result.hint}`);
    return;
  }
  ...
  if (group === 'cron' && action === 'run-now') {
    ...
    console.log(`  runs:    hamuna cron runs ${data.taskId ?? '<id>'} --limit 1`);
    return;
  }
  if (group === 'cron' && action === 'stop') {
    ...
    if (status === 'blocked') {
      console.log(`  restart: hamuna cron start ${taskId}`);
    }
    return;
  }
  ...
  if (group === 'diagnose') {
    // `hamuna diagnose runtime <type>` — sugar for `runtime diagnose`.
    printRuntimeDiagnose(result.data as Record<string, unknown>);
    return;
  }
  if (group === 'status') {
  ...
  if (group === 'session' && action === 'send') {
    ...
    if (result.replyBack === false) {
      console.log('  notification: one-way; HamunaAgent will not push the target turn result back here.');
    } else {
      console.log('  notification: HamunaAgent will push the target turn result back as a <hamuna-session-event type="send.result"> block.');
    }
    return;
  }
  if (group === 'session' && action === 'watch') {
    ...
    console.log('  result: HamunaAgent will push a <hamuna-session-event type="watch.completed"> block when the target finishes.');
    return;
  }
  if (group === 'help') {
  ...
  // `ls -lt ~/.hamuna/tasks/`. JSON mode above returns the full payload.
 ...
 *     task_id:   <uuid>
 *     name:      <string>
 *     docs_path: ~/.hamuna/tasks/<uuid>/
 *     next:      hamuna task run <uuid>          # non-attached tasks
 *     complete:  hamuna task update-status ...   # attached tasks
 */
function printTaskCreateResult(data: Record<string, unknown>): void {
  ...
  const absDocs = `${home}/.hamuna/tasks/${id}/`;
  ...
  const dispatch = nextSteps?.dispatch ?? (!isAttached && id ? `hamuna task run ${id}` : '');
...
function printSpaceClaimAttachedResult(data: Record<string, unknown>): void {
  ...
  const sessionId = String(data.localSessionId ?? task.currentSessionId ?? process.env.HAMUNA_SESSION_ID ?? '');
  ...
  if (taskId) console.log(`  inspect:   hamuna task get ${taskId}`);
  if (issueId && taskId) {
    console.log(`  finish:    hamuna space issue complete ${shellQuoteArg(issueId)}${spaceArg}${workspaceArg} --taskId ${shellQuoteArg(taskId)} --body-file result.md --message "completed Space issue"`);
  } else if (issueId) {
    console.log(`  complete:  hamuna space issue complete ${shellQuoteArg(issueId)}${spaceArg}${workspaceArg}`);
  }
}
...
function printSpaceGoalList(data: Record<string, unknown>, flags: Record<string, unknown>): void {
  ...
  console.log(`  create: hamuna space issue create --space ${shellQuoteArg(slug)} ... --goal <goalId>`);
  console.log(`  move:   hamuna space issue update <issueId> --space ${shellQuoteArg(slug)} --goal <goalId>`);
  console.log(`  inbox:  hamuna space issue update <issueId> --space ${shellQuoteArg(slug)} --clear-goal`);
}
...
function printSpaceIssueUpdate(data: Record<string, unknown>, flags: Record<string, unknown>): void {
  ...
  if (issueId && slug) {
    console.log(`  inspect: hamuna space issue view ${shellQuoteArg(issueId)} --space ${shellQuoteArg(slug)} --json`);
  }
}

/**
 * Format `hamuna runtime list` output.
 *
...
function printRuntimeList(rows: Array<Record<string, unknown>>): void {
  ...
  console.log('');
  console.log('Describe a runtime:  hamuna runtime describe <runtime>');
}

/**
 * Format `hamuna runtime describe <runtime>` output.
 *
...
/**
 * Format `hamuna diagnose runtime <type>` output.
 *
...
function printRuntimeDiagnose(data: Record<string, unknown>): void {
  ...
  console.log(`  proxyPolicy: ${env.proxyPolicy ?? 'hamuna'}`);
  console.log(`  HAMUNA_PROXY_INJECTED: ${env.hamunaProxyInjected ? 'yes' : 'no'}`);
...
/**
 * Format `hamuna agent show <id>` output.
 *
...
function printAgentShow(data: Record<string, unknown>): void {
  ...
  console.log('');
  console.log('Describe this runtime:  hamuna runtime describe <runtime>');
}
...
/**
 * Format `hamuna mcp show <id>` output.
 *
...
function printCronStatus(data: Record<string, unknown>): void {
  ...
  if (data.lastExecutedAt || data.nextExecutionAt) {
    console.log(`Display timezone: ${localTz} (${timezoneOffset(Date.now(), localTz, 'utc')}); per-task cron tz is shown by 'hamuna cron list'.`);
  }
}
...
function printTaskDetail(task: Record<string, unknown>): void {
  ...
  console.log('\nNext steps:');
  console.log('  hamuna task update-status <id> <status> [--message ...]  # transition state machine');
  console.log('  hamuna task run <id>                                     # dispatch immediately');
  console.log('  hamuna task rerun <id>                                   # re-arm stopped/blocked task');
  console.log('  hamuna task --help                                       # full Task CLI reference');
}
  ...
  if (!PORT) {
    if (groupIsSpaceCommand(positional[0]) && jsonMode) {
      return exitAgentCliError(flags, {
        code: 'HAMUNA_PORT_REQUIRED',
        error: 'The HamunaAgent local API port is unavailable.',
        suggestion: 'Run this command from an active HamunaAgent Session or start the app and retry.',
      }, 3);
    }
    console.error('Error: HAMUNA_PORT not set. This CLI runs within the HamunaAgent app.');
    process.exit(3);
  }
...
export function rejectUnsupportedSpaceDryRun(
  ...
  exitSpaceInputError(
    flags,
    'DRY_RUN_UNSUPPORTED',
    `hamuna ${command} does not support --dry-run. No changes were applied.`,
    `Read hamuna ${command} --help; remove --dry-run only when ready to apply the mutation.`,
  );
}
...
export function buildRoute(group: string, action: string, rest: string[]): string {
  ...
  // Tool readmes: `hamuna cron readme`, `hamuna im readme`, `hamuna widget ...`,
  // `hamuna thought readme`. `thought` is included so the AI's natural
  ...
  // (`hamuna widget`, `hamuna widget chart`, `hamuna widget readme chart`)
...
function requireSpaceSlug(flags: Record<string, unknown>): string {
  ...
  return exitAgentCliError(flags, {
    ...
    suggestion: 'Run `hamuna space list --json`, then retry with one returned slug.',
    suggestedCommand: 'hamuna space list --json',
  });
}
...
function requireSpacePositional(
  ...
  return exitAgentCliError(flags, {
    ...
    suggestion: `Run \`hamuna ${command} --help\` and retry with <${argName}>${flagAlternative ? ` or --${flagAlternative} <${argName}>` : ''}.`,
    suggestedCommand: `hamuna ${command} --help`,
  });
}
...
function optionalSpaceStringFlag(
  ...
  return exitSpaceInputError(
    ...
    `Run \`hamuna ${command} --help\`, then retry with a non-empty --${flagName} value.`,
  );
}
...
function spaceCommandContext(flags: Record<string, unknown>, workspacePath: string): Record<string, unknown> {
  return {
    ...
    sessionId: process.env.HAMUNA_SESSION_ID,
...
async function claimSpaceIssueWithAttachedTask(
  ...
  const currentSessionId =
    typeof flags.currentSessionId === 'string' && flags.currentSessionId.trim()
      ...
      : process.env.HAMUNA_SESSION_ID;
  if (!currentSessionId) {
    return exitSpaceInputError(
      flags,
      'SESSION_REQUIRED',
      'space issue claim --create-attached requires an active HamunaAgent Session.',
      'Run this command inside the HamunaAgent AI Session that will own the attached Task.',
    );
  }
...
async function completeSpaceIssueWithLocalFollowup(
  ...
  if (taskId) {
    ...
    if (!taskRead.success) {
      return {
        ...
        suggestion: `Run \`hamuna task get ${shellQuoteArg(taskId)} --json\` to verify the Task id, then retry without changing the cloud Issue.`,
  ...
  const operationKey = buildSpaceCompleteOperationKey({
    issueId,
    taskOrSessionId: taskId ?? process.env.HAMUNA_SESSION_ID ?? 'no-local-task',
    resultComment,
  });
  const completeResult = await callApi('space/issue-complete', {
    ...
    resultComment,
    operationKey,
    operationKeySubject: taskId ?? process.env.HAMUNA_SESSION_ID ?? 'no-local-task',
  });
  if (!completeResult.success) {
    return {
      ...
      suggestion: stringValue(completeResult, 'suggestion')
        ?? 'Inspect the Issue with `hamuna space issue view <issueId> --space <slug> --json`, then retry only after resolving the reported cloud conflict.',
      data: {
        issueId,
    ...
    if (!taskResult.success) {
      ...
      if (!taskAlreadyDone) {
        return {
          ...
          recoveryHint: {
            recoveryCommand: `hamuna space issue complete ${shellQuoteArg(issueId)} --space ${shellQuoteArg(String(completeBody.spaceSlug))} --workspacePath ${shellQuoteArg(workspacePath)} --taskId ${shellQuoteArg(taskId)} --message "completed Space issue"`,
...
export function buildRequestBody(
  ...
  if (group === 'goal') {
    ...
    if (action === 'create') {
      ...
      if (!objective) {
        ...
        console.error('  Usage: hamuna goal create --objective-file goal-objective.txt');
        process.exit(2);
      }
    ...
    if (action === 'update') {
      ...
      if (status !== 'complete' && status !== 'blocked') {
        ...
        console.error('  Usage: hamuna goal update --status complete');
        process.exit(2);
      }
      const reason = (
  ...
  if (group === 'tool') {
    if (action === 'add') {
      // `hamuna tool add <dir>` — dir is positional; --dir also accepted
      ...
      if (issueAction === 'update') {
        ...
        if (
          ...
          return exitSpaceInputError(
            ...
            'Run `hamuna space issue update --help` and provide title, body, Goal, or human-only metadata.',
          );
        }
        return {
          ...context,
    ...
    if (action === 'claim') {
      ...
      if (claimAction === 'local-task') {
        ...
        const localSessionId = requireSpacePositional(
          flags,
          (flags.localSessionId ?? process.env.HAMUNA_SESSION_ID) as string | undefined,
          'localSessionId',
          'space claim local-task',
          'localSessionId',
        );
  ...
  if (group === 'issue') {
    ...
    if (action === 'comment' || action === 'comments' || action === 'status') {
      return exitSpaceInputError(
        flags,
        'COMMAND_MOVED',
        `hamuna issue only reads an Issue; "${action}" is not supported by this alias.`,
        `Use hamuna space issue ${action} ... with an explicit --space <slug>.`,
      );
    }
  ...
  // Runtime discovery commands (v0.1.69+): `hamuna runtime list|describe`
  ...
  // Sugar form: `hamuna diagnose runtime <type>` (issue #194).
  if (group === 'diagnose' && action === 'runtime') {
    return {
  ...
  //   hamuna widget                         → action='list',    rest=[]           → modules=[]
  //   hamuna widget readme                  → action='readme',  rest=[]           → modules=[]
  //   hamuna widget readme chart            → action='readme',  rest=['chart']    → modules=['chart']
  //   hamuna widget readme chart interactive → rest=['chart','interactive']       → modules=['chart','interactive']
  //   hamuna widget chart                   → action='chart',   rest=[]           → modules=['chart']
  //   hamuna widget chart interactive       → action='chart',   rest=['interactive'] → modules=['chart','interactive']
  ...
  // Task Center (v0.1.69) — covers all `hamuna task <action>` subcommands.
  //
  ...
  // (HAMUNA_PORT present → agent subprocess; otherwise user terminal).
  if (group === 'task') {
    if (action === 'list') {
      return {
        workspaceId: flags.workspaceId,
    ...
    if (action === 'create-attached') {
      ...
      const currentSessionId =
        typeof flags.currentSessionId === 'string' && flags.currentSessionId.trim()
          ...
          : process.env.HAMUNA_SESSION_ID;
      if (!currentSessionId) {
        console.error('Error: task create-attached requires HAMUNA_SESSION_ID. Run it from inside a HamunaAgent AI session.');
        process.exit(2);
      }
      const source = (flags.source as string | undefined) ?? 'space-issue';
      if (source !== 'space-issue') {
  ...
  // Thought (v0.1.69) — `hamuna thought <list|create>`
  if (group === 'thought') {
    if (action === 'list') {
      return {
        tag: flags.tag,
    ...
    if (action === 'create') {
      // Issue #149: on Windows the AI-emitted `hamuna thought create '<text>'`
  ...
  // ===== Session events (PRD 0.2.37) — `hamuna session send/watch` =====
  if (group === 'session') {
    if (action === 'send') {
      ...
      if (!promptText || promptText.length === 0) {
        ...
        console.error('  → Tip: see `hamuna session send --help` for usage examples');
        process.exit(3);
      }
      ...
      if (!flags.promptFile) {
        if (promptText.includes('\n')) {
          ...
          console.error('         hamuna session send <sid> --prompt-file <path>');
          process.exit(3);
        }
    ...
    if (action === 'watch') {
      ...
      if (unsupportedFlag) {
        console.error('Error: session watch does not accept prompt/then flags. Use `hamuna session send` to ask the target session to do new work.');
        process.exit(3);
      }
      if (rest.length > 1) {
...
export function normalizeScheduleFlag(
  ...
  if (trimmed.startsWith('{')) {
    ...
    if (kind === 'loop') {
      ...
      console.error('  hamuna goal create --objective-file <path>');
      process.exit(2);
    }
 ...
 *
 * Flags supported:
 *   --notificationBotChannelId <bot-id>     IM bot id (see `hamuna im channels`)
...
function buildNotificationFromFlags(
  ...
  if (channel !== undefined) {
    ...
    if (typeof channel !== 'string' || channel.length === 0) {
      console.error('Error: --notificationBotChannelId requires a bot id (e.g. --notificationBotChannelId feishu_main). See: hamuna im channels');
      process.exit(2);
    }
  ...
  main().catch(err => {
    ...
    if (isSpaceJsonInvocation()) {
      console.log(JSON.stringify({
        ...
        suggestedCommand: `hamuna ${rawArgs.slice(0, 4).filter(arg => !arg.startsWith('-')).join(' ')} --help`,
      }, null, 2));
    } else {
