...
function readConfig(): Record<string, unknown> {
  return JSON.parse(readFileSync(join(scratch, '.hamuna', 'config.json'), 'utf-8')) as Record<string, unknown>;
}
...
beforeEach(() => {
  scratch = mkdtempSync(join(tmpdir(), 'hamuna-admin-api-'));
  mkdirSync(join(scratch, '.hamuna'), { recursive: true });
...
describe('admin-api help registry', () => {
  it('documents the official vision command group for hamuna vision --help', async () => {
    ...
    expect(text).toContain('hamuna vision');
    expect(text).toContain('analyze');
    expect(text).not.toContain('Unknown command group');
  });

  it('documents Goal Mode for hamuna goal --help', async () => {
    const { handleHelp } = await import('./admin-api');
    ...
    expect(result.success).toBe(true);
    expect(text).toContain('hamuna goal');
  ...
  it('uses the longest Space command path so leaf help is an executable Agent contract', async () => {
    ...
    expect(result.success).toBe(true);
    expect(text).toContain('hamuna space issue attachment add');
    ...
    expect(text).not.toContain('hamuna space — Work with');
  });
  ...
  it('provides an independent Agent contract for every Space Issue leaf', async () => {
    ...
    for (const path of leaves) {
      ...
      expect(text, path.join(' ')).not.toContain('hamuna space — Work with');
    }
  });
  ...
  it('provides exact nine-section contracts for Goal discovery and Issue metadata update', async () => {
    ...
    expect(goalText).not.toContain('hamuna space — Discover');
    expect(updateText).not.toContain('hamuna space — Discover');
  });
...
describe('admin-api Space workspace identity', () => {
  ...
  it('enriches Space commands with the stable project id for the requested workspace', async () => {
    ...
    mkdirSync(workspace);
    writeJson(join(scratch, '.hamuna', 'projects.json'), [{
...
describe('admin-api goal', () => {
  it('creates a current-session Goal without Cron delivery ownership', async () => {
    ...
    sessionEngineMocks.state.context = {
      ...
      workspacePath: '/tmp/hamuna-goal-workspace',
    };
    setImCronContext({
      botId: 'bot-feishu',
      chatId: 'chat-123',
...
describe('admin-api cron create', () => {
  it('leaves execution routing unset when the caller did not request a Task override', async () => {
    const workspacePath = '/tmp/hamuna-managed-codex-workspace';
    writeJson(join(scratch, '.hamuna', 'config.json'), {
  ...
  it('rejects loop schedules before ordinary cron creation reaches Rust', async () => {
    ...
    expect(result.error).toContain('Use hamuna goal create');
    expect(managementApiMocks.managementApi).not.toHaveBeenCalled();
  });
});
...
describe('admin-api agent set configuration intent', () => {
  it('normalizes managed Codex permission vocabulary without changing provider or model', async () => {
    const workspacePath = '/tmp/hamuna-agent-set-managed-codex';
    writeJson(join(scratch, '.hamuna', 'config.json'), {
      agents: [{
        id: 'agent-managed-codex',
        name: 'Managed Codex',
    ...
    expect(readJson(join(scratch, '.hamuna', 'projects.json'))[0]).toMatchObject({
      providerId: 'codex-sub',
      model: 'gpt-5.6-sol',
      permissionMode: 'fullAgency',
    });
  ...
  it('rejects an invalid permission mode without mutating either config store', async () => {
    const workspacePath = '/tmp/hamuna-agent-set-invalid';
    const config = {
      agents: [{
        id: 'agent-invalid',
        name: 'Invalid Guard',
    ...
    writeJson(join(scratch, '.hamuna', 'config.json'), config);
    writeJson(join(scratch, '.hamuna', 'projects.json'), projects);
    const { handleAgentSet } = await import('./admin-api');
    ...
    expect(readJson(join(scratch, '.hamuna', 'projects.json'))).toEqual(projects);
    expect(managementApiMocks.managementApi).not.toHaveBeenCalled();
  });

  it('rejects managed Codex full-auto instead of escalating it to no-restrictions', async () => {
    const workspacePath = '/tmp/hamuna-agent-set-full-auto';
    const config = {
      agents: [{
        id: 'agent-full-auto',
  ...
  it.each([
    ...
    const workspacePath = '/tmp/hamuna-agent-set-invalid-provider-model';
    const config = {
  ...
  it.each([
    ...
    const workspacePath = `/tmp/hamuna-agent-effective-model-${value}`;
    writeJson(join(scratch, '.hamuna', 'config.json'), {
      providerVerifyStatus: {
        'anthropic-sub': { status: 'valid', verifiedAt: '2026-07-21T00:00:00.000Z' },
      },
    ...
    if (addCustomModel) {
      ...
      expect(readJson(join(scratch, '.hamuna', 'projects.json'))[0]).toMatchObject({ model: value });
    } else {
  ...
  it('rolls back the Agent write when the Project mirror cannot be saved', async () => {
    const workspacePath = '/tmp/hamuna-agent-set-project-failure';
    const config = {
      agents: [{
  ...
  it('serializes concurrent Agent intents so Agent and Project end on the same value', async () => {
    const workspacePath = '/tmp/hamuna-agent-set-concurrent';
    writeJson(join(scratch, '.hamuna', 'config.json'), {
      providerVerifyStatus: {
        'anthropic-sub': { status: 'valid', verifiedAt: '2026-07-21T00:00:00.000Z' },
      },
    ...
    const projectModel = readJson(join(scratch, '.hamuna', 'projects.json'))[0].model;
  ...
  it('validates a delayed model intent against the provider committed before its lock turn', async () => {
    const workspacePath = '/tmp/hamuna-agent-set-validation-lock';
    writeJson(join(scratch, '.hamuna', 'config.json'), {
      providerVerifyStatus: {
        'anthropic-sub': { status: 'valid', verifiedAt: '2026-07-21T00:00:00.000Z' },
  ...
  it.each([
    ...
    const workspacePath = '/tmp/hamuna-agent-set-mirror';
    ...
    expect(result.success).toBe(true);
    expect(readJson(join(scratch, '.hamuna', 'projects.json'))[0]).toMatchObject({ [key]: value });
    expect(managementApiMocks.managementApi).toHaveBeenCalledWith(
      '/api/agent/reload-config',
      'POST',
      {
  ...
  it.each([
    ...
    const workspacePath = `/tmp/hamuna-agent-managed-${key}`;
  ...
  it('rejects an unverified subscription provider before mutating either store', async () => {
    const workspacePath = '/tmp/hamuna-agent-unverified-subscription';
    const config = {
      agents: [{
  ...
  it('shows managed Codex as its effective runtime instead of the stored builtin carrier', async () => {
    writeJson(join(scratch, '.hamuna', 'config.json'), {
      agents: [{
        ...
        name: 'Managed Show',
        workspacePath: '/tmp/hamuna-agent-managed-show',
        providerId: 'codex-sub',
        model: 'gpt-5.6-sol',
        runtime: 'builtin',
        permissionMode: 'fullAgency',
...
describe('admin-api MCP remove/disable legacy HTTP servers', () => {
  ...
  it('cascades custom MCP remove across config, projects, sessions, legacy Bot payloads, and Rust stores', async () => {
    ...
    writeJson(join(scratch, '.hamuna', 'sessions.json'), [{
      id: 'session-1',
    ...
    const result = await handleMcpRemove({ id: 'yuandian-law' });
    const config = readConfig();
    const project = readJson(join(scratch, '.hamuna', 'projects.json'))[0];
    const session = readJson(join(scratch, '.hamuna', 'sessions.json'))[0];
    const agent = (config.agents as Array<Record<string, unknown>>)[0];
  ...
  it('keeps AppConfig definition when Rust Task/Cron cleanup fails', async () => {
    ...
    managementApiMocks.managementApi.mockResolvedValueOnce({
      ...
      recoveryHint: { recoveryCommand: 'hamuna status', message: 'retry later' },
    });
    ...
    expect(result.success).toBe(false);
    expect(result.recoveryHint).toEqual({ recoveryCommand: 'hamuna status', message: 'retry later' });
  ...
  it('keeps AppConfig definition when session snapshot cleanup cannot be written', async () => {
    ...
    mkdirSync(join(scratch, '.hamuna', 'sessions.json.tmp'));
...
describe('admin-api Agent workspace archive', () => {
  it('archives a linked agent workspace and pauses proactive agent state', async () => {
    ...
    const projects = readJson(join(scratch, '.hamuna', 'projects.json'));
  ...
  it('keeps restore intent when archive is called repeatedly', async () => {
    ...
    expect((await handleAgentArchive({ id: 'agent-1' })).success).toBe(true);
    let projects = readJson(join(scratch, '.hamuna', 'projects.json'));
    expect(projects[0].archivedAgentEnabledBeforeArchive).toBe(true);
    ...
    projects = readJson(join(scratch, '.hamuna', 'projects.json'));
  ...
  it('rejects plain enable for archived agent workspaces', async () => {
    ...
    expect(result.recoveryHint).toMatchObject({
      recoveryCommand: 'hamuna agent unarchive agent-1',
    });
    const config = readConfig();
    expect((config.agents as Array<Record<string, unknown>>)[0].enabled).toBe(false);
  });
