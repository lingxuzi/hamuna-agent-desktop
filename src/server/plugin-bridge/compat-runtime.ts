...
export function createCompatRuntime(rustPort: number, botId: string, pluginId: string) {
  ...
  const runtime = {
    ...
    channel: {
      ...
      // ===== Activity tracking =====
      // No-op — HamunaAgent doesn't need OpenClaw activity tracking.
      activity: {
      ...
      media: {
        ...
        async saveMediaBuffer(
          ...
          const dir = join(tmpdir(), 'hamuna-media', safeSubdir);
          await ensureDir(dir);
      ...
      // No-op — HamunaAgent uses its own allowedUsers mechanism via BIND codes.
      pairing: {
      ...
      // ===== Group policies =====
      // HamunaAgent handles access control at Rust layer (group approval UI).
