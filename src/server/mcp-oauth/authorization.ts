...
function buildCallbackHtml(success: boolean, message: string): string {
  ...
  const icon = success ? '&#10003;' : '&#10007;';
  return `<!DOCTYPE html><html><head><meta charset="utf-8"><title>HamunaAgent OAuth</title>
        ...
        if (code && reqState) {
          ...
          void (async () => {
            try {
              ...
              res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8' });
              res.end(buildCallbackHtml(false, 'Authorization failed while saving credentials. Return to HamunaAgent and try again.'));
              resolveToken(null);
            } finally {
              cleanupFlow(flow);
            }
