...
import {
  ...
  normalizeProxyScope,
  shouldUseHamunaAgentProxyForGeneralRequests,
  shouldUseHamunaAgentProxyForProvider,
...
const proxyWasInjectedByRust = process.env.HAMUNA_PROXY_INJECTED === '1';
const proxyInheritedEnvJson = process.env.HAMUNA_PROXY_INHERITED_ENV_JSON;
delete process.env.HAMUNA_PROXY_INJECTED;
delete process.env.HAMUNA_PROXY_INHERITED_ENV_JSON;
 ...
 * NO_PROXY matcher. HamunaAgent' localhost invariant includes the whole 127/8
...
function readInitialProxySettings(): ProxySettings | null {
  try {
    const raw = readFileSync(join(homedir(), '.hamuna', 'config.json'), 'utf8');
...
function copyProxyEnvVars(
  ...
  delete target.HAMUNA_PROXY_INJECTED;
  delete target.HAMUNA_PROXY_INHERITED_ENV_JSON;
...
function applyGeneralProxyEnv(): void {
  if (
    shouldUseHamunaAgentProxyForGeneralRequests(currentProxySettings)
    && appProxyEnvSnapshot
  ) {
...
async function applyProcessProxyConfig(
  ...
  if (proxySettings.protocol === 'socks5') {
    ...
    try {
      ...
      console.log(`[proxy-state] owner=general path=${shouldUseHamunaAgentProxyForGeneralRequests(proxySettings) ? 'hamuna-proxy' : 'inherited'} protocol=socks5-bridge`);
      return;
    } catch (err) {
      if (generation !== proxyConfigGeneration) {
        return;
  ...
  commitProxyState(proxySettings, createAppProxyEnvSnapshot(proxyUrl));
  console.log(`[proxy-state] owner=general path=${shouldUseHamunaAgentProxyForGeneralRequests(proxySettings) ? 'hamuna-proxy' : 'inherited'} protocol=${proxySettings.protocol || 'http'}`);
}
...
function generalRequestEnvSnapshot(): Record<string, string | undefined> {
  return shouldUseHamunaAgentProxyForGeneralRequests(currentProxySettings)
    ? (appProxyEnvSnapshot ?? {})
...
/**
 * Dispatcher for generic HamunaAgent-owned network requests. The selected source
...
export function applyProviderProxyPolicyToEnv(
  ...
  const useAppProxy = shouldUseHamunaAgentProxyForProvider(currentProxySettings, providerId);
  if (useAppProxy) {
    copyProxyEnvVars(env, appProxyEnvSnapshot ?? {});
  } else {
    copyProxyEnvVars(env, inheritedProxySnapshot);
  }
  console.log(`[proxy-state] owner=provider provider=${providerId} path=${useAppProxy ? 'hamuna-proxy' : 'inherited'}`);
}

export function getProxyForProviderUrl(providerId: string, url: string): string | undefined {
  const source = shouldUseHamunaAgentProxyForProvider(currentProxySettings, providerId)
    ? (appProxyEnvSnapshot ?? {})
    : inheritedProxySnapshot;
...
/**
 * Resolve only the proxy explicitly configured by HamunaAgent for general
 * requests. Security-sensitive callers use this to distinguish the trusted
 * app overlay from the inherited baseline.
 */
export function getHamunaAgentProxyForGeneralUrl(url: string): string | undefined {
  if (!shouldUseHamunaAgentProxyForGeneralRequests(currentProxySettings)) return undefined;
