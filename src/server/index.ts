...
async function writeSpaceSkillExportPackages(
  ...
  const exportDir = join(homedir(), '.hamuna', 'tmp', 'skill-url-export', exportId);
...
function requiredMemorySystemSkill(managedKind: string | undefined): RequiredSystemSkill | undefined {
  switch (managedKind) {
    case 'memory_auto_update_batch': return 'hamuna-memory-update';
    case 'memory_gardener': return 'hamuna-memory-gardener';
    case 'memory_molt': return 'hamuna-memory-molt';
...
function getSkillsConfigPath(): string {
  const homeDir = getHomeDirOrNull() || '';
  return join(homeDir, '.hamuna', 'skills-config.json');
}
...
function seedBundledSkills(): void {
  try {
    ...
    const config = readSkillsConfig();
    const homeDir = getHomeDirOrNull() || '';
    const userSkillsDir = join(homeDir, '.hamuna', 'skills');
    ...
    for (const folder of bundledFolders) {
      ...
      // (~/.hamuna/skills/docx pointed at a deleted target).
...
/**
 * Ensure the ~/.hamuna/plugins/ directory tree exists for Claude Plugin
 ...
 *
 *   ~/.hamuna/plugins/                  plugin install root
 *   ~/.hamuna/plugins/data/             ${CLAUDE_PLUGIN_DATA} parent
 *
...
function ensurePluginsDirs(): void {
  try {
    ...
    const root = join(homeDir, '.hamuna', 'plugins');
...
async function routeAdminApi(pathname: string, payload: Record<string, unknown>): Promise<Record<string, unknown>> {
  ...
  // Official HamunaAgent CLI tools
  ...
  // HamunaAgent Cloud Space — Registered Agent CLI bridge.
  ...
  // Session Inbox (PRD 0.2.18) — `hamuna session send`
  if (route === 'session/send') {
    ...
    // in cli/hamuna.ts:1627-1633 can read it without destructuring the
    // nested error object.
...
function startupBeacon(step: string): void {
  ...
  try {
    ...
    const logsDir = join(homedir(), '.hamuna', 'logs');
...
async function main() {
  ...
  process.env.HAMUNA_SIDECAR_ROLE = sidecarRole;
  ...
  //      (OpenAI bridge loopback URL + HAMUNA_PORT injection both need it).
  //   2. setSidecarPort's process.env.HAMUNA_PORT side effect is in place
  //      before any external runtime subprocess (or `hamuna` CLI invocation
  ...
  (globalThis as { __hamunaDeferredInit?: Promise<void> }).__hamunaDeferredInit =
    deferredInitPromise;
  ...
  honoServe({
    ...
    fetch: async (request) => {
      ...
      // X-HamunaAgent-Session-Id / X-HamunaAgent-Tab-Id; the server generates a
      // fresh requestId (or honours an inbound `X-HamunaAgent-Request-Id` from
      // the Rust proxy if it pre-populated one).
      const incomingRequestId = request.headers.get('x-hamuna-request-id') ?? undefined;
      const requestId = incomingRequestId ?? randomUUIDv4Short();
      const sessionId = request.headers.get('x-hamuna-session-id') ?? undefined;
      const tabId = request.headers.get('x-hamuna-tab-id') ?? undefined;
  ...
  async function handleRequest(request: Request): Promise<Response> {
    {
      ...
      //   2. Miss → fall back to the trusted attachment root <home>/.hamuna/generated/
      ...
      // The trusted-root fallback is by construction inside the HamunaAgent-owned tree.
      ...
      // Production uses the Tauri `hamuna://attachment/<path>` custom protocol
      ...
      if (pathname.match(/^\/sessions\/[^/]+\/since\/[^/]+$/) && request.method === 'GET') {
        ...
        // resolved via the hamuna:// custom protocol on the client.
      ...
      if (pathname === '/api/image' && request.method === 'GET') {
        try {
          ...
          // Security: allow reading from workspace/hamuna_files/{generated_images,temp}/ or legacy paths
          const resolvedPath = resolve(imagePath);
          const legacyDir = join(homedir(), '.hamuna', 'generated');
          const legacyDirSep = legacyDir.endsWith(sep) ? legacyDir : legacyDir + sep;
          // New unified paths + backward compat with hamuna-generated/images/
          const allowedDirs = currentAgentDir ? [
            join(currentAgentDir, 'hamuna_files', 'generated_images'),
            join(currentAgentDir, 'hamuna_files', 'temp'),
            join(currentAgentDir, 'hamuna-generated', 'images'), // backward compat
          ] : [];
      ...
      if (pathname === '/api/audio' && request.method === 'GET') {
        try {
          ...
          // Security: allow reading from workspace/hamuna_files/generated_audio/ or legacy paths
          const resolvedPath = resolve(audioPath);
          const legacyAudioDir = join(homedir(), '.hamuna', 'generated_audio');
          const legacyAudioDirSep = legacyAudioDir.endsWith(sep) ? legacyAudioDir : legacyAudioDir + sep;
          // New unified path + backward compat with hamuna-generated/audio/
          const allowedAudioDirs = currentAgentDir ? [
            join(currentAgentDir, 'hamuna_files', 'generated_audio'),
            join(currentAgentDir, 'hamuna-generated', 'audio'), // backward compat
          ] : [];
      ...
      if (pathname === '/api/logs/export' && request.method === 'GET') {
        try {
          ...
          const logsDir = joinPath(homedir(), '.hamuna', 'logs');
          ...
          const zipName = `HamunaAgent-logs-${timestamp}.zip`;
                ...
                response = await fetchWithGeneralProxy(server.url, {
                  ...
                  body: JSON.stringify({
                    ...
                    params: {
                      ...
                      capabilities: {},
                      clientInfo: { name: 'HamunaAgent', version: '0.1.29' },
                    },
                  }),
      ...
      const homeDir = getHomeDirOrNull() || '';
      const userSkillsBaseDir = join(homeDir, '.hamuna', 'skills');
      const userCommandsBaseDir = join(homeDir, '.hamuna', 'commands');
      ...
      if (pathname === '/api/skill/toggle-enable' && request.method === 'POST') {
        try {
          ...
          if (!enabled && isRequiredSystemSkill(folderName)) {
            return jsonResponse({
              success: false,
              error: `${folderName} is a required HamunaAgent system skill and cannot be disabled`,
            }, 409);
          }
          const config = readSkillsConfig();
          if (enabled) {
      ...
      if (pathname === '/api/skill/sync-check' && request.method === 'GET') {
        try {
          ...
          // Get existing folders in HamunaAgent skills directory.
          ...
          const hamunaFolders = new Set<string>();
          if (existsSync(userSkillsBaseDir)) {
            ...
            for (const entry of entries) {
              if (isDirEntry(entry, join(userSkillsBaseDir, entry.name))) {
                hamunaFolders.add(entry.name);
              }
            }
          }

          // Find folders that can be synced (exist in Claude but not in HamunaAgent)
          const syncableFolders = claudeFolders.filter(folder => !hamunaFolders.has(folder));
      ...
      // POST /api/skill/sync-from-claude - Sync skills from Claude Code to HamunaAgent
      // NOTE: This route MUST be before /api/skill/:name to avoid being captured by the wildcard
      if (pathname === '/api/skill/sync-from-claude' && request.method === 'POST') {
        try {
          ...
          // Ensure HamunaAgent skills directory exists
          if (!existsSync(userSkillsBaseDir)) {
            ensureDirSync(userSkillsBaseDir);
          }

          // Get existing folders in HamunaAgent skills directory (follow junctions — issue #104)
          const hamunaFolders = new Set<string>();
          const entries = readdirSync(userSkillsBaseDir, { withFileTypes: true });
          for (const entry of entries) {
            if (isDirEntry(entry, join(userSkillsBaseDir, entry.name))) {
          ...
          const syncableFolders = claudeFolders.filter(folder =>
            !hamunaFolders.has(folder) && isValidFolderName(folder)
          );
      ...
      // POST /api/skill/copy-to-global - Copy a project skill to global (~/.hamuna/skills/)
      // NOTE: This route MUST be before /api/skill/:name to avoid being captured by the wildcard
      ...
      //
      // This deliberately does not write to ~/.hamuna/skills or a workspace.
      ...
      const userAgentsBaseDir = join(homeDir, '.hamuna', 'agents');
      ...
      if (pathname === '/api/agent/sync-check' && request.method === 'GET') {
        try {
          ...
          const hamunaAgents = scanAgents(userAgentsBaseDir, 'user');
          const hamunaSet = new Set(hamunaAgents.map(a => a.folderName));
          ...
          const newFolders = claudeAgents.filter(a => !hamunaSet.has(a.folderName)).map(a => a.folderName);
          const conflictFolders = claudeAgents.filter(a => hamunaSet.has(a.folderName)).map(a => a.folderName);
      ...
      // POST /api/agent/sync-from-claude - Sync agents from Claude Code to HamunaAgent
      // NOTE: Must be before /api/agent/:name to avoid wildcard capture
      // Supports conflict handling: mode = 'skip' (default) | 'overwrite'
      //
      // Preserves the source agent's layout:
      //   folder  (.claude/agents/foo/foo.md)        → ~/.hamuna/agents/foo/foo.md  + _meta.json
      //   flat    (.claude/agents/foo.md)            → ~/.hamuna/agents/foo.md       (no _meta.json — flat has no home for it)
      //   nested  (.claude/agents/team/reviewer.md)  → ~/.hamuna/agents/team/reviewer.md  (ditto)
      //
      ...
      if (pathname === '/api/im/enqueue' && request.method === 'POST') {
        try {
          ...
          if (payload.botId && process.env.HAMUNA_MANAGEMENT_PORT) {
            ...
            // appear "disconnected".
            //
            // (v0.2.11) `im-media` was retired here — `hamuna im send-media`
        ...
        try {
          const payload = await request.json() as {
            ...
            pendingCronEvents?: Array<{
              ...
              // the IM Bot AI can `hamuna session send <fromSessionId>` to
...
`;
      ...
      if (pathname === '/api/memory/update' && request.method === 'POST') {
        try {
          ...
          // disable the versioned hamuna-memory-update system skill.
      ...
      //
      // Note: `hamuna session send` CLI hits `/api/admin/session/send`
      ...
      if (pathname === '/api/inbox/drain' && request.method === 'POST') {
        try {
          ...
          // HAMUNA_AGENT_DIR env is not reliable for sidecar-to-sidecar inbox.
          const engine = getSessionEngine();
  ...
  // Routes (except /health) `await __hamunaDeferredInit` before running,
  ...
  (async () => {
    try {
      ...
      // Diff-merge bundled extended MCP entries (extended_buildin_mcp/mcp.json)
      // into ~/.hamuna/config.json::mcpServers so they spawn automatically on
      // first install without the user enabling them in Settings. Best-effort —
      // atomicModifyConfig failures (lock contention) just retry next startup.
  ...
  // then — sufficient for common binary lookups (.hamuna/bin, homebrew, nvm,
