...
//
// 由 CLI `hamuna session send` 通过 sidecar admin API 调用。流程:
...
export interface AdminInboxResponse {
  ...
  messageId?: string;
  /** Whether HamunaAgent will push the target turn result back to the caller. */
  replyBack?: boolean;
...
export async function handleAdminInbox(
  ...
  const managementPort = process.env.HAMUNA_MANAGEMENT_PORT;
  if (!managementPort) {
    return {
      status: 500,
      response: {
        delivered: false,
        error: { code: 'delivery_failed', message: 'HAMUNA_MANAGEMENT_PORT not set' },
      },
    };
  }
