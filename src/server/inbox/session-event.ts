  ...
  switch (event.type) {
    case 'send.request':
      return event.sourceNotification === 'none'
        ? "Another HamunaAgent session sent this session a one-way request or notification. The source session will not automatically receive this turn's final result."
        : "Another HamunaAgent session sent this session a request. Work on it normally in this session. When this turn finishes, HamunaAgent will automatically deliver this turn's final result back to the source session.";
    case 'send.result':
      return event.status === 'error'
        ? 'HamunaAgent attempted to deliver the final result of the target session turn triggered by your previous `session send` request, but that turn did not complete successfully.'
        : 'HamunaAgent automatically delivered the final result of the target session turn triggered by your previous `session send` request.';
    ...
    case 'watch.error':
      return 'HamunaAgent could not confirm normal completion for the watched target session.';
  }
}
