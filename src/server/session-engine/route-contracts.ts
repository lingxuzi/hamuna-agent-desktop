...
export const SESSION_ENGINE_ROUTE_CONTRACTS: SessionEngineRouteContract[] = [
  ...
  },
  {
    ...
    failureStatuses: [200, 500],
    behavior: 'Injects memory maintenance through the active engine and gates completion on true turn success; manual and automatic turns both require the exact official hamuna-memory-update skill at the Runtime dispatch boundary, managed auto-update carries exact Task queue ownership for authorization and stop, and timeout/turn_failed remain body-level status values on HTTP 200.',
  },
  {
  ...
  },
  {
    ...
    failureStatuses: [500],
    behavior: 'Applies per-session HamunaAgent official CLI tool selections through the active engine; prompt-changing external runtimes persist metadata and drop idle/prewarm processes.',
  },
  {
    path: '/api/agents/set',
    method: 'POST',
