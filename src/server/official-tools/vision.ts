...
const SYSTEM_PROMPT = [
  'You are HamunaAgent Vision Helper.',
...
export function getVisionToolReadme(): string {
  return `hamuna vision — official image-understanding helper

Usage:
  hamuna vision analyze --image <path> [--image <path> ...] [--prompt "..."] [--json]
  hamuna vision analyze --image <path> --prompt-file <workspace-relative-text-file>
  hamuna vision analyze --image @hamuna_files/screenshot.png --prompt "Read the error message"
  hamuna vision readme

The tool only accepts local image paths inside the current HamunaAgent workspace.
Use @hamuna_files/... paths when the conversation shows attached images as
...
export async function analyzeImages(input: VisionAnalyzeInput): Promise<VisionAnalyzeResult> {
  ...
  if (!workspacePath) {
    throw new VisionToolError('No current workspace is available for image path resolution.', 400, {
      message: 'Run this command from an active HamunaAgent session with a workspace.',
    });
  }
...
async function runVisionQueryInner(args: {
  ...
  const visionQuery = query({
    ...
    options: {
      maxTurns: 1,
      sessionId,
      cwd: args.workspacePath || resolve(homedir(), '.hamuna', 'projects'),
      settingSources: [],
