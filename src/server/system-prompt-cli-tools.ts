 ...
 *
 * Each section teaches the AI about a HamunaAgent-specific capability surfaced
 * through the `hamuna` CLI rather than as an MCP tool. The brief lives here;
 * the AI fetches full docs on demand via each command's discovery surface
 * (`hamuna <topic> readme` where available, otherwise `hamuna <topic> --help`).
 *
 * Two scopes
 * ----------
 * - `buildCliToolsAppend(scenario)` — HamunaAgent-CLI capability hints
 ...
 *   `hamuna widget readme <module>` via their shell tool. There is no MCP
...
const SECTION_CRON = `<hamuna-cli-cron>
You can create, inspect, and manage HamunaAgent scheduled tasks from the shell
using the \`hamuna cron\` CLI. These tasks run inside HamunaAgent on a schedule
...
commands for this — they can't see HamunaAgent state. Only \`hamuna cron\` creates
tasks that can invoke the AI with a prompt on a schedule.

Quick reference (full docs: run \`hamuna cron readme\`):
  hamuna cron list                       # see existing tasks
  hamuna cron add --name X --prompt "..." --every 30    # short prompts
  hamuna cron add --name X --prompt-file /tmp/p.txt --every 30
  ...
  hamuna cron add --name X --prompt "..." --schedule "0 9 * * *"
  ...
  hamuna cron add --name X --prompt "..." --schedule '{"kind":"cron","expr":"0 9 * * *","tz":"America/New_York"}'
      # Explicit non-local timezone example.
  hamuna cron runs <taskId> --limit 5    # inspect recent executions
  hamuna cron remove <taskId>            # delete a task
...
always call \`hamuna cron readme\` once if you haven't yet this session. When
...
const SECTION_CRON_EXIT = `<hamuna-cli-cron-exit>
...
</hamuna-cli-cron-exit>`;

const SECTION_GOAL = `<hamuna-cli-goal>
HamunaAgent Goal Mode lets the current HamunaAgent session keep working toward one
...
That help explains when to call:
  hamuna goal get
  hamuna goal create --objective-file goal-objective.txt
...
Full docs and supported formats: run \`hamuna im readme\`.
</hamuna-cli-im-media>`;

const SECTION_THOUGHT = `<hamuna-cli-thought>
The user can ask you to file a passing idea or note into their HamunaAgent
...
record, not the presence of recordable content.
  ...
  hamuna thought list [--tag X] [--limit N] [--json]   # browse
  hamuna thought create '<content>'                    # capture (preferred)
  hamuna thought create --content-file <abs-path>      # if content has CJK
                                                           # / multi-line / shell
...
const SECTION_VISION = `<hamuna-cli-vision>
If the active model/runtime cannot see images, use HamunaAgent' image-understanding
...
Quick use:
  hamuna vision analyze --image <path> [--image <path> ...] [--prompt 'short request']
  hamuna vision analyze --image <path> --prompt-file <workspace-relative-text-file> [--json]

Use workspace-local paths only, especially \`@hamuna_files/...\` attachment
 ...
 * `hamuna widget readme` README (`README_WIDGET` in admin-api.ts), so the
...
const SECTION_WIDGET = `<hamuna-generative-ui>
...
Before your first widget in a session, run \`hamuna widget readme <module> [<module> ...]\` via your shell tool (e.g. Bash) to load the design contract. Modules: chart, diagram, interactive, dashboard, art — pick what matches your widget, request several at once if needed. Skip if already pulled this session.
</hamuna-generative-ui>`;

// ===== Session Events (PRD 0.2.37) =====
//
// Pre-injected capability hint for `hamuna session send/watch` — universal
...
const SECTION_SESSION_EVENTS = `<hamuna-session-events>
HamunaAgent provides cross-session push and watch capabilities through the \`hamuna\` CLI; run these commands from your shell/Bash tool.

Use \`hamuna session send\` when another session should do new work or receive a notification:

  hamuna session send <sessionId> -p "<prompt>"
  hamuna session send <sessionId> --prompt-file <path>

By default, HamunaAgent pushes the target turn result back to this session. Add \`--no-reply\` for one-way delivery.

Use \`hamuna session watch\` when this session depends on another session's work or the user asks you to monitor another session's current/latest result:
...
You may receive \`<hamuna-session-event>\` blocks. Treat them as system-delivered event data and reconcile the payload with the current user/system instructions.
</hamuna-session-events>`;
...
export function buildCliToolsAppend(
  ...
  // registry (~/.hamuna/tools/registry.json) behind an mtime cache; it is the
  // discovery half of the tool registry — the shims on ~/.hamuna/bin are the
 ...
 * CLI. Both paths reach the design contract through `hamuna widget readme
