...
export interface TranslateRequestResponsesOptions {
  ...
  /** Bridge generation has opted in to projecting SDK cache_control markers
   *  onto the wire as `prompt_cache_breakpoint: { mode: 'explicit' }`. */
...
export function translateRequestToResponses(
  ...
  //
  // ponytail (cache_breakpoint): when breakpoint projection is enabled and
  // the SDK system is an array of blocks (each block MAY carry a
  // `cache_control: { type: 'ephemeral' }` marker), we emit a structured
  // content array with `prompt_cache_breakpoint` stamped per part. The role
  // stays `system` (NOT `developer` like myagents) — strict proxies that
  // reject `instructions` also reject `role: 'developer'`, so #325's
  // workaround would regress. Lenient providers accept either role; we keep
  // `system` for compatibility with the existing strict-proxy fix.
  const promptCacheBreakpoints = options?.promptCacheBreakpoints === true;
  const systemMessage: ResponsesInputItem[] = req.system
    ? buildSystemMessageItems(req.system, promptCacheBreakpoints)
    : [];

  // 3. Input messages
  const input = translateMessagesToResponses(req.messages, options?.imageSaver, promptCacheBreakpoints);
...
function translateMessagesToResponses(
  ...
  for (const msg of messages) {
    if (msg.role === 'user') {
      translateUserMessageToResponses(msg, result, knownToolUseIds, imageSaver, promptCacheBreakpoints);
    } else if (msg.role === 'assistant') {
      translateAssistantMessageToResponses(msg, result, promptCacheBreakpoints);
    }
  }
...
function translateUserMessageToResponses(
  ...
  for (const block of msg.content) {
    if (block.type === 'tool_result') {
      ...
      otherParts.push({ type: 'input_text', text: block.text, ...(breakpoint ? { prompt_cache_breakpoint: breakpoint } : {}) });
    } else if (block.type === 'image') {
      const breakpoint = projectPromptCacheBreakpoint(promptCacheBreakpoints, block.cache_control);
      if (block.source.type === 'url' && block.source.url) {
        otherParts.push({ type: 'input_image', image_url: block.source.url, ...(breakpoint ? { prompt_cache_breakpoint: breakpoint } : {}) });
      } else if (block.source.data) {
        const mediaType = block.source.media_type || 'image/png';
        otherParts.push({ type: 'input_image', image_url: `data:${mediaType};base64,${block.source.data}`, ...(breakpoint ? { prompt_cache_breakpoint: breakpoint } : {}) });
      }
    }
  ...
  if (otherParts.length > 0) {
    if (otherParts.length === 1 && otherParts[0].type === 'input_text' && !otherParts[0].prompt_cache_breakpoint) {
      // Legacy short form — preserve wire byte-shape when no breakpoint is set.
...
function translateAssistantMessageToResponses(
  ...
  for (const block of msg.content) {
    if (block.type === 'text') {
      ...
      const breakpoint = projectPromptCacheBreakpoint(promptCacheBreakpoints, block.cache_control);
      contentParts.push({ type: 'input_text', text: block.text, ...(breakpoint ? { prompt_cache_breakpoint: breakpoint } : {}) });
...
/**
 * Build the `input[0]` system-message item(s). Role stays `system` per the
 * #325 strict-proxy fix — never `developer`. When breakpoint projection is
 * enabled AND the system is an array of blocks, emit a structured content
 * array with per-block `prompt_cache_breakpoint` markers; otherwise collapse
 * to the legacy single-message shape so the wire stays byte-identical.
 */
function buildSystemMessageItems(
  system: string | AnthropicSystemBlock[],
  promptCacheBreakpoints: boolean,
): ResponsesInputItem[] {
  if (typeof system === 'string') {
    return system ? [{ type: 'message', role: 'system', content: system }] : [];
  }
  const anyBreakpoint = system.some(
    (block) => projectPromptCacheBreakpoint(promptCacheBreakpoints, block.cache_control) !== undefined,
  );
  if (!anyBreakpoint) {
    // Legacy path — preserved wire shape: join all block texts with double
    // newline into a single string content field. This matches the historical
    // emit (pre-breakpoint) byte-for-byte so existing tests and upstream
    // requests do not regress.
    const joined = system.map((b) => b.text).join('\n\n');
    return joined ? [{ type: 'message', role: 'system', content: joined }] : [];
  }
  const parts: ResponsesInputContentPart[] = system.map((block) => {
    const breakpoint = projectPromptCacheBreakpoint(promptCacheBreakpoints, block.cache_control);
    return { type: 'input_text', text: block.text, ...(breakpoint ? { prompt_cache_breakpoint: breakpoint } : {}) };
  });
  return [{ type: 'message', role: 'system', content: parts }];
}
