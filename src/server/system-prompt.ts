...
const TMPL_BASE_IDENTITY = `<hamuna-identity>
...
const TMPL_CHANNEL_DESKTOP = `<hamuna-interaction-channel>
用户正通过 HamunaAgent 桌面客户端与你对话。
</hamuna-interaction-channel>`;

const TMPL_CHANNEL_IM = `<hamuna-interaction-channel>
你正通过 {{platformLabel}} 作为 IM 聊天机器人与用户对话，{{sourceTypeLabel}}。{{#if botName}}你的昵称为「{{botName}}」。{{/if}}
</hamuna-interaction-channel>`;

const TMPL_CRON_TASK = `<hamuna-cron-task-instructions>
...
const TMPL_HEARTBEAT = `<hamuna-heartbeat-instructions>
...
const TMPL_REGISTERED_AGENT = `<hamuna-registered-agent-instructions space-id="{{spaceId}}" registered-agent-id="{{registeredAgentId}}">
...
const TMPL_FLOATING_BALL = `<hamuna-floating-ball-instructions>
...
const TMPL_BROWSER_STORAGE_STATE = `<hamuna-browser-storage-instructions>
...
export interface SystemPromptOptions {
  ...
  runtime?: RuntimeType;
  /**
   * Append the `hamuna` CLI capability hints (cron / IM media) to the
  ...
  /**
   * Include user-registered CLI tools from ~/.hamuna/tools/registry.json in
  ...
  /** Effective HamunaAgent official CLI tools enabled for this session. */
...
export function buildSystemPromptAppend(scenario: InteractionScenario, options?: SystemPromptOptions): string {
  ...
  // `hamuna widget readme <module>` invoked through their shell tool.
