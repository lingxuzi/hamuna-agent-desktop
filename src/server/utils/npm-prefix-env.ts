...
export function getHamunaAgentNpmGlobalPrefix(
  home: string,
  platform = process.platform,
): string | null {
  if (!home) return null;
  return isWindowsPlatform(platform)
    ? pathWin32.resolve(home, '.hamuna', 'npm-global')
    : `${home}/.hamuna/npm-global`;
}

export function getHamunaAgentNpmGlobalBinDir(
  home: string,
  platform = process.platform,
): string | null {
  const prefix = getHamunaAgentNpmGlobalPrefix(home, platform);
...
export function scrubHamunaAgentNpmPrefixEnv(
