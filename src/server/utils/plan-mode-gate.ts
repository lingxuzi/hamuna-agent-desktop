 ...
 * WHY THIS EXISTS
 * ---------------
 * HamunaAgent launches the Claude Agent SDK subprocess with
 ...
 * `canUseTool` callback (where HamunaAgent enforces the plan-mode read-only rules)
 ...
 * `mcp__*` tools, the `hamuna` CLI, …). Returns `false` (allow normal flow)
