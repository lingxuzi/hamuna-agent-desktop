...
export interface CuseSkillCacheDiagnostic extends CuseBinaryDiagnostic {
  source: 'workspace' | 'hamuna-user' | 'codex-user' | 'claude-user';
  label: string;
    ...
    candidates.push(
      {
        source: 'hamuna-user',
        label: '~/.hamuna skill',
        path: join(homeDir, '.hamuna', 'skills', 'cuse', 'scripts', binaryName),
      },
      {
        source: 'codex-user',
        label: '~/.codex skill',
...
async function inspectSkillCaches(
  ...
  const safeRoots = existingRealRoots([
    workspacePath,
    homeDir ? join(homeDir, '.hamuna', 'skills', 'cuse') : null,
