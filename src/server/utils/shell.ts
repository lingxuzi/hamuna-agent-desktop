...
import { getHamunaAgentNpmGlobalBinDir } from './npm-prefix-env';
...
export function getFallbackPaths(options: FallbackPathOptions = {}): string[] {
    ...
    if (platform === 'win32') {
        ...
        // then bundled Node, HamunaAgent-managed npm installs, and HamunaAgent CLI.
        ...
        pushPath(paths, localAppData ? joinForPlatform(platform, localAppData, 'HamunaAgent', 'nodejs') : '', platform);
        pushPath(paths, getHamunaAgentNpmGlobalBinDir(userProfile, platform), platform);
        pushPath(paths, userProfile ? joinForPlatform(platform, userProfile, '.hamuna', 'bin') : '', platform);
    ...
    pushPath(paths, bundledNodeDir, platform);
    pushPath(paths, getHamunaAgentNpmGlobalBinDir(home, platform), platform);
    // ~/.hamuna/bin stays before generic user package-manager dirs so external
    // runtime shell tools can still find the `hamuna` CLI.
    pushPath(paths, home ? `${home}/.hamuna/bin` : '', platform);
 ...
 * env so HamunaAgent behaves consistently with terminal invocations.
 *
...
export function warmupShellPath(): Promise<void> {
    ...
    warmupInFlight = new Promise<void>((resolve) => {
        ...
        const pathMarker = `__HAMUNA_PATH_${process.pid}__`;
        const proxyMarker = `__HAMUNA_PROXY_${process.pid}__`;
 ...
 *     caller can safely strip HamunaAgent-injected proxy vars from the subprocess
