...
function makeRoot(): string {
  const root = mkdtempSync(join(tmpdir(), 'hamuna-plan-files-'));
  roots.push(root);
  return root;
}
...
describe('session plan files', () => {
  it('uses a sanitized session-scoped plansDirectory setting', () => {
    ...
    expect(getSessionPlansDirectorySetting('sid/../bad:value')).toBe('.claude/plans/hamuna/sid_bad_value');
    expect(getSessionPlansDirectoryPath('/workspace', 'sid/../bad:value')).toContain(join('.claude', 'plans', 'hamuna', 'sid_bad_value'));
    expect(SESSION_PLANS_GITIGNORE_PATTERN).toBe('.claude/plans/hamuna/');
  });

  it('reads the latest markdown plan newer than the current-turn cutoff', async () => {
    const root = makeRoot();
    const dir = join(root, '.claude', 'plans', 'hamuna', 'session-1');
  ...
  it('retries briefly for a plan file created during approval handoff', async () => {
    const dir = join(makeRoot(), '.claude', 'plans', 'hamuna', 'session-1');
    mkdirSync(dir, { recursive: true });
  ...
  it('does not read a plans directory symlink that resolves outside the expected root', async () => {
    ...
    const parent = join(root, '.claude', 'plans', 'hamuna');
