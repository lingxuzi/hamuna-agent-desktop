...
export interface SystemSkillExposureInput {
  ...
  /** Test seam; production defaults to ~/.hamuna. */
  hamunaRoot?: string;
}
...
export function assertOfficialSystemSkillExposed(input: SystemSkillExposureInput): void {
  const hamunaRoot = input.hamunaRoot
    ?? join(getHomeDirOrNull() || '', '.hamuna');
  if (!hamunaRoot || hamunaRoot === '.hamuna') {
    throw new Error('HamunaAgent home directory is unavailable');
  }

  const versionPath = join(hamunaRoot, '.system-skills-version');
  ...
  const officialDir = join(hamunaRoot, 'skills', input.skillName);
