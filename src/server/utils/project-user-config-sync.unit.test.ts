...
import { getHamunaAgentUserDir, syncProjectUserConfigFiles, trySyncProjectUserConfigFiles } from './project-user-config-sync';
...
describe('project-user-config-sync', () => {
  ...
  function makeEnv(): { root: string; home: string; workspace: string } {
    const root = join(tmpdir(), `hamuna-project-sync-${Date.now()}-${Math.random().toString(16).slice(2)}`);
  ...
  function writeUserSkill(home: string, name: string): void {
    const dir = join(home, '.hamuna', 'skills', name);
    mkdirSync(dir, { recursive: true });
    writeFileSync(join(dir, 'SKILL.md'), `---\nname: ${name}\ndescription: ${name}\n---\n`);
  }

  function writeUserCommand(home: string, name: string): void {
    const dir = join(home, '.hamuna', 'commands');
    mkdirSync(dir, { recursive: true });
    writeFileSync(join(dir, `${name}.md`), `# ${name}\n`);
  }

  it('links enabled HamunaAgent user skills into the project .claude/skills directory', () => {
    ...
    expect(getHamunaAgentUserDir()).toBe(join(home, '.hamuna'));
  });
  ...
  it('removes managed skill symlinks when the skill is disabled', () => {
    ...
    const configPath = join(home, '.hamuna', 'skills-config.json');
  ...
  it('keeps all required system skills exposed while honoring an optional system skill disable', () => {
    ...
    writeFileSync(
      join(home, '.hamuna', 'skills-config.json'),
  ...
  itNonWindows('links HamunaAgent user commands into the project .claude/commands directory', () => {
    const { home, workspace } = makeEnv();
