...
const MGMT_PORT = process.env.HAMUNA_MANAGEMENT_PORT;
const SIDECAR_GENERATION = process.env.HAMUNA_SIDECAR_GENERATION;
...
export async function resolveManagedOAuthCredential(
  ...
  const sessionId = process.env.HAMUNA_SESSION_ID?.trim();
  if (!sessionId) {
