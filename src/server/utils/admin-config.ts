 ...
 * instead of Tauri plugin-fs. Both read/write the same ~/.hamuna/config.json.
...
export interface AdminAppConfig {
  ...
  // mcpServerEnv. Read at launch by the ~/.hamuna/bin shims — env changes
  ...
  // HamunaAgent official CLI tools
...
export function getProvidersDir(): string {
  const home = getHomeDirOrNull();
  if (!home) throw new Error('Cannot determine home directory');
  return resolve(home, '.hamuna', 'providers');
}

/** Find a provider by ID: checks PRESET_PROVIDERS first, then custom files in ~/.hamuna/providers/ */
...
/** Load all custom provider files from ~/.hamuna/providers/ */
...
export function resolveWorkspaceConfig(
  ...
  // --- Resolve HamunaAgent official CLI tools ---
