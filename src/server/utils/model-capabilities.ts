 ...
 *   1. `~/.hamuna/providers/*.json` — user-defined custom providers.
 *   2. `~/.hamuna/config.json::presetCustomModels[providerId][]` — models
 ...
 *   4. `~/.hamuna/cache/litellm_model_prices.json` — LiteLLM community data,
...
// Safety caps for malicious / runaway inputs. A rogue ~/.hamuna/providers/
...
const LITELLM_CACHE_REL = ['.hamuna', 'cache', 'litellm_model_prices.json'] as const;
...
function loadCustomProvidersFromDisk(home: string): Array<Record<string, unknown>> {
  const dir = resolve(home, '.hamuna', 'providers');
  ...
  try {
    ...
    console.warn('[model-caps] readdir failed for ~/.hamuna/providers/:', (err as Error)?.message ?? err);
    return [];
  }
  if (files.length > MAX_PROVIDER_FILES) {
    console.warn(`[model-caps] ~/.hamuna/providers/ has ${files.length} files; processing first ${MAX_PROVIDER_FILES}`);
...
function loadPresetCustomModels(home: string): Record<string, unknown> | null {
  const configPath = resolve(home, '.hamuna', 'config.json');
  ...
  try {
    ...
    console.warn('[model-caps] failed to read ~/.hamuna/config.json:', (err as Error)?.message ?? err);
    return null;
  }
}
 ...
 *   - `~/.hamuna/providers/`  (mtime changes when a file is created /
 *     deleted / replaced via tmp+rename — the canonical edit pattern)
 *   - `~/.hamuna/config.json` (mtime changes on `presetCustomModels` edits)
 *
...
function buildRegistry(): Map<string, ModelCapability> {
  ...
  const providersDir = home ? resolve(home, '.hamuna', 'providers') : '';
  const configPath = home ? resolve(home, '.hamuna', 'config.json') : '';
  ...
  // 1) Custom providers from ~/.hamuna/providers/*.json — disk-first override.
  if (home) {
  ...
  // 4) LiteLLM cached catalog (Rust-maintained ~/.hamuna/cache/) — LOWEST
 ...
 * auto-compact threshold / free-space rows, which stay correct. HamunaAgent' own
