 ...
 *
 * 磁盘布局：
 *   ~/.hamuna/tools/<name>/          工具目录（tool.json + entry，自包含）
 *   ~/.hamuna/tools/registry.json    产品侧状态（enabled/registeredAt）+ description 缓存
 *   ~/.hamuna/bin/<name>             POSIX 启动器（shim，已在所有 runtime 与终端的 PATH 上）
 *   ~/.hamuna/bin/<name>.cmd         Windows 启动器（+ <name>.launcher.cjs）
 *
 * 关键设计：
...
function getHamunaAgentDir(): string {
...
export function getCliToolsDir(): string {
  return join(getHamunaAgentDir(), 'tools');
}
...
export function getCliToolsBinDir(): string {
  return join(getHamunaAgentDir(), 'bin');
}
 ...
 * ~/.hamuna/bin 在 PATH 中先于系统路径（buildClaudeSessionEnv），
...
export function findPathCollision(name: string, effectivePath?: string): string | null {
  ...
  const candidates = process.platform === 'win32'
    ...
    : [name];
  // 显式补上 HamunaAgent 自有的 AI 自装 CLI 落点，防止注册工具遮蔽 AI 装过的命令
  const extraDirs = [join(getHamunaAgentDir(), 'npm-global', 'bin')];
...
// Auto-generated by \`hamuna tool add\` — do not edit (re-running tool add regenerates it).
// Launcher for ${JSON.stringify(name)}: injects per-tool env from ~/.hamuna/config.json
...
let toolEnv = {};
try {
  const cfg = JSON.parse(readFileSync(join(homedir(), '.hamuna', 'config.json'), 'utf8'));
...
if (result.error) {
  console.error('[hamuna-tool] failed to launch ' + TOOL_NAME + ': ' + result.error.message);
  process.exit(1);
}
...
/**
 * 构建 `<hamuna-user-tools>` 段（纯函数，单测覆盖保险丝行为）。
...
export function buildUserToolsSectionText(tools: ReadonlyArray<{ name: string; description: string }>): string {
  ...
  const overflowLine = overflow > 0
    ? `\n\n…and ${overflow} more registered tool(s) — run \`hamuna tool list\` to see all.`
    : '';
  return `<hamuna-user-tools>
The user has registered the following CLI tools in HamunaAgent. They are on PATH —
