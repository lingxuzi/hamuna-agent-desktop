...
describe('buildCronTaskReminder', () => {
  it('puts operational cron instructions in the hidden reminder and leaves prompt as visible text', () => {
    ...
    expect(wrapped).toBe([
      ...
      'allowExit: true',
      '',
      'If this HamunaAgent scheduled task goal is complete and future executions should stop, run:',
      '  hamuna cron exit --reason "<brief reason>"',
      '',
  ...
  it('omits exit command guidance when AI exit is disabled', () => {
    ...
    expect(wrapped).not.toContain('hamuna cron exit');
  });
