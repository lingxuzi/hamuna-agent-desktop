...
/**
 * HamunaAgent-reserved MCP server ids — names used by context-injected builtins
 ...
 * `hamuna` CLI commands + system prompt guidance (single CLI surface usable
 ...
 */
export const HAMUNA_CONTEXT_INJECTED_MCP_IDS = [
  'im-bridge-tools',
] as const;
 ...
 *
 * Skills (directories):
 * - Creates symlinks for enabled skills: <project>/.claude/skills/<name> → ~/.hamuna/skills/<name>
 ...
 *
 * Commands (.md files):
 * - Creates symlinks for all commands: <project>/.claude/commands/<name>.md → ~/.hamuna/commands/<name>.md
...
function publishCurrentSessionEnv(): void {
  process.env.HAMUNA_SESSION_ID = sessionId;
}
 ...
 *
 *  Side effect: exports `HAMUNA_PORT` to `process.env` so every subprocess
 ...
 *  `hamuna` CLI without the CLI bailing with `HAMUNA_PORT not set`. This is
 ...
 *
 *  The builtin SDK path still sets `env.HAMUNA_PORT` explicitly in
...
export function setSidecarPort(port: number): void {
  sidecarPort = port;
  if (port > 0) {
    process.env.HAMUNA_PORT = String(port);
  }
}
...
// (i.e. the message came in via /api/inbox/drain from a `hamuna session send`
...
/**
 * Per-Tab override for HamunaAgent official CLI tools. This only changes the
 ...
 * restart the builtin SDK only when the current provider's effective HamunaAgent
 ...
 */
/**
 * Force a session restart triggered by an explicit `hamuna reload`.
 *
...
/**
 * Predicate for each HamunaAgent-reserved context-injected builtin MCP id.
 ...
 *
 * The `Record<typeof HAMUNA_CONTEXT_INJECTED_MCP_IDS[number], …>` shape
...
const CONTEXT_INJECTED_BUILTIN_PREDICATES: Record<
  typeof HAMUNA_CONTEXT_INJECTED_MCP_IDS[number],
  () => boolean
> = {
...
function getActiveContextInjectedBuiltinIds(): Set<string> {
  ...
  for (const id of HAMUNA_CONTEXT_INJECTED_MCP_IDS) {
...
function checkMcpToolPermission(toolName: string): { allowed: true } | { allowed: false; reason: string } {
  ...
  // For ids in HAMUNA_CONTEXT_INJECTED_MCP_IDS but NOT currently active,
 ...
 * - Our product uses ~/.hamuna/ for user-level config
 ...
 *    capabilities through the `hamuna` CLI + system prompt guidance,
...
async function buildSdkMcpServers(): Promise<Record<string, McpServerEntry>> {
  ...
  const servers = userServers.filter(s => {
    ...
    // Reserve HamunaAgent context-injected builtin ids: a user MCP with the same id
    // would otherwise overwrite the builtin in result[id] and inherit its
    // auto-trust in canUseTool — see issue #148.
    if ((HAMUNA_CONTEXT_INJECTED_MCP_IDS as readonly string[]).includes(normalized)) {
      console.warn(`[agent] MCP "${s.id}" skipped: id is reserved by HamunaAgent (context-injected builtin). Rename to avoid this.`);
      return false;
    }
    return true;
  });
...
const WINDOWS_UTF8_BASH_ENV_SENTINEL = 'HAMUNA_WINDOWS_UTF8';
const WINDOWS_UTF8_BASH_ENV_FILENAME = 'windows-utf8-bash-env.sh';
const WINDOWS_UTF8_BASH_ENV_CONTENT = [
  '# HamunaAgent-managed Bash prelude for Windows SDK tool output.',
  `if [ -n "\${HAMUNA_ORIGINAL_BASH_ENV:-}" ] && [ "\${HAMUNA_ORIGINAL_BASH_ENV}" != "\${BASH_ENV:-}" ] && [ -r "\${HAMUNA_ORIGINAL_BASH_ENV}" ]; then`,
  '  . "${HAMUNA_ORIGINAL_BASH_ENV}"',
  'fi',
...
function ensureWindowsUtf8BashEnvScript(home: string): string | undefined {
  ...
  const scriptDir = resolve(home, '.hamuna', 'runtime');
...
export function applyWindowsUtf8SubprocessEnv(
  ...
  if (existingBashEnvForShell && existingBashEnvForShell !== bashEnvScriptForShell) {
    env.HAMUNA_ORIGINAL_BASH_ENV = existingBashEnvForShell;
  }
...
export function buildClaudeSessionEnv(
  ...
  const myAgentsNpmGlobalPrefix = getHamunaAgentNpmGlobalPrefix(home);
  const myAgentsNpmGlobalBinDir = getHamunaAgentNpmGlobalBinDir(home);
  ...
  // HamunaAgent-managed npm global bin dir. It stays on PATH so tools installed
  // by HamunaAgent-localized npm commands (see HAMUNA_NPM_GLOBAL_PREFIX below)
  // are immediately invocable. This dir comes BEFORE `~/.hamuna/bin` in
  // essentialPaths so:
  //   1. AI-installed tools (e.g. agent-browser) shadow any legacy
  //      `~/.hamuna/bin/<name>` wrapper from older app versions —
  //      legacy wrappers naturally fall idle without explicit cleanup.
  //   2. Existing installs made by older HamunaAgent versions remain discoverable
  //      after we stopped leaking npm_config_prefix globally.
  if (myAgentsNpmGlobalBinDir) {
    essentialPaths.push(myAgentsNpmGlobalBinDir);
  }

  // HamunaAgent bin directory — user-facing commands (the `hamuna` CLI itself).
  ...
  if (home) {
    const hamunaBinDir = isWindows
      ? resolve(home, '.hamuna', 'bin')
      : `${home}/.hamuna/bin`;
    essentialPaths.push(hamunaBinDir);
  }
  ...
  //   npm_config_prefix="$HAMUNA_NPM_GLOBAL_PREFIX" npm install -g <pkg>
  if (myAgentsNpmGlobalPrefix) {
    env.HAMUNA_NPM_GLOBAL_PREFIX = myAgentsNpmGlobalPrefix;
    if (myAgentsNpmGlobalBinDir) {
      env.HAMUNA_NPM_GLOBAL_BIN = myAgentsNpmGlobalBinDir;
    }
    scrubHamunaAgentNpmPrefixEnv(env, myAgentsNpmGlobalPrefix);
  }
  ...
  // Declare HamunaAgent as the inference-routing host for non-subscription
  ...
  // vars into user settings cannot silently redirect HamunaAgent requests to a
  ...
  // Self-Config CLI: expose sidecar port so the `hamuna` CLI can call back
  if (sidecarPort > 0) {
    env.HAMUNA_PORT = String(sidecarPort);
  }
  env.HAMUNA_SESSION_ID = sessionId;
  ...
  if (!effectiveProviderId) {
    console.warn('[env] Provider-owned SDK env missing providerId; HamunaAgent proxy will not be injected for this subprocess');
  }
...
async function attachBuiltinMediaIfAny(
  ...
  try {
    ...
    // `<workspace>/hamuna_files/<tool-name>/` location (#293-followup).
...
export async function initializeAgent(
  ...
  if (initialSessionId) {
    ...
    // creates HamunaAgent metadata before the SDK has ever persisted a transcript,
...
export async function enqueueUserMessage(
  ...
  // `hamuna session send`). Carries reply-back instruction + caller identity;
  ...
  if (hasImages && imagesAllowed) {
    ...
    // model lacks image support → write the images into `<agentDir>/hamuna_files/`
    ...
    if (agentDir) {
      const targetDir = join(agentDir, 'hamuna_files');
      try {
    ...
    if (fallbackPaths.length > 0) {
      ...
      ensureGitignorePattern(agentDir, 'hamuna_files/');

      const refs = fallbackPaths.map((p) => `@${p}`).join(' ');
      effectiveText = effectiveText ? `${effectiveText}\n\n${refs}` : refs;
      console.log(`[agent] modality fallback: ${fallbackPaths.length} image(s) → hamuna_files/ (model=${modelForFilter ?? '(unknown)'})`);
...
export async function rewindSession(userMessageId: string): Promise<{
  ...
  const doRewind = async () => {
    ...
    //
    //    **注意**：这两个集合只是 HamunaAgent 自己的视角，**不是 SDK 持久化状态的权威 proxy**。
    //    HamunaAgent 的 JSONL 与 SDK 的 JSONL (~/.claude/projects/.../*.jsonl) 是双份存储、
    //    异步独立写入（CLAUDE.md「双重存储」节）。SDK subprocess 在 flush 完成前被
    //    interrupt，会留下 HamunaAgent 有 / SDK 没有 的 UUID。所以"UUID 不在本地集合"
...
async function tryEagerFork(opts: {
  ...
  // HamunaAgent session (corrupt sessions.json / hypothetical SDK reuse) — clean up + fall back.
  if (getSessionMetadata(newSid)) {
    return fail(`fork id collides with an existing HamunaAgent session: ${newSid}`);
  }
...
async function startStreamingSession(preWarm = false): Promise<void> {
  ...
  try {
    ...
    if (process.env.HAMUNA_MANAGEMENT_PORT && !getImCronContext()) {
    ...
    const commonQueryOptions = {
      ...
      settings: {
        ...
        // an outward data flow HamunaAgent has not product-decided to expose.
        // Keep the tool surface frozen; revisit as its own feature if wanted.
        disableArtifact: true,
        // CC's own bundled skills duplicate the skill set HamunaAgent ships and
        // seeds itself (bundled-skills/ → ~/.hamuna/skills → <cwd>/.claude/skills
      ...
      systemPrompt: {
        ...
        append: buildSystemPromptAppend(currentScenario, {
          ...
          // those got dropped in favour of `hamuna` CLI calls so builtin needs the
      ...
      // HamunaAgent only hands it the enabled set — getEnabledPluginSdkConfigs
      ...
      canUseTool: async (toolName: string, input: unknown, options: { signal: AbortSignal }) => {
        ...
        // bot / bridge plugin) and are HamunaAgent-managed, not third-party. In IM
        ...
        // HAMUNA_CONTEXT_INJECTED_MCP_IDS guarantees no user MCP can take
        // the same name (filtered out in buildSdkMcpServers), so this auto-allow
        // can't be hijacked.
        const parts = toolName.split('__');
        if (
          parts.length >= 3 &&
          (HAMUNA_CONTEXT_INJECTED_MCP_IDS as readonly string[]).includes(parts[1])
        ) {
        ...
        // Auto-allow read-only `hamuna` CLI Bash invocations. After the v0.2.11
        // cron / im-cron / im-media → CLI migration, the AI reaches HamunaAgent'
        // own scheduling / IM / widget surface through `hamuna <group> …`
        ...
        // on `hamuna cron list` or `hamuna im channels`. Mutating commands
        ...
        // match `\n`/`\r`, letting `hamuna widget readme\nrm` slip through
        ...
        if (toolName === 'Bash') {
          ...
          // 1. Widget design contract: `hamuna widget [readme|list|<module>] [<module>...]`
          //    — module names limited to `[a-z][a-z0-9-]*`.
          if (/^hamuna[ \t]+widget(?:[ \t]+(?:readme|list))?(?:[ \t]+[a-z][a-z0-9-]*)*[ \t]*$/.test(cmd)) {
            console.log(`[permission] hamuna widget readme auto-allowed: ${cmd}`);
            return {
              behavior: 'allow' as const,
              updatedInput: input as Record<string, unknown>
            };
          ...
          //    `hamuna cron list|status|readme [--json]`
          //    `hamuna im channels|readme [--json]`
          if (/^hamuna[ \t]+(?:cron[ \t]+(?:list|status|readme)|im[ \t]+(?:channels|readme))(?:[ \t]+--json)?[ \t]*$/.test(cmd)) {
            console.log(`[permission] hamuna readonly CLI auto-allowed: ${cmd}`);
            return {
              behavior: 'allow' as const,
              updatedInput: input as Record<string, unknown>
            };
          ...
          // 3. Cron run history: `hamuna cron runs <taskId> [--limit N] [--full] [--json]`
          ...
          if (/^hamuna[ \t]+cron[ \t]+runs[ \t]+[a-zA-Z0-9_-]{1,64}(?:[ \t]+(?:--limit[ \t]+\d{1,4}|--full|--json))*[ \t]*$/.test(cmd)) {
            console.log(`[permission] hamuna cron runs auto-allowed: ${cmd}`);
            return {
              behavior: 'allow' as const,
              updatedInput: input as Record<string, unknown>
            };
          ...
          // 4. Thought inbox browse: `hamuna thought list [--tag <slug>] [--limit N] [--json]`.
          ...
          if (/^hamuna[ \t]+thought[ \t]+list(?:[ \t]+(?:--tag[ \t]+[a-z0-9][a-z0-9-]{0,31}|--limit[ \t]+\d{1,4}|--json))*[ \t]*$/.test(cmd)) {
            console.log(`[permission] hamuna thought list auto-allowed: ${cmd}`);
            return {
              behavior: 'allow' as const,
              updatedInput: input as Record<string, unknown>
            };
          ...
          // 5. Thought capture: `hamuna thought create '<content>'`
          ...
          if (/^hamuna[ \t]+thought[ \t]+create[ \t]+'[^']*'[ \t]*$/.test(cmd)) {
            console.log(`[permission] hamuna thought create auto-allowed: ${cmd}`);
            return {
              behavior: 'allow' as const,
              updatedInput: input as Record<string, unknown>
            };
          ...
          // 6. Thought capture via file: `hamuna thought create --content-file <path>`
          ...
          if (/^hamuna[ \t]+thought[ \t]+create[ \t]+--content-file[ \t]+[^ \t\n\r;|&<>$`'"]+[ \t]*$/.test(cmd)) {
            console.log(`[permission] hamuna thought create --content-file auto-allowed: ${cmd}`);
            return {
              behavior: 'allow' as const,
              updatedInput: input as Record<string, unknown>
            };
    ...
    // resolved at +337ms in a clean repro; in HamunaAgent production with project
    ...
    //     (SDK builds internal caches for large directories like ~/.hamuna with 20k+ files).
