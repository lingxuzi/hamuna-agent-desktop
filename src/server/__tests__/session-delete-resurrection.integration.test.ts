 ...
 * import of SessionStore, so the module-level ~/.hamuna paths bind to the
...
const sessionsDir = () => join(home, '.hamuna', 'sessions');
const sessionsJson = () => join(home, '.hamuna', 'sessions.json');
const sessionsTmpJson = () => join(home, '.hamuna', 'sessions.json.tmp');
...
beforeAll(async () => {
    home = mkdtempSync(join(tmpdir(), 'hamuna-336-'));
...
describe('issue #336 — delete vs persist resurrection', () => {
    ...
    it('backs up a corrupt sessions.json before creating metadata for a new session', async () => {
        ...
        const backupNames = readdirSync(join(home, '.hamuna'))
            .filter(name => name.startsWith('sessions.json.corrupt-'));
        expect(backupNames.length).toBeGreaterThan(0);
        expect(readFileSync(join(home, '.hamuna', backupNames[0]), 'utf-8')).toBe('{"truncated"');
    ...
    it('salvages valid metadata from a malformed sessions.json array before metadata creation', async () => {
        const beforeBackups = readdirSync(join(home, '.hamuna'))
        ...
        expect(readFileSync(join(home, '.hamuna', latestBackup!), 'utf-8')).toContain(preserved.id);
    ...
    it('salvages complete metadata objects from truncated sessions.json before metadata creation', async () => {
        ...
        const latestBackup = backupNames.sort().at(-1);
        expect(latestBackup).toBeTruthy();
        expect(readFileSync(join(home, '.hamuna', latestBackup!), 'utf-8')).toBe(corruptContent);
