...
describe('buildClaudeSessionEnv npm prefix isolation', () => {
  ...
  it('does not leak HamunaAgent npm prefix variables into the SDK shell env', () => {
    const home = process.platform === 'win32'
      ? 'C:\\Users\\hamuna-test'
      : '/tmp/hamuna-env-home';
    const prefix = process.platform === 'win32'
      ? resolve(home, '.hamuna', 'npm-global')
      : `${home}/.hamuna/npm-global`;
    ...
    expect(env.HAMUNA_NPM_GLOBAL_PREFIX).toBe(prefix);
...
describe('Windows SDK subprocess UTF-8 env', () => {
  ...
  it('installs a Git Bash UTF-8 BASH_ENV prelude without touching an existing shell prefix', () => {
    const home = mkdtempSync(resolve(tmpdir(), 'hamuna-env-home-'));
    tempHomes.push(home);
    const env: NodeJS.ProcessEnv = {
    ...
    expect(env.HAMUNA_ORIGINAL_BASH_ENV).toBe('C:/custom/bash-env.sh');
    expect(env.CLAUDE_CODE_SHELL_PREFIX).toBe('echo existing;');
    const prelude = readFileSync(env.BASH_ENV!, 'utf-8');
    expect(prelude).toContain('HAMUNA_WINDOWS_UTF8');
    expect(prelude).toContain('HAMUNA_ORIGINAL_BASH_ENV');
...
describe('Claude SDK context window env', () => {
  ...
  it('keeps Anthropic subscription 4.6 at 200K when another provider discovers the same model id as 1M (#444)', () => {
    ...
    const tempHome = mkdtempSync(join(tmpdir(), 'hamuna-env-home-'));
    try {
      mkdirSync(join(tempHome, '.hamuna'), { recursive: true });
      writeFileSync(
        join(tempHome, '.hamuna', 'config.json'),
        JSON.stringify({
          presetCustomModels: {
