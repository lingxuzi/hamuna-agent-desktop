...
const originalConfigDir = process.env.HAMUNA_CONFIG_DIR;
...
describe('mcp oauth', () => {
  beforeEach(() => {
    configDir = mkdtempSync(join(tmpdir(), 'hamuna-oauth-test-'));
    process.env.HAMUNA_CONFIG_DIR = configDir;
    resetStateStoreCacheForTests();
    ...
    // Do not let the developer machine's persisted HamunaAgent proxy route the
  ...
  afterEach(() => {
    ...
    if (originalConfigDir === undefined) {
      delete process.env.HAMUNA_CONFIG_DIR;
    } else {
      process.env.HAMUNA_CONFIG_DIR = originalConfigDir;
    }
  ...
  test('successful token response is reported failed when the credential cannot be persisted', async () => {
    ...
    globalThis.fetch = (async () => {
      ...
      process.env.HAMUNA_CONFIG_DIR = invalidConfigDir;
      return new Response(JSON.stringify({
        access_token: 'unpersisted-access',
        refresh_token: 'unpersisted-refresh',
        expires_in: 3600,
    ...
    const outcome = await refreshToken('notion');
    process.env.HAMUNA_CONFIG_DIR = validConfigDir;
    resetStateStoreCacheForTests();
  ...
  test('cross-process refresh lock permits one HTTP refresh and followers observe it', async () => {
    ...
    const runChild = () => new Promise<string>((resolve, reject) => {
      const child = spawn(process.execPath, ['--import', 'tsx/esm', fixture, 'notion'], {
        env: {
          ...process.env,
          HAMUNA_CONFIG_DIR: configDir,
          HAMUNA_SIDECAR_ROLE: 'session',
          HTTP_PROXY: '',
          HTTPS_PROXY: '',
          ALL_PROXY: '',
