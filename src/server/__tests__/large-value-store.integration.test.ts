...
let scratch: string;
const ORIGINAL_REFS_DIR = process.env.HAMUNA_REFS_DIR;

beforeEach(() => {
  scratch = mkdtempSync(join(tmpdir(), 'hamuna-refs-'));
  process.env.HAMUNA_REFS_DIR = scratch;
});

afterEach(() => {
  rmSync(scratch, { recursive: true, force: true });
  if (ORIGINAL_REFS_DIR === undefined) {
    delete process.env.HAMUNA_REFS_DIR;
  } else {
    process.env.HAMUNA_REFS_DIR = ORIGINAL_REFS_DIR;
  }
});
