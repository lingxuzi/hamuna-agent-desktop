 ...
 * that ALREADY lives in ~/.hamuna/plugins/<name> used to 409 with
...
describe('plugin install — register-in-place (#239)', () => {
  ...
  beforeEach(() => {
    home = mkdtempSync(join(tmpdir(), 'hamuna-plugin-inplace-'));
    savedHome = process.env.HOME;
    savedUserProfile = process.env.USERPROFILE;
    // getHomeDirOrNull() reads HOME (unix) / USERPROFILE (win) at call time.
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    const hamuna = join(home, '.hamuna');
    mkdirSync(hamuna, { recursive: true });
    // Seed an empty config so withConfigLock has a file to lock + rewrite.
    writeFileSync(join(hamuna, 'config.json'), JSON.stringify({}, null, 2), 'utf-8');
  });
  ...
  it('registers a valid plugin dir that already sits in plugins/<name> instead of 409ing', async () => {
    // User manually created ~/.hamuna/plugins/test-echo with a valid layout.
    const pluginDir = join(home, '.hamuna', 'plugins', 'test-echo');
  ...
  it('still 409s a name collision when the source is OUTSIDE plugins/ (orphan dir)', async () => {
    ...
    const occupied = join(home, '.hamuna', 'plugins', 'foo');
  ...
  it('rolls back the moved dir when the config commit fails (W4 — no orphan)', () => {
    ...
    const configPath = join(home, '.hamuna', 'config.json');
    rmSync(configPath, { force: true });
    mkdirSync(configPath, { recursive: true });
    writeFileSync(join(configPath, 'block'), 'x', 'utf-8');

    const installPath = join(home, '.hamuna', 'plugins', 'rollback-plugin');
