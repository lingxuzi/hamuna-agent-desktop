 ...
 * - Moonshot: Uses API key from ~/.hamuna/config.json
 */
...
/**
 * Load Moonshot API key from ~/.hamuna/config.json
 */
function loadMoonshotApiKey(): string | undefined {
  const configPath = join(homedir(), '.hamuna', 'config.json');

  if (!existsSync(configPath)) {
    console.warn('[test-env] ~/.hamuna/config.json not found');
    return undefined;
  }
 ...
 * Uses API key from ~/.hamuna/config.json
