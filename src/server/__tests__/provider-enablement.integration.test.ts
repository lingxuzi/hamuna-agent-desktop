...
beforeEach(() => {
  scratch = mkdtempSync(join(tmpdir(), 'hamuna-provider-enable-'));
  const configDir = join(scratch, '.hamuna');
...
function writeCustomProvider(id: string): void {
  writeFileSync(
    join(scratch, '.hamuna', 'providers', `${id}.json`),
    JSON.stringify({
      id,
...
describe('server-side provider enablement', () => {
  it('does not resolve disabled providers for background runtime startup', async () => {
    ...
    writeFileSync(join(scratch, '.hamuna', 'projects.json'), '[]', 'utf-8');
