...
describe('required system skill API contract', () => {
  it('projects ownership/required state, rejects required disables, and normalizes legacy config', async () => {
    const scratch = mkdtempSync(join(tmpdir(), 'hamuna-required-skills-'));
    const home = join(scratch, 'home');
    const workspace = join(scratch, 'workspace');
    const userSkills = join(home, '.hamuna', 'skills');
    ...
    writeSkill(projectSkills, 'hamuna-cli');
    ...
    try {
      ...
      expect(byFolder(projectBody.skills, 'hamuna-cli')).toMatchObject({
        scope: 'project',
        systemOwned: false,
        required: false,
        enabled: true,
