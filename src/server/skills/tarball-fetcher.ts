 ...
 *
 * Responsibilities:
 *   - Fetch trusted GitHub tarballs through the HamunaAgent general proxy overlay
  ...
  try {
    ...
    for (let hop = 0; hop <= MAX_REDIRECTS; hop++) {
      ...
      try {
        ...
        if (src.kind === 'github') {
          // GitHub coordinates resolve to a HamunaAgent-constructed codeload URL;
        ...
        resp = await undiciFetch(currentUrl, {
          ...
          redirect: 'manual',
          headers: {
            'User-Agent': 'HamunaAgent-Skill-Installer/1.0',
