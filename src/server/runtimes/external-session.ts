...
// Pre-warm can create a runtime thread before HamunaAgent has a durable
...
const MANAGED_CODEX_STRUCTURED_USER_INPUT_DISABLED_PROMPT = `<hamuna-managed-codex-interaction-limits>
...
</hamuna-managed-codex-interaction-limits>`;
...
export async function handleExternalProxyConfigChange(input: {
  ...
  const usesProcessProxyEnv =
    ...
    (activeExternalEnvPolicy?.proxy ?? 'hamuna') === 'hamuna';
  ...
  if (!usesManagedProviderProxy && !usesProcessProxyEnv) {
    return { success: true, skipped: 'proxy-not-owned-by-hamuna' };
  }
  if (oldKey === newKey) {
...
export async function startExternalSession(options: {
  ...
  /** Issue #194 — per-agent env policy (proxy: hamuna/terminal/direct). */
...
async function _doStartExternalSession(options: {
  ...
  // Build system prompt using HamunaAgent' three-layer architecture.
  // Pass the current runtime so L1 identity text reports the correct CLI
  // (e.g. "Google Gemini CLI" instead of the builtin default).
  //
  // cliToolsEnabled: true — teach the AI about `hamuna cron …` / `hamuna
  // im send-media` / `hamuna im wake|channels` via a progressive-disclosure
 ...
 *   - External runtime models aren't in HamunaAgent' PRESET_PROVIDERS registry,
...
export async function sendExternalMessage(
  ...
  const activeProcess = getExternalActiveProcess();
  if (!activeProcess || activeProcess.exited) {
    // CC supports custom session IDs (--session-id) — resume with our HamunaAgent session ID.
    ...
    try {
      await startExternalSession({
        ...
        analyticsOrigin: turnAnalyticsOrigin,
        birthOrigin: context?.birthOrigin,
        resumeSessionId: resumeId, // CC: --resume <hamuna-session-id>; Codex: --resume <threadId>
        metadataBirthPending: context?.metadataBirthPending,
        recordConfigState: !hasQueuedExternalConfigOperation(),
        dispatchPromotion: dispatchPromotion ?? undefined,
        activityFacts,
