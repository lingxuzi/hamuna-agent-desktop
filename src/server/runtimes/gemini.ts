...
// System prompt: merged "HamunaAgent 3-layer + Gemini official prompt" written to a tmp file,
 ...
 *   <HamunaAgent three-layer prompt (verbatim from options.systemPromptAppend)>
 ...
 *
 * Returns the path, or null if no HamunaAgent prompt was supplied (in which case we let
...
async function writeSessionSystemPrompt(
  ...
  let content = `<!-- HamunaAgent Gemini runtime session prompt, generated at ${timestamp} -->\n`;
  ...
  if (basePrompt && basePrompt.trim().length > 0) {
    ...
    content +=
      ...
      'tool usage, safety, and tone unless they conflict with the HamunaAgent instructions above, ' +
      'in which case the HamunaAgent instructions take precedence.\n\n';
    ...
    content +=
      ...
      'Follow the HamunaAgent instructions above strictly, and fall back to conservative ' +
...
class GeminiProcess implements RuntimeProcess {
   ...
   * ~/Downloads/hamuna-logs-2026-04-14T17-28-53.txt:169-173 where
...
export class GeminiRuntime implements AgentRuntime {
  ...
  async startSession(
    ...
    // Issue #194 — honor agent.runtimeConfig.envPolicy (proxy: hamuna/terminal/direct).
    const spawnEnv: Record<string, string | undefined> = { ...augmentedProcessEnv(options.envPolicy) };
    spawnEnv.PWD = options.workspacePath;
    spawnEnv.HAMUNA_SESSION_ID = options.sessionId;
    if (promptFile) {
    ...
    try {
      ...
      if (options.resumeSessionId) {
        ...
        // rebuild their UI. HamunaAgent has its own SessionStore and does not want
  ...
  private parseNotification(
    ...
    // tool_call_update, etc.). HamunaAgent already has its own SessionStore for the loaded
 ...
 * tool_call_update notifications (verified with /tmp/hamuna-verify-gemini/
...
/**
 * Map Gemini internal tool name (from toolCallId prefix) to the HamunaAgent
 ...
 * `input` object that HamunaAgent frontend expects.
 *
