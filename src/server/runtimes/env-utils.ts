 ...
 *  - `hamuna` (default): inherit Sidecar's process.env proxy vars (Rust
 *    injects HamunaAgent-configured proxy into the Sidecar's env at spawn).
 ...
 * `'direct'` values fall through validation and default to `'hamuna'`.
 */
export function augmentedProcessEnv(
  ...
  // that slipped past upstream validation) MUST behave as `'hamuna'` —
  // i.e. don't strip the user's proxy env on a guess.
  const rawPolicy = policy?.proxy;
  const proxyPolicy: 'hamuna' | 'terminal' =
    rawPolicy === 'terminal' ? 'terminal' : 'hamuna';
 ...
 * defense-in-depth (unknown → `hamuna`), so behaviour stays safe, but the
...
export async function resolveAgentEnvPolicy(
  ...
  try {
    ...
    const proxy: 'hamuna' | 'terminal' | undefined =
      proxyRaw === 'hamuna' || proxyRaw === 'terminal'
        ? proxyRaw
        : undefined;
    if (proxyRaw !== undefined && proxy === undefined) {
      ...
      // showing `'HamunaAgent 代理'` selected and user can pick `'terminal'` if
      // they previously relied on stripping proxy.
      console.warn(
        `[env-utils] Ignoring unsupported envPolicy.proxy=${JSON.stringify(proxyRaw)} for ${workspacePath} — defaulting to 'hamuna'`,
      );
    }
    return { proxy };
  } catch (err) {
