 ...
 *    into the per-tool workspace dir `<workspace>/hamuna_files/<tool-name>/`
...
/** generated dir segment names (`~/.hamuna/<name>` or `<ws>/hamuna_files/<name>`). */
...
export interface BuiltinAttachmentCtxBase {
   ...
   * unified workspace location `<workspace>/hamuna_files/<tool-name>/` —
   * the same `hamuna_files/` convention edge-tts / gemini-image use, but
   ...
   * workspace) → falls back to `~/.hamuna/generated/<tool-name>/`.
   */
  workspace?: string;
}

/**
 * Folder name for a tool's generated files, under `hamuna_files/`. Uses the
 ...
 * workspace `hamuna_files/` umbrella on first write, matching edge-tts /
 * gemini-image).
 */
function ensureToolGeneratedDir(toolName: string, workspace?: string): string {
  const dir = workspace
    ? path.join(workspace, 'hamuna_files', toolDirName(toolName))
    : path.join(homedir(), '.hamuna', 'generated', toolDirName(toolName));
  ensureDirSync(dir);
  if (workspace) ensureGitignorePattern(workspace, 'hamuna_files/');
  return dir;
}
 ...
 * `<workspace>/hamuna_files/<tool-name>/` (its `sourcePath`, what the tool
