...
async function createHarness(
  ...
  vi.resetModules();
  const home = mkdtempSync(join(tmpdir(), 'hamuna-external-mock-'));
  mkdirSync(join(home, '.hamuna'), { recursive: true });
  if (options.config) {
    writeFileSync(join(home, '.hamuna', 'config.json'), JSON.stringify(options.config));
  }
  previousHome = process.env.HOME;
  previousUserProfile = process.env.USERPROFILE;
  previousRuntime = process.env.HAMUNA_RUNTIME;
  process.env.HOME = home;
  process.env.USERPROFILE = home;
  process.env.HAMUNA_RUNTIME = 'codex';
...
function restoreEnv(): void {
  ...
  if (previousRuntime === undefined) delete process.env.HAMUNA_RUNTIME;
  else process.env.HAMUNA_RUNTIME = previousRuntime;
...
describe('external SessionEngine with fake runtime', () => {
  it('spills oversized completed tool input before top-level and nested result events', async () => {
    ...
    for (const index of [topStopIndex, nestedStopIndex]) {
      ...
      expect(JSON.parse(readFileSync(join(harness.home, '.hamuna', 'refs', refId!), 'utf-8')))
        .toEqual(finalInput);
    }
  });
