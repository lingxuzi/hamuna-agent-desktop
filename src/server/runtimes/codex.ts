...
export function buildCodexInitializeParams(): Record<string, unknown> {
  return {
    clientInfo: { name: 'HamunaAgent', title: null, version: process.env.HAMUNA_VERSION || '0.1.60' },
...
/**
 * Runtime-native startup barrier for the MCP servers HamunaAgent injected into a
...
function codexMcpEnvVarName(serverName: string, key: string): string {
  ...
  return `HAMUNA_MCP_${safeServer}_${safeKey}`.slice(0, 180);
}
...
function unsafeCodexMcpStdioValueReason(value: string): string | null {
  if (hasCodexMcpTemplate(value)) return 'contains HamunaAgent env placeholder';
...
function unsafeCodexMcpUrlReason(rawUrl: string): string | null {
  if (hasCodexMcpTemplate(rawUrl)) return 'contains HamunaAgent env placeholder';
  let parsed: URL;
  try {
...
function buildManagedCodexMcpConfigArgs(
  ...
  for (const server of servers) {
    ...
    skipped += 1;
    console.warn(`[codex] managed MCP ${server.id} skipped: Codex app-server does not support HamunaAgent MCP type ${server.type}`);
  }
...
/**
 * Notification methods that drive the MAIN HamunaAgent session and carry a
...
class CodexProcess implements RuntimeProcess {
  ...
  /** HamunaAgent sessionId (from SessionStartOptions). Used as the attachment scope key
   *  so refPath /api/attachment/tool/<sessionId>/<turnId>/<file> stays consistent
   *  across runtime resumes within the same HamunaAgent session. */
  sessionId = '';
...
const CODEX_STDERR_PATTERNS: StderrPattern[] = [
  ...
  { re: /proxyconnect tcp: dial tcp 127\.[0-9.]+:\d+: connect: operation not permitted/i, level: 'error', prefix: 'Codex sandbox blocks HamunaAgent proxy' },
...
function buildEffectiveEnvSnapshot(
  env: Record<string, string | undefined>,
  cwd: string,
  proxyPolicy: RuntimeProxyPolicy = 'hamuna',
): RuntimeEffectiveEnv {
  ...
  return {
    ...
    // start (issue #194). 'hamuna' = HamunaAgent-configured proxy is injected;
    // 'terminal' = inherited from user's interactive shell.
    proxyPolicy,
    pathHead,
    hamunaProxyInjected: env.HAMUNA_PROXY_INJECTED === '1',
...
async function collectCodexDiagnostics(
  ...
  // a false-positive "需要登录 Codex" banner in HamunaAgent (cross-bugfix #1).
    ...
    if (auth.requiresLogin) {
      issues.push({
        ...
        hint: runtimeSource === 'managed-provider'
          ...
          : 'Run `codex login` in a terminal, then retry from HamunaAgent.',
      });
    }
  ...
  if (sandboxProbe) {
    ...
    if (proxyProbe && !proxyProbe.reachable && (sandboxProbe.detected || sandboxProbe.networkDisabled)) {
      issues.push({
        code: 'codex_sandbox_blocks_hamuna_proxy',
        severity: 'error',
        title: 'Codex sandbox blocks the HamunaAgent proxy',
...
export class CodexRuntime implements AgentRuntime {
  ...
  /**
   * Standalone diagnostic run (issue #194 — used by `hamuna diagnose runtime
   ...
   * `hamuna` proxy view even when the agent is set to `terminal`/`direct`
  ...
  async startSession(
    ...
    codexEnv.HAMUNA_SESSION_ID = options.sessionId;
      ...
      void (async () => {
        try {
          const diagnostics = await collectCodexDiagnostics(
            ...
            codexProc.threadId,
            options.envPolicy?.proxy ?? 'hamuna',
  ...
  private parseNotification(
    ...
    // Those MUST NOT drive the MAIN HamunaAgent session: a child's turn/completed
  ...
  private handleServerRequest(
    ...
    switch (method) {
      ...
      case 'item/tool/call':
        codexProc.rpc.respondError(rpcId, -32000, 'Codex dynamic tool host is not supported by HamunaAgent yet');
        break;

      case 'account/chatgptAuthTokens/refresh':
        codexProc.rpc.respondError(rpcId, -32000, 'HamunaAgent does not refresh Codex ChatGPT tokens; run `codex login` in a terminal');
        break;

      case 'attestation/generate':
        codexProc.rpc.respondError(rpcId, -32000, 'HamunaAgent did not request Codex attestation');
        break;
