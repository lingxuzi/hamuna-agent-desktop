...
/**
 * Reserved plugin names — they would collide with `~/.hamuna/plugins/`'s
...
export async function installPlugin(
  ...
  // ~/.hamuna/plugins/<name> and ran `cc-plugin install file://…/<name>`.
...
export function getEnabledPluginSdkConfigs(
  ...
  for (const p of entries) {
    ...
    // ~/.hamuna/plugins/foo → /tmp/evil, we won't hand /tmp/evil to SDK.
    try {
...
export function getDefaultEnabledPluginIdsForWorkspace(workspacePath: string): string[] {
  ...
  try {
    ...
    const home = getHomeDirOrNull();
    if (!home) return [];
    const projectsPath = resolve(home, '.hamuna', 'projects.json');
