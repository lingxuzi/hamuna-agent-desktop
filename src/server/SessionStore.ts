...
const HAMUNA_DIR = join(homedir(), '.hamuna');
const SESSIONS_FILE = join(HAMUNA_DIR, 'sessions.json');
const SESSIONS_DIR = join(HAMUNA_DIR, 'sessions');
const ATTACHMENTS_DIR = join(HAMUNA_DIR, 'attachments');
const SESSIONS_TMP_FILE = join(HAMUNA_DIR, 'sessions.json.tmp');
const SESSIONS_LOCK_FILE = join(HAMUNA_DIR, 'sessions.lock');
const SESSIONS_LOCK_DIR = join(HAMUNA_DIR, 'session-locks');
...
function createCorruptBackupPath(): string {
    ...
    const base = join(HAMUNA_DIR, `sessions.json.corrupt-${stamp}`);
    if (!existsSync(base)) {
        return base;
    }
...
function ensureStorageDir(): void {
    if (!existsSync(HAMUNA_DIR)) {
        ensureDirSync(HAMUNA_DIR);
    }
