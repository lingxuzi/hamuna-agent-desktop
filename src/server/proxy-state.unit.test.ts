...
const PROXY_ENV_KEYS = [
  ...
  'HAMUNA_PROXY_INHERITED_ENV_JSON',
] as const;
...
describe('proxy-state provider scope', () => {
  ...
  it.each([
    ...
    process.env.HAMUNA_PROXY_INJECTED = '1';
    process.env.HAMUNA_PROXY_INHERITED_ENV_JSON = JSON.stringify({
      HTTPS_PROXY: 'http://system.proxy:8080',
      NO_PROXY: '.corp.local',
    });
    process.env.HTTP_PROXY = 'http://hamuna.proxy:7890';
    process.env.HTTPS_PROXY = 'http://hamuna.proxy:7890';
    process.env.http_proxy = 'http://hamuna.proxy:7890';
    process.env.https_proxy = 'http://hamuna.proxy:7890';
    ...
    expect(process.env.HTTPS_PROXY).toBe(
      generalUsesApp ? 'http://hamuna.proxy:7890' : 'http://system.proxy:8080',
    );
    expect(process.env.HTTP_PROXY).toBe(
      generalUsesApp ? 'http://hamuna.proxy:7890' : undefined,
    );
    expect(proxyState.getProxyForUrl('https://space.example.com/v1')).toBe(
      generalUsesApp ? 'http://hamuna.proxy:7890' : 'http://system.proxy:8080',
    );
    expect(proxyState.getHamunaAgentProxyForGeneralUrl('https://space.example.com/v1')).toBe(
      generalUsesApp ? 'http://hamuna.proxy:7890' : undefined,
    );
    ...
    expect(providerEnv.HTTPS_PROXY).toBe(
      providerUsesApp ? 'http://hamuna.proxy:7890' : 'http://system.proxy:8080',
    );
    expect(providerEnv.HTTP_PROXY).toBe(
      providerUsesApp ? 'http://hamuna.proxy:7890' : undefined,
    );
    expect(providerEnv.NO_PROXY).toContain('localhost');
    if (!providerUsesApp) expect(providerEnv.NO_PROXY).toContain('.corp.local');
    expect(providerEnv.HAMUNA_PROXY_INJECTED).toBeUndefined();
    expect(providerEnv.HAMUNA_PROXY_INHERITED_ENV_JSON).toBeUndefined();
  ...
  it.each([
    ...
    const appUrl = `${protocol}://hamuna.proxy:7890`;
