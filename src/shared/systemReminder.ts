...
export const SPACE_ISSUE_CONTEXT_TAG = 'hamuna-space-issue';
...
export const SESSION_EVENT_TAG = 'hamuna-session-event';
...
export function parseSessionSendRequestDisplay(
  ...
  const openingTag = reminder.body.match(/^\s*<hamuna-session-event\b([\s\S]*?)>/);
...
export function buildFloatingBallContextReminder(input: FloatingBallContextReminderInput): string {
  ...
  const parts: string[] = [
    ...
    '<interaction>',
    'This message comes from the HamunaAgent floating window. Keep the reply concise and directly useful for a small desktop-adjacent window.',
    '</interaction>',
    '',
    '<context>',
...
function goalTerminalGuidance(input: GoalReminderInput): string[] {
  if (input.aiCanExit === false) {
    return [
      ...
      '- Do not call `hamuna goal update` for this Goal.',
    ];
  }
  return [
    ...
    '',
    'Do not call hamuna goal update unless the Goal is complete or the strict blocked audit above is satisfied. Do not mark a Goal complete merely because you are stopping, because the user interrupted a turn, or because you made partial progress.',
  ];
}
...
function goalTerminalCommandGuidance(input: GoalReminderInput): string[] {
  if (input.aiCanExit === false) {
    return [
      'The user disabled autonomous Goal termination. Do not call `hamuna goal update`; report completion evidence or blockers without changing the Goal status.',
    ];
  }
  return [
...
export function buildGoalContinuationReminder(input: GoalReminderInput): string {
  const reminder = [
    ...
    '<instruction>',
    'Continue working toward the active HamunaAgent Goal.',
    '',
...
export function buildGoalContextReminder(input: GoalContextReminderInput): string {
  return [
    ...
    `<${GOAL_CONTEXT_TAG}>`,
    '<instruction>',
    'This session is currently working toward a HamunaAgent Goal.',
    '',
