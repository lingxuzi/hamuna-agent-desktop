...
// `.lsp.json`, `monitors/`, `bin/`, etc. HamunaAgent downloads the directory
// to `~/.hamuna/plugins/<name>/` and hands the absolute path to the
...
export interface PluginEntry {
  ...
  id: string;
  /** plugin.json::name (kebab-case) — also the directory basename under ~/.hamuna/plugins/ */
  name: string;
   ...
   */
  sourceUrl: string;
  /** Absolute path on disk (`~/.hamuna/plugins/<name>/`) */
  installPath: string;
...
/**
 * Parsed `.claude-plugin/plugin.json` metadata. Only the subset HamunaAgent
