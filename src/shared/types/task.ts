...
// Workspace-scoped execution units. Persisted to ~/.hamuna/tasks.jsonl.
...
export interface Task {
   ...
   *  env from `~/.hamuna/config.json` on every tick, so credential
   *  rotation propagates without re-saving the task and no credential
   *  copies land in `~/.hamuna/tasks/...jsonl`.
   *
...
export interface TaskDocs {
  /** Absolute path to the docs directory: `~/.hamuna/tasks/<id>/`. */
  dir: string;
...
export interface TaskCreateFromAlignmentInput {
  ...
  /** PRD 0.2.9 — Per-task provider id override. MUST be paired with `model`. */
  providerId?: string;
  /** Per-task permission mode override. Runtime-specific values — see `hamuna runtime describe <runtime>`. */
  permissionMode?: string;
  runtime?: RuntimeType;
  runtimeConfig?: RuntimeConfigSnapshot;
  /** Per-task MCP enable list override (PRD 0.2.4 §需求 4). */
...
/** Payload for `cmd_task_create_attached` / `hamuna task create-attached`. */
