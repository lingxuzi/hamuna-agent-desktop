 ...
 * through so a new runtime release does not require a HamunaAgent release first.
 */
 ...
 *
 * - `hamuna` (default, legacy) — HamunaAgent unconditionally injects its own
 *    `proxySettings` into the runtime's env, overriding whatever the parent
 *    shell or system has configured. Best for "HamunaAgent proxy is THE proxy"
 *    setups.
 * - `terminal` — Drop HamunaAgent-injected proxy vars; restore whatever proxy
 ...
 *    claude from terminal and want HamunaAgent to behave the same."
 * - `direct` — Strip all proxy vars. Best when system-level proxy (Clash
 *    TUN, transparent proxy) handles routing.
 */
export type RuntimeProxyPolicy = 'hamuna' | 'terminal';
 ...
 * validator and default to `'hamuna'`; users who relied on stripping
 * HamunaAgent proxy can pick `terminal` from the UI.
 */
...
export interface RuntimeConfig {
   ...
   * `{ proxy: 'hamuna' }` (the legacy behaviour) for backwards compat.
   */
 ...
 * selector, `hamuna agent set runtime <v>` CLI) behaves identically.
 *
...
// `hamuna runtime describe builtin` show the same allowlist other runtimes
 ...
 * always `'hamuna'` (the legacy behaviour).
 */
export interface RuntimeEffectiveEnv {
  ...
  pathHead?: string[];
  /** True when HAMUNA_PROXY_INJECTED=1 reached the runtime. */
  hamunaProxyInjected?: boolean;
