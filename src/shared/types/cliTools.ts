 ...
 * - `~/.hamuna/tools/registry.json`（CliToolRegistryFile）只存产品侧状态
...
export interface CliToolManifest {
  ...
  entry: string;
  runtime?: 'node';
  /** 工具需要的环境变量名（API key 等），值经 `hamuna tool env` 存 config.cliToolEnv */
  envKeys?: string[];
...
export interface CliToolRegistryEntry {
  ...
  envKeys?: string[];
  deps?: string[];
  /** 工具目录绝对路径（规范位置 ~/.hamuna/tools/<name>） */
  dir: string;
  /** 入口脚本绝对路径 */
  entryPath: string;
...
/** 注入 prompt 的工具数保险丝，超出部分降级为一行 `hamuna tool list` 指引 */
...
/**
 * 保证危险的保留名黑名单。`~/.hamuna/bin` 在 PATH 中先于系统路径，
...
export const CLI_TOOL_RESERVED_NAMES: ReadonlySet<string> = new Set([
  'hamuna',
  // 外部 Runtime 的 CLI：重名工具会被 resolveCommand 解析到（~/.hamuna/bin
...
export function validateCliToolName(name: unknown): CliToolValidationResult {
  ...
  if (CLI_TOOL_RESERVED_NAMES.has(name)) {
    return {
      ok: false,
      code: 'NAME_RESERVED',
      error: `Tool name "${name}" is reserved: ~/.hamuna/bin precedes system paths on PATH, so it would silently shadow the system command in every agent session and terminal`,
...
export function validateCliToolManifest(raw: unknown): CliToolValidationResult {
  ...
  // description 会被原文包进 <hamuna-user-tools> 段注入 system prompt——
  // 含该 token 的文本能闭合包裹标签、把后续内容伪装成 prompt 指令。
  // 注册时直接打回（该 token 在工具描述里没有任何合法用途）。
  if (/hamuna-user-tools/i.test(m.description)) {
    return {
      ok: false,
      code: 'DESCRIPTION_FORBIDDEN_TOKEN',
      error: 'description must not contain "hamuna-user-tools" — it could break out of the prompt section that wraps tool descriptions',
