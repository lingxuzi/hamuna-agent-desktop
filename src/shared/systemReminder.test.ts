...
describe('systemReminder', () => {
  it('builds floating-ball context as a plain system-reminder envelope', () => {
    ...
    expect(reminder).toContain('This message comes from the HamunaAgent floating window.');
  ...
  it('parses mixed cron reminders with hidden operational context and visible task text', () => {
    const raw = [
      '<system-reminder>',
      '<CRON_TASK>',
      'You are running inside a HamunaAgent scheduled task execution.',
      'cronTaskId: cron_123',
      '</CRON_TASK>',
      '</system-reminder>',
  ...
  it('parses Space issue reminders with the badge tag and visible status text', () => {
    const raw = [
      '<system-reminder>',
      `<${SPACE_ISSUE_CONTEXT_TAG}>`,
      '<hamuna-space-event version="1" type="issue-delivery">',
      '<issue-instruction>hidden instructions</issue-instruction>',
      '</hamuna-space-event>',
      `</${SPACE_ISSUE_CONTEXT_TAG}>`,
      '</system-reminder>',
      'HamunaAgent Space 已投递一个 Issue 通知，Registered Agent 开始处理。',
    ].join('\n');
    ...
    expect(parsed.visibleText).toBe('HamunaAgent Space 已投递一个 Issue 通知，Registered Agent 开始处理。');
    expect(stripLeadingSystemReminder(raw)).toBe('HamunaAgent Space 已投递一个 Issue 通知，Registered Agent 开始处理。');
  });
  ...
  it('removes every autonomous terminal command when Goal exit is disabled', () => {
    ...
    expect(parsed.body).not.toContain('hamuna goal update --status');
