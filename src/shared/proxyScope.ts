...
export function shouldUseHamunaAgentProxyForGeneralRequests(
...
export function shouldUseHamunaAgentProxyForProvider(
  proxySettings: ProxySettings | null | undefined,
...
export function effectiveProxyScopeKey(
  ...
  const id = providerId?.trim();
  if (!proxySettings?.enabled || !id) return 'hamuna-proxy:none';
  if (!shouldUseHamunaAgentProxyForProvider(proxySettings, id)) {
    return `hamuna-proxy:disabled-for-provider:${id}`;
  }
  const protocol = proxySettings.protocol || 'http';
  const host = proxySettings.host || '127.0.0.1';
  const port = proxySettings.port || 7890;
  return `hamuna-proxy:${id}:${protocol}://${host}:${port}`;
}

export function effectiveGeneralProxyScopeKey(
  proxySettings: ProxySettings | null | undefined,
): string {
  if (!shouldUseHamunaAgentProxyForGeneralRequests(proxySettings)) {
    return 'hamuna-proxy:general:inherited';
  }
  const protocol = proxySettings?.protocol || 'http';
  const host = proxySettings?.host || '127.0.0.1';
  const port = proxySettings?.port || 7890;
  return `hamuna-proxy:general:${protocol}://${host}:${port}`;
}
