...
export interface Project {
  ...
  /** HamunaAgent official CLI tools enabled for this workspace. Separate from MCP ids. */
  enabledOfficialToolIds?: OfficialToolId[];
  /** Internal projects (e.g. ~/.hamuna diagnostic workspace) hidden from Launcher */
  internal?: boolean;
...
export interface WorkspaceTemplate {
  ...
  path?: string;        // User template: absolute path under ~/.hamuna/templates/
...
export interface ProxyScopeSettings {
  mode: ProxyScopeMode;
  /**
   * Whether non-provider-owned network requests use the HamunaAgent app proxy.
...
export interface AppConfig {
   ...
   *  只控制工具箱里的 CLI 工具注册/管理/AI 自动发现；不影响 hamuna CLI
  ...
  /** 实验室门控：团队 Space（HamunaAgent Space / Cloud Space）。默认关。
  ...
  // (~/.hamuna/tools/). Same shape as mcpServerEnv. The ~/.hamuna/bin
  // launcher shims read this at runtime, so env changes apply on next launch
  // without re-registration.
  cliToolEnv?: Record<string, Record<string, string>>;

  // ===== Official CLI Tools =====
  // Global visibility gate for HamunaAgent-owned CLI tools (not user registry tools).
  enabledOfficialToolIds?: OfficialToolId[];
   ...
   *  under ~/.hamuna/plugins/<name>/ containing .claude-plugin/plugin.json.
...
export const PRESET_MCP_SERVERS: McpServerDefinition[] = [
  ...
  },
  {
    ...
    args: ['mcp', '--caller-app', 'HamunaAgent'],
    isBuiltin: true,
    isFree: true,
