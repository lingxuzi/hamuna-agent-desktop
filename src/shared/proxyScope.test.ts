...
describe('general request proxy decision', () => {
  it('requires the global switch and defaults legacy scopes to enabled', () => {
    expect(shouldUseHamunaAgentProxyForGeneralRequests(proxy(undefined, false))).toBe(false);
    expect(shouldUseHamunaAgentProxyForGeneralRequests(proxy())).toBe(true);
    expect(shouldUseHamunaAgentProxyForGeneralRequests(proxy({ mode: 'custom', providerIds: ['deepseek'] })))
      .toBe(true);
  });

  it('honors explicit custom general selection independently from providers', () => {
    expect(shouldUseHamunaAgentProxyForGeneralRequests(proxy({
  ...
  it('changes the effective key only when the general path changes', () => {
    ...
    expect(effectiveGeneralProxyScopeKey(proxy()))
      .toBe('hamuna-proxy:general:http://127.0.0.1:7897');
  });
});

describe('provider-owned proxy decision', () => {
  it('does not use HamunaAgent proxy when disabled', () => {
    expect(shouldUseHamunaAgentProxyForProvider(proxy(undefined, false), 'deepseek')).toBe(false);
  });

  it('uses HamunaAgent proxy for all providers by default', () => {
    expect(shouldUseHamunaAgentProxyForProvider(proxy(), 'deepseek')).toBe(true);
  });

  it('uses HamunaAgent proxy only for selected custom providers', () => {
    const settings = proxy({ mode: 'custom', providerIds: ['anthropic-sub'] });
    expect(shouldUseHamunaAgentProxyForProvider(settings, 'anthropic-sub')).toBe(true);
    expect(shouldUseHamunaAgentProxyForProvider(settings, 'deepseek')).toBe(false);
  });

  it('includes provider and proxy url in the effective restart key', () => {
    expect(effectiveProxyScopeKey(proxy(), 'deepseek')).toBe('hamuna-proxy:deepseek:http://127.0.0.1:7897');
    expect(effectiveProxyScopeKey(proxy({ mode: 'custom', providerIds: ['anthropic-sub'] }), 'deepseek'))
      .toBe('hamuna-proxy:disabled-for-provider:deepseek');
  });
