...
# 获取带架构后缀的 tar.gz 文件名
# 用法: get_tar_upload_name "HamunaAgent.app.tar.gz" "aarch64-apple-darwin"
# 输出: HamunaAgent_aarch64.app.tar.gz
get_tar_upload_name() {
    local base="$1"
    local target="$2"
...
echo ""
echo -e "${CYAN}╔═══════════════════════════════════════════════════════╗${NC}"
echo -e "${CYAN}║${NC}  ${GREEN}🚀 HamunaAgent 发布到 Cloudflare R2${NC}                     ${CYAN}║${NC}"
...
for TARGET in "${FOUND_TARGETS[@]}"; do
    ...
    # HamunaAgent.app.tar.gz -> HamunaAgent_aarch64.app.tar.gz 或 HamunaAgent_x86_64.app.tar.gz
