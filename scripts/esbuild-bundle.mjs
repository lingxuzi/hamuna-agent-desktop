...
const TARGETS = {
  ...
  bridge: {
    ...
    // `C:\Users\hackl\AppData\Local\HamunaAgent\`), Node's default classifier
  ...
  cli: {
    entryPoints: ['src/cli/hamuna.ts'],
    outfile: 'src-tauri/resources/cli/hamuna.js',
    format: 'cjs',
    sourcemap: false,
    banner: { js: CLI_SHEBANG_BANNER },
    /** Post-build: drop the Windows launcher next to the bundle. Rust's
     *  `cmd_sync_cli` reads `resources/cli/hamuna.js` AND `hamuna.cmd`,
     ...
     */
    postBuild: async () => {
      const src = 'src/cli/hamuna.cmd';
      const dst = 'src-tauri/resources/cli/hamuna.cmd';
...
await build({
  ...
  define: {
    ...
    // fallbacks across the codebase so `hamuna version` reports the real
    // shipped build instead of a stale hardcoded string in production.
    __HAMUNA_VERSION__: JSON.stringify(PKG_VERSION),
  },
