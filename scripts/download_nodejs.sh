#!/bin/bash
# Download Node.js LTS binaries for bundling with HamunaAgent.
#
...
write_metadata() {
    ...
    printf "%s\n" "$NODE_VERSION" > "${dir}/.hamuna-nodejs-version"
    printf "%s\n" "$platform" > "${dir}/.hamuna-nodejs-platform"
    printf "%s\n" "$arch" > "${dir}/.hamuna-nodejs-arch"
}
...
check_existing() {
    ...
    if [[ -f "$node_bin" ]]; then
        case "$dir" in
            "$CACHE_ROOT"/*)
                if [[ ! -f "${dir}/.hamuna-nodejs-version" ]]; then
                    return 1
                fi
                ;;
        esac
        ...
        local existing_ver
        if [[ -f "${dir}/.hamuna-nodejs-version" ]]; then
            existing_ver=$(cat "${dir}/.hamuna-nodejs-version" 2>/dev/null || echo "")
        else
        ...
        if [[ -f "${dir}/.hamuna-nodejs-platform" ]]; then
            local existing_platform
            existing_platform=$(cat "${dir}/.hamuna-nodejs-platform" 2>/dev/null || echo "")
            if [[ "$existing_platform" != "$platform" ]]; then
                return 1
            fi
        fi
        if [[ -n "$expected_arch" && -f "${dir}/.hamuna-nodejs-arch" ]]; then
            local existing_arch
            existing_arch=$(normalize_arch "$(cat "${dir}/.hamuna-nodejs-arch" 2>/dev/null || echo "")")
