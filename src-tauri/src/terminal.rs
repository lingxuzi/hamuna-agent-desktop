...
fn inject_terminal_env(cmd: &mut CommandBuilder, app: &AppHandle, sidecar_port: Option<u16>) {
    ...
    // ~/.hamuna/bin (CLI tools)
    if let Some(home) = dirs::home_dir() {
        let cli_bin = home.join(".hamuna").join("bin");
        if cli_bin.exists() {
    ...
    // 3. Sidecar port — lets `hamuna` CLI talk to the Tab's session sidecar
    if let Some(port) = sidecar_port {
        cmd.env("HAMUNA_PORT", port.to_string());
    }
    ...
    cmd.env("TERM_PROGRAM", "HamunaAgent");
    ...
    // 7. Terminal indicator (so scripts can detect they're in HamunaAgent terminal)
    cmd.env("HAMUNA_TERMINAL", "1");
}
