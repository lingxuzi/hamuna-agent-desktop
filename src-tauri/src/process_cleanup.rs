...
pub struct CleanupReport {
    ...
    /// processes that may not match HamunaAgent command-line/root patterns
...
/// executing from the current HamunaAgent install/resource directory can hold
...
/// Query whether a specific PID corresponds to a HamunaAgent process.
...
/// filesystem case quirks (`HamunaAgent` vs `hamuna`).
pub fn is_hamuna_pid(pid: u32) -> bool {
    let mut system = System::new();
...
mod tests {
    ...
    fn root_match_accepts_windows_separator_variants() {
        let roots = normalized_roots(&[PathBuf::from(
            r"C:\Users\alice\AppData\Local\HamunaAgent\resources",
        )]);
        assert_eq!(
            process_match_reason(
                "c:/users/alice/appdata/local/hamuna/resources/nodejs/node.exe server-dist.js",
                Some("c:/users/alice/appdata/local/hamuna/resources/nodejs/node.exe"),
                &[],
                &roots,
            )
            .as_deref(),
            Some("exe under 'c:/users/alice/appdata/local/hamuna/resources'")
        );
    }
    ...
    fn root_match_strips_extended_length_prefix() {
        ...
        assert!(process_match_reason(
            "node c:/users/alice/appdata/local/hamuna/resources/plugin-bridge-dist.mjs",
            None,
            &[],
            &roots,
        )
    ...
    fn root_match_does_not_match_prefix_siblings_by_exe() {
        ...
        assert!(process_match_reason(
            "",
            Some("c:/users/alice/appdata/local/hamuna/resources-old/node.exe"),
            &[],
            &roots,
        )
        .is_none());
    ...
    fn root_match_does_not_match_prefix_siblings_by_cmd() {
        ...
        assert!(process_match_reason(
            "node c:/users/alice/appdata/local/hamuna/resources-old/server-dist.js",
            None,
            &[],
            &roots,
        )
    ...
    fn root_match_accepts_file_url_command_arguments() {
        ...
        assert!(process_match_reason(
            "node --import=file:///c:/users/alice/appdata/local/hamuna/resources/tsx-runtime/loader.mjs",
            None,
            &[],
            &roots,
        )
