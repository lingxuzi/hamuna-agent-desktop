...
//! `~/.hamuna/config.json` with such a tool, HamunaAgent would log the parse
//! error and fall back to the `.bak` backup — an opaque failure mode that
//! looked like data loss. HamunaAgent itself never writes BOM (`serde_json::to_string_pretty`
