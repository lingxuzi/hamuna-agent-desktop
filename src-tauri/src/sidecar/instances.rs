...
pub fn start_tab_sidecar<R: Runtime>(
    ...
    let effective_agent_dir = if let Some(ref dir) = agent_dir {
        ...
        let temp_dir = std::env::temp_dir().join(format!("hamuna-global-{}", std::process::id()));
    ...
    if mgmt_port > 0 {
        cmd.env("HAMUNA_MANAGEMENT_PORT", mgmt_port.to_string());
    }
    ...
    if !is_global {
        if let Some(ref dir) = agent_dir {
            if let Some(runtime) = resolve_agent_runtime_from_config(dir) {
                cmd.env("HAMUNA_RUNTIME", &runtime);
            }
        }
    }
...
pub async fn monitor_session_sidecars(
    ...
    struct RecoveryEntry {
        ...
        /// Snapshot of the dead sidecar's `runtime` field (HAMUNA_RUNTIME env
        ...
        /// Snapshot of HAMUNA_RUNTIME_SOURCE captured with `runtime`.
