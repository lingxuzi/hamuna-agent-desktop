...
/// Look up the `runtime` field from the agent config in ~/.hamuna/config.json
...
pub(crate) fn resolve_agent_runtime_identity_from_config(
    ...
    let config_path = dirs::home_dir()?.join(".hamuna").join("config.json");
...
/// Look up the `runtime` field from session metadata in ~/.hamuna/sessions.json.
...
/// runtime B under the same HamunaAgent session_id.
#[allow(dead_code)]
...
pub fn resolve_session_runtime_identity_full(session_id: &str) -> Option<RuntimeIdentity> {
    let sessions_path = dirs::home_dir()?.join(".hamuna").join("sessions.json");
...
/// exists in `~/.hamuna/sessions.json` and (b) its workspace directory still
...
pub fn cmd_can_restore_session(sessionId: String, agentDir: String) -> bool {
    ...
    let Some(sessions_path) = dirs::home_dir().map(|h| h.join(".hamuna").join("sessions.json"))
    else {
        return false;
    };
