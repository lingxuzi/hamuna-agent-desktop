...
/// Write the Global Sidecar port to ~/.hamuna/sidecar.port so the CLI can discover it.
pub(super) fn write_global_port_file(port: u16) {
    if let Some(home) = dirs::home_dir() {
        let port_file = home.join(".hamuna").join(PORT_FILE_NAME);
...
//   kill a concurrent HamunaAgent instance's sidecars during any
