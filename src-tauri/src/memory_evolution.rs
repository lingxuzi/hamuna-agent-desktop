...
async fn backfill_system_maintenance_session_markers(
    ...
    let hamuna_dir = crate::app_dirs::hamuna_data_dir()
        .ok_or_else(|| "无法定位 HamunaAgent 数据目录".to_string())?;
    let sessions_path = hamuna_dir.join("sessions.json");
    let tmp_path = hamuna_dir.join("sessions.json.tmp");
    let lock_path = hamuna_dir.join("sessions.lock");
