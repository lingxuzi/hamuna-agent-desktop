...
/// Persisted ball placement — `~/.hamuna/floating_ball.json`.
...
/// Mirror of the renderer's gate fields in `~/.hamuna/config.json`.
...
mod imp {
            ...
            loop {
                ...
                let dispatched = app
                    .run_on_main_thread(move || {
                        ...
                        // Peek is intentionally nonactivating: HamunaAgent is
    ...
    fn placement_path() -> Option<PathBuf> {
        crate::app_dirs::hamuna_data_dir().map(|d| d.join("floating_ball.json"))
    }
    ...
    fn ensure_windows(app: &AppHandle) -> Result<(), String> {
        ...
        if app.get_webview_window(SHIELD_LABEL).is_none() {
            let win = WebviewWindowBuilder::new(app, SHIELD_LABEL, WebviewUrl::default())
                .title("HamunaAgent Floating Shield")
                .inner_size(1.0, 1.0)
                .resizable(false)
                .decorations(false)
                .transparent(true)
    ...
    pub fn hide_companion(app: &AppHandle) {
        ...
        // activates HamunaAgent so IME/text services attach to WKWebView, so the
    ...
    fn frontmost_window_info() -> FrontmostInfo {
        match active_win_pos_rs::get_active_window() {
            ...
            // saw "正在看 HamunaAgent — HamunaAgent Ball" in the title row. Filter
    ...
    fn frontmost_app_name() -> Option<String> {
        ...
        if name.is_empty() || name == "HamunaAgent" {
            None
        } else {
            Some(name)
        }
    ...
    pub fn screenshot() -> Result<super::FbScreenshot, String> {
        ...
        let tmp = std::env::temp_dir().join(format!("hamuna-fb-shot-{id}.png"));
        ...
        let jpg = std::env::temp_dir().join(format!("hamuna-fb-shot-{id}.jpg"));
...
mod imp {
    ...
    fn process_name_for_hwnd(hwnd: HWND) -> Option<String> {
        unsafe {
            ...
            name.filter(|s| !s.is_empty() && s != "HamunaAgent")
        }
    }
...
"#,
    ...
    pub fn screenshot() -> Result<FbScreenshot, String> {
        ...
        for (idx, (max_dim, quality)) in ATTEMPTS.iter().copied().enumerate() {
            let jpg =
                std::env::temp_dir().join(format!("hamuna-fb-shot-{id}-{max_dim}-{quality}.jpg"));
            ulog_info!(
