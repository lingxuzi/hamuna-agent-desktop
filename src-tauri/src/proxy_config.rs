...
//!
//! Configuration is read from `~/.hamuna/config.json` and can be enabled/disabled
...
pub const PROXY_INJECTED_MARKER_ENV: &str = "HAMUNA_PROXY_INJECTED";
pub const PROXY_INHERITED_ENV_JSON: &str = "HAMUNA_PROXY_INHERITED_ENV_JSON";
...
/// Proxy settings from `~/.hamuna/config.json`
///
/// # Example JSON
/// ```json
/// {
...
fn read_proxy_settings_from_disk() -> Option<ProxySettings> {
    ...
    let config_path = home.join(".hamuna").join("config.json");
...
/// Read raw proxy settings from ~/.hamuna/config.json.
...
/// Read proxy settings from ~/.hamuna/config.json
...
/// Snapshot proxy env vars before HamunaAgent overwrites them for a child process.
...
/// Preserve inherited bypass rules while adding HamunaAgent' mandatory localhost
...
/// Apply HamunaAgent proxy policy to a child process `Command`.
///
...
pub fn apply_to_subprocess(cmd: &mut Command) -> bool {
    if let Some(proxy_settings) = read_proxy_settings_for_general_requests() {
        match get_proxy_url(&proxy_settings) {
            Ok(proxy_url) => {
                ulog_info!("[proxy_config] owner=general path=hamuna-proxy");
                ...
                // explicit HamunaAgent proxy injection from inherited system env.
        ...
        // No HamunaAgent proxy configured: inherit system network behavior.
...
/// HamunaAgent proxy only when the selected provider is in scope; otherwise it
...
pub fn apply_to_subprocess_for_provider(cmd: &mut Command, provider_id: &str) -> bool {
    if let Some(proxy_settings) = read_proxy_settings_for_provider(provider_id) {
        match get_proxy_url(&proxy_settings) {
            Ok(proxy_url) => {
                ulog_info!(
                    "[proxy_config] owner=provider providerId={} path=hamuna-proxy",
                    provider_id
                );
                cmd.env("HTTP_PROXY", &proxy_url);
                cmd.env("HTTPS_PROXY", &proxy_url);
            ...
            Err(e) => {
                ulog_error!(
                    "[proxy_config] Invalid proxy configuration for provider {}: {}. \
                     Provider subprocess will start without HamunaAgent proxy.",
                    provider_id,
                    e
                );
                for &var in PROXY_ENV_KEYS {
...
pub fn build_blocking_client_with_proxy_for_provider(
    ...
    let final_builder = if let Some(proxy_settings) = read_proxy_settings_for_provider(provider_id)
    {
        let proxy_url = get_proxy_url(&proxy_settings)?;
        ulog_info!(
            "[proxy_config] owner=provider providerId={} path=hamuna-proxy blocking=true",
            provider_id
        );
        let proxy = reqwest::Proxy::all(&proxy_url)
            .map_err(|e| format!("[proxy_config] Failed to create proxy: {}", e))?
