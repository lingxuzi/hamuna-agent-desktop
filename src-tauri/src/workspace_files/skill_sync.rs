...
//!    `~/.hamuna/skills` (resp. `~/.hamuna/commands`). Anything else
...
fn sync_workspace_skills_with_home(workspace: &Path, home: &Path) -> Result<(), String> {
    ...
    let hamuna_root = home.join(".hamuna");
    sync_skills_subtree(workspace, &hamuna_root);
    sync_commands_subtree(workspace, &hamuna_root);
    Ok(())
}

fn sync_skills_subtree(workspace: &Path, hamuna_root: &Path) {
    let user_skills = hamuna_root.join("skills");
    ...
    let disabled = read_disabled_list(hamuna_root);
    let cli_tool_registry_enabled = read_cli_tool_registry_enabled(hamuna_root);
...
fn sync_commands_subtree(workspace: &Path, hamuna_root: &Path) {
    let user_commands = hamuna_root.join("commands");
...
/// `~/.hamuna/skills/foo` source → project-side `foo` link is now broken).
...
mod tests {
    ...
    fn add_user_skill(root: &Path, name: &str) {
        let user_skills = root.join(".hamuna").join("skills");
    ...
    fn user_root_with_skill(name: &str) -> std::path::PathBuf {
        // Create a stand-in `~/.hamuna` rooted at a tempdir for tests so
    ...
    fn skips_skill_without_skill_md() {
        ...
        let user_skill = home.join(".hamuna/skills/empty");
    ...
    fn skips_real_project_skill_dir() {
        ...
        // `~/.hamuna/skills/foo`. Sync MUST NOT clobber the real dir.
        let home = user_root_with_skill("foo");
    ...
    fn cleans_up_dangling_symlinks() {
        ...
        // be cleaned up.
        fs::remove_dir_all(home.join(".hamuna/skills/foo")).unwrap();
        sync_workspace_skills_with_home(&workspace, &home).unwrap();
        assert!(
    ...
    fn respects_disabled_list() {
        ...
        fs::write(
            home.join(".hamuna/skills-config.json"),
    ...
    fn removes_tool_creator_symlink_when_cli_tool_registry_gate_turns_off() {
        let home = user_root_with_skill("tool-creator");
        let config_path = home.join(".hamuna/config.json");
