...
///
/// `target_dir` is workspace-relative (e.g. `"hamuna_files"`). Empty string
...
mod tests {
    ...
    async fn imports_single_file() {
        ...
        let res = cmd_workspace_import_files_b64(
            ...
            payload,
            Some("hamuna_files".to_string()),
        )
        .await
        .unwrap();
        assert_eq!(res.files, vec!["hamuna_files/hello.txt".to_string()]);
        assert_eq!(
            fs::read(ws.join("hamuna_files").join("hello.txt")).unwrap(),
            b"hi"
        );
