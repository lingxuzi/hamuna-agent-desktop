...
//!   Skill/Command detail panels to reveal `~/.hamuna/skills/...` files
...
/// `~/.hamuna/skills/<name>/`); `Some(path)` for callers that know the
...
/// Skill/Command detail panels to open `~/.hamuna/skills/<name>/SKILL.md`
...
pub(crate) fn validate_external_open_path(
    full_path: &str,
...
mod tests {
    ...
    fn validate_external_open_workspace_does_not_bypass_home_blacklist() {
        ...
        let stub = ssh_dir.join(format!("hamuna_ws_bypass_test_{}", std::process::id()));
    ...
    fn validate_external_open_blocks_ssh_dir() {
        ...
        let Some(home) = home else { return };
        let ssh_dir = home.join(".ssh");
        let stub = ssh_dir.join(format!("hamuna_test_stub_{}", std::process::id()));
    ...
    fn workspace_open_blocks_symlink_to_protected_path() {
        ...
        let ws_root = home.join(format!(".hamuna-test-ws-sym-out-{}", std::process::id()));
    ...
    fn workspace_open_allows_inside_workspace_symlink() {
        ...
        let Some(home) = home_dir() else { return };
        let ws_root = home.join(format!(".hamuna-test-ws-sym-in-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&ws_root);
