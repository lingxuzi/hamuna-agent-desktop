...
/// HamunaAgent-managed directory (see `is_trusted_managed_target`). Blocks the
...
/// junctions / symlinks we sync ourselves from `~/.hamuna/skills` etc.
...
///   isn't under a trusted HamunaAgent-managed root, returns
...
/// Canonicalized roots of HamunaAgent-managed directories that we sync into
/// workspaces via junctions/symlinks. Targets under any of these roots are
/// safe to follow from in-workspace links because HamunaAgent owns the source —
...
fn trusted_managed_roots() -> Vec<PathBuf> {
    ...
    let hamuna = home.join(".hamuna");
    ["skills", "commands", "agents"]
        .iter()
        .filter_map(|sub| fs::canonicalize(hamuna.join(sub)).ok())
        .collect()
}
...
fn atomic_write_file_inner(
    ...
    let tmp_name = format!(".{}.hamuna-{}-{}.tmp", file_name, std::process::id(), n);
...
mod tests {
    ...
    fn atomic_write_if_current_cleans_tmp_when_current_read_fails() {
        ...
        let leftovers: Vec<String> = fs::read_dir(&ws)
            ...
            .filter(|name| name.contains(".hamuna-"))
            .collect();
    ...
    fn no_follow_workspace_writer_skips_precreated_temp_symlinks() {
        ...
        let predictable = ws.join(format!(".result.txt.hamuna-{}-0.tmp", std::process::id()));
    ...
    fn trusted_target_empty_roots_rejects_everything() {
        // Defence: if `trusted_managed_roots()` returns empty (no `.hamuna`
    ...
    fn resolve_existing_allows_symlink_into_trusted_root() {
        ...
        // `~/.hamuna/`) and inject our tmp root via the pure helper.
        let lexical =
