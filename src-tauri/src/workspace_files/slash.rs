...
//! 2. `~/.hamuna/commands/*.md`                — user commands
//! 3. `<workspace>/.claude/skills/*/SKILL.md`    — project skills
//! 4. `~/.hamuna/skills/*/SKILL.md`            — user skills (respects skills config disabled list)
...
pub async fn cmd_list_slash_commands(workspace: String) -> Result<SlashCommandsResponse, String> {
    ...
    let hamuna_root = home_dir.join(".hamuna");

    let disabled = disabled_skill_names_for_slash(&hamuna_root);
    ...
    // 2. User commands
    scan_commands_dir(&hamuna_root.join("commands"), "user", &mut commands);
    // 3. Project skills
    if workspace_exists {
        scan_skills_dir(
...
fn disabled_skill_names_for_slash(hamuna_root: &Path) -> Vec<String> {
    let mut disabled = read_disabled_list(hamuna_root);
    if !read_cli_tool_registry_enabled(hamuna_root)
...
mod tests {
    ...
    fn disabled_skill_names_include_tool_creator_when_cli_tool_registry_gate_is_off() {
        let root = make_test_workspace("slash_gate_off_home").join(".hamuna");
    ...
    fn disabled_skill_names_do_not_include_tool_creator_when_cli_tool_registry_gate_is_on() {
        let root = make_test_workspace("slash_gate_on_home").join(".hamuna");
        fs::create_dir_all(&root).unwrap();
        fs::write(
    ...
    fn disabled_skill_names_keep_required_system_skills_available() {
        let root = make_test_workspace("slash_required_home").join(".hamuna");
        fs::create_dir_all(&root).unwrap();
        fs::write(
            root.join("skills-config.json"),
            r#"{"disabled":["hamuna-memory-update","hamuna-memory-gardener","hamuna-memory-molt","hamuna-cli","hamuna-docs","prompt-writer"]}"#,
        )
        .unwrap();
