...
//! `~/.hamuna/attachments/<session>/` store and return only a relative ref
...
pub struct PreparedUserImageAttachment {
    ...
    pub size_bytes: u64,
    /// Relative path under `~/.hamuna/attachments/`, suitable for
    /// `hamuna://attachment/<relativePath>`.
...
async fn prepare_one_user_image_attachment(
    ...
    let Some(data_dir) = hamuna_data_dir() else {
        return Err(make_err(
            "storage_unavailable",
            "HamunaAgent data directory is unavailable".to_string(),
        ));
    };
