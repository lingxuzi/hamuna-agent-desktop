//! Read-only access to `~/.hamuna/skills-config.json`.
//!
...
/// Read the user's disabled-skill list from `~/.hamuna/skills-config.json`.
...
pub fn read_disabled_list(hamuna_root: &Path) -> Vec<String> {
    let path = hamuna_root.join("skills-config.json");
    if !path.is_file() {
        return Vec::new();
    }
...
/// HamunaAgent surface. Keep in sync with `src/shared/systemSkills.ts`.
...
/// `~/.hamuna/config.json`. Omitted/malformed/unreadable means disabled,
/// matching the TypeScript `isCliToolRegistryEnabled()` helper.
pub fn read_cli_tool_registry_enabled(hamuna_root: &Path) -> bool {
    let path = hamuna_root.join("config.json");
    if !path.is_file() {
        return false;
    }
    match fs::read_to_string(&path) {
...
mod tests {
    ...
    fn required_system_skills_are_not_reported_as_disabled() {
        ...
        fs::write(
            ...
            r#"{"disabled":["hamuna-memory-update","hamuna-memory-gardener","hamuna-memory-molt","hamuna-cli","hamuna-docs","ordinary-skill"]}"#,
        )
