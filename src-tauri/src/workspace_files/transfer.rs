    ...
    for source in source_paths {
        match copy_one_path(&source, &target_root, &workspace_root, auto_rename) {
            ...
            Err(err) => {
                ...
                // doesn't reach `~/.hamuna/logs/unified-{date}.log`.
...
mod tests {
    ...
    async fn copies_file_to_target() {
        ...
        assert_eq!(res.copied_files.len(), 1);
        assert_eq!(res.copied_files[0].target_path, "hamuna_files/foo.txt");
        assert_eq!(
            fs::read(ws.join("hamuna_files").join("foo.txt")).unwrap(),
            b"abc"
        );
        let _ = fs::remove_dir_all(&ws);
