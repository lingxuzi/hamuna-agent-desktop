...
fn load_disk_memory_auto_update_agents() -> Result<Vec<DiskMemoryAutoUpdateAgent>, String> {
    let Some(data_dir) = crate::app_dirs::hamuna_data_dir() else {
        return Ok(Vec::new());
    };
...
fn collect_candidates(
    ...
    let Some(hamuna_dir) = crate::app_dirs::hamuna_data_dir() else {
        return vec![];
    };
    let sessions_path = hamuna_dir.join("sessions.json");
    ...
    for session in sessions {
        ...
        let jsonl_modified_at = session_jsonl_modified_at(&hamuna_dir, &session.id);
        ...
        let analysis = analyze_session_jsonl(&hamuna_dir, &session.id);
...
fn analyze_session_jsonl(hamuna_dir: &Path, session_id: &str) -> SessionJsonlAnalysis {
    let jsonl_path = session_jsonl_path(hamuna_dir, session_id);
...
fn session_jsonl_path(hamuna_dir: &Path, session_id: &str) -> PathBuf {
    hamuna_dir
        .join("sessions")
        .join(format!("{}.jsonl", session_id))
}

fn session_jsonl_modified_at(hamuna_dir: &Path, session_id: &str) -> Option<DateTime<Utc>> {
    std::fs::metadata(session_jsonl_path(hamuna_dir, session_id))
...
async fn update_successful_completion<R: Runtime>(
    ...
    let result = tauri::async_runtime::spawn_blocking(move || -> Result<(), String> {
        let config_path = crate::app_dirs::hamuna_data_dir()
            .ok_or("No data dir")?
            .join("config.json");
...
{"role":"user","content":"<system-reminder><CRON_TASK>scheduled</CRON_TASK></system-reminder>","attachments":[{"path":"maintenance.txt"}],"timestamp":"2026-01-01T02:00:00.000Z"}"#;
    ...
    fn marker_detection_uses_the_standard_leading_reminder_protocol() {
        ...
        assert!(is_automatic_user_text(
            "<system-reminder><hamuna-session-event type=\"watch.completed\">result</hamuna-session-event></system-reminder>"
        ));
        assert!(is_automatic_user_text(
            "<system-reminder><hamuna-space-issue>issue</hamuna-space-issue></system-reminder>"
        ));
