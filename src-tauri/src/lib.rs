...
// `--paper` tokens in hamuna-default.css; the unit guard below fails if this
...
const THEME_BOOTSTRAP_APPEARANCE_MARKER: &str = "__HAMUNA_APPEARANCE_MODE__";
const THEME_BOOTSTRAP_RUN_ID_MARKER: &str = "__HAMUNA_BOOTSTRAP_RUN_ID__";
...
fn classify_navigation(url: &Url) -> NavDecision {
    ...
    // - hamuna / hamuna-internal: app's custom protocols
    if matches!(
        scheme,
        "tauri" | "ipc" | "asset" | "hamuna" | "hamuna-internal"
    ) {
...
pub fn run() {
    ...
    let data_dir = app_dirs::hamuna_data_dir().unwrap_or_else(|| std::path::PathBuf::from("."));
    ...
    let builder = tauri::Builder::default()
        ...
        .register_asynchronous_uri_scheme_protocol("hamuna", attachment_protocol::handle)
    ...
    let app = builder
        ...
        .setup(|app| {
            ...
            let theme_bootstrap_selection = app_dirs::hamuna_data_dir()
            ...
            // ── Boot Banner: single-line consolidated diagnostics for AI grep ──
            {
                ...
                let data_dir = app_dirs::hamuna_data_dir();
            ...
            // create a Sidecar that needs HAMUNA_MANAGEMENT_PORT.
            ...
            if let Some(data_dir) = app_dirs::hamuna_data_dir() {
                ...
                // KB now lives in the Node sidecar (TypeGraph + SQLite).
            }
...
mod nav_guard_tests {
    ...
    fn allows_internal_and_local_schemes() {
        ...
        assert_eq!(decide("hamuna://x/y"), NavDecision::Allow);
    ...
    fn native_bootstrap_surface_tracks_canonical_theme_paper_tokens() {
        let css = include_str!("../../src/renderer/theme/themes/hamuna-default.css");
