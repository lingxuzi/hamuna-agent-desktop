// Global shortcut: summon-or-toggle HamunaAgent from anywhere on the OS.
//
// Behaviour:
...
/// Canonical default; M = HamunaAgent, three-platform-safe.
...
fn config_path() -> Option<PathBuf> {
    crate::app_dirs::hamuna_data_dir().map(|d| d.join("config.json"))
}
