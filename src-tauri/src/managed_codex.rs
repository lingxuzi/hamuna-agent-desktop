...
fn data_dir() -> Result<PathBuf, String> {
    crate::app_dirs::hamuna_data_dir()
        .ok_or_else(|| "[managed-codex] Cannot determine ~/.hamuna directory".to_string())
}
...
fn managed_minisign_public_key() -> Result<PublicKey, String> {
    let decoded = base64_to_string(HAMUNA_MINISIGN_PUBKEY, "public key")?;
...
fn managed_env() -> Result<HashMap<String, String>, String> {
    ...
    env.insert(
        "HAMUNA_RUNTIME_SOURCE".to_string(),
...
On a remote or headless machine? Use `codex login --device-auth` instead.";
    ...
    #[test]
    fn manifest_rejects_non_hamuna_https_urls() {
        let mut manifest = valid_manifest("darwin-arm64");
    ...
    fn managed_codex_pubkey_matches_tauri_updater_pubkey() {
        ...
        assert_eq!(updater_pubkey, HAMUNA_MINISIGN_PUBKEY);
    }
}
