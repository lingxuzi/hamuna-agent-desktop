...
//! is stored as a single `.md` file under `~/.hamuna/thoughts/<YYYY-MM>/<id>.md`
...
//! ---
//!
//! 帮我把 OpenClaw 的 lark 适配器升一下版本，#HamunaAgent #维护
//! ```
//!
...
pub struct ThoughtStore {
    ...
    /// `~/.hamuna/thoughts/` — parent dir; month subdirs are created lazily.
    root: PathBuf,
}
...
impl ThoughtStore {
    ...
    /// Absolute path to the `~/.hamuna/thoughts/` root. Used by
...
/// Reveal `~/.hamuna/thoughts/` in the OS file manager so users can
...
mod tests {
    ...
    #[test]
    fn parse_tags_cjk() {
        let tags = parse_tags("升级 #HamunaAgent #维护 适配器");
        assert_eq!(tags, vec!["HamunaAgent".to_string(), "维护".to_string()]);
    }
