//! CLI mode handler for `hamuna` binary.
//!
...
//! ~/.hamuna/bin/hamuna (JavaScript, shebang `#!/usr/bin/env node`). The
...
///
/// Keep this list in sync with the command groups in `src/cli/hamuna.ts` TOP_HELP.
...
const CLI_COMMANDS: &[&str] = &[
    ...
    // Issue #194 — `hamuna diagnose runtime <type>` sugar. Without this, the
    ...
    "diagnose",
    // PRD 0.2.36 — CLI tool registry. Without this, `hamuna tool list` via the
...
pub fn run(args: &[String]) -> i32 {
    ...
    // 2. Find the CLI script at ~/.hamuna/bin/hamuna
    let cli_script = match find_cli_script() {
        Some(p) => p,
        None => {
            eprintln!("Error: CLI script not found at ~/.hamuna/bin/hamuna");
            eprintln!("Please launch the HamunaAgent app at least once to initialize the CLI.");
            return 1;
        }
    };
    ...
    // Inject sidecar port if available (the Node script reads HAMUNA_PORT)
    if let Some(ref p) = port {
        cmd.env("HAMUNA_PORT", p);
    }
...
/// macOS: /Applications/HamunaAgent.app/Contents/Resources/nodejs/bin/node
...
fn find_cli_script() -> Option<PathBuf> {
    ...
    // Primary: ~/.hamuna/bin/hamuna
    let script = home.join(".hamuna").join("bin").join("hamuna");
    if script.exists() {
        return Some(script);
    }

    // Windows: ~/.hamuna/bin/hamuna.cmd
    #[cfg(windows)]
    {
        let cmd_script = home.join(".hamuna").join("bin").join("hamuna.cmd");
...
/// Read the Global Sidecar port from ~/.hamuna/sidecar.port.
...
fn discover_sidecar_port() -> Option<String> {
    let home = dirs::home_dir()?;
    let port_file = home.join(".hamuna").join("sidecar.port");
