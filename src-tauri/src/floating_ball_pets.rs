...
//! concerns for user-installed pet packs under `~/.hamuna/pets`.
...
fn pets_dir() -> Result<PathBuf, String> {
    crate::app_dirs::hamuna_data_dir()
        ...
        .ok_or_else(|| "无法定位 ~/.hamuna 目录".to_string())
}
...
fn import_pet_from_root(root: &Path) -> Result<FbPetEntry, String> {
    ...
    if root_canon == dest {
        return read_pet_entry(&dest, "hamuna");
    }
...
fn build_external_http_client(timeout_secs: u64) -> Result<reqwest::Client, String> {
    ...
    let builder = reqwest::Client::builder()
        ...
        .user_agent(format!("HamunaAgent/{}", env!("CARGO_PKG_VERSION")));
