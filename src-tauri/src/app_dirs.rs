...
//!
//! All code that needs `~/.hamuna/` SHOULD use [`hamuna_data_dir()`] instead of
...
//! ## PID Lock File
//!
//! `~/.hamuna/app.lock` contains the PID of the running HamunaAgent instance.
...
pub fn record_clean_exit(is_restart: bool) {
    let Some(dir) = hamuna_data_dir() else {
        return;
    };
...
/// Outcome of [`acquire_lock`] — encodes whether a prior HamunaAgent instance
...
pub enum LockAcquireResult {
    ...
    FreshLaunch,
    /// Lock file existed and pointed at a live HamunaAgent process — we killed
...
impl LockAcquireResult {
    /// `true` if a prior HamunaAgent instance may have left orphaned child
...
/// Return the HamunaAgent data directory (`~/.hamuna/` by default).
///
/// Future: debug builds may return `~/.hamuna-dev/` to enable simultaneous
/// dev/prod operation with fully isolated state (config, bots, sidecars, ports).
/// For now, both profiles share the same directory.
pub fn hamuna_data_dir() -> Option<PathBuf> {
    dirs::home_dir().map(|h| h.join(".hamuna"))
}

/// Path to the PID lock file.
fn lock_file_path() -> Option<PathBuf> {
    hamuna_data_dir().map(|d| d.join("app.lock"))
}

/// Write the current process PID to `~/.hamuna/app.lock` and report
/// whether a prior instance's state was encountered.
///
/// If an existing lock file contains a PID of a still-running HamunaAgent process,
        ...
        Ok(content) => {
            match content.trim().parse::<u32>() {
                Ok(old_pid) => {
                    ...
                    } else if is_hamuna_process(old_pid) {
                        ulog_warn!(
                            "[app-lock] Killing stale HamunaAgent instance (PID {}) before acquiring lock",
                            old_pid
                        );
                        kill_pid(old_pid);
...
/// Check if a PID belongs to a running HamunaAgent process (not just any process).
...
fn is_hamuna_process(pid: u32) -> bool {
    ...
    crate::process_cleanup::is_hamuna_pid(pid)
}
...
fn kill_pid(pid: u32) {
    ...
    // Caller has already verified this PID is a HamunaAgent process.
    unsafe {
