...
/// endpoint (`https://analytics.hamuna.io/api/track`) through this same path.
///
...
/// written to `~/.hamuna/refs/<id>` instead of being base64-encoded /
...
pub async fn proxy_http_request(
    ...
    //   ~/.hamuna/refs/<id>, dump the buffered bytes, then continue
...
enum StreamOutcome {
    ...
    Buffered(Vec<u8>),
    /// Body exceeded the threshold; written to ~/.hamuna/refs/<id>.
    Spilled(SpilledBody),
...
async fn stream_or_spill_response_body(
    ...
    async fn init_spill(app: &AppHandle) -> Result<SpillState, String> {
        ...
        let refs_dir: PathBuf = home.join(".hamuna").join("refs");
...
mod tests {
    ...
    fn external_urls_are_not_loopback() {
        ...
        assert!(!request_target_is_loopback(
            "https://user:pass@analytics.hamuna.io/track"
        ));
