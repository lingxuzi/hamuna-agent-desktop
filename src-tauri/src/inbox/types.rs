...
/// `<hamuna-session-event>` 注入 enqueueUserMessage / sendExternalMessage。
///
...
pub struct PendingInboxMessage {
    ...
    /// Structured HamunaAgent Session Event Protocol v1 payload. Node sidecar
    /// renders this into `<hamuna-session-event>` before injecting it into
