...
/// Persist a partial patch to a single bot's entry in `~/.hamuna/config.json`.
...
/// Add a new bot entry to `~/.hamuna/config.json`.
...
/// Remove a bot entry from `~/.hamuna/config.json`.
...
/// Read `availableProvidersJson` from the top-level field of `~/.hamuna/config.json`.
...
/// loopback Management API used by `hamuna agent set`.
...
pub async fn cmd_delete_agent(
    ...
    tokio::task::spawn_blocking(move || {
        ...
        // Clean up agent data directory (~/.hamuna/agents/{agentId}/)
        let agent_data_dir = home.join(".hamuna").join("agents").join(&aid);
