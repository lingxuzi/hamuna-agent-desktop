...
/// Config shape from ~/.hamuna/config.json (only what we need)
...
fn read_archived_agent_workspaces_from_disk() -> ArchivedAgentWorkspaces {
    ...
    let path = home.join(".hamuna").join("projects.json");
...
/// Read IM bot configs from ~/.hamuna/config.json
...
pub(super) fn read_im_configs_from_disk() -> Vec<(String, ImConfig)> {
    ...
    let config_dir = home.join(".hamuna");
...
/// Persist a partial patch to a single agent's entry in `~/.hamuna/config.json`.
