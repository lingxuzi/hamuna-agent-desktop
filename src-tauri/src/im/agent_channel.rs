...
async fn create_bot_instance_with_pending_cron_events<R: Runtime>(
    ...
    let default_workspace = config
        ...
        .unwrap_or_else(|| {
            ...
            dirs::home_dir()
                .map(|h| h.join(".hamuna").join("projects").join("mino"))
                .filter(|p| p.exists())
            ...
            for attachment in &msg.attachments {
                match attachment.attachment_type {
                    ImAttachmentType::File => {
                        let target_dir = workspace_path.join("hamuna_files");
                        if let Err(e) = tokio::fs::create_dir_all(&target_dir).await {
                            ulog_error!("[im] Failed to create hamuna_files dir: {}", e);
                            continue;
                        }
                    ...
                    if is_telegram_bind || is_feishu_bind || is_dingtalk_bind || is_openclaw_bind {
                        ...
                        if code == bind_code_for_loop {
                            ...
                            if let Err(e) = send_immediate_reply(
                                ...
                                &msg,
                                "❌ 绑定码无效或已过期，请在 HamunaAgent 设置中重新获取二维码。",
                            ).await {
                    ...
                    if text.starts_with("/provider") {
                        ...
                        if is_external_runtime_type(&current_runtime) {
                            ...
                            let reply = if arg.is_empty() {
                                format!(
                                    "📡 当前 Runtime：{}\n\n供应商/账号由 {} 管理，IM Bot 不能通过 /provider 切换 HamunaAgent 供应商。\n如需切换模型，请使用 /model 查看 {} 可用模型。",
                                    runtime_name,
                                    runtime_name,
                                    runtime_name,
                                )
                                ...
                                format!(
                                    "❌ 当前 Runtime 是 {}，不能通过 /provider 切换 HamunaAgent 供应商。\n供应商/账号由 {} 管理。如需切换模型，请使用 /model。",
                                    runtime_name,
                                    runtime_name,
                                )
                            };
