// IM Health State — periodic persistence to ~/.hamuna/im_bots/{botId}/state.json
...
/// ~/.hamuna/
fn hamuna_dir() -> PathBuf {
    dirs::home_dir()
        .unwrap_or_else(|| PathBuf::from("."))
        .join(".hamuna")
}

/// ~/.hamuna/im_bots/{botId}/
...
pub fn bot_data_dir(bot_id: &str) -> PathBuf {
    ...
    hamuna_dir().join("im_bots").join(bot_id)
}

/// v3 path: ~/.hamuna/im_bots/{botId}/state.json
pub fn bot_health_path(bot_id: &str) -> PathBuf {
    bot_data_dir(bot_id).join("state.json")
}

/// v3 path: ~/.hamuna/im_bots/{botId}/buffer.json
pub fn bot_buffer_path(bot_id: &str) -> PathBuf {
    bot_data_dir(bot_id).join("buffer.json")
}

/// v3 path: ~/.hamuna/im_bots/{botId}/dedup.json
...
/// ~/.hamuna/agents/{agentId}/channels/{channelId}/
...
/// v1 legacy: single-bot era — ~/.hamuna/im_state.json
fn legacy_health_path() -> PathBuf {
    hamuna_dir().join("im_state.json")
}
/// v1 legacy: single-bot era — ~/.hamuna/im_buffer.json
fn legacy_buffer_path() -> PathBuf {
    hamuna_dir().join("im_buffer.json")
}

/// v2 flat: multi-bot era — ~/.hamuna/im_{botId}_state.json
fn flat_health_path(bot_id: &str) -> PathBuf {
    hamuna_dir().join(format!("im_{}_state.json", bot_id))
}
/// v2 flat: multi-bot era — ~/.hamuna/im_{botId}_buffer.json
fn flat_buffer_path(bot_id: &str) -> PathBuf {
    hamuna_dir().join(format!("im_{}_buffer.json", bot_id))
}
/// v2 flat: multi-bot era — ~/.hamuna/im_{botId}_dedup.json
fn flat_dedup_path(bot_id: &str) -> PathBuf {
    hamuna_dir().join(format!("im_{}_dedup.json", bot_id))
}
...
/// Scan ~/.hamuna/ for orphaned v2 flat files (bot IDs not in config.json).
...
fn read_active_bot_ids(hamuna: &Path) -> Option<HashSet<String>> {
    let config_path = hamuna.join("config.json");
