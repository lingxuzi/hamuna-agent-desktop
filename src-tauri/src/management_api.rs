...
fn request_sidecar_generation(headers: &HeaderMap) -> Result<u64, Json<serde_json::Value>> {
    let generation = headers
        .get("x-hamuna-sidecar-generation")
...
pub async fn start_management_api() -> Result<u16, String> {
    ...
    let app = Router::new()
        ...
        // Task Center (v0.1.69) — HTTP surface for the `hamuna task` CLI.
        ...
        // Knowledge base (资料库) — Node sidecar queries/tools.
...
// ── Knowledge base (资料库) handlers ─────────────────────────────────────

#[derive(Deserialize)]
struct KbMountsQuery {
    workspace: String,
}
...
#[derive(Deserialize)]
struct KbPendingRelationsQuery {
    #[serde(rename = "kbId")]
    kb_id: String,
    limit: Option<usize>,
}
...
async fn kb_mounts_handler(
    Query(query): Query<KbMountsQuery>,
) -> Json<serde_json::Value> {
    let Some(engine) = crate::kb::get_kb_engine() else {
        return Json(serde_json::json!({ "ok": false, "error": "kb engine not ready" }));
    };
    match engine.mounts_for_workspace(&query.workspace).await {
        Ok(kb_ids) => Json(serde_json::json!({ "ok": true, "kbIds": kb_ids })),
        Err(e) => Json(serde_json::json!({ "ok": false, "error": e })),
    }
}

async fn kb_query_handler(
    ...
    match engine
        .query(req.kb_ids, req.query, req.limit.unwrap_or(20))
        .await
    {
...
async fn kb_pending_relations_handler(
    Query(query): Query<KbPendingRelationsQuery>,
) -> Json<serde_json::Value> {
    let Some(engine) = crate::kb::get_kb_engine() else {
        return Json(serde_json::json!({ "ok": false, "error": "kb engine not ready" }));
    };
...
async fn kb_relations_handler(
    Json(req): Json<KbRelationsRequest>,
) -> Json<serde_json::Value> {
    let Some(engine) = crate::kb::get_kb_engine() else {
        return Json(serde_json::json!({ "ok": false, "error": "kb engine not ready" }));
    };
...
// turn is called by the `hamuna task` CLI. The CLI is the **entry point of
// trust inference** for `actor` / `source` (PRD §10.2.1 caller-inference table):
//
// - `HAMUNA_PORT` env var set → AI sub-process → `actor=agent, source=cli`
// - Otherwise (user terminal reading `~/.hamuna/sidecar.port`) →
...
struct ThoughtListQuery {
    ...
    /// archive feature so `hamuna thought list --archived` works.
...
fn space_cli_error(error: String) -> serde_json::Value {
    ...
    let (suggestion, suggested_command) = match code.as_str() {
        "SPACE_REQUIRED" | "SPACE_NOT_AVAILABLE" => (
            Some("Run `hamuna space list --json`, then retry with one returned slug."),
            Some("hamuna space list --json"),
        ),
        "ASSIGNEE_ID_INVALID" => (
        ...
        "SPACE_AGENT_BINDING_INVALID" | "SPACE_CONTEXT_INVALID" | "SPACE_CONTEXT_MISMATCH" => (
            Some("Verify the selected Space and current identity with `hamuna space whoami --space <slug> --json`."),
            None,
        ),
        code if code.contains("PERMISSION")
            ...
            Some("hamuna space whoami --space <slug> --json"),
        ),
...
pub(crate) async fn run_task_by_id_with_control(
    ...
    if ta.status != task::TaskStatus::Todo {
        return Err(format!(
            "task is in state '{}'; use 'hamuna task rerun {}' to re-dispatch it",
            ta.status.as_str(),
            ta.id
        ));
    }
...
/// `GET /api/task/read-doc?id=&doc=` — used by the `hamuna task show-doc`
...
mod tests {
    ...
    fn space_cli_error_separates_failure_fact_from_recovery_direction() {
        ...
        assert!(response
            ...
            .is_some_and(|value| value.starts_with("Run `hamuna space list")));
    }
