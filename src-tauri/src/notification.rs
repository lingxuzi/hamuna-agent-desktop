...
/// from `~/.hamuna/config.json` (defaults to enabled if missing). The
...
/// User notification preferences read from `~/.hamuna/config.json`.
///
...
fn read_notification_prefs() -> NotificationPrefs {
    ...
    let parsed: Option<PartialAppConfig> = crate::app_dirs::hamuna_data_dir()
...
/// difference is the toast attribution ("PowerShell" instead of "HamunaAgent").
...
/// to render a toast attributed to HamunaAgent.
///
...
/// not strictly toast clicks — alt-tab back to HamunaAgent within 30s of a
