...
//! `~/.hamuna/logs/unified-2026-05-19.log`.
//!
//! ## What this prevents
//!
...
/// Read `~/.hamuna/config.json` for `forceWakeLock`. Defaults to `false`
...
fn toggle_lock_inner(guard: &mut Option<WakeLock>, enabled: bool) {
    match (enabled, guard.is_some()) {
        (true, false) => match WakeLock::acquire("HamunaAgent force wake-lock") {
            Ok(lock) => {
                *guard = Some(lock);
...
mod tests {
    ...
    //
    // The public `should_force_wake_lock()` reads `~/.hamuna/config.json`,
    ...
    fn force_wake_lock_default_false_when_file_missing() {
        ...
        let p = std::env::temp_dir().join(format!(
            "hamuna_force_wl_test_nonexistent_{}.json",
            std::process::id()
        ));
        if p.exists() {
            std::fs::remove_file(&p).ok();
