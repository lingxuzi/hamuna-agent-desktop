...
pub fn augmented_path() -> std::ffi::OsString {
    ...
    #[cfg(not(target_os = "windows"))]
    {
        ...
        if let Some(home) = dirs::home_dir() {
            push_path_part(
                &mut parts,
                home.join(".hamuna").join("npm-global").join("bin"),
            );
            push_path_part(&mut parts, home.join(".hamuna").join("bin"));
...
fn append_app_local_runtime_dirs(parts: &mut Vec<String>) {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        // uvx.exe is a sibling of server-dist.js so it's already on PATH
        // via the install-dir entry Tauri injects — no extra push needed.
        // Python is NOT pushed here: the installer runs the official
        // Python 3.12 installer which registers python.exe on user PATH,
        // so sidecar / MCP subprocesses find it via the standard PATH
        // walk. (The previous embeddable-Python-via-resources layout
        // shipped a private python312/ that didn't satisfy pip / venv
        // and was rejected by product feedback.)
    }
...
fn append_windows_runtime_dirs(parts: &mut Vec<String>) {
    if let Some(home) = dirs::home_dir() {
        push_path_part(parts, home.join(".hamuna").join("npm-global"));
        push_path_part(parts, home.join(".hamuna").join("bin"));
        push_path_part(parts, home.join(".bun").join("bin"));
        push_path_part(parts, home.join("AppData").join("Roaming").join("npm"));
    }

    push_env_dir(parts, "LOCALAPPDATA", &["HamunaAgent", "nodejs"]);
...
fn detect_shell_path() -> Option<String> {
    ...
    CACHED
        .get_or_init(|| {
            ...
            let marker = format!("__HAMUNA_PATH_{}__", std::process::id());
