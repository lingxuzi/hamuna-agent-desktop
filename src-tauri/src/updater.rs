// HamunaAgent Auto-Updater Module
...
/// Get the ~/.hamuna/ directory path
#[cfg(target_os = "windows")]
fn get_hamuna_dir() -> Result<std::path::PathBuf, String> {
...
/// Reads proxy settings from ~/.hamuna/config.json:
...
pub async fn test_update_connectivity(app: AppHandle) -> Result<String, String> {
    ...
    let builder = reqwest::Client::builder()
        .user_agent(format!("HamunaAgent-Updater/{}", current_version))
...
mod tests {
    ...
    fn parses_windows_updater_temp_dir_names() {
        ...
        assert_eq!(
            parse_windows_updater_temp_dir_version(
                "HamunaAgent-1.2.3-beta.1+build.7-updater-random",
                "HamunaAgent"
            ),
        ...
        assert_eq!(
            parse_windows_updater_temp_dir_version("Other-0.2.27-updater-abcd", "HamunaAgent"),
            None
        );
        assert_eq!(
            parse_windows_updater_temp_dir_version("HamunaAgent-0.2-updater-abcd", "HamunaAgent"),
            None
        );
        assert_eq!(
            parse_windows_updater_temp_dir_version(
        ...
        assert_eq!(
            parse_windows_updater_temp_dir_version("HamunaAgent-0.2.27-updater-", "HamunaAgent"),
            None
        );
        assert_eq!(
            parse_windows_updater_temp_dir_version(
        ...
        assert_eq!(
            parse_windows_updater_temp_dir_version("HamunaAgent-0.2.27", "HamunaAgent"),
            None
        );
    }
    ...
    fn cleanup_removes_only_stale_owned_updater_dirs() {
        ...
        let owned_dir = root.path().join("HamunaAgent-0.2.9-updater-old");
        let owned_file = root.path().join("HamunaAgent-0.2.10-updater-file");
    ...
    fn cleanup_keeps_fresh_owned_updater_dirs() {
        let root = tempfile::tempdir().unwrap();
        let fresh_dir = root.path().join("HamunaAgent-0.2.9-updater-fresh");
    ...
    fn cleanup_skips_symlinked_updater_dirs() {
        ...
        let link = root.path().join("HamunaAgent-0.2.9-updater-link");
