...
// `local_http.rs` to propagate `X-HamunaAgent-Request-Id /
// X-HamunaAgent-Session-Id / X-HamunaAgent-Tab-Id` from inbound headers.
...
/// Get logs directory path (~/.hamuna/logs/)
fn get_logs_dir() -> PathBuf {
    static LOGS_DIR: OnceLock<PathBuf> = OnceLock::new();
    LOGS_DIR
        .get_or_init(|| {
            let home = dirs::home_dir().unwrap_or_else(|| PathBuf::from("."));
            home.join(".hamuna").join("logs")
        })
        .clone()
}
