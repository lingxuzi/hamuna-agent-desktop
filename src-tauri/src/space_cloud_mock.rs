...
pub const MOCK_BASE_URL: &str = "https://space.mock.hamuna.local";
...
impl Drop for MockSpaceTestGuard {
    fn drop(&mut self) {
        std::env::remove_var("HAMUNA_SPACE_MOCK_DATA");
    }
}
...
pub(crate) fn enable_for_test() -> MockSpaceTestGuard {
    ...
    std::env::set_var("HAMUNA_SPACE_MOCK_DATA", "true");
    reset();
...
pub fn is_enabled() -> bool {
    ...
    std::env::var("HAMUNA_SPACE_MOCK_DATA")
        .map(|value| {
            matches!(
...
pub fn download_attachment(
    ...
    let bytes = format!(
        "Mock attachment {}\nGenerated by HamunaAgent Space mock data.\n",
        attachment_id
    )
    .into_bytes();
...
fn initial_state() -> MockState {
    ...
    let issue_specs = vec![
        ...
        ("iss_mock_014", "中文正文和英文 CLI 命令混排的阅读节奏", "in_progress", vec!["docs", "ux"], "详情页正文里会同时出现中文说明、`hamuna space issue view iss_mock_014 --space hamuna` 命令和较长段落，需要稳定行高。"),
    ...
    for (idx, (id, title, status, tag_names, body)) in issue_specs.into_iter().enumerate() {
        ...
        issues.push(json!({
            ...
            "creator": {
                ...
                "avatarUrl": if idx % 3 == 0 { "https://space.mock.hamuna.local/mock-avatar/ethan.png" } else { "https://space.mock.hamuna.local/mock-avatar/lin.png" }
            },
            "author": {
                "id": if idx % 3 == 0 { MOCK_OWNER_USER_ID } else { "usr_lin" },
                "name": if idx % 3 == 0 { "Ethan" } else { "Lin Qiao" },
    ...
    while issues.len() < 500 {
        ...
        issues.push(json!({
            ...
            "creator": {
                ...
                "avatarUrl": if offset % 2 == 0 { "https://space.mock.hamuna.local/mock-avatar/ethan.png" } else { "https://space.mock.hamuna.local/mock-avatar/lin.png" }
            },
            "author": {
                "id": if offset % 2 == 0 { MOCK_OWNER_USER_ID } else { "usr_lin" },
                "name": if offset % 2 == 0 { "Ethan" } else { "Lin Qiao" },
    ...
    let mut skills = vec![
        ...
        skill_record(
            skill(
                ...
                "frontend-taste-review",
                "Reviews React UI for HamunaAgent design-system consistency.",
                3,
            ),
    ...
    let mut agents = vec![
        ...
        agent(
            ...
            "Windows QA Runner",
            "offline",
            "C:/Users/Ethan/Projects/HamunaAgent",
            "Windows VM",
...
fn goal(id: &str, parent_goal_id: Option<&str>, title: &str, context: &str) -> Value {
    ...
    json!({
        ...
        "goalPathLabel": if parent_goal_id.is_some() { format!("HamunaAgent社区 / {}", title) } else { title.to_string() }
    })
}
...
fn seeded_goal_label(index: usize) -> &'static str {
    match seeded_goal_id(index) {
        "goal_mock_runtime" => "HamunaAgent社区 / Runtime Delivery",
        "goal_mock_ui" => "HamunaAgent社区 / UI Quality",
        "goal_mock_docs" => "HamunaAgent社区 / Docs Alignment",
        _ => "HamunaAgent社区",
    }
}
    ...
    let mut comments = vec![
        json!({
            ...
            "author": {
                ...
                "type": "user",
                "name": "Maya Chen",
                "avatarUrl": "https://space.mock.hamuna.local/mock-avatar/maya.png"
            },
...
fn skill(id: &str, name: &str, slug: &str, description: &str, revision: u32) -> Value {
    json!({
        ...
        "uploader": {
            "id": MOCK_OWNER_USER_ID,
            "name": "Ethan",
            "avatarUrl": "https://space.mock.hamuna.local/mock-avatar/ethan.png"
        },
        "createdAt": "2026-06-10T08:00:00.000Z",
        "updatedAt": format!("2026-06-{:02}T12:00:00.000Z", 12 + (revision % 10)),
        "source": {
            "type": "github",
            "url": format!("https://github.com/hamuna/mock-skills/tree/main/{}", slug),
            "resolvedUrl": "https://codeload.github.com/hamuna/mock-skills/zip/refs/heads/main",
            "owner": "hamuna",
            "repo": "mock-skills",
            "ref": null,
            "effectiveRef": "main",
...
fn delivery_item(id: &str, agent: &LocalRegisteredAgent, issue: &Value, status: &str) -> Value {
    ...
    json!({
        ...
        "goalMeta": {
            "id": goal_id,
            "path": agent.goal_path_label.clone().unwrap_or_else(|| "HamunaAgent社区".to_string()),
            "title": agent.goal_path_label.clone().unwrap_or_else(|| "HamunaAgent社区".to_string())
        }
    })
}
...
fn mock_account_plan() -> Value {
    if std::env::var("HAMUNA_SPACE_MOCK_PLAN")
...
fn mock_membership() -> Value {
    json!({
        "id": "mship_mock_owner",
        "role": std::env::var("HAMUNA_SPACE_MOCK_ROLE").unwrap_or_else(|_| "owner".to_string())
    })
}
    ...
    fn goal_mutation_routes_create_update_and_archive_subtrees() {
        ...
        assert_eq!(
            created
                ...
                .and_then(Value::as_str),
            Some("HamunaAgent社区 / Runtime Quality")
        );
        ...
        assert_eq!(
            updated
                .pointer("/goal/goalPathLabel")
                .and_then(Value::as_str),
            Some("HamunaAgent社区 / Runtime Reliability")
        );
