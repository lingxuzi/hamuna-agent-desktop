...
//! `~/.hamuna/tasks.jsonl` (one task per line, atomic full-rewrite on change).
//! Associated markdown documents live under `~/.hamuna/tasks/<taskId>/{task.md,
...
pub struct Task {
    ...
    /// docs live in `~/.hamuna/tasks/<id>/` (user-scoped, v0.1.69+), not
    ...
    /// `~/.hamuna/config.json`, so credential rotation propagates without
...
pub struct TaskCreateFromAlignmentInput {
    ...
    /// Source directory `~/.hamuna/tasks/<alignmentSessionId>/` — its
    /// contents are moved (renamed) to `~/.hamuna/tasks/<newTaskId>/`.
...
/// Sidecar metadata persisted to `~/.hamuna/tasks/<alignmentSessionId>/metadata.json`
...
pub struct TaskUpdateInput {
    ...
    /// `~/.hamuna/tasks/<id>/task.md` under the same write lock that persists the
...
impl TaskStore {
    ...
    pub async fn create_from_alignment(
        ...
        if !matches!(input.execution_mode, TaskExecutionMode::Once) {
            return Err(format!(
                "create-from-alignment only supports executionMode=once; \
                 to set a schedule, create the task and then `hamuna task update <id> \
        ...
        // 2. Move the alignment dir to `~/.hamuna/tasks/<newId>/`. If this fails, we unwind
    ...
    /// Check-and-write `~/.hamuna/tasks/<id>/<filename>` atomically with respect to
...
/// Resolve `~/.hamuna/tasks/<id>/` and verify the resolved path stays inside
/// `~/.hamuna/tasks/`. This is the pit-of-success guard — centralizing path
...
pub fn task_docs_dir(task_id: &str) -> Result<PathBuf, String> {
    ...
    // path must still lexically start with `~/.hamuna/tasks/`. This catches
...
/// Root dir for all task documents — `~/.hamuna/tasks/`.
///
/// Honors `HAMUNA_TASK_DOCS_ROOT` **only in debug / test builds** so tests
...
fn task_docs_root() -> Result<PathBuf, String> {
    ...
    {
        if let Ok(override_path) = std::env::var("HAMUNA_TASK_DOCS_ROOT") {
            let p = PathBuf::from(&override_path);
            if !p.is_absolute() {
                return Err(format!(
                    "HAMUNA_TASK_DOCS_ROOT must be absolute, got {}",
                    override_path
                ));
            }
            return Ok(p);
    ...
    // Route through `app_dirs::hamuna_data_dir()` so future dev/prod data
    // isolation (see `app_dirs.rs` doc — e.g. `~/.hamuna-dev/` for debug
    // builds) picks up this path automatically. Don't hardcode home dir.
    crate::app_dirs::hamuna_data_dir()
        .map(|d| d.join("tasks"))
        .ok_or_else(|| "cannot resolve hamuna data dir for task docs".to_string())
}
...
///    reads `~/.hamuna/tasks/<id>/{task,verify,progress,alignment}.md` on its own)
///
...
fn compose_dispatch_prompt(task: &Task) -> Result<String, String> {
    match task.dispatch_origin {
        TaskDispatchOrigin::AiAligned => {
            ...
            // `~/.hamuna/tasks/<taskId>/` on its own. We just need to invoke it.
...
/// `~/.hamuna/tasks/<alignmentSessionId>/metadata.json`.
///
...
///
/// - `task`: the executor prompt (`~/.hamuna/tasks/<id>/task.md`). Authored by the user
///   at dispatch, editable from the task detail overlay.
/// - `verify`: acceptance criteria (`~/.hamuna/tasks/<id>/verify.md`). Optional; may
///   be authored by the user or produced by the alignment flow. Returns an
///   empty string when the file does not yet exist.
/// - `progress`: read-only execution log (`~/.hamuna/tasks/<id>/progress.md`). Agents
///   append to this file during runs; the UI renders it but does not write.
/// - `alignment`: AI-discussion decision record (`~/.hamuna/tasks/<id>/alignment.md`).
...
/// Reveal `~/.hamuna/tasks/<id>/` in the OS file manager so the user
...
mod tests {
    ...
    fn ensure_test_docs_root() {
        TEST_DOCS_ROOT.get_or_init(|| {
            ...
            std::env::set_var("HAMUNA_TASK_DOCS_ROOT", dir.path());
            dir
        });
    }
    ...
    fn sample_direct_input(ws: &Path) -> TaskCreateDirectInput {
        TaskCreateDirectInput {
            ...
            description: None,
            workspace_id: "ws-hamuna".to_string(),
            ...
            tags: vec!["HamunaAgent".to_string()],
            notification: None,
        }
    }
        ...
        let created = store
            .create_attached_with_session_probe(
                TaskCreateAttachedInput {
                    ...
                    executor: TaskExecutor::Agent,
                    description: Some("HamunaAgent Space Issue iss_123".to_string()),
                    workspace_id: "ws-hamuna".to_string(),
                    workspace_path: ws.to_string_lossy().into_owned(),
