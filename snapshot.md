...
（landed: commit `bfc62de` + `references/mcp-usage-guide.md §3.3` 四态 / `§3.4` 4 步硬门控 + `SKILL.md` 视频生成门禁末尾短句锚点 + `drama/prompt.md`「持久化」段 cross-link；遗留：4 步硬门控 + commercial 3 路 cross-link 未配 lint，依赖 SKILL.md 锚点）
...
（用户选"修复路径 3（手 bump）" → `npm version patch --no-git-tag-version` 把 4 文件 0.3.114 → 0.3.115 + 手改 commands.rs + systemSkills.ts 43→44 + commit `ed5894b`。**未做**：(a) git stash pop stash@{0} Cargo.lock post-commit hook artifact（path #2 不选）；(b) bump-on-commit.mjs 第 2 skip 条件不改（**尊重并发 writer 纪律**，外人主动 bump 不破坏）。**下一 commit 行为**：hook 重新走 wcVer==headVer 路径，下次 commit 若 working tree package.json version 0.3.115 == HEAD 0.3.115 → bump 0.3.115→0.3.116；若是 creative-video-suite 改动 SYSTEM_SKILLS_VERSION 44→45）
