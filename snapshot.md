...
最后更新：**2026-09-18**（任务 #167 Windows 用户更新后广电供应商 ENOENT — root cause = `src/server/nxgd-auth.ts:149` 用 `AUTH_FILE.substring(0, AUTH_FILE.lastIndexOf('/'))` 切父目录。Windows 路径全是 `'\\'`，`lastIndexOf('/')` 返 -1 → `substring(0, -1)` 返空字符串（Node spec：不是切最后一字符）→ `mkdirSync('')` 抛 ENOENT → `writePersistedAuth` 写 `nxgd-auth.json` 失败。根因不是路径前缀（CLAUDE.md `\\?\` 红线不适用）是逻辑 bug）。**改动 2 文件**：(1) `src/server/nxgd-auth.ts` line 16 import 加 `dirname`，line 149 `substring(0, lastIndexOf('/'))` → `dirname(AUTH_FILE)`（Node stdlib 跨平台；Windows 上 Node 自动走 `path.win32.dirname`）。(2) `src/server/nxgd-auth.unit.test.ts` 加 `describe('nxgd-auth 写盘路径 helper 契约（Windows ENOENT regression guard, #167')` block + 1 个顶层 `import { win32 as pathWin32 } from 'node:path'`：case 1 断言 `pathWin32.dirname('C:\\Users\\foo\\.hamuna\\nxgd-auth.json') === 'C:\\Users\\foo\\.hamuna'`；case 2 断言旧 substring + lastIndexOf 实现（`buggy = ''`）确实**不**等于 dirname 结果，**任何回滚会被这个 contract test + 源码 review 拦截**。**为什么 Linux dev 不能直接 reproduce**：vitest 在 Linux 跑，Linux 上 `'C:\\fake\\home'` 不是合法路径，mkdirSync 不会创建 → 不能直接 mock 出 ENOENT；且 `path.dirname` 在 Linux 走 `path.posix.dirname`，看到 `'\\'` 不是 separator 返 `'.'`，所以测试必须**显式 `path.win32.dirname`** 才能测 Windows 语义。**#167 scope-out**：(a) 不改 `withFileLock` line 304 的 `mkdirSync(lockPath)` 不递归（lock dir 不存在时也 ENOENT，但不在本 bug 路径上；writePersistedAuth 内部 mkdirSync `{recursive:true}` 已覆盖，是另一条 latent bug）；(b) 不抽写盘 helper（CLAUDE.md「>3 引用才抽象」拦）；(c) 不动 AUTH_FILE 的 join / 文件名（Node 跨平台，那段本身 OK）。**grllling — substring vs dirname**：原本考虑 `AUTH_FILE.replace(/[^\\/]+$/, '')`，但 regex 同样路径分隔符敏感。`path.dirname` 是 Node stdlib 内置、跨平台、语义清晰（直译"父目录"），零新依赖，唯一选择。**踩坑 — `require()` eslint 拦**：`const { win32 } = require('node:path')` 触发 `@typescript-eslint/no-require-imports`，改顶层静态 `import { win32 as pathWin32 } from 'node:path'`。**踩坑 — Node `substring(0, -1)` 返空串**：我原以为 "切最后一字符"，实际 Node spec 是 `start=0, end=-1` 时 clamp `end=0` → 返空串。修复了测试断言。**踩坑 — Linux `path.dirname` 走 posix**：case 1 第一次跑用 `path.dirname` 失败（posix 看到 `'\\'` 不当 separator），改 `path.win32.dirname` 通过。**踩坑 — vitest glob 不匹配 `.path.test.ts`**：vitest include filter 只匹配 `*.unit.test.ts` / `*.test.tsx`，`.path.test.ts` 不进 unit pool；改回加到 `nxgd-auth.unit.test.ts` 末尾新 describe block。**验证**：18/18 unit pass（17 既有 + 1 contract guard）；typecheck 0 errors；eslint 0 errors；commit `5bae3f1` version bumped 0.3.182 → 0.3.183。

最后更新：**2026-09-18**（任务 #166 wizard step 2 结构化错误路由 + 陈旧数据提示 — 承接 #165 留的 2 个 follow-up；**user 拍板 A+B**：「错误详情结构化 + silent fallback 加陈旧提示」）。**设计 result + error class 混用**：`stale-cache` 不是失败（数据可用，要标陈旧）→ 走 result 形态 `NxgdDiscoveryResult { models, checkedAt? }`；`rate-limit` 带可恢复信号 + `network` 未知失败 → 走 throw `NxgdDiscoveryError({ kind:'rate-limit'|'network', ... })`；caller 兼容 `Array.isArray` 分流（ModelManagementPanel signature 从 `DiscoveredModel[]` → `DiscoveredModel[] | NxgdDiscoveryResult`）。**改动 8 文件**：(1) `src/server/nxgd-auth.ts` 新增 `getCachedNxgdModelsSnapshotWithCheckedAt()` helper 返 `{ models, checkedAt }` 形态（带时间戳）；`getCachedNxgdModelsSnapshot` 删除（无人引用）—— 单一 helper 收敛职责。(2) `src/server/index.ts` 第 693 行 import 切到新 helper；`/api/nxgd/models` handler 4661-4681 行 429 + 502 envelope 加 `checkedAt` 字段（让 renderer 从错误体 regex 抽出 stale-cache timestamp；不动业务 envelope）。(3) `src/renderer/config/services/nxgdSubscriptionService.ts` 全重写：新增 `NxgdDiscoveryResult` interface + `NxgdDiscoveryErrorKind` discriminated union + `NxgdDiscoveryError` class（`detail` 字段携带结构化 detail，`message` 仍人类可读兼容老 caller）；`discoverNxgdModels` 返回类型改 `Promise<NxgdDiscoveryResult>`；429 envelope regex `/retryAfterSeconds["':\s]+(\d+)/` 抽 retryAfterSeconds 抛 rate-limit；502 + cached envelope regex `/"checkedAt":\s*(\d+)/` 抽 checkedAt 返 stale-cache result；其它错误抛 network。(4) `src/renderer/components/NxgdOnboardingWizard.tsx` 加 `AlertTriangle` + `NxgdDiscoveryError` + `NxgdDiscoveryResult` import；state `candidatesError: string|null` → `{ i18nKey, params? }|null` + 新增 `candidatesStale: { ageSeconds }|null`；useEffect `.then` 检测 `result.checkedAt` 设 stale；`.catch` 按 `err.detail.kind` 分流 rate-limit / network / 兜底；JSX 错误条按 i18n key 渲染 + 新增 stale warning banner（AlertTriangle + `var(--warning)`，无 retry 按钮 — 数据可用仅标陈旧）；useEffect 加 `eslint-disable-next-line react-hooks/exhaustive-deps` 注释（deps 故意省略 `candidates.length`/`candidatesLoading` —— 它们是 result 不是 trigger，trigger 仅是 expanded/primaryModel/retryCounter）。(5) `src/renderer/components/ModelManagementPanel.tsx` signature `discoveryAction?: () => Promise<DiscoveredModel[] | NxgdDiscoveryResult>` + `setDiscoveredModels(Array.isArray(result) ? result : result.models)` 兼容旧 caller。(6) `src/renderer/i18n/locales/zh-CN/app.json` wizard.step2 删 `errorTitle` + 加 `errorRateLimited.title`「上游限流」/`errorRateLimited.body`「请在 {{seconds}} 秒后点重试」+ `errorNetwork.title`「获取最新模型列表失败」/`errorNetwork.body`「{{message}}」+ `warningStale.title`「数据可能不是最新」/`warningStale.body`「上次更新于 {{age}} 秒前，可继续使用」；en-US 镜像全 4 段。(7) 新增 `src/renderer/config/services/nxgdSubscriptionService.test.ts` 6 case（happy path / 429 rate-limit / 502 stale-cache / 502 parse fail / network / Error.message 含结构化信息）。(8) 改 `NxgdOnboardingWizard.test.tsx` 用 `vi.importActual` 保留真 `NxgdDiscoveryError` class（其余函数 mock），9 处 `mockResolvedValue([...])` 改 `mockResolvedValue({ models:[...], checkedAt:undefined })`，加 2 个新 case（rate-limit 渲染 `errorRateLimited.title` + retry 按钮 / stale 渲染 `warningStale` testid 不渲染 retry）。**grlling — server 端 502 envelope 加 `checkedAt`**：原 plan 写「checkedAt 怎么传」待 server 现状 grep；实读 server 端 `/api/nxgd/models` 502 envelope 后，envelope `checkedAt` 字段已被剥离 → 必须 server 端加；用 `getCachedNxgdModelsSnapshotWithCheckedAt()` 透传，避免抛错时丢 timestamp。**grlling — `Array.isArray` 分流**：`Promise<DiscoveredModel[] | NxgdDiscoveryResult>` 是 discriminated union，`Array.isArray` 是 stdlib 内置 runtime guard（无新依赖），比 `[Symbol.toStringTag]` 之类靠谱。**踩坑 — `vi.mock` 不暴露 `NxgdDiscoveryError`**：初版 mock 完全覆盖 module 导致 `instanceof` 失效 → 用 `vi.importActual` 保留真 class 仅 mock 函数，vitest 兼容。**#166 scope-out**：(a) 不抽 reusable error-state 组件（仅 1 处用，CLAUDE.md 拦）；(b) 不动 server `fetchModels` silent fallback 行为（graceful degradation 设计）；(c) 不加客户端 UI 限流倒计时（错误条静态文案 + retry 重新触发 fetch 即可，避免再发请求撞限流墙）；(e) 不 i18n 网络错误原文（保留作为「Failed to fetch / <原文>」占位符，避免翻译语义错位）；(f) 不改 server error envelope 字段命名（沿用 `models-unavailable` / `rate-limited`，仅补 `checkedAt`）。**验证**（全绿）：`npm run typecheck` 0 errors；`npx vitest run --project unit src/renderer/config/services/nxgdSubscriptionService.test.ts` 6/6；`npx vitest run --project dom src/renderer/components/NxgdOnboardingWizard.test.tsx` 29/29；pre-existing 5 failed tests 不增不减。
