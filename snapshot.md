...
最后更新：**2026-09-16**（任务 #146 v2 nxgd 充值 payFormHtml auto-submit — 紧接 #146 iframe 不渲染修复，用户追加指示「请查看充值接口返回的api规范，要渲染payFormHtml」。API 规范（`docs/广电token平台API接口.md` §309-323）明示 `payFormHtml` 仅含 `<form action="https://openapi.alipay.com/gateway.do">` 裸 markup，**无 submit 按钮**，客户端必须按规范包裹 `<div id="alipay-wap-pay">${payFormHtml}</div><script>document.querySelector('#alipay-wap-pay form').submit();</script>` 触发跳转。修复前 iframe 渲染了 form HTML 但用户无法付款（卡在 form 上无按钮可点），i18n 文案承诺"会自动跳转"也没兑现。**修复**：(1) `NxgdRechargeForm.tsx::iframeSrcDoc` 包 spec 三件套（DOCTYPE + utf-8 meta + `<div id="alipay-wap-pay">` + auto-submit script）；(2) sandbox 不加 `allow-top-navigation`（pit-of-success 红线 — 否则 iframe 能把整个 app 跳到 alipay.com）—— 支付流程留在 iframe 内即可；(3) i18n `iframeHint` 文案改写，准确承诺"页面会自动提交并跳转至支付宝收银台，付款后回此窗口点稍后关闭"（zh-CN + en-US）。**测试增强**：原"iframe 渲染"case 加 2 条断言 —— (a) `srcdoc` 含原始 `payFormHtml` 不丢；(b) `srcdoc` 含 `<script>...form...submit()</script>`（API 规范契约）。**scope-out**：(1) 仍不在 iframe 内做支付宝回调成功页检测（用户手动点稍后关闭，依赖下次 balance 刷新 + 下次发消息触发 modal）；(2) 仍不暴露"我已支付"独立按钮 — 「稍后」按钮就是「我完成了」自然信号（v1 已拍）；(3) iframe sandbox 不放宽到 `allow-top-navigation`（保留用户上下文不丢）。4 文件 / +20 -8（form +13/-7 + 2 locale +6/-0 + test +1/-1），`npm run typecheck` 0 errors + `test:dom` 6/6 + `eslint` 0 errors。**版本流转**：未 commit → patch bump-on-commit。
