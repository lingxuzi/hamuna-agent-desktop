...
最后更新：**2026-09-18**（任务 #164 wizard transition 强化 + #165 wizard step 2 discovery 错误条 + retry — `/gsap-react` 触发；user 原始冲动是引入 GSAP 库做堆叠，grlling 拒了，DESIGN-LANGUAGE.md:48 / DESIGN-HEURISTICS.md:26 红线「≤200ms / 极克制 / 无弹跳 / prefers-reduced-motion 强制」+ wizard 头注释「不堆砌多动效」。**用户拍板 A. 强化现有 transition；B. CSS keyframes + Tailwind（拒绝 GSAP 依赖）**。**#164 改动 3 文件**：(1) `src/renderer/index.css` 新增 `@keyframes wizard-step-in`（opacity 0→1 + scale 0.98→1，比 `popoverIn` 的 0.95 更克制，匹配 wizard 卡片视觉权重；不放 translateY 因为 step body 已浮在卡片内部）+ 5 行 rationale 注释。(2) `NxgdOnboardingWizard.tsx` step body 加 `motion-reduce:animate-none animate-[wizard-step-in_180ms_cubic-bezier(0.25,1,0.5,1)_both]`（`both` fill mode 防 mount 闪烁；`cubic-bezier(0.25,1,0.5,1)` = DESIGN 红线规定的 ease-out-quart；`key={step}` 已存在 — React remount 触发 keyframe 重放）；expanded 卡片外层复用既有 `@keyframes popoverIn`（项目已有，零新增）—— 184 行 className 加 `motion-reduce:animate-none animate-[popoverIn_180ms_cubic-bezier(0.25,1,0.5,1)_both]`；加 `data-testid="nxgd-wizard-step-body"` 给 step body 测试断言用。(3) `NxgdOnboardingWizard.test.tsx` 新增 `describe('wizard animation')` 3 case：expanded 卡片含 popoverIn className / step body 含 wizard-step-in className + step 切换后仍存在 / pill 态不含 animate-*（保留低优先通知语义）。**grlling — pill 不加动画**：pill 是低优先通知元素，加动画抢注意力破坏语义；expanded pop 已足够传达"展开"。**#165 改动 5 文件**（接 #164 后立刻 iterate — user 预判风险「首次启动 network 挂时 wizard step 2 静默 fallback 到 primaryModel，user 看不出是失败」，拍板方案 C 「fallback primaryModel + 给重试按钮」 — 仍可推进但 user 看到错误条 + 点得到重试）：(1) `NxgdOnboardingWizard.tsx` imports 加 `AlertCircle` + `RefreshCw`（lucide-react 已有，零新装包）；state 加 `candidatesError: string | null`（Error.message 原文透传不 i18n 详情避免翻译语义错位）+ `retryCounter: number`（retry trigger）；useEffect catch 分支 setCandidatesError(msg || 'unknown')；新增 `retryDiscovery` useCallback 清 error + 清 candidates + 重置 loading + bump retryCounter（让 useEffect deps 变化重跑 —— 原 deps `[expanded, primaryModel, primaryModelLabel]` 不含 retryCounter，retry 不改它们，必须独立 trigger）；useEffect deps 加 retryCounter；JSX step 2 在列表上方插错误条（AlertCircle + errorTitle + candidatesError + RefreshCw retry 按钮，`role="alert"` 屏幕阅读器可读，颜色 `var(--error)` + `var(--accent-warm-subtle)` 复用项目 token），错误条不挡 CTA，candidate 列表仍渲染 primaryModel fallback。(2) `zh-CN/app.json` wizard.step2 加 `errorTitle`「获取最新模型列表失败」 + `errorRetry`「重试」。(3) `en-US/app.json` 镜像 `errorTitle`「Failed to fetch latest model list」 + `errorRetry`「Retry」。(4) `NxgdOnboardingWizard.test.tsx` 新增 `describe('wizard step 2 discovery error')` 3 case：discovery 失败 → 错误条 + retry 按钮可见 + CTA 不 disabled + candidate 列表 fallback primaryModel / 点 retry → 错误条消失 + candidate 列表更新 + discoverNxgdModels 调用 2 次 / discovery 成功 → 不渲染错误条。(5) `snapshot.md` 本条（折叠 anchor）。**#165 grlling — 502 + cached silent fallback 不动**：`discoverNxgdModels:44-53` 在 502 + cached 时 silent return 上次成功缓存（"降级返回上次成功缓存"）。本任务不动 — silent fallback 是 graceful degradation 设计，加上「陈旧数据」提示需要 `DiscoveredModel` 类型透传 `checkedAt`（server 端 `NxgdModelsEnvelope.checkedAt` 已存但 renderer 没透），是另一个 refactor。**#165 scope-out**：(a) 不改 `discoverNxgdModels` hardcoded 中文「上游限流，N 秒后重试」 + silent cache fallback；(b) 不抽 reusable error-state 组件 — 用率 < 3（当前 ModelManagementPanel 是唯一同 pattern 用户，wizard 是第二处但不算「抽出」门槛），CLAUDE.md「代码块 >3 引用才抽象」直接拦；(c) 不 i18n 错误详情 — Error.message 原文透传；(d) 不改 closeAndMarkSetup / onPinModel / pickedModelId 默认逻辑；(e) 不动 wizard 头注释「单节奏 200ms」 — 本次是错误态，不是入场动效；(f) 不加 motion-reduce:animate-none — 错误条是条件渲染非 mount 动画。**#165 实现踩坑 — retry 必须有独立 deps trigger**：初版 retryDiscovery 只 setCandidatesError(null) + setCandidates([]) + setCandidatesLoading(false)，依赖 useEffect 原 deps `[expanded, primaryModel, primaryModelLabel]`，retry 不触发 effect 重跑，错误条消失但 candidate 列表不更新，测试 1/2 fail。修法 = 加 `retryCounter: number` state，retry bump 它，useEffect deps 加 retryCounter —— 26 → 27/27 pass。**验证**（全绿）：`npm run typecheck` 0 errors；`npx vitest run --project dom src/renderer/components/NxgdOnboardingWizard.test.tsx` 27/27（21 + 3 #164 animation + 3 #165 discovery error）；pre-existing 5 failed tests 不增不减（与本次无关）。
