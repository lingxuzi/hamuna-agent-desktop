...
#### TODO #113 — multimedia-creator MCP: 每次 spawn force-refresh 到 PyPI latest（去 0.1.4 pin）✅ DONE（落地详见 `extended_buildin_mcp/mcp.json` multimedia-creator + commit（即将）；`uvx --refresh-package agnes-video-25-mcp --from agnes-video-25-mcp`，每次 spawn 强制从 PyPI re-resolve + upgrade 到 latest；bundled uvx 0.5.11 已 `strings` 验支持 `--refresh-package`。**副作用**：每次 spawn +100-500ms PyPI round-trip + PyPI rate-limit 高频撞（>100 req/min 警戒）+ 上游 breaking change 自动传染（pin 模式下不可见）+ 网络/PyPI 故障 → MCP spawn 失败 → AI 端 hard error。**回滚**：改回 `uvx --from agnes-video-25-mcp==<ver>` 即可）
