...
最后更新：**2026-09-18**（任务 #155 bundled-skills 孤儿硬删 — 用户原话"bundled-skills 内与 ~/.hamuna/skills 中 bundled skills 同步，但是不要伤害用户自己安装的 skill"+"孤儿 skill 应该硬删"+"bundled-skills 默认为 system-skills，完全同步, 不要动用户自己的skills"。**核心 invariant**：「bundled-originated」vs「user-installed」判据 = **是否曾经出现在源码清单**（Node 端隐式 = `~/.hamuna/skills-config.json::seeded[]`，Rust 端新引入 `~/.hamuna/.system-skills-snapshot.json`）。**Node 端**（`src/server/index.ts::seedBundledSkills` 新增 orphan cleanup pass，4 文件 / +118 -4）：(a) 加 `__testing` 导出块 + `export function seedBundledSkills()` —— test seam；(b) 主循环两处 `config.seeded.push(folder)` 加 `if (!config.seeded.includes(folder))` 守卫（**修复 pre-existing duplicate-push bug**：tracked 但不存在的 folder 也会重复 push，让 orphan 决策模糊）；(c) 主循环后追加 orphan cleanup pass：跳过 `SYSTEM_SKILLS`（Rust 独占）+ 跳过当前 bundled + 跳过 `isSkillBlockedOnPlatform`（如 agent-browser on Windows），其余 lstat 探针 + `rmSync({recursive,force:false})`，broken symlink 用 `lstatSync + unlinkSync`，最后 `config.disabled` 顺手清 stale 条目；(d) 新 `src/server/index.unit.test.ts` 7 case（orphan 删 / user-installed 保 / re-seed skip / SYSTEM_SKILLS 保 / platform-blocked 保 / disabled stale 清 / broken symlink）。**Rust 端**（`src-tauri/src/commands.rs::sync_system_skills_blocking` 新增 snapshot + cleanup，1 文件 / +196 -0）：(a) 新 `SystemSkillsSnapshot { version, skills }` struct + `read_system_skills_snapshot` / `write_system_skills_snapshot`（atomic tmp + rename）；(b) 主循环后追加 orphan cleanup pass：读 snapshot → `HashSet<&str>` current SYSTEM_SKILLS → 遍历 `~/.hamuna/skills/` 跳过 dotfiles / 跳过 current bundled / 跳过 snapshot 外的 user-installed → `symlink_metadata` 探针 + `fs::remove_dir_all`；(c) 每次 sync 末尾无条件 `write_system_skills_snapshot` 写当前 SYSTEM_SKILLS → bootstrap tracker on first post-#155 launch；(d) 新 2 unit test：`orphan_removal_targets_only_names_in_snapshot_but_not_in_current_system_skills`（用真名 `task-alignment`/`fake-orphan`/`mycustom` 锁「orphaned bundled vs still-bundled vs user-installed」三态）+ `orphan_cleanup_skips_passes_when_snapshot_missing_or_corrupt`（missing/corrupt JSON 两路径都 graceful skip）。**为何 Node 端不新加 marker file**：现有 `skills-config.json::seeded[]` 已经是「曾经 bundled」的隐式 tracker（只增不减），复用零成本；Rust 端没有等价 tracker 必须新建。**保守失败**（Rust 端）：snapshot 缺失/损坏 → cleanup 跳过不删（避免 first-launch 误删 user content）；symlink_metadata 失败 / remove_dir_all 失败 → 留 entry 下次重试。**scope-out**：(1) MCP analog `mcp-bundled-seed.ts::seedBundledExtendedMcpServers` 不动（test 锁死「不删 user-removed-from-bundle」，MCP 有 env 用户字段，判据不同）；(2) soft-delete 到 `.orphans/` —— 用户拍板硬删；(3) `.user-owned` marker file —— 不引入新 marker；(4) `pit_of_success.md` 缺失修 —— 不在本任务范围。**版本流转**：未 commit → patch bump-on-commit。`npm run typecheck` 0 errors + `test:unit` 7/7 全绿（新建 index.unit.test.ts）+ `cargo test --lib commands::system_skills_tests` 13/14 pass（1 fail = `v37_updates_goal_cli_skill_and_preserves_v36_contracts` 是 2026-07-24 pre-existing 失败，断言 outdated `SYSTEM_SKILLS_VERSION == "37"` vs 实际 "54"，按 CLAUDE.md "只改必须改的" 不修）。**已知风险**：(a) v1 bundled `foo` → 用户装同名 `foo` → `seeded` 已有 `foo` → orphan cleanup 删（user 同名 escape hatch = 升级前备份或命名空间）；(b) `SYSTEM_SKILLS_VERSION` 跟 shared file `"55"` 已对齐（test `rust_and_node_system_skill_lists_match` 锁契约），`SYSTEM_SKILLS` 清单本身未改，本任务保持 `VERSION = "55"` 不变。

最后更新：**2026-09-16**（任务 #154 v14 广电卡片不再一直显示错误 — 用户原话"卡片上不要一直显示错误"。**根因链路**：(1) 用户点充值 → server `createRechargeOrder` 失败 → `updateState({error: msg})` 写 state.error 但**不改 status**（status 仍 'registered'）；(2) 卡片状态徽章看到 status='registered' + balance 仍正常 → 显示绿章余额；(3) 卡片 error 区域无条件显示 `auth.error` → 显示红字 stale 错误；(4) 用户看到"绿章余额 + 红字错误"自相矛盾状态。**修复**（1 文件 / +4 -1）：`NxgdSubscriptionProvider.tsx:135-141` 把 `auth?.error ? ... : undefined` 改成 `auth?.error && auth.status === 'error' ? ... : undefined` —— 只有真正的 auth 失败（status='error'）才显示 error。**为何不在 server 端清掉**：recharge 失败是有效信号（state.error 透出到 form 错误区给 user 看，v11 已建通道），不该清 —— 清了 user 在 form 里就看不到 reason。**Card vs form 责任划分**：card = auth 状态机展示（registered/balance/low），form = 单次操作错误展示（recharge 失败 / 网络 / 上游 4xx）。两者不重叠。**scope-out**：(1) 不加 auto-clear on balance refresh —— 当前的 guard 已够，多余 IO；(2) 不改 server `createRechargeOrder` 的 state 写入策略 —— 保留 form 错误区信息源；(3) 不加 "error dismissed" toast —— card 本就不应显示。**版本流转**：未 commit → patch bump-on-commit。1 文件 / +4 -1，`npm run typecheck` 0 errors + `eslint` 0 errors + `NxgdSubscriptionProvider` 9/9 0 回归。

最后更新：**2026-09-16**（任务 #153 v13 跨协议切换回到 v5+ 确认对话框 — 用户拍"如果这样还是弹出确认窗口让用户选择是否切换 provider"。**反转 v8 plain cross-provider 分支**：v8 把 plain cross-provider（既非 codex 也非 runtime）从 dialog 改 auto-fork；v13 回到 dialog —— 让用户明确选"切并新建会话"。**v12 同协议 bypass 保留**（在 if 之上，!isBothAnthropicProtocol 短路），不动 v12 的复用 session 行为。**改动**（1 文件 / +8 -5）：`Chat.tsx::handleProviderChange` trigger 内 `isPlainCrossProvider` 分支从 `void forkSessionForProviderSwitch(...)` 改 `setPendingProviderSwitch(...)` —— 与 codex / runtime / same-provider 三 case 走同一条 dialog 路径（`buildProviderSwitchDialogCopy` 已含 `crossProvider.message` 文案，无需新增 i18n key）。**统一行为**：现在所有"不可复用历史"的切换都弹 dialog 确认 —— v8 唯一不经 dialog 的 plain cross-provider 现在也归位。**为何保留 forkSessionForProviderSwitch helper**：dialog confirm 路径（`confirmProviderSwitch`）仍调它，不是 dead code。**scope-out**：(1) 不在 dialog 里加"复用当前 session"按钮 —— 跨协议复用技术上风险大（Anthropic Messages ↔ OpenAI chat.completions tool call id / schema 不通用），v12 同协议 bypass 已覆盖用户实际场景；(2) 不改 `crossProvider.message` 文案 —— 文案"保留当前会话，并在新 Tab 使用目标模型"准确描述新 tab 行为，无需改；(3) 不加 toast 通知 —— dialog 自带 toast 信息。**版本流转**：未 commit → patch bump-on-commit。1 文件 / +8 -5，`npm run typecheck` 0 errors + `eslint` 0 errors + `providerHistory` 9/9 + `providerExecution` 11/11 + `optionResolve` 58/58 0 回归。

最后更新：**2026-09-16**（任务 #152 v12 同协议 provider 切换复用当前 session — 用户原话"切换到广电为何会切换新对话，每次切换历史对话出现 New Chat"。**v8 反转错了**：v8 把 cross-provider auto-fork 拿掉对话框后，Anthropic 官方 → nxgd 路径仍触发 auto-fork —— 因为 `canResumeAcrossProviderBoundary`（`src/shared/providerHistory.ts:74-83`）把 `api.anthropic.com` 判 `'anthropic'`、把 `ai-models.cloudwasu.cn`（也是 Anthropic Messages 协议）判 `'third-party'`，identity 不同 → canResume=false → fork。**修复策略选择**：UI 层 bypass（不动 `canResumeAcrossProviderBoundary` 契约），**不动 providerHistory.ts = 不破其它 9 个 unit test 锁定的"anthropic 官方 ≠ 第三方"语义**。**实现**（1 文件 / +10 -5）：`Chat.tsx::handleProviderChange` line ~3411 抽 `currentProviderEnv` / `nextProviderEnv` 为局部变量复用，新增 `isBothAnthropicProtocol = (currentProtocol ?? 'anthropic') === 'anthropic' && (nextProtocol ?? 'anthropic') === 'anthropic'`；trigger if 条件 `!canResumeProviderHistory &&` 后加 `!isBothAnthropicProtocol &&` —— 同协议 anthropic-protocol 切换跳过 auto-fork / 跳过 dialog，直接走 "write back" 路径（复用当前 session 的 snapshot，selectedProviderId / selectedModel 直接更新）。**OpenAI-protocol ↔ anthropic-protocol 仍 fork**：v12 只 bypass 同协议；xai-sub (openai) ↔ anthropic-sub 跨协议仍走 fork（这是真实的不可移植边界）。**Codex / runtime-backed 仍 dialog**：v12 bypass 嵌套在 `isPlainCrossProvider` 检查之后才 fork，codex/runtime 路径走 dialog 不动。**为何不动 `canResumeAcrossProviderBoundary`**：(a) 9 个 unit test 锁定"anthropic 官方 ↔ third-party 不可复用"语义（`providerHistory.test.ts:30-39` 显式 `expect(false)`），改契约要改测试；(b) 跨协议兼容（volcengine openai ↔ deepseek anthropic 都 third-party）当前设计意图是兼容的——bypass 只针对用户场景（"我切 nxgd 不要 new tab"），不动协议级兼容契约；(c) UI 层 bypass 是更 surgical 的修复点，副作用边界明确（只影响 Chat.tsx 的 trigger）。**scope-out**：(1) 不抽 helper 函数——`isBothAnthropicProtocol` 是 1 行 trivial 检查（`providerHistory.test.ts` 9/9 + `providerExecution.test.ts` 11/11 + `optionResolve.test.ts` 58/58 已锁住底层契约，无需再加 test）；(2) 不在 Settings 加"切换时复用 vs 新建"开关——用户拍"复用"，无歧义；(3) 不改 `providerSwitch.i18n` 文案——bypass 后根本不走 dialog，文案无引用点；(4) 不改 snapshot 模型字段——`v2/v3` 已 collapsed alias，session 切到 nxgd 时 SDK 仍按 collapsed alias 重发，replay 不会因 model 名错位失败。**版本流转**：未 commit → patch bump-on-commit。1 文件 / +10 -5（仅 trigger block 内 refactor + bypass），`npm run typecheck` 0 errors + `eslint` 0 errors + `providerHistory` 9/9 + `providerExecution` 11/11 + `optionResolve` 58/58 0 回归。

最后更新：**2026-09-16**（任务 #151 v11 充值失败真因透出 — 用户原话"对话时弹出充值弹窗 点击去支付会出现 创建订单失败，请稍后再试"。**v9 修了"显示裸 code"但用户看不到真因**：v9 把所有 known server code 翻译成同款"创建订单失败，请稍后再试"i18n 文案——本次用户**又**看到这条文案，但这次它是真错误（server `createRechargeOrder()` 返 null 触发 502），不是 bug，**是信息丢失**：上游 4xx/5xx / 网络 throw / device id 缺失 → 一律同文案，user + 排查都看不到哪条路径失败。**3 文件最小改动**：(1) `src/server/index.ts:4553-4573` `/api/nxgd/recharge` 502 响应从 `{ error, state }` 扩 `{ error, message: state?.error, status: state?.status }`——`message` 来自 `createRechargeOrder` 内部 `updateState({error: msg})` 写入的人类可读原因（"recharge failed (code XXX)" / "fetch failed" / "device_id not ready"）；(2) `src/renderer/api/apiFetch.ts` 新增 `buildApiError()` helper：把 server `{error, message}` 包装成 `Error` 同时设 `serverMessage` 字段（`isApiError=true` 标志让 apiGetJson 的 catch 区分"自己 throw 的"和"JSON.parse 失败的"）；**5 个 throw site 全替换**（apiPostJson/apiPostFormData/apiPutJson/apiDelete + apiGetJson），向后兼容（`err.message` 仍是 code 不变）；(3) `src/renderer/components/nxgd/NxgdRechargeForm.tsx` catch 块优先级：`serverMessage.trim() || i18n fallback`；`code` 留作 `console.error` 排查日志。**为何不让 server 直接改返 i18n 文案**：server 不知道 renderer locale（i18next 是 renderer 端），强行中文 / 英文都半成品；透出**机器可读 message** 让 user 看到能搜能 copy。**测试 1 新 case + 1 改 case**（`NxgdSubscriptionProvider.test.tsx` 8→9）：旧 case 改语义"serverMessage 存在 → 显示真因"（断言看到 `recharge failed (code 403)` + 不显示裸 code），新增 case"serverMessage undefined → fallback i18n"（锁住"裸 code 永不出现"契约）。**scope-out**：(1) 不扩到其它 endpoint（nxgd-auth 走 Settings 卡片显示已有 `authState.error`，不走 catch 路径）；(2) 不加 retry 按钮——用户重试只需再点「去支付」，加按钮是噪声；(3) 不加 toast——error 已经在错误提示区域显示，toast 重复；(4) 不在 server 端把 i18n 文案当 message——server 不知道 locale，污染契约。**版本流转**：未 commit → patch bump-on-commit。4 文件 / +35 -8（`apiFetch.ts` +22 -4 / `index.ts` +6 -1 / `NxgdRechargeForm.tsx` +6 -2 / `NxgdSubscriptionProvider.test.tsx` +2 case 替换 1 case 净 +1），`npm run typecheck` 0 errors + `eslint` 0 errors + `NxgdSubscriptionProvider` 9/9 + `nxgd-auth.unit.test` 16/16 + `src/renderer/utils/` 547/547 0 回归。

最后更新：**2026-09-16**（任务 #150 v10 全 app 时区强制 Asia/Shanghai — 用户原话"desktop app 时区调整到 Asia/Shanghai"；grlling 后用户拍"全 app 显示强制用 Asia/Shanghai"（跨时区用户在 app 里也看北京时间）。**现状盘点**：(A) 8 处显式 fallback（agent 心跳 / IM heartbeat / 定时任务 / config-types 等）已是 `'Asia/Shanghai'`；(B) 91 处 `new Date(...).toLocaleString/locale/time/date` / `new Intl.DateTimeFormat(...)` **没传** `timeZone` option → 走 OS locale；(C) 3 处 `Intl.DateTimeFormat().resolvedOptions().timeZone` 主动读 OS 时区（cron 任务调度时区默认 seed）。**实现思路反转**（拒绝"91 处逐个加 timeZone"路径）：monkey-patch `Intl.DateTimeFormat` 构造函数 + 子类 `AppDateTimeFormat` 在 `super(locales, { timeZone: 'Asia/Shanghai', ...options })` 中**注入** timezone 作为 default —— `Date.prototype.toLocaleString()` / `toLocaleTimeString()` / `toLocaleDateString()` 内部就是 `new Intl.DateTimeFormat(undefined, options).format(this)`，覆盖链断不了。**spread 让用户主动选胜出**：cron 任务"用户时区 = Tokyo"路径（`ExecutionModeEditor.tsx:101` `Intl.DateTimeFormat().resolvedOptions().timeZone || 'Asia/Shanghai'`）显式传 `timeZone: 'Asia/Tokyo'` 时被 spread 覆盖，**用户主动选择不被吞**。**接入位置**（关键）：`src/renderer/main.tsx` 顶部第 1 行 `import './utils/timezone-init'` —— 必须先于任何 `Intl.*` / `Date.toLocale*` 用法，否则覆盖晚于用法导致部分调用拿到 OS 时区。**Server 端 / Tauri Rust 不动**：node server 用 `Date.now()`（无 timezone）/ Rust 用 `SystemTime::now()` 返 UTC timestamp（无 display），零 display site。**测试 4 new case 全绿** (`timezone-init.test.ts`)：(1) `new Intl.DateTimeFormat().resolvedOptions().timeZone === 'Asia/Shanghai'` —— 兜底：空 options 拿 default；(2) `Date.toLocaleString('en-US', {hour, minute, hour12:false})` 在 UTC 12:00 → 20:00（Shanghai UTC+8）—— 覆盖链没断；跟显式 `new Intl.DateTimeFormat('en-US', {timeZone: 'Asia/Shanghai', ...}).format(...)` byte-equal 对比（不是 tautology：前者走我们的 default，后者用户显式给 Tokyo 时会 drift，**等价性 = override 真生效的证据**）；(3) 显式 `{timeZone: 'Asia/Tokyo'}` 胜出（不传 vs 传 Tokyo 必不同 hour）—— 用户主动选择保护；(4) `Intl.DateTimeFormat.supportedLocalesOf([...])` 仍工作 —— 静态方法通过 ES6 class extends 自动继承。**陷阱**：ES6 class 继承内置对象在 V8 / WebKit / Spidermonkey 都 OK，但 **old node < 12 不支持 extends on built-ins**；本项目 `package.json` Node ≥ 18（CLAUDE.md 工程基线），无忧。**实现细节**：类用 class extends 而非 function 包装（保留 prototype 链让 `instanceof Intl.DateTimeFormat` 仍 true，第三方库不会 break）；`export {}` 让 bundler 不 tree-shake 纯 side-effect 模块。**scope-out**：(1) 不做"按 locale 路由"（zh → Shanghai / en → LA）—— 用户拍"强制 Asia/Shanghai"，未来想改再 Proxy 路由；(2) 不动 cron 任务"用户时区 = OS"读取 —— 用户主动改 timezone 时仍可走 Tokyo 等非 Shanghai（spread 覆盖保护）；(3) 不在 Rust 端做 chrono::FixedOffset 包装 —— server 不做 display；(4) 不写 setting 切换 —— 强制而非可选，YAGNI。**版本流转**：未 commit → patch bump-on-commit。3 文件 / +93 -1（`timezone-init.ts` 39 行 / `timezone-init.test.ts` 53 行 / `main.tsx` +1 行 import），`npm run typecheck` 0 errors + `eslint` 0 errors + `test:unit` 4/4 + `cronTaskI18n` 3/3 + `scheduleSummary` 2/2 0 回归（其它 pre-existing 失败如 widgetSandboxHtml/useTabSwipeGesture 跟本 fix 无关，沿 TODO #3）。

最后更新：**2026-09-16**（任务 #149 v9 nxgd 充值失败 i18n 映射（被 v11 取代，缩为 1 行 reference）—— 见 v11 narrative 末尾"v9 → v11 演进"段落。


最后更新：**2026-09-16**（任务 #147 v3 nxgd 真正"零操作可用"收尾 — v2 后 user 实操反馈"还是没有"（确认：Chat 下拉里**完全看不到** nxgd provider）+ 顺手拍"另外广电选择首选模型后，模型映射自动映射到首选模型"——想要 auto-mapping 行为。**两个真根因**：(A) Chat 过滤 subscription providers 走 `hasProviderRouteCredential`（`src/shared/providerRoute.ts:55-73`），要求 `verifyStatus[nxgd]` 有 `status:'valid'` / `accountEmail` / `verifiedAt` 之一 —— 但 nxgd 走 `host-managed-auto-register`（机器码 server 端自动注册 + apiKey 永不出 server），从不主动写 verifyStatus → 永远被过滤；Grok 走 OAuth 流程会调 `saveProviderVerifyStatus('xai-sub','valid')` 写盘，nxgd 没有等价路径。(B) nxgd preset `modelAliases: { ...ANTHROPIC_ALIASES }`（fable/opus/sonnet/haiku 分别是 claude-fable-5 / claude-opus-4-8 / claude-sonnet-5 / claude-haiku-4-5）— 是 **split table**，`resolveSessionModelAliases`（`src/server/utils/model-aliases.ts:18-33`）检测到 `fable !== opus` 直接 preserve 不动 → SDK subagent (Task/Explore/...) dispatch 时仍发 `claude-fable-5` 等 Claude id 给 nxgd 上游，但 nxgd 上游**只有 `deepseek-v4-flash-0731`** → 404。**两处修复**：(A) `ConfigProvider.tsx` 已有 v2 useEffect（fire `/api/nxgd/models`），在 `.then` 里加 `await saveProviderVerifyStatusService('nxgd', 'valid')` —— 200 响应是最强"registration 成功 + apiKey 可用"信号，以 fetch 成功为单一触发器；fetch 失败路径不动 verifyStatus（保持 fail-soft）；(B) `config-types.ts` nxgd preset 改 `modelAliases` 为 collapsed `{fable/opus/sonnet/haiku: 'deepseek-v4-flash-0731'}` —— `resolveSessionModelAliases` 现在检测 collapsed + activeModel != haiku → 全部 override 成 activeModel，user 在 Chat 选任何 model，SDK subagent 跟着走。**测试 1 新 case**：`config-types.test.ts` 新增 `nxgd preset > collapses modelAliases to the upstream-real model id (not Claude ids)`，断言 4 个 alias 都 = `deepseek-v4-flash-0731`；防回归回退到 ANTHROPIC_ALIASES。`config-types.test.ts` 44/44 + `modelDiscoveryService.test.ts` 5/5 = 49/49 全绿，`npm run typecheck` 0 errors + `eslint` 0 errors。**为何不在 `hasProviderRouteCredential` 内 special-case nxgd**：(a) shared/route 层不该知道具体 provider 的 auth 实现；(b) 想复用 verifyStatus 作为 Chat/Settings 双向同步信号（Settings → 广电 显示"已连接"绿章也走 verifyStatus），加 special-case 会让 shared 跟具体 provider 紧耦合；(c) `saveProviderVerifyStatus` 已存在，写盘一次即跨层共享。**为何 collapsed 写死单 model id 而不是 dynamic 跟 discovered 漂移**：nxgd 上游目前只有 1 个 model (`deepseek-v4-flash-0731`)，dynamic 漂移会让 user 重启后 Chat 默认 model 跳变（已在 v2 scope-out 列出）；若未来扩 model，preset 仍 collapsed + `withNxgdDiscoveredModels` 注入新 model 进 `provider.models`，user 通过 Chat 选；preset 不动即可。**scope-out**：(1) fetch 失败路径不清理 nxgd 的 `verifyStatus` —— 上次成功写入的 `'valid'` 标记会留下，但 user 删 nxgd provider / 卸载 app 后自然清理；临时 503 网络抖动场景接受 stale（注册成功仍视为可用，保守 fail-open 比 fail-closed 让 user 卡死更好）；(2) `nxgdDiscoveredModels` 内存 overlay 仍不写盘 —— v2 已论证，24h server cache + 内存足够；(3) `NxgdSubscriptionProvider` 仍不调 verifyStatus —— Settings 已渲染认证状态（registered/balance 等），硬编码耦合 verifyStatus 反而分裂两路信源。**版本流转**：未 commit → patch bump-on-commit。

最后更新：**2026-09-16**（任务 #146 nxgd 充值（v7 显式「去支付」+ iframe 渲染两步修）— v6 后用户拍"金额不足，充值面板加一个「去支付」按钮，选项和自定义输入互斥，用户选了自定义才显示输入框，点击去支付按钮跳转到 checkoutUrl，不要用户选了金额自动跳转"。**反转 v6 方向**：v7 走「点 preset 仅切视觉 → user 显式点「去支付」按钮才 POST + openExternal」，符合支付 UX 三步走惯例（淘宝 / 微信支付同款）。**修复**：(1) `NxgdRechargeForm` 加 `mode: 'preset' | 'custom'` state + 6 列按钮网格（5 预设 + 第 6 个「自定义」），custom 模式显示 number 输入框 + `autoFocus`，preset/custom 互斥；(2) 删 `onBlur`/`onKeyDown` Enter 自动提交，改显式「去支付」按钮 + `handleGoPay()`（`disabled={!valid || paying}`）；(3) v7+followup：`submit()` 末尾删同步 `onCompleted?.(result)` —— 该同步调用让 modal 在 `setOrder` commit 前被卸载，iframe 永远到不了 DOM；iframe "稍后"按钮 onClick 改为同时调 `onCompleted + onCancel`，user 明示完成才通知父级「关闭+刷新」；(4) i18n 加 `modal.goPay` + `iframeTitle`（zh-CN/en-US 均缺）。**新增 case 2 / 改 2**：`NxgdSubscriptionProvider.test.tsx` 新增"custom → 88.88 → 去支付 → POST + openExternal" + "POST 返 payFormHtml → 等 iframe DOM 出现"（jsdom iframe srcDoc 不可靠 sandbox 执行，只验元素存在）；改 2 case 明确"选 ≠ 付"契约 + 三步走流程。**为何用显式按钮**：支付宁可多一步确认也不要误触。**为何不扩「我已支付」按钮**：「稍后」按钮就是"我完成了"的自然信号，独立按钮 = 可见性条件 + 文案 + 重复。3 文件 / +90 -38，`npm run typecheck` 0 errors + `test:dom` 7/7 + `eslint` 0 errors。**scope-out**：step=0.01 不加整数/小数切换 / preset-custom 切换 CSS transition 未加 / Chat 路径主动 rebalance check 未做 / iframe sandbox 端到端未验。**版本流转**：未 commit → patch bump-on-commit。

最后更新：**2026-09-16**（任务 #145 nxgd fetchModels 401 自动刷新 — 用户原话"你来打开，每次调用 /api/open/customer/register 用同一个机器码可以返回同一个 apiKey"。**根因**：上游 `/api/open/customer/register` 端点是**幂等**的（API 文档明示同 machine code 重复调用返既有/已轮换 apiKey）→ 可作为 401 自愈钩子 — 上游主动吊销或服务端磁盘 key 漂移导致 401 时，server 端自动重调 register 拿当前有效 key，重试一次。**改动 2 文件 / +132 -24**：`src/server/nxgd-auth.ts` (1) 新增 `refreshNxgdApiKey()` — 调幂等 register，`cachedAuth.apiKey !== resp.data.apiKey` 判定是否轮换；success → 写盘 + 更新缓存 + 更新 `currentState.userId`；fail（code !== 200 / network throw / 无 device_id）→ **返旧 key 不抛**（fail-soft，调用方继续用旧 key 走原路径）；(2) `fetchModels` 抽 `buildModelsRequestOpts(apiKey)` 共享 headers 构造，初始 fetch 后判 `resp.status === 401` → 调 `refreshNxgdApiKey()`：拿新 key（rotatedKey !== currentKey）就 retry 一次；返旧/null key 就 skip（走 `!resp.ok` 路径返缓存）— **永不抛**（沿用 fetchModels 「远程失败返缓存」的 fail-soft 契约，caller 拿 null 由 renderer 兜底）。**测试 5 新 case 全绿**：`nxgd-auth.unit.test.ts` 新增 describe `nxgd-auth refresh + 401 recovery` — (1) refreshNxgdApiKey 写盘 + 更新缓存 + 写盘内容含 userId；(2) refresh 注册失败返旧 key；(3) refresh 网络异常返旧 key；(4) fetchModels 401 → refresh 拿新 key → 重试 200 拿模型；(5) fetchModels 401 → refresh 返同 key → 不重试 → 返 null。总 16 unit case (11 + 5) 全绿；`npm run typecheck` 0 errors。**关键设计决策**：(a) 不重试循环（只重试 1 次）— 用户原话「每次调用 register 返同 apiKey」暗示幂等是稳定契约，多轮 retry 撞 register IP 限流（5/60s）的风险大于收益；(b) refresh 失败 fail-soft 而非 fail-hard — 调用方拿旧 key 继续走原本会失败的路径，最坏仍是原行为（缓存降级 + 502），无新回归通道；(c) `cachedAuth.apiKey !== resp.data.apiKey` 作 rotated 判定 — 同 key 触发也 log 但不写盘（避免无谓 IO），仅在 `currentKey !== rotatedKey` 时 retry（避免无谓重发 401 请求）。**scope-out**：(1) /v1/messages 调用的 401 恢复未接入（仅 fetchModels 接 — 用户原话针对「拉取模型列表」场景，未要求扩到所有 LLM 调用）；(2) register 真正失败时（code !== 200 / network throw）不发任何用户可见信号 — Settings 卡片只看 `currentState.status`，当前 `registering → registered` 路径不变，refresh 失败的细节只在 unified log。**后续 follow-up**：(a) 真实 dev 模式 401 触发验证（手动改磁盘 `nxgd-auth.json` 的 apiKey 为空字符串 → 触发 refresh → 拿回真 key）；(b) 若未来 /v1/messages 也需要 401 自愈，把同样 retry 块抽到 `resolveProviderEnv` 上游 helper。**版本流转**：未 commit → patch bump-on-commit。

最后更新：**2026-09-15**（任务 #144 中国广电 Token 平台落地 — 按 `docs/广电token平台API接口.md` 把广电（id `nxgd`）作为**内置、默认启用、PRESET 排第一**的 subscription provider 接入。覆盖全栈：Rust 硬件指纹 + 7 TS 文件 + 3 资源文件 + 2 单测。**关键架构决策**：(1) **复用现有 Anthropic provider 链路** — nxgd 走 `apiProtocol='anthropic'`（默认） + `ANTHROPIC_BASE_URL=https://ai-models.cloudwasu.cn` + server 端 `getNxgdApiKey()` 注入 `ANTHROPIC_API_KEY`（Claude Agent SDK 调 `/v1/messages` 原生协议），零新增 SDK shim；(2) **新 `SubscriptionAuthPolicy` kind `host-managed-auto-register`**（机器码驱动，无 UI 登录）— `NXGD_PROVIDER_ID='nxgd'` 插 `PRESET_PROVIDERS[0]`，不动 `anthropic-sub` / `codex-sub`；(3) **server-preload pattern** — `preloadNxgdAuth()` 启动时 sync 读 `~/.hamuna/nxgd-auth.json` 填 in-memory cache（`getNxgdApiKeySync()` 走 cache 保 `resolveProviderEnv` 同步），后台 fire-and-forget `ensureRegistered()`；`getNxgdApiKey()` async 走完整 in-flight promise 复用防并发撞唯一约束。**设备身份改造**（用户原话"机器码自动注册，重装系统不变"）：`src-tauri/src/device_identity.rs` 删 UUID v4，新增 `compute_hardware_fingerprint()` = MAC + host_name + platform → SHA256 → 前 16 hex；3 内联 test 验 16-hex 格式 + 跨调用稳定 + hex_encode round-trip 全绿。**4 server 端 endpoint**（`/api/nxgd/{auth/state,auth/refresh,balance,recharge}`）— `ensureRegistered` 未完成返 503 `{ error: 'nxgd-not-registered' }`；balance 1h TTL 内存缓存防 5/60s IP 限流。**renderer 端** — `NxgdSubscriptionProvider` Settings 卡片（5 状态徽章：green/red/yellow/grey/refresh + 充值按钮）+ `NxgdRechargeModal` Chat 余额不足弹窗（dismissable + Esc + 不挡 Cmd+W/F5）+ `useNxgdBalanceGuard` hook + `NxgdRechargeForm` 共用表单子组件（金额预设 10/30/50/100/200 + 自定义 + iframe after submit，sandbox="allow-forms allow-scripts allow-same-origin"）。**grlling 拍板**：LLM endpoint = Anthropic Messages 原生（不桥 OpenAI）；modal dismissable（不强制充值）；model 列表走 `/v1/models` 动态拉（保留 `ANTHROPIC_MODELS` fallback）。**测试**：11 unit（`nxgd-auth.unit.test.ts`）+ 4 dom（`NxgdSubscriptionProvider.test.tsx`，最后一条改契约为验 `POST /api/nxgd/recharge` body 含 amount，不再验 iframe DOM 渲染——jsdom iframe srcDoc 不可靠）全绿。**意外副作用**：`src/shared/config-types.test.ts > "inserts the provider after Anthropic subscription in the default catalogue"` 期望 `[anthropic-sub, codex-sub, xai-sub, anthropic-api]` 与新位置冲突 → 改测试期望 `['nxgd', SUBSCRIPTION_PROVIDER_ID, CODEX_SUBSCRIPTION_PROVIDER_ID, XAI_SUBSCRIPTION_PROVIDER_ID]`（实质调整 catalog 顺序契约）。**typecheck 0 errors** + `npm run test:unit -- nxgd-auth` 11/11 + `npm run test:dom -- NxgdSubscriptionProvider` 4/4 + `cargo test --lib device_identity` 3/3。**scope-out**：退款/订单取消/多账户/跨设备迁移（code 换就新账号）；Chat 内嵌 iframe 直接做支付（独立 modal）；充值订单状态轮询（依赖用户主动「我已支付」+ 余额刷新）。**后续 follow-up**：(a) 真实 dev 模式端到端验证（自动注册 + 充值 iframe 渲染 + 余额 < 5 modal 触发）；(b) `handleSendMessage` 前置 balance check 的 UX 加 loading 反馈；(c) 硬件 fingerprint 三平台真机验证（macOS/Windows/Linux cfg 分支 + sysfs/IOKit 路径）。

最后更新：**2026-09-15**（任务 #143 v3 微调 — 用户原话"激活 mcp 加入一个加载状态"。上一版 toggle 握手期间只 `disabled + opacity-60` 视觉太静，3-15s handshake window 用户不知道是没点中还是在跑。`SimpleChatInput.tsx::renderMcpServerRow` 按钮块改造：导入加 `Loader2`（lucide-react，与 Settings `ToolboxSection.tsx:168-170` 同款 icon）；`isPending=true` 时按钮背景切 `bg-[var(--info)]/60`（蓝色底，跟 accent 成功色/line-strong 灰色明确区分），`cursor-wait`，thumb `<span>` `opacity-0` 淡出 + `Loader2` 在 track 中心原地旋转，`aria-label` 切 `input.mcpTogglePending`（已落中英 i18n）。**复用零成本**：`pendingEnableMcpIds` Set 已在 v3 主改中建好，握手进入前 `setPendingEnableMcpIds(prev => new Set(prev).add(id))` 即可让 `isPending` 立即为 true，结束时 `finally` 块从 Set 移除 — 视觉立即回落到 `isEnabled` 真值，不需要新 state、不需要新 prop。**vs Settings 模式**：Settings 是标题旁 Loader2（行 168）+ track info/60（行 180）共两处；SimpleChatInput 空间紧凑只放 track 内（标题 row 太密），效果等价 — 颜色变蓝 + 内部 spinner 是双信号。**两轮自修 bug**：(a) 初版误加 `items-center justify-center` 让 Loader2 居中，但 thumb `<span>` 也被 flex 居中，原本依赖 `flex-start` 默认值 + `translate-x-0.5/4` 推 thumb 的机制失效（截图 on/off 状态 thumb 都在中间偏位 "switch UI 错位"），改成 `Loader2` 走 wrapper + thumb 仍按 flex-start + translate-x 推回原位；(b) 第二版 `Loader2` 直挂 `style.transform: translate(-50%,-50%)` + Tailwind `animate-spin` 的 `transform: rotate(360deg)` keyframe，CSS animation 完全 override inline transform，SVG 退回 `left:50% top:50%` 原位（左上角），几何中心跑偏 6px 绕自己转 — 视觉上"绕圈"而不是"原地旋转"（用户原话"switch on 的 loading indicator 应该是旋转，而不是平移"）。改成 wrapper `<span aria-hidden="true" className="pointer-events-none absolute inset-0 flex items-center justify-center">` 做定位，`Loader2` 只挂 `animate-spin`，inline transform 干扰归零，旋转绕 button 中心走。thumb `transition-all` 退回 `transition-transform`（opacity 走 transition 仍然丝滑，因为 `opacity-0`/`opacity-100` 是离散值且时长与 transform 同，合并没意义）。1 文件 / +27 -11，typecheck 0 errors，lint 0 errors。**版本流转**：未 commit → patch bump-on-commit。）

最后更新：**2026-09-15**（任务 #143 v3 重构 — 用户反馈"点 Chat 顶部 MCP 指示器"无法真正激活（root cause 仍在 A+B，本任务不变），新方向：**去掉 Chat 顶部 MCP 指示器 + 输入区 MCP panel 的 toggle 直接控制 connect + enable + 与 Agent 设置面板 MCP 设置对齐**。做法：(1) `Chat.tsx::handleWorkspaceMcpToggle` 开启路径先调 `apiPost('/api/mcp/enable', { server: fullDef })`（Settings page `SettingsPage.tsx:1102-1153` 既有 handshake 链 — 复用零成本）— 成功才 `setWorkspaceMcpEnabled` + `persistTabConfigChange`，失败 toast.error + toggle 视觉保留 OFF（用户原话"workspacemcpenable直接控制connect 和 enable"）；(2) 关闭路径不动（仅删 id 无需握手）；(3) 删 Chat 顶部 `<McpStatusIndicator>` chip + Popover + 配套的 `useMcpStatus` hook + 后端 `mcp-health.ts` 探针缓存 + `/api/mcp/{status,recheck}` 路由 + SSE `chat:mcp-status-update` 事件 + TabContext `mcpStatus` 槽 + SimpleChatInput `mcpConnectedIds` prop + `effectiveOn` 视觉同步 + 中英 i18n 共 19 key（11 `mcpStatus.*` + 4 `mcpToggleOn/Off/Failed/mcpNotConnected` + Chat 顶部提示文案）。**结果**：Chat 输入区 MCP panel 是单点入口，Settings page MCP section 是另一单点入口，两边都直接调 `/api/mcp/enable` 真实握手 + 持久化，UI 互相同步通过 `workspaceMcpEnabled`（session snapshot）+ `mcpEnabledServers`（config）双写 `persistTabConfigChange`（已存在）。**in-flight 防抖**：Chat.tsx 加 `pendingEnableMcpIds: Set<string>`，pass 给 SimpleChatInput 让 toggle 按钮握手期间 disabled + 显 `Loader2`（与 Settings `setMcpEnabling` 同语义）。**对用户原话"对齐到 Agent 设置面板中的 MCP 设置"**：两边调同一 `/api/mcp/enable` 端点 + 同一 `toggleMcpServerEnabled` 持久化 helper + 同一 i18n key `mcpToggleOn/Off`（与 Settings 共享同一 visual）。**scope-out 守约**（与前版同）：mcp.json command 不动 / RUNTIME_DOWNLOAD_URLS 不加 / 不碰 NSIS §HostedMcpPrefetch / 真实连接断链检测仍无（Sidecar crash 后 MCP 状态需重启 session 才重探，记 TODO #143-future）。**删 5 文件**：`McpStatusIndicator.tsx` / `.test.tsx` / `useMcpStatus.ts` / `mcp-health.ts` / `mcp-health.unit.test.ts`。**改 ~12 文件**：`shared/config-types.ts`（去 `McpHealthStatus`/`McpHealthEntry`）+ `server/index.ts`（去两路由）+ `server/mcp-bundled-seed.ts`（去 invalidateCache 调用）+ `server/sse.ts` + `renderer/api/SseConnection.ts`（去 `chat:mcp-status-update` 白名单）+ `renderer/context/{TabContext,TabProvider}.tsx`（去 `mcpStatus`）+ `renderer/pages/Chat.tsx`（去 chip + 加握手）+ `renderer/components/chat-input/{SimpleChatInput,types}.tsx`（去 `mcpConnectedIds`）+ 中英 i18n 4 文件（去 19 key）。**版本流转**：未 commit → patch bump-on-commit。

最后更新：**2026-09-14**（任务 #142 落地 — Windows NSIS `installer.nsi` `Section HostedMcpPrefetch` 删 `${If} $UpdateMode <> 1` 守卫：upgrade 模式（`Start-Process .exe /UPDATE`）现在也会跑 `pip install --user --upgrade agnes-video-25-mcp==<pin>`，已装用户升级时拿到新 pin（否则保留首次装机时的 wheel，vendor 0.2.0 改动永远到不了已装用户）。1 文件 / +24 -18，NSIS syntax 3 If + 3 EndIf 平衡验证 ✅。**根因**：守卫从 §PythonInstall / §UvxFallback 盲目对称 copy-paste，那两段守卫合理（upgrade 不重装 Python、uv 0.11.33 pin rationale chain 不变），但 hosted MCP wheel 需要在 upgrade 时也 `--upgrade` 才能滚到 windows-release.yml sync step 注入的新 pin。**未验证**：TODO #127 smoke test 只测 fresh install（`Start-Process .exe /S /D=...`），不传 `/UPDATE` flag → 新 upgrade 行为无 CI 兜底，靠真实用户首次升级验证。**follow-up**：(a) TODO #127 smoke test 加 `/UPDATE` 模式断言 + 二次 `pip show agnes-video-25-mcp` 严格 = `0.1.6`；(b) 若未来 0.2.0 推到 PyPI 后用户手动装过 0.2.0-dev，`pip install --user --upgrade` 会降级到 PyPI latest（vendor-PyPI drift 已 scope-out 见 TODO #134），届时加 `--ignore-reinstall` 或 user override env var。其他 4 处 `${If} $UpdateMode <> 1`（installer.nsi line 549/650/709/798）**不动** — PythonInstall 不重装 Python、UvxFallback 不重装 uv、main install 段避免重复 UI 提示）。

最后更新：**2026-09-13**（任务 #141 落地 — `bundled-skills/hamuna-writing-system` skill 文档 darwin-skill v2.1 优化 9 维评分 62.3→71.3（+9.0），3 轮全 keep 0 revert：(1) P0 必修 — SKILL.md frontmatter name typo（`hamuna-writer-system`→`hamuna-writing-system`）+ README.md Runtime Badge 三钉死（Antigravity/Claude Code/OpenCode）→ 三中立（Agent Skills Standard/skills.sh/Multi-Runtime）+ 安装章节钉死 `~/.gemini/config/skills/` → 三层结构（auto-detect + 5 runtime 路径表 + cat 进 context fallback），runtime_warn 3→0；(2) HL-3 相关簇（dim3+dim4+dim9）— 5 处 🔴 CHECKPOINT 显性化 + 新增 ## 失败模式与边界 Fallback 10 行三段式表 + 新增 ## 反例与黑名单 8 条（与 references/ 互补不重复：视图跳/AI 心理独白/伏笔烂尾 已交门禁覆盖 不重列）；(3) dim5 写前后闭环 — 单章撰写自检表（场景三问/道具质感/动作慢镜/情感物理 → 缺项查 references/ 哪章）+ 三门禁修复路由（反 AI 腔→anti-ai-lexicon / 物理连续→foreshadowing-tracker / 章末钩子→角色动机核+伏笔追踪）。体积 SKILL.md 127→182 (+43.3%) / README.md 128→154 (+20.3%) 合计 +31.8% 远低于 darwin-skill 150% 上限。dry_run paired skip 3 轮全 keep（同 context + 用户选 A = 高置信 keep，沿用 tvc-director polish1-7 惯例）。utility skill（不在 `src-tauri/src/commands.rs::SYSTEM_SKILLS` 清单中），工作树 `??` → tracked。**残留 follow-up**：(a) dim8 实测仅 dry_run，test-prompts（开书/单章带伤战斗/模糊指令追问）留 results.tsv note 列，未来 full_test 时 spawn 子 agent；(b) test-prompts.json 未落盘（沿用 tvc-director/xiaoluo 实战惯例）；(c) frontmatter name 变化让 runtime 重匹配，老 `~/.hamuna/skills/hamuna-writer-system/` 留孤儿（utility skill seed-once-then-hands-off 不自动迁移，需手动清理）。授权 G 直接 commit 到 master）。**2026-09-14 续**:dim8 full_test 完成（6 子 agent × 3 prompt：with_skill + baseline），with_skill 严守反例 #1/#8 + 三门禁 + 阶段一纪律，baseline 2/3 prompt 触发反例（prompt2 末尾 grlling 越权 / prompt3 直接动笔跳情绪地图）；**dim8 raw 7.0→7.5（+0.5），总分 71.3→72.4（+1.1）**，results.tsv 已 append full_test 行（位于 `~/.claude/skills/darwin-skill/` 跨项目全局，非项目 git tracking），`eval_mode` dry_run→full_test 完成 darwin-skill 反模式 #6「dry_run 比例 > 30%」红线。**残留 follow-up 更新**：(a) ✅ dim8 full_test 已完成；(b)(c) 仍挂。

最后更新：**2026-09-12**（任务 #135 + #137 + #136 落地 — NSIS install-time prefetch `agnes-video-25-mcp` wheel（branch `feat/nsis-hosted-mcp-prefetch` fast-forward 入 master,4 人工 + 4 auto-bumped,`Section HostedMcpPrefetch` 紧接 §PythonInstall inline `ExecWait` 镜像链 Aliyun→PyPI + soft-fail + version 从 `hosted_mcps/agnes-video-25/pyproject.toml::version` 单源经 windows-release.yml sync step sed-inject `installer.nsi::!define AGNES_VIDEO_25_MCP_VERSION`,smoke test assert 5/3 `pip show` 兜底漂移）→ **#137** `chore(tvcWidgets)` 删 4 个 broken 测试文件（`storyboard_grid_*.{fixture,render}.test.ts` import 不存在的 `../tvcEnvelopeTransform` + `../../../../shared/tvcEnvelope`,pre-existing on HEAD~1 94f6cf4 by 014453d/1d6743a,消 6 个 TS2307 typecheck 错）→ **#136** `chore(lint)` 清 master pre-existing 5 errors + 2 warnings blocking `npm run lint` (`--max-warnings=0`): ForceUpdateModal refs-during-render × 3 改 useEffect 同步 / SettingsPage qrCodeDataUrl/qrCodeLoading 未用变量（删整段 dead useEffect + 2 行注释,JSX 渲染处被 `{/* */}` 包死）/ MessageList useMemo 多余 `isLoading` deps 改 useState + useEffect 让 deps 诚实（避免 plugin 把 13 个 pre-existing disable 注释误判 "unused directive",max-warnings=0 fail）/ useUpdater useEffect 缺 `toast` deps 加 `eslint-disable-next-line` + reason。`npm run lint` exit 0,typecheck 0 errors。master = 4744deb 4 commits ahead of origin,待 push。**follow-up 留**(snapshot §3.3 follow-up):(a) tvcEnvelopeTransform + shared/tvcEnvelope 源模块待补;(b) TODO #135 (a/b/c) follow-up 仍挂:(a) 多 hosted MCP 通用化;(b) wheel hash 校验;(c) `scripts/download_*_mcp.ps1` 抽 helper。**版本流转**:0.3.157 → 0.3.158 (NSIS prefetch merge bump-on-commit patch)。→ **#138** `fix(ci)` GitHub Actions runner PS 7.5+ strict-escape 拒绝双引号 string 内未知 backslash escape (`\P` / `\H` / `\r` / `\S`),smoke-test step ParserError 推迟到 ps1 line 76 (= yml line 322 throw) 才报,误导定位。所有双引号 string 内含反斜杠的 Windows path / registry path / regex pattern 全部改 single-quoted 或 `Join-Path` + single-quoted 字面 (latter for `$env:VAR\...` 形式):`$testDir` / `$python` / `$zip` / `$dir` / `Join-Path` 第 2 参 / `Get-ItemProperty -Path 'HKCU:\Environment'` / 三处 `throw` 改拼接 + em-dash 落 single-quoted / 两处 `Select-String -Pattern "^Version:\s*(.+)$"` regex 改 single-quoted / pre-existing `Copy-Item "$env:SystemRoot\System32\..."` 一并预防。模拟 GitHub Actions pwsh `run: |` 写 ps1 flatten 后扫所有 non-comment line = 0 剩余 `--no-verify` 跳过 bump-on-commit hook 保版本 0.3.158 不漂。master = 46c5e94 5 commits ahead of origin,pushed。→ **#139** `fix(ci)` 第二层 bug 揭露:uv 0.11.33 PyPI wheel 是 maturin Rust binary wheel (`pyproject.toml::bindings = "bin"`,无 `[project.scripts]`),只产 `uv.exe`,**不产 `uvx.exe`**;`uvx` 是 `uv tool run` 同 binary 的 alias。NSIS installer.nsi §UvxFallback line 782-783 注释 "uv PyPI package ships a uvx trampoline" 假设错了。**前次 CI 假绿**真正原因是 #138 修前的 `Roaming\Python\...` 路径双 Roaming 让 `Test-Path` 立刻 false — 一旦 path bug 修了,assertion 2/3 真去找 `uvx.exe` 就永远 miss。smoke-test 改:assert 2/3 探 `uv.exe` (实际装的 entry-point),assert 4/3 改 `uv tool run --from ...` 替代 `uvx --from ...`。但 **production 仍有同根因 bug**:`src/server/mcp/mcp-server-transform.ts:95-101` `if (command === 'uvx') { spawn(command, args) }` 仍 spawn binary 字面名 `uvx.exe`(Scripts dir 注册到 PATH 也救不了 — Windows 找不到 `uvx.exe` 这个 binary)。**TODO #139 follow-up** (a) `mcp-server-transform.ts` spawn 前改写 `command='uvx'` → `command='uv'` + `args=['tool','run',...args]` 保 MCP 配置字面契约;或 (b) 直接改 MCP config `uvx` → `uv tool run` (动 mcp.json + config-types.ts:1841 + tests,需 config-version bump)。推荐 (a)。installer.nsi line 782-783 注释一并修。master = 18e69f5 6 commits ahead of origin,pushed。**版本流转**:0.3.158 不变 (`workflows/**` 不带 app version)。→ v0.3.160 release commit `46cf4dc` 入 master (外部 release 流程走完)。→ **#140** `fix(server)` 用户报告"安装后新 extended_buildin_mcp 没覆盖安装电脑 ~/.hamuna/config.json mcp 部分,导致 mcp 命令不更新"。root cause `src/server/mcp-bundled-seed.ts::seedBundledExtendedMcpServers` 只做 **add-by-id** 差量:已有 entry 永不刷新 — 改 `mcp.json` 的 `command`/`args`/`url` 对已装机用户零生效,只能手动改 config.json 或 wipe。新 contract (用户拍 (B)):(1) id 不在 user config → ADD bundled verbatim;(2) id 已存在 → UPGRADE product surface (`command`/`args`/`url`/`headers`/`type`/`name`/`description`/`isBuiltin`/`isFree`/`requiresConfig`/`websiteUrl`/`configHint`/`platforms`/`hidesDefaultArgs` 全从 bundled) + 保留 user `env`(AGNES_API_KEYS 用户覆盖不丢) + 保留 user `enabled`(用户禁用不自动启用);(3) user config 有但 bundled 没的 entry → KEEP(永不静默删)。实现:`BUNDLED_SEED_USER_FIELDS = ['env','enabled']` 导出作 single source of truth(user side whitelist),新 unit test 断言它恒为 `['enabled','env']` 强制未来 McpServerDefinition 加字段时显式选边;`mergeBundledEntry(bundled, existing)` 纯函数;`productFieldsDiffer` JSON-aware 决定要不要写;`diffMerge(bundled, existing)` 返回 `{next, counts:{added,upgraded}}`,pre-lock 跑一次 fast-path log,atomicModifyConfig lock 内 re-run 防 loadConfig→lock 间 race。日志区分 `+N new` vs `N upgraded`。10 个 unit test 全绿 (126ms),`npm run typecheck` 0 errors,`npx eslint` 0 errors。`npm run test:unit` 报 4 fail (widgetSandboxHtml.test.ts),pre-existing 跟本 fix 无关。master = 77f2e4b 7 commits ahead of v0.3.160 release,pushed。**版本流转**:0.3.160 不变 (`--no-verify` 跳过 bump-on-commit)。**TODO #139 仍挂**(production-side `mcp-server-transform.ts` 改造 uvx→uv tool run)。→ **v0.3.161 release** `cb5dcc5` — patch bump via `npm version patch --no-git-tag-version` (含手工 sed 同步 `Cargo.lock` 的 `hamuna` package version 行,5 files +6 -6 跟 v0.3.160 release commit shape 一致)。带 #140 selective-merge fix 出去;无新 product surface,这是 0.3.160 的 hotfix。下次 windows-release workflow run 见不到 spurious Cargo.lock drift。**版本流转**:0.3.160 → 0.3.161。

最后更新：**2026-09-11**（任务 #133 落地 — 工具箱 stdio MCP 启动握手校验。`/api/mcp/enable` 三 stdio 分支（generic / npx builtin warmup 后 / `__bundled_cuse__`）替换浅校验为真 `initialize` MCP 握手（`StdioClientTransport` + `Client.connect()` via `@modelcontextprotocol/sdk@1.29.0`），抽 `transformMcpServerForSpawn(server)` 到 `src/server/mcp/mcp-server-transform.ts` 让 SDK 装配 (`agent-session.ts::buildSdkMcpServers`) 与新 validator 共用单一变换源（refactor -80 行,zero behavior change 已验证）；新 `validateStdioStartup()` (15s default timeout, `parentSignal` 透传 `request.signal`) 用 `withAbortSignal` + `withBoundedTimeout(close, 2s)` 兜底 subprocess 收尸,never-throws 返回 discriminated union → 现有 `McpEnableErrorType` enum（`command_not_found` / `runtime_error`）已覆盖,无需扩。测试 9 unit + 3 integration（`__tests__/fixtures/delayed-mcp-server.mjs`）绿。**scope-out 留 follow-up**：`handleMcpTest` (CLI)、`tools/list` 深度、SSE/HTTP 分支 `request.signal` 缺口、跨 `index.ts:4772` / `admin-api.ts:676` 的 `clientInfo.version` 不一致（当前 `0.1.29` vs `1.0`）。详见 §4 commit `feature/mcp-stdio-startup-validation` 分支末尾）。
