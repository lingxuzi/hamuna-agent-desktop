...
最后更新：**2026-09-18**（任务 #164 wizard transition 强化 — `/gsap-react` 触发；user 原始冲动是引入 GSAP 库加堆叠动效，grlling 拒了：DESIGN-LANGUAGE.md:48 / DESIGN-HEURISTICS.md:26 红线「≤200ms / 极克制 / 无弹跳 / prefers-reduced-motion 强制」+ wizard 头注释「不堆砌多动效」。**用户拍板 A. 强化现有 transition；B. CSS keyframes + Tailwind（拒绝 GSAP 依赖）**。**改动 3 文件**：(1) `src/renderer/index.css` 新增 `@keyframes wizard-step-in`（opacity 0→1 + scale 0.98→1，比 `popoverIn` 的 0.95 更克制，匹配 wizard 卡片视觉权重；不放 translateY 因为 step body 已浮在卡片内部）+ 5 行 rationale 注释。(2) `NxgdOnboardingWizard.tsx` step body 加 `motion-reduce:animate-none animate-[wizard-step-in_180ms_cubic-bezier(0.25,1,0.5,1)_both]`（`both` fill mode 防 mount 闪烁；`cubic-bezier(0.25,1,0.5,1)` = DESIGN 红线规定的 ease-out-quart；`key={step}` 已存在 — React remount 触发 keyframe 重放）；expanded 卡片外层复用既有 `@keyframes popoverIn`（项目已有，零新增）—— 184 行 className 加 `motion-reduce:animate-none animate-[popoverIn_180ms_cubic-bezier(0.25,1,0.5,1)_both]`；加 `data-testid="nxgd-wizard-step-body"` 给 step body 测试断言用。(3) `NxgdOnboardingWizard.test.tsx` 新增 `describe('wizard animation')` 3 case：expanded 卡片含 popoverIn className / step body 含 wizard-step-in className + step 切换后仍存在 / pill 态不含 animate-*（保留低优先通知语义）。**grlling — pill 不加动画**：pill 是低优先通知元素，加动画抢注意力破坏语义；expanded pop 已足够传达"展开"。**scope-out**：(a) 不引入 GSAP — 项目无依赖 + Tailwind v4 CSS-driven 已有标准 `animate-[name_Xms_easing]` 任意值 pattern（5+ 组件用此 pattern：ModelManagementPanel / CronTaskDetailPanel / TaskCenterOverlay / Toast / MessageList）；(b) 不动 wizard 头注释「不堆砌多动效」 — 本次强化仍属单节奏（step body + expanded 两个节点，< 200ms）；(c) 不抽 utility helper — 1 keyframe + 2 className，复用率 < 3，CLAUDE.md「代码块 >3 引用才抽象」直接拦；(d) 不加 prefers-reduced-motion 全局 kill switch — 项目约定每组件局部 opt-out（grep 5+ 处 `motion-reduce:`）；(e) 不改 card hover / CTA hover / ProgressDots / BalanceProgress — 已合规不动；(f) 不改 useCloseLayer / OverlayBackdrop — 已自管；(g) 不改 key={step} — React remount 触发 keyframe 是正确选择；(h) 不动 closeAndMarkSetup / onPinModel / discoverNxgdModels 路径。**验证**（全绿）：`npm run typecheck` 0 errors；`npx vitest run --project dom src/renderer/components/NxgdOnboardingWizard.test.tsx` 24/24（21 旧 + 3 新）；pre-existing 5 failed tests 不增不减（与本 fix 无关）。
