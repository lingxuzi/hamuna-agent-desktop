...
最后更新：**2026-09-10**（任务 #123 落地 — Windows 用户报"拷到 `~/.hamuna/bin/` 后 spawn uvx 仍报 not found"根因：`c845bec` 的 `seedBundledUvToHamunaBin()` 把 uvx 拷到 `~/.hamuna/bin/`，但 `agent-session.ts::spawn MCP uvx` 时只 prepend `getBundledUvPath()` 的 dirname（即 install-dir），**完全不读 `~/.hamuna/bin/`** —— 拷过去的 uvx 是死代码。修法：扩 `runtime.ts::getBundledUvPath()` 探测链加 slot 3 = `~/.hamuna/bin/uvx.exe`，让 spawn 路径真正发现 seed 拷过去的 copy。Task #122 uv 0.5.11→0.11.33 重 pin 已 commit `0b3a75d`）。
...
#### TODO #123 — getBundledUvPath 探测链补 ~/.hamuna/bin/ uvx.exe（待 commit）🔄
**触发**：用户报"`c845bec` commit 后装机，拷贝 uvx 到 `~/.hamuna/bin/` 仍报 uvx not found" —— grlling 后定位死代码根因：`mcp-bundled-seed.ts::seedBundledUvToHamunaBin()` 把 uvx.exe 拷到 `~/.hamuna/bin/`，但 `agent-session.ts:3635-3641` spawn MCP `command: 'uvx'` 时只 prepend `getBundledUvPath()` 的 dirname（即 install-dir `resources/`），**完全不读 `~/.hamuna/bin/`** —— 拷过去的 uvx 是死代码，从未被任何 spawn 进程发现。**额外矛盾**：`mcp-bundled-seed.ts:116` 注释说"`system_binary.rs::augmented_path`"会注入 `~/.hamuna/bin/` 到 PATH，但 `augmented_path()` 只在用户开新 shell 时生效，Sidecar 是 Node 进程直接读 `process.env.PATH`，跟 `augmented_path()` 无关。
**用户拍板 B 方案**：扩 `runtime.ts::getBundledUvPath()` 探测链加 slot 3 = `~/.hamuna/bin/uvx.exe`。3 caller 都接受：`agent-session.ts` MCP spawn（命中后 prepend `~/.hamuna/bin/` 到 PATH）/ `mcp-bundled-seed.ts` seed copy 源（已有 fallback）/ `index.ts:9685` 无显式使用（注释）。**2 文件改动**：(1) `src/server/utils/runtime.ts` +38/-7（import `homedir from 'node:os'` + 新增 `getUserHomeBinUvxPath()` 辅助 + 探测链加 slot 3 + JSDoc 重写 probe order 表 4 步）；(2) `src/server/utils/runtime.unit.test.ts` NEW 88 行 / 5/5 通过（覆盖：no-staged null / 用户 bin fallback / install-dir vs user dir 优先级 / linux null / darwin null）。**关键架构决策**：(1) **slot 3 在 dev walk-up（slot 4）之前**——prod install 命中 slot 1/2，未命中 → slot 3 user dir（mcp-bundled-seed 拷过去的）→ slot 4 dev walk-up；(2) **`getUserHomeBinUvxPath()` 不 memoize**——`homedir()` Node 内部 cache，函数调用是 nanosecond 级，不需 memoize；(3) **不强制 `seedBundledUvToHamunaBin()` 必须在 spawn 之前跑完**——startup hook `index.ts:9679+9686` 已经串入顺序，spawn 时命中 slot 3 是 startup seed 的自然产物；(4) **不退 slot 1/2**——install-dir 仍 prefer，避免 user dir 拷贝陈旧时仍被优先。**验证**：`runtime.unit.test.ts` 5/5 + `mcp-bundled-seed.unit.test.ts` 11/11 = 16/16 通过；typecheck 0 新错误。**待 user 拍板 commit**。
