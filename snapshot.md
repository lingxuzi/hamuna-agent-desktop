...
最后更新：**2026-09-18**（任务 #156 bundled-skills/ 作为 SYSTEM_SKILLS 单一权威源（auto-derive） — 用户原话"SYSTEM_SKILLS 在编译时自动检测 bundled-skills 目录中 skills，对齐列表"。**根因链路**：任务 #155 commit `568b704` orphan cleanup 落地但没生效 —— `bundled-skills/creative-video-suite/` 已删但 Rust `commands.rs::SYSTEM_SKILLS` + Node `index.ts::SYSTEM_SKILLS` 两份手写常量仍含 `creative-video-suite`，orphan cleanup pass 故意跳过 `SYSTEM_SKILLS` 集合 → 名字两边不出现 → 不删 → 用户机器 `~/.hamuna/skills/creative-video-suite/` 永远留着。**根治**：bundled-skills/ = 单一权威源，两份常量 build-time 自动 derive（人手不再维护，drift = by construction 不可达）。**新增 `scripts/generate-system-skills.mjs`** —— Node 脚本 `readdir(bundled-skills/)` 过滤有 `SKILL.md` 的目录，按字典序排序，同时 emit 两个文件：(a) `src-tauri/src/system_skills.generated.rs` (`pub const SYSTEM_SKILLS: &[&str] = &[ "a", "b", ... ]`)；(b) `src/shared/systemSkills.generated.ts` (`export const SYSTEM_SKILLS = [ 'a', 'b', ... ] as const` + `type SystemSkillName = typeof SYSTEM_SKILLS[number]`) —— 一份脚本两份产物 = cross-language parity by construction。**Rust include! macro 替换手写 const**：`commands.rs` 删 57 行 `pub const SYSTEM_SKILLS` block，改 `include!("system_skills.generated.rs")` —— 仍是真 `const &[&str]`（保 `.iter().copied()` / `for x in` / `includes()` 兼容性），callsite 零改动。**TS re-export 替换手写 const**：`src/shared/systemSkills.ts` 加 `export { SYSTEM_SKILLS, type SystemSkillName } from './systemSkills.generated';`（与已有 `SYSTEM_SKILLS_VERSION` / `REQUIRED_SYSTEM_SKILLS` 同模块对齐）；`src/server/index.ts` 删 49 行手写 const，改 `import { SYSTEM_SKILLS } from '../shared/systemSkills';` —— 4 处 `SYSTEM_SKILLS.includes(x)` callsite 因 `as const` 收紧为 literal union 类型 + `folder: string` widening 报 TS2345 错误，**修复** = `const SYSTEM_SKILLS_SET = new Set<string>(SYSTEM_SKILLS)` + 改 `.includes(x)` 为 `.has(x)`（`as readonly string[]` cast 实测不生效 —— literal type 是 still frozen）。**build 触发链**：(1) `package.json` 加 `prebuild:server` / `prebuild:web` / `pretest` / `prelint` npm hooks + `generate:system-skills` 入口；(2) `src-tauri/build.rs` 加 `cargo:rerun-if-changed=../bundled-skills` + `cargo:rerun-if-changed=../scripts/generate-system-skills.mjs` + 兜底 spawn `node ../scripts/generate-system-skills.mjs`（bare `cargo build` 不走 npm hooks 也能跑）。**`verify-system-skills-sync.mjs` 改造**：Check 1（Rust ↔ Node parity）**删除**（同源生成，tautology）；Check 2（bundled-skills/ ↔ generated sync）**重写**为「filesystem ↔ 两份生成文件双向 sync」捕「改 generator 忘 regen」；新增 Check 9「生成文件存在」（防 skip `npm install` 导致生成器从未跑）。**`commands.rs::system_skills_tests::rust_and_node_system_skill_lists_match` 重写**为 `system_skills_matches_bundled_skills_with_skill_md`（filesystem invariant test）—— cross-language parity test 由 by-construction 取代，没必要测。**`bump-on-commit.mjs`**：删 L63-156 两个 creative-video-suite-specific bump 块（目录已不存在，dead code），保留 package.json + AGNES_MCP_VERSION auto-bump；顶部加 11 行 rationale 注释说明 SYSTEM_SKILLS_VERSION 仍是手动 bump（仅「SKILL.md 内容变更 → force-overwrite 所有 system skill」场景）。**`.gitignore`**：3 行加生成文件 entry (`src-tauri/src/system_skills.generated.rs` + `src/shared/systemSkills.generated.ts` + 头部注释)。**`SYSTEM_SKILLS_VERSION` 本次保持 `"55"` 不变** —— orphan cleanup 走 `.system-skills-snapshot.json` 增量机制不受 version 影响，下次任意 build 出来的 binary 启动后自动删用户机器 creative-video-suite（v55 snapshot 含 creative-video-suite + 当前 SYSTEM_SKILLS 不含 → orphan → 删），无需 force-overwrite 14 个 system skill。**scope-out**：(a) `REQUIRED_SYSTEM_SKILLS` 不动（业务策略 5 名字独立）；(b) `PLATFORM_BLOCKED_SKILLS` 不动（事件性 override）；(c) 5 处 `include_str!("../../bundled-skills/<name>/SKILL.md")`（commands.rs content-pinning）不动（与 list 派生正交）；(d) `mcp-bundled-seed.ts::seedBundledExtendedMcpServers` 不动（MCP 与 skills 判据不同，test 锁「不删 user-removed-from-bundle」）；(e) `tauri.conf.json::bundle.resources` 不改（bundled-skills/ 仍整体 copy）；(f) pre-existing v37 stale test 不修（CLAUDE.md "只改必须改的"）；(g) creative-video-suite §5.2 决策不动；(h) creative-video-suite §5.6 promote 决策不动（已 promote，但用户机器该目录硬删走 .system-skills-snapshot.json orphan cleanup 路径 = 本任务前置）。**验证**（全绿）：`npm run generate:system-skills` → 23 skills written（之前 24 含 creative-video-suite），creative-video-suite 在 `src/server/index.ts` / `src/` / `src-tauri/src/` / `src/shared/` grep = 0（orphan cleanup 无需 bump version 自动生效）；`npm run typecheck` 0 errors；`npm run test:unit` 3043 passed + 5 failed（4 pre-existing 跟本 fix 无关，详见 TODO #3）+ 5 skipped；`cargo test --lib commands::system_skills_tests::system_skills_matches_bundled_skills_with_skill_md` 1/1 pass；`node scripts/verify-system-skills-sync.mjs` 8/8 ok（23 bundled-skills ↔ 23 system ↔ 5 required all in lockstep）；`npm run build:server` OK（prebuild:server hook 跑 generator）；`cargo build --manifest-path src-tauri/Cargo.toml` Finished 48.30s；`grep -c creative-video-suite src-tauri/resources/server-dist.js` = 0。**版本流转**：未 commit → patch bump-on-commit。
...
#### TODO #163 — nxgd: 删除 ConfigProvider auto-discovery + wizard step 2 改 pin 1 model 🔄
