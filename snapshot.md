...
**触发**：用户报"用 `/home/hmcz/Projects/XiaoLuo-AI-Drama-Skill/小逻-视频拉片拆解.md` 规范分析 reference_videos, 优化 creative-video-suite 产品广告的创意方向，总结创意模板"——`reference_videos/` 已落 9 个抖音产品广告 mp4（34 MiB 总，详 §5.8 / TODO #36），需要把它们从"原始素材"提炼成"AI 可复用的创意模板"。**目标**：从 9 视频拉片 → 提炼 9 个互补且可复用的创意模板 → 写独立 reference 文件 cross-ref 现有 3 路 commercial ref。**1 文件落地（待 bump）**：`bundled-skills/creative-video-suite/references/commercial/creative-templates-from-9-references.md`（含 9 视频速览表 + 9 模板详细展开 + 跨模板共性 + 模板选择决策树）。**待用户拍板**：(a) **保持 cosmetic 不动**——只本地写文件，不 commit 不 bump；(b) **commit 不 bump**——只 commit 新文件，下次 creative-video-suite 改动才走 §5.6 已生效的自动 bump；(c) **commit + 手动 bump `SYSTEM_SKILLS_VERSION`**——同步触发老用户 force-overwrite；(d) **commit + 等下次 hook 自动 bump**——本 commit 不动版本号，下次 bundled-skills/creative-video-suite/ 任意文件改动时 bump-on-commit.mjs 自动 +1。
