...
#### TODO #116 — bump-on-commit.mjs: SKILL frontmatter version auto-bump（creative-video-suite scope） 🔄
（landed: `scripts/bump-on-commit.mjs` 新增 SKILL_VERSION auto-bump 段，触发条件 = staged `bundled-skills/creative-video-suite/` 任意非空 → bump `SKILL.md` frontmatter `version: "X"` → `"X+1"`；跳过条件 4 项：CI / wcSkill==headSkill / wcVer!=headVer / regex miss；scope 硬编码 creative-video-suite 1 个，因 `prompt-writer: 20260707` / `hamuna-strategy-v2: 20260818` 日期格式 + `agent-browser: 1` body 内非 frontmatter 不能复用同一 regex，**扩展前 MUST 先统一版本格式策略**；遗留：dry-run 测试 `node -e "import(...)"` 会真实跑 hook 污染 working tree，下次验证改用 `node --check` 或 `git diff --cached` 模拟 staged）
#### TODO #117 — creative-video-suite: 单 segment 视频生成前拆分资产清单 + required_assets 全 ready 硬门控 + recipe 三件套必填 🔄 — landed: 用户 brief 2 件事 → user mid-turn 澄清 scope = "单 segment 视频生成" → user **mid-task grlling 扩展 schema**：分镜生成 JSON 应包括 资产图片 + Prompt + mcp tool name + tool params（即 recipe 三件套）→ 3 文件改动：`output-conventions.md §2.2` 新增 `notes.video_segments[].required_assets[]` schema 10 字段（6 基础 + 4 新：`generation_prompt` 中文 prompt / `mcp_tool_name` 6 选 1 multimedia-creator 三件套 / `tool_params` JSON 化 params / `generated_attempt` 0-2 重试计数）+ `§5.1` 4 路 asset 来源决策表；`mcp-usage-guide.md §3.4` pre-flight 加第 4 项 required_assets 全 ready 检查 + 第 5 项 recipe 三件套必填检查（缺失 → 停下问用户，**不**自动补避免 AI 自由发挥 prompt 漂移）+ post-call 回写 `asset_url_consumed[]`；`SKILL.md` 视频生成门禁段补 cross-link 含 recipe 必填项。**单一权威 = 嵌入 `notes.video_segments[]`** 避免双源；写时机 = frame 阶段末尾；recipe 三件套任意状态都必填（missing/pending 时记录"原计划怎么生成"，跨 session 续跑靠复读 recipe）；遗留：第 4 项未配 lint + §5.1 △ 行无自动判定 + `asset_url_consumed[]` append-only 无 compaction
