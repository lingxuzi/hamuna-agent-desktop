...
最后更新：**2026-09-16**（任务 #146 v4 nxgd 充值解析 payFormHtml + iframe 只读预览 — v3 提交后用户实操反馈"payFormHtml 没有渲染，现在还是自动跳转到浏览器了"。**真根因**：上游（Alipay 标准 PC 收银台）常在 payFormHtml 内**自带** `<script>form.submit()</script>`（spec §309-323 设计的"页面级嵌入"模式默认就靠这段），v3 删了我们自己注入的 script 没用，原生 form HTML 里的 script 一样跑。第二个问题：payFormHtml 只含 `<form action="...">` + hidden inputs，**本身没可见内容**，原样嵌入 iframe 也是空白 — user 看到白屏。**修复**：(1) `parsePayFormHtml(html)` 用 DOMParser 抽 `action` / `method` / `inputs` 数组，**不嵌入原始 HTML**；(2) iframe srcDoc 改为只读预览 —— 把 inputs 渲染成 key-value table（"支付表单字段（只读预览 · N 项）"），让 user 看到订单数据；(3) 真正提交的 form 由 JS 在 iframe.contentDocument 里**动态构建** —— 用 `createElement('form')` + `createElement('input')` 设 hidden 属性再 appendChild，没有任何上游 markup / script，零 auto-submit 风险；(4) sandbox **去掉** `allow-scripts`（我们不再在 iframe 内跑 JS）+ 保持 `allow-forms allow-same-origin`（form submit + 父 frame 注入节点），最紧能力集；(5) modal 上方加订单摘要（orderNo / 金额 / 过期时间），不依赖 iframe 也能核对订单；(6) 「去支付宝支付」按钮加 black-accent 实色（之前灰文字看不清），按 `parsedPayForm` 为空时 disable + 显示「支付表单解析失败」错误（解析失败也能 surface 给 user 而非默默 noop）。**i18n 新增**：3 个 `summary.*` key + `parseFailed` + 更新 `iframeHint` 强调"新窗口打开支付宝收银台"。**测试改写**：原"iframe 渲染 payFormHtml 原样"case 改为：(a) srcdoc **不含**原始 payFormHtml；(b) srcdoc **不含** `form.submit`；(c) srcdoc 含所有解析字段（out_trade_no + total_amount + 它们的 value）；(d) sandbox 严格 `allow-forms allow-same-origin`；(e) 订单摘要可见；(f) 「Go to Alipay」按钮 enabled。6 条断言一起防回归。**为何不直接 window.open alipay.com**：会丢 POST 数据（sign 必须在 body 不能在 query），且 `window.open` 在 Tauri WebView 可能被拦 / 弹 OS 浏览器丢 app 上下文，仍走 iframe 保持 in-app。1 文件 / +130 -45（form 主重写）+ 2 locale +12 -7 + test +18 -8，`npm run typecheck` 0 errors + `test:dom` 6/6 + `eslint` 0 errors。**scope-out**：(1) 仍不在 iframe 内做支付宝回调成功页检测（user 手动点稍后关闭）；(2) 仍不暴露"我已支付"独立按钮（v1 已拍「稍后」复用）；(3) iframe sandbox 不放宽到 `allow-top-navigation`（保留用户上下文不丢）。**版本流转**：未 commit → patch bump-on-commit。

最后更新：**2026-09-16**（任务 #146 nxgd 充值 iframe 不渲染修复 — 用户原话"广电充值页面选择金额之后应该弹出充值HTML，目前没有"。**根因**：`src/renderer/components/nxgd/NxgdRechargeForm.tsx::submit()` 调 `apiPostJson('/api/nxgd/recharge')` 拿到 `payFormHtml` 后立即 `setOrder(result)` **然后同步** `onCompleted?.(result)`；两条调用方（`NxgdSubscriptionProvider.tsx:153` `onCompleted={handleRechargeCompleted}` → `setRechargeOpen(false) + loadBalance()`；`NxgdRechargeModal.tsx:61` 经 `onPaid={nxgdGuard.hide}` 透传）都在 `setOrder` 同步触发的 commit 之前把 modal 卸载掉，iframe 永远到不了 DOM。**修复**：(1) 删 `submit()` 末尾同步 `onCompleted?.(result)`；(2) iframe 分支"稍后"按钮的 onClick 改为同时调 `onCompleted?.(order)` + `onCancel?.()` — 用户明示完成本轮流程时再通知父级「关闭 + 刷新」；(3) 补 i18n `providers.nxgd.recharge.iframeTitle`（代码引用，zh-CN/en-US 均缺）。**未扩「我已支付」按钮**：用户跳支付宝后从浏览器回 app，「稍后」按钮就是"我完成了"的自然信号；再加独立按钮需维护额外 visible 条件 + 文案 + 与「稍后」重复 — 复用现有按钮最小化改动。**Chat 路径无回归**：`onPaid={nxgdGuard.hide}` 行为从"订单创建即关"变"用户点稍后才关"，符合 dismissable 模态 UX（用户原话"可关"）；用户即使未真付就点稍后，下次发消息触发 balance check < 5 时 modal 重新弹出。**测试 1 新 case**：`NxgdSubscriptionProvider.test.tsx` 新增「点 ¥50 → POST 返回 payFormHtml → 等 iframe DOM 出现」 — 旧 case 验证 POST 被调，新 case 验证 iframe 真的渲染（之前 0.2.x 期间因 jsdom iframe srcDoc 渲染不可靠曾故意不验，但 jsdom 实际能渲染 `<iframe srcDoc>` 元素，只是 innerHTML 不执行；用 `querySelector('iframe[srcDoc]')` 验元素存在即可，不依赖 sandbox 执行）。3 文件 / +30 -8（其中 iframeTitle 2 locale +3/-1），`npm run typecheck` + `test:dom` 全绿。**scope-out**：(1) Chat 路径"用户明示完成时是否要主动 rebalance check"未做 — 当前依赖下次发消息触发 balance check；(2) iframe 内部 sandbox 是否真允许支付宝 form submit 未做端到端验证（dev 模式实测为准）。**版本流转**：未 commit → patch bump-on-commit。
