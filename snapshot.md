...
最后更新：**2026-09-10**（任务 #124 落地 — extended builtin MCP 默认 vendor 命令/参数在 Settings UI 不暴露：扩 `McpServerDefinition.hidesDefaultArgs?: boolean` 字段 + `extended-builtin-mcp.ts::coerceServer()` 解析（严格 `=== true` 防 poisoned bundle）+ `extended_buildin_mcp/mcp.json` 3 entry（stock-datasource / mobile-control / multimedia-creator）显式 `hidesDefaultArgs: true` + ToolboxSection list row 在 `hidesDefaultArgs` 时不显示 mono 命令行 + SettingsPage builtin dialog 显示「默认命令（供应商私有）」placeholder + 中英文 i18n 各 1 key + parser 单测 4 新 case（true / 缺席→false / 5 种 truthy 值收敛 false / http 类型保留）+ 27/27 通过）。
...
（落地详见 §5.9 + §4 `c58e0f3` commit，B 方案扩 slot 3 = `~/.hamuna/bin/uvx.exe`，runtime unit 7/7 通过；`72ca789 uvx` 是 bump-on-commit 自动 patch 的衍生 commit）。

#### TODO #122 — bundled uv 0.5.11 → 0.11.33 重 pin ✅ DONE
（落地详见 §5.9 + §4 `4812fbe` commit，跨 minor 拿稳定性 fix，mcp.json `--from` 语法未踩 0.12+ 收紧线）。**后续 follow-up（未 commit）**：`download_uv.ps1` 默认 pin `0.11.33`（**字符串字面量**，原 commit 写成 `0.11.33` 裸数字字面量 → PowerShell 解析器把 `0.11` 当 decimal + `.33` 当属性访问，静默赋值 `$null`，脚本报「GitHub response missing tag_name」误导；CI 跑 `-Force` 撞上这个 bug）。**修法**：字符串字面量 + 注释锁版本 rationale（**不** auto-track Astral latest 0.12.x，因 0.12 收紧 `--default-index` 语义可能与 bundled mcp.json `--from <pkg> <cmd>` 不兼容；TODO #122 决策是 user 拍板 pin）。后续要 bump 必须手动验证 spawn 兼容性 + 改这一行。

#### TODO #125 — `download_uv.ps1` 默认 pin 字符串字面量 + 锁版本 rationale（待 commit）🔄
**触发**：CI 跑 `download_uv.ps1 -Force` → 报「GitHub response missing tag_name」失败。**根因**：原 line 106 `$Version = 0.11.33` 是裸数字字面量，PowerShell 解析器把 `0.11` 当 decimal + `.33` 当属性访问，**静默**赋值为 `$null`（不抛错，因为不是 cmdlet），line 113 `if (-not $Version)` 误触发报错。**grlling**：(a) 是否 auto-track Astral latest？**用户拍板：保持 0.11.33 pin**，因 uv 0.12.x 收紧 `--default-index` 语义，与 bundled mcp.json `--from agnes-video-25-mcp==0.1.6 agnes-video-25-mcp` 可能不兼容；(b) 改 API 调用读 `$latest.tag_name` auto-track？被用户否决（pin 优先）；(c) 还是直接字符串字面量？选 c，最小改动 + 不引入 auto-track 风险。**1 文件改动（待 commit）**：`scripts/download_uv.ps1` line 99-117 删 15 行 GitHub API 调用 + 替换为字符串字面量 `$Version = "0.11.33"` + 详细注释（pin rationale + 不 auto-track 原因 + 裸数字字面量 bug 复述）。**验证**：CI `-Force` 应正常下载 0.11.33 uvx.exe + PE header 短路正常 exit 0。**待 user 拍板 commit**。



#### TODO #124 — extended builtin MCP 默认 vendor 命令/参数在 Settings UI 不暴露（hidesDefaultArgs 字段，待 commit）🔄
**触发**：用户原话"extended_bulidin_mcp在windows下不要暴露默认配置参数"——Windows Settings UI 三处把 extended builtin 的 `command`/`args`/`env` 完整字面量展示给终端用户：(a) `ToolboxSection.tsx:182-186` list row mono font 命令行 + (b) `ToolboxSection.tsx:183` `title` tooltip hover + (c) `SettingsPage.tsx:5460` builtin dialog preset command 段。**grlling 4 矛盾**：(1) 配置 vs 实现分离（mcp.json args 是 vendored 实现细节，Windows 终端用户改不了也不该改）；(2) 安全/合规（部分 vendor 命令含 API key / 私有 registry URL，泄露给截图/录屏用户是 P0 风险）；(3) 自定义 vs 内置一致性（in-process builtin MCP 已经隐藏 `__builtin__` sentinel，用户对 extended builtin 也应有同等隐私保护）；(4) 与 in-process builtin 不可合并（protocol layer / 懒加载红线 / process isolation / 现有 fix 闭环 / dual registry 5 维理由不合并）。**用户拍板 B 方案**（4 选项 AskUserQuestion：layout 改写 / **hidesDefaultArgs 字段（推荐）** / vendor id 白名单 / 运行时解析）：`McpServerDefinition.hidesDefaultArgs?: boolean` 是 metadata 字段，bundle / future-extended-builtin / user-defined 都可声明；UI 看到 `true` → 隐藏 vendor 默认参数，显示「默认命令（供应商私有）」。**9 文件改动**：(1) `src/shared/config-types.ts::McpServerDefinition` 加 `hidesDefaultArgs?: boolean` 字段 + 详细 JSDoc；(2) `src/server/utils/extended-builtin-mcp.ts::RawExtendedServer` 加 `hidesDefaultArgs?: unknown` + `coerceServer()` 严格 `raw.hidesDefaultArgs === true` 解析（防 poisoned bundle 漏 `1` / `"yes"` / `{}` / `null` / `"true"` 字面量）；(3) `extended_buildin_mcp/mcp.json` 3 entry（stock-datasource / mobile-control / multimedia-creator）显式 `hidesDefaultArgs: true`；(4) `src/renderer/pages/settings/sections/ToolboxSection.tsx:182-186` 条件显示——`hidesDefaultArgs` 时隐藏 mono 命令行 + 显示 i18n 提示；(5) `src/renderer/pages/settings/SettingsPage.tsx:5460` builtin dialog 在 `hidesDefaultArgs` 时直接显示 placeholder 而非 vendor 命令；(6) `src/renderer/i18n/locales/en-US/settings.json` 加 `toolbox.tools.defaultArgsHidden: "Default command (vendor private)"`；(7) 同 zh-CN 加 `"默认命令（供应商私有）"`；(8) `src/server/utils/extended-builtin-mcp.unit.test.ts` +4 新 case（parses true / absent→false / 5 种 truthy 值收敛 false / http 类型保留）；(9) `snapshot.md` closeout。**关键架构决策**：(1) **统一 metadata 字段而非 id 白名单**——未来新增 vendor 不需改代码；(2) **严格 `=== true` 解析**——poisoned bundle 不会偷偷开 hide；(3) **仅影响 UI 显示，不影响 spawn**——spawn 仍走 `command`/`args` 原值，UX 与功能解耦；(4) **http/sse 也支持**——stock-datasource 是 http 类型也加 `hidesDefaultArgs: true`（虽然没 args，但 URL 也是 vendored 配置）；(5) **不新增独立 page 或新 dialog**——复用现有 builtin dialog，避免 UI surface 增长。**验证**：`extended-builtin-mcp.unit.test.ts` 27/27 + `runtime.unit.test.ts` 7/7 = 34/34 通过；typecheck 0 新错误（pre-existing 的 `SettingsPage.tsx:620-621` qrCodeDataUrl/qrCodeLoading 未读是前人遗留，未波及）。**待 user 拍板 commit**。
...
#### TODO #120 — creative-ad-director: 抖音/创意广告 5 阶段 SKILL（独立 utility）🔄 — `bundled-skills/creative-ad-director/`（NEW）SKILL.md + 3 个 references + 3 evals + 6 子代理对比脚本。**触发**：用户 `/skill-creator:skill-creator` 用 `XiaoLuo-AI-Drama-Skill/小逻-视频拉片拆解.md` 拉片 reference_videos/ 9 抖音产品广告 mp4，**新建**独立 skill（聚焦"创意方向"，与 creative-video-suite 3 路 commercial ref + tvc-director 互补，不重复）。**5 阶段工作流**：(1) 模板决策 9 选 1（古风IP / 经典IP解救 / 都市倦怠 / 反转悬念 / 拟人IP / 异族神迹 / 历史人物 / 电商促销 / 纯产品镜头）+ `reuse_conditions_met` + `duration_boundary` + `required_assets` 4 字段硬门控；(2) 小逻方法论 storyboard（5 维物理表演 / 6 表演范式 → 具象动作 / 运镜按景别分级）；(3) multimedia-creator MCP 13 硬编码模板 T01-T13 决策（4 反模式产品图硬门控 / 14 项 gate / 2-retry 0 微调铁律 / `aspect_ratio` 9:16 / `seconds` ∈ `"4"`-`"12"` 字符串 / `model: agnes-video-2.5-flash` 显式锁）；(4) 跨段一致性 5 维铁律（asset/space/time/emotion/visual 25 项静默自检）；(5) ≥13s 视频用 `long_video_stitch_mode` match_cut。**Iter-1/2/3 三轮实测**：trigger accuracy OLD 6/9 → NEW 9/9 → standalone 9/9 (+33.3pp / 100%)；workflow with_skill vs without **iter-1 +38.5pp / iter-3 wide +21.6pp**（3 eval: iPhone17pro +16.6pp / Mercedes TVC +23.0pp / B2B 智能制造 +25.0pp）。**grlling 矛盾点**：standalone + 9 模板扩展到 TVC/UGC/B2B **wide-scope 增益更大**（+23.0/+25.0pp vs narrow +16.6pp）—— 9 模板 + stitch 多段机制对长片是强需求；但**绝对值**70.3% 持平，说明 wide-scope 测试更严苛。**整体结论**：standalone + wide-scope 不反噬 trigger/workflow 两维度，trigger 9/9 + workflow +21.6pp 都达标，可进 commit 阶段。**剩余未做**：(a) commit `bundled-skills/creative-ad-director/`（待 user 拍板 + bump SYSTEM_SKILLS_VERSION）

#### TODO #121 — Windows install: bundled MCP auto-merge + uvx PATH 闭环（待 commit）🔄
**触发**：用户报"windows 安装完后 multimedia-creator 启动依然找不到 uvx.exe" → grlling 根因 2 条：(H1) install-dir 不在 Windows PATH（NSIS / .app 不依赖 install-dir 入 PATH），multimedia-creator spawn `command: 'uvx'` 撞 ENOENT；(H2) `extended_buildin_mcp/mcp.json::servers` 3 个 entry（stock-datasource / mobile-control / multimedia-creator）从不自动落到 `~/.hamuna/config.json::mcpServers`（`buildSdkMcpServers` 显式 "user config is single source of truth"），用户必须 Settings 手动加每个 bundled entry。**用户拍板 A 路径** = auto-merge + bundled uvx 拷到 `~/.hamuna/bin/`（PATH shim 已注册）。**5 文件改动**：(1) **`src/server/mcp-bundled-seed.ts` NEW（143 行）**— `seedBundledExtendedMcpServers()` diff-merge `loadExtendedBuiltinMcpServers()` 进 user config（in-lock re-check 防 race + `ConfigBusyError` 静默 swallow + never throws 阻塞 startup）；`seedBundledUvToHamunaBin()` Windows-only 把 bundled `uvx.exe` copy 到 `~/.hamuna/bin/uvx.exe`（size-based idempotent + broken-symlink guard + tmp+rename atomic + `lstatSync`+`existsSync` 双探避免 v0.2.5 C++ exception crash）；(2) **`src/server/mcp-bundled-seed.unit.test.ts` NEW（240 行 / 11/11 通过）**— 覆盖 auto-merge 空 / 全已 / 部分 / ConfigBusyError / loader throw + uvx copy no-op / 缺 / 同 size / 不同 size / 断链 symlink / 非 win32；(3) **`src/server/index.ts`** seed hook 在 `seedBundledSkills()` 后串入两个 seed（注释说明职责 + 失败 best-effort 不阻塞 startup）；(4) **`extended_buildin_mcp/mcp.json`** multimedia-creator args `@latest` → `==0.1.6`（description 已承诺 pin 0.1.6，PyPI latest 也是 0.1.6；TODO #113 bump-on-commit.mjs auto-bump 段需要 `==X.Y.Z` 格式才能生效，`@latest` 会被静默 skip → breaking change 自动传染风险）；(5) **`scripts/bump-on-commit.mjs`** AGNES_MCP auto-bump regex miss 分支加 `@latest` stderr 警告（之前静默 skip → description vs args 漂移无人知，fdd624e 实战）。**关键架构决策**：(a) **auto-merge vs 手动 opt-in** = 用户拍板 auto-merge（trade-off：Settings UI 删 entry → 下次 startup 仍重 merge，follow-up PR 加 `mcpDisabledBundledMcpServers: string[]`）；(b) **uvx 拷到 `~/.hamuna/bin/` 不放 install-dir** = 后者已在 user PATH（CLI shim 注册 `hamuna.cmd` 同理），NSIS / .app 不依赖 install-dir 入 PATH；(c) **不在 mcp.json 加 disable flag**（YAGNI）；(d) **不自动 disable `AGNES_API_KEYS` 硬编码**——安全备忘已立 TODO（仓内 fixture key 暴露在 PyPI vendor source + mcp.json 是开发产物）。**验证**：`npm run typecheck` 0 新错误（6 个 pre-existing tvcEnvelope 错误确认与本次改动无关）；`extended-builtin-mcp.unit.test.ts` 23/23 + `mcp-bundled-seed.unit.test.ts` 11/11 = 34/34 通过。**待 user 拍板 commit**。
...
| **#19 P3 Step 2 — 改代码** | ✅ DONE（0.1.4 in-memory）+ ✅ DONE（0.1.6 持久化 + 30s 窗口） | 改 `server.py` + `tests/test_key_pool_cooldown.py` 6 assert self-check（不是 pytest）；in-memory pool + persisted state + atomic tmp+replace + `AGNES_KEY_POOL_STATE_DIR` 覆盖 + 30s 窗口 reason split（`quota_429` 首次 / `consecutive_429_30s` 30s 内二次） + cooldown 完 refresh `last_429_at=0` |
| **#20 P3 Step 3 — bump + PyPI** | ✅ DONE（v0.1.6） | `pyproject.toml` 0.1.5 → 0.1.6 + `python -m build` + `twine upload dist/agnes_video_25_mcp-0.1.6-{py3-none-any.whl,tar.gz}`；PyPI v0.1.6 whl 15122B + tar.gz 81054B，upload_time 2026-09-09T16:24:15/18，https://pypi.org/project/agnes-video-25-mcp/0.1.6/ |
| **#21 P3 Step 4 — vendor + mcp.json + e2e** | 🔄 pending | `hosted_mcps/agnes-video-25/src/agnes_video_25/server.py` 同步 0.1.6（PyPI 已有仓内 vendor 仍是 0.1.5 代码）；`extended_buildin_mcp/mcp.json` pin 0.1.5 → 0.1.6（**或**靠 TODO #113 bump-on-commit.mjs AGNES_MCP auto-bump 段下次 commit 自动 patch）；跑 2nd UGC 剩余 5 段验证 fallback 真生效（2 个 key，1 个 daily quota 撞顶看是否自动切到第 2 个） |
...
| `<pending>` | **fix(uvx-spawn): extend getBundledUvPath probe chain with ~/.hamuna/bin/ uvx.exe (slot 3 of 4; mcp-bundled-seed copy 真正生效)** |
...
**+ agnes-video-25-mcp: v0.1.6 multi-key cooldown state 持久化 + 30s 窗口 reason 升级(commit `d2403e6`,PyPI v0.1.6,2026-09-09)**:用户 sequence "持久化 state 删 deprecated utilities commit 发pypi"。`hosted_mcps/agnes-video-25/src/agnes_video_25/server.py` +317/-33:新增 `_state_file()` / `_load_persisted_state()` / `_persist_state()`(写 `~/.hamuna/state/agnes-key-pool.json` + atomic tmp+replace + `AGNES_KEY_POOL_STATE_DIR` override);`_load_key_pool()` merge future-disabled only;`_pick_key()` cooldown 完 refresh `last_429_at=0` + dirty flag 出 with-block 调 `_persist_state`(Lock 非 RLock 防重入);`_mark_disabled()` 末尾调 `_persist_state()`;429 分支 fixed 60s + 30s 窗口 reason split(`quota_429` 首次 / `consecutive_429_30s` 30s 内二次);删 deprecated `_parse_quota_reset` / `_next_utc_midnight` + orphan `import re`;加 `import json`;pyproject.toml 0.1.5→0.1.6;`tests/test_key_pool_cooldown.py` NEW 6 assert self-check(不用 pytest)。self-check 6/6 passed;PyPI v0.1.6 已发(whl 15122B + tar.gz 81054B,upload_time 2026-09-09T16:24)。**已知遗留**:Step 4 vendor sync 0.1.6 to hosted_mcps + mcp.json pin update + 2nd UGC e2e 验证 fallback 真生效(§3.2 #21 仍 pending)。
