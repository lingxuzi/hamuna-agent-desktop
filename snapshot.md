...
最后更新：**2026-09-16**（任务 #146 v3 nxgd 充值去 auto-submit + 显式「去支付宝支付」按钮 — 用户实操反馈 v2 的 auto-submit 让 iframe 立刻跳到 alipay.com，原 form HTML 一闪即过看不清；且 form 本身只含 hidden inputs，没可见内容，auto-submit 一走 iframe 就空。**修复**：(1) 删 iframe srcDoc 里的 auto-submit script，iframe 只纯渲染 `payFormHtml` 原样（让用户看到订单号 / 金额 / sign 等 hidden input 值）；(2) 加 `iframeRef` + 显式「去支付宝支付」按钮（i18n key `providers.nxgd.recharge.modal.goPay`，zh-CN `去支付宝支付` + en-US `Go to Alipay`），按钮 onClick 调 `iframeRef.current.contentDocument.querySelector('form').submit()` —— sandbox 已含 `allow-same-origin` 父 frame 可访问 contentDocument；(3) i18n `iframeHint` 重写为"已为您创建充值订单（5 分钟内有效）— 下方是支付表单预览，检查无误后点「去支付宝支付」跳转收银台"。**vs v2 auto-submit**：v2 按 spec 字面 spec 注入 script 自动 submit，但 spec 设计场景是"页面级嵌入（display:none 包裹 + 立即 submit）"——意图就是 main window 立刻跳 Alipay 收银台（不是让用户看 form）。iframe 嵌入场景下"让用户看 form"和"auto-submit 跳走"是矛盾的；本版选择前者（用户实操原话"目前 iframe 渲染看不到"）。**测试改写**：原"iframe 渲染 + auto-submit 脚本"case 改"iframe 渲染 payFormHtml 原样 + NOT 含 auto-submit 脚本 + 「去支付宝支付」按钮存在"，三条断言一起验。3 文件 / +24 -16（form +15/-12 + 2 locale +7/-0 + test +2/-4），`npm run typecheck` 0 errors + `test:dom` 6/6 + `eslint` 0 errors。**scope-out**：(1) 仍不在 iframe 内做支付宝回调成功页检测（用户手动点稍后关闭）；(2) 仍不暴露"我已支付"独立按钮（v1 已拍「稍后」复用）；(3) iframe sandbox 不放宽到 `allow-top-navigation`（保留用户上下文不丢）。**版本流转**：未 commit → patch bump-on-commit。
