...
#### TODO #114 — creative-video-suite: video 阶段轮询 + 串行 + project.json 状态机同步 🔄
用户 brief 4 点（"轮询视频生成 / 不要并发 / 避免限流 / 生成前检查 project.json / 每完成同步状态"）→ `/skill-optimizer:skill-optimizer` 5 步走完 → 按计划执行。**改动 3 文件**：(1) `references/mcp-usage-guide.md §3.3` 扩展 status 枚举为四态 `pending / in-progress / completed / failed` + 字段补 `started_at` / `finished_at` / `attempt_count` / `last_error_code` / `last_error_message`；(2) 同文件新增 `§3.4 video 阶段 4 步硬门控` 段（pre-flight → pre-call → serial-call → post-call + 段间冷却 2-5s 建议 + 串行契约 + pre-flight 三项检查）；(3) `references/drama/prompt.md`「持久化」段前补 cross-link + `SKILL.md`「视频生成门禁」末尾补"段间串行 + project.json 状态机"短句锚点。**关键架构决策**：(1) "轮询"语义 = project.json 状态机 `in-progress` 中间态（CLAUDE.md 红线禁 busy-wait，AI 不主动 poll MCP）；(2) "不要并发" = 段间串行 + 段内串行（单批 ≤ 2 是数量上限，不是并发起跑 2 个 MCP 调用）；(3) "避免限流" = 段间冷却建议 + MCP 内部 429 退避 + AGNES_API_KEYS 多 key fallback 三层防御（SKILL 层只补"段间冷却"，不重复造 MCP 已做的轮子）；(4) status 四态 + 字段 schema 单一权威在 §3.3，drama + commercial 3 路共享；(5) 4 步硬门控对 drama / ugc / marketing / corporate 全适用。**不改**：`hosted_mcps/agnes-video-25/server.py`（MCP 内部 5s/600s poll 已对）/ `extended_buildin_mcp/mcp.json`（AGNES_API_KEYS 多 key fallback 已对）/ `bump-on-commit.mjs`（已有 SYSTEM_SKILLS_VERSION auto-bump，commit 时自动 +1，强制 sync 老用户）。**已知遗留**：(a) 4 步硬门控未配 lint 拦截（cross-link 文本约定，依赖 SKILL.md 锚点）；(b) commercial 3 路 ref 未单独补 cross-link（drama prompt.md 已补，commercial 同源结构未单独提）；(c) `references/commercial/{ugc,marketing,corporate}-*-ref.md` 各自 video_generate 调用段未单独加"段间串行"提示（共享 §3.4 锚点）。**版本流转**：commit 时 bump-on-commit.mjs 自动 patch `SYSTEM_SKILLS_VERSION` +1 + cargo + tauri.conf.json（与 #108 / #109 / #111 触发模式一致）。
