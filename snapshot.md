...
#### TODO #120 — creative-ad-director: 抖音/创意广告 5 阶段 SKILL（独立 utility）🔄 — `bundled-skills/creative-ad-director/`（NEW）SKILL.md + 3 个 references + 3 evals + 6 子代理对比脚本。**触发**：用户 `/skill-creator:skill-creator` 用 `XiaoLuo-AI-Drama-Skill/小逻-视频拉片拆解.md` 拉片 reference_videos/ 9 抖音产品广告 mp4，**新建**独立 skill（聚焦"创意方向"，与 creative-video-suite 3 路 commercial ref + tvc-director 互补，不重复）。**5 阶段工作流**：(1) 模板决策 9 选 1（古风IP / 经典IP解救 / 都市倦怠 / 反转悬念 / 拟人IP / 异族神迹 / 历史人物 / 电商促销 / 纯产品镜头）+ `reuse_conditions_met` + `duration_boundary` + `required_assets` 4 字段硬门控；(2) 小逻方法论 storyboard（5 维物理表演 / 6 表演范式 → 具象动作 / 运镜按景别分级）；(3) multimedia-creator MCP 13 硬编码模板 T01-T13 决策（4 反模式产品图硬门控 / 14 项 gate / 2-retry 0 微调铁律 / `aspect_ratio` 9:16 / `seconds` ∈ `"4"`-`"12"` 字符串 / `model: agnes-video-2.5-flash` 显式锁）；(4) 跨段一致性 5 维铁律（asset/space/time/emotion/visual 25 项静默自检）；(5) ≥13s 视频用 `long_video_stitch_mode` match_cut。**Iter-1 benchmark**（`/tmp/creative-ad-director-workspace/iteration-1/`）：6 子代理全完成 · with_skill 70.3% vs without 31.8% pass_rate = **+38.5pp delta** · time +216s / tokens +122 · viewer `/tmp/creative-ad-director-workspace/iteration-1/review.html`（静态 459KB）。**Description 优化 + iter-2 trigger accuracy 实测**：459→479 chars · 加 **6-66s 时长硬门**挡掉 3 分钟 TVC · 4 条显式 NOT 重定向到 drama/corporate/commercial/tvc-director · 替代原 pushy "MUST" 措辞为引导式 · **实测 9 query × claude -p sonnet**：OLD 6/9 (66.7%) vs NEW 9/9 (100%) = **+33.3pp trigger accuracy delta** · 修掉 3 个 false trigger（短剧/TVC/UGC口播）+ 1 个 false negative（节日营销 H5）· `/tmp/creative-ad-director-workspace/iteration-2/trigger_benchmark.json`。**正文同步**：§角色段"何时走"加 NOT 边界表 + commercial 3 路展开为 ugc/marketing/corporate 三行 · Phase 1 时长默认从"<≤ 12s" 改 "<≤ 12s / 6-66s stitch / >66s 走 tvc-director"。**未做**：(a) commit `bundled-skills/creative-ad-director/`（待 user 拍板 + bump SYSTEM_SKILLS_VERSION）；(b) workflow 质量 iter-2 实测（6 子代理跑完整流程，看 description 优化是否反噬质量）。**iter-3 standalone 改造**（用户拍板"不要调用其他 skill"）· description 重写为"任何视频广告 brief (6s-3min)" + 删所有 `→ tvc-director / → creative-video-suite` 重定向 · SKILL.md + 3 个 references 全文清零 cross-refs（grep 已验证 0 match）· §角色 触发边界表改"能力边界"表（短剧/TVC/UGC/B2B 都走本 skill 内 stitch + 模板机制扩展）· 删 `## 何时跳转到其他 skill` 整段（user 要求 standalone 后语义失效）· `## 关键架构决策` 重写为 4 条 standalone 决策 · **iter-3 trigger 实测**（10 query × claude -p sonnet）· 9/10 正确（1 timeout）· 实际判定 9/9 = 100% 准确率（ground truth 重定义：视频广告含短剧/TVC/UGC/B2B/H5 → True；hero 图/纯代码 → False）· `/tmp/creative-ad-director-workspace/iteration-3/trigger_benchmark.json`。**grlling 矛盾点**：standalone 决策让 description 范围从"6-66s 抖音"扩到"6s-3min 任何视频广告"，**预期**会让 6 子代理 workflow 质量从 +38.5pp 退化（短剧/TVC/B2B 走 9 模板机制是否能保持 70.3% pass_rate 待验证），但 trigger accuracy 100% 反而提升（少了 3 个 false trigger 的复杂度）。**iter-3 workflow 实测**（6 子代理 = 3 eval × 2 config 全完成，`/tmp/creative-ad-director-workspace/iteration-3/grade.py` 自动断言）· eval-0 iPhone17pro 抖音 12s regression: **with_skill 83.3% (10/12) vs without 66.7% (8/12) = +16.6pp** · eval-1 Mercedes TVC 3min wide-scope: **with_skill 69.2% (9/13) vs without 46.2% (6/13) = +23.0pp** · eval-2 B2B 智能制造 60s wide-scope: **with_skill 58.3% (7/12) vs without 33.3% (4/12) = +25.0pp** · **OVERALL with_skill 70.3% vs without 48.6% = +21.6pp**（vs iter-1 的 +38.5pp，**绝对值回归 -16.9pp，delta 退化 -16.9pp**）· `/tmp/creative-ad-director-workspace/iteration-3/benchmark.json` · **grlling 反预期**：standalone + 9 模板扩展到 TVC/UGC/B2B **wide-scope 增益更大**（+23.0/+25.0pp vs narrow +16.6pp）—— 9 模板 + stitch 多段机制对长片是强需求，baseline 子代理完全不知道 stitch 被扣分；但**绝对值**从 70.3%（iter-1 narrow）→ 70.3%（iter-3 wide）持平，说明 wide-scope 测试更严苛。**整体结论**：standalone + wide-scope 不反噬 trigger/workflow 两维度，trigger 9/9 + workflow +21.6pp 都达标，可进 commit 阶段。**剩余未做**：(a) commit `bundled-skills/creative-ad-director/`（待 user 拍板 + bump SYSTEM_SKILLS_VERSION）
