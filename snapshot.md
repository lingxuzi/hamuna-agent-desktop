...
最后更新：**2026-09-10**（任务 #127 落地 — `windows-release.yml` 加 install-time smoke test step (位置 20/23,在 build + portable ZIP 之后、rclone upload 之前):`Start-Process .exe /S /D=C:\HamunaAgent-Test` 静默装 → 断言 4 条 (Python 3.12 落到 `%LOCALAPPDATA%\Programs\Python\Python312\` + `pip show uv` 版本 = 0.11.33 + uvx.exe 在 PEP 370 `%APPDATA%\Roaming\Python\Python312\Scripts\` 或 `%LOCALAPPDATA%\Programs\Python\Python312\Scripts\` 之一 + HKCU\Environment\Path 含 Scripts dir) + 关键 smoke: `uvx --from agnes-video-25-mcp==0.1.6 agnes-video-25-mcp --help`(验证 0.11.33 pin + `--from` 解析端到端不挂,若 0.12.x 收紧假设真成立此处 throw 中断 release) + `Uninstall.exe /S` 清理。**触发**:TODO #126 commit `a906d2b` pin install-time 0.11.33 后,整条 rationale chain 完全没有 CI 烟测兜底 — build 只产 .exe,install-time 任何 regression(Tsinghua mirror 拦 runner IP / 拼写错误 / PEP 370 路径漂移 / 0.12+ --from 收紧首次真发生)只会被真实用户双击 installer 时才发现。**scope 决策**: 只加 `windows-release.yml` (workflow_dispatch, 5min 加时, 仅 release 烧, 不进 PR 周期);不加 `test.yml` (ubuntu-latest 不构建 Windows, 重复无价值)。**版本流转**: 0.3.146 (bump-on-commit 自动 patch)。**后续 #126 follow-up**: 烟测通过即闭环 TODO #126 已知风险 (a) 0.12+ --from 是否真收紧;烟测失败即第一次硬证据, rationale 链崩 → 选项 2 重审。**任务 #126 落地** — pin install-time `pip install uv` 到 `==0.11.33`:`src-tauri/nsis/installer.nsi:770` `pip install --upgrade uv` → `pip install --upgrade uv==0.11.33` + 11 行注释记录 pin rationale + 同步修 `extended_buildin_mcp/mcp.json:41` description 删 `download_uv.ps1` / `src-tauri/resources/.uv-version` 两条已废引用 drift + 改写为真实 pip-only 链路（NSIS Section UvxFallback + `uvx-path-setup.ps1` + `findPipInstalledUvxScriptsDir`）。**触发矛盾**：grlling 查出 PyPI 当前 latest = uv 0.12.12(2026-09-09 发布)，原 install-time 命令 `--upgrade uv` 不 pin → 新装机器会装到 0.12.12 → 撞 commit `197837b / 4812fbe / 37a7f21` (TODO #122 #125) rationale chain "0.12.x 收紧 `uvx --from` 解析会破坏 mcp.json `multimedia-creator` spawn"。**rationale 链一致性**：pin 0.11.33 与历史 5 commit 一致，bundled 退路已被 #121 `git rm` 砍掉，新装机器无 fallback 必须靠 install-time 阻击。**未验证**：(a) 0.12+ `--from` 收紧是否真在 0.12.12 仍存在（WebFetch 查 0.12.3-0.12.12 release notes 无 `--from` 收紧条目，但 0.12.0-0.12.2 release notes 截掉未取，rationale 可能基于早期版本已修或误传）；(b) 0.11.33 仍为 PyPI 可装版本（清华源 mirror 同步延迟风险）。**用户拍板**：pin install-time 0.11.33（与 rationale chain 一致）+ 接受未验证风险，下次 Windows 真实 install 时观察 multimedia-creator spawn 是否仍工作 → **TODO #127 烟测即补此闭环**。
