...
**触发**：用户报"用 `/home/hmcz/Projects/XiaoLuo-AI-Drama-Skill/小逻-视频拉片拆解.md` 规范分析 reference_videos, 优化 creative-video-suite 产品广告的创意方向，总结创意模板"——`reference_videos/` 已落 9 个抖音产品广告 mp4（34 MiB 总，详 §5.8 / TODO #36），需要把它们从"原始素材"提炼成"AI 可复用的创意模板"。**目标**：从 9 视频拉片 → 提炼 9 个互补且可复用的创意模板 → 写独立 reference 文件 cross-ref 现有 3 路 commercial ref。**5 文件落地**：(1) `bundled-skills/creative-video-suite/references/commercial/creative-templates-from-9-references.md`（NEW，369 行）——含 9 视频速览表 + 9 模板详细展开 + 跨模板共性 + 模板选择决策树 + 与 3 路 commercial ref 的边界；(2-4) 3 路 commercial ref 各加 `🔗 创意方向横切` blockquote 引用新文件（planner 阶段触发 + 各自推荐适用模板编号）；(5) `bundled-skills/creative-video-suite/SKILL.md` 在 `🔒 MCP 调用模板硬编码铁律` 段后加 `🔗 创意方向横切` 段（planner 阶段入口 + 横切 vs 纵切边界）。**用户拍板落地**：用户选 4 选项中"commit + 加 3 路 cross-ref + 手动 bump SYSTEM_SKILLS_VERSION（推荐）"——但**执行时发现 SYSTEM_SKILLS_VERSION 已在 HEAD = "41"**（`1466dd9 feat(creative-video-suite): version infrastructure + sibling-skill cleanup` 已隐式 bump 40→41，见 §5.6 已生效的 bump-on-commit.mjs 自动触发链），无需重复手动 patch 双源——本次只 commit 内容变更，下一次任何 creative-video-suite/ 改动时由 hook 再 +1。**关键架构决策**：(1) **9 模板 vs 原计划 5-8**——9 个全部独立且互补（古风IP/经典IP痛苦解救/都市倦怠/反转悬念/拟人IP冷暖反转/异族部落神迹/历史人物原料拟人/现实痛感怼脸/纯产品镜头叙事），无重叠/合并；(2) **不与 3 路 commercial ref 重复**——3 路是按"场景分"（UGC口播/Marketing无分镜/Corporate品牌），新文件是按"创意机制分"的横切总览，决策树按"产品类型+品牌调性+投放目标"3 维度匹配；(3) **时长边界遵循 MCP 上限 12s**——超 12s 的模板（#4/#6/#7/#8）显式标注走 `long_video_stitch_mode`，详见 `references/agnes-ai-api.md §视频时长边界（单一权威）`；(4) **不做剧情总结/评价**——严格遵循小逻规范"不要总结剧情，不要评价好坏"原则，新文件是"创意机制横切总览"；(5) **9 拉片总报告留在 subagent task output**——`/tmp/claude-1000/.../tasks/*.output` 不入仓（同 §5.6 reasoning：临时文件不入仓，体积过大）。**已知遗留**：(a) **模板 #9 与现有 3 路"无角色叙事"分支有重叠**——但本模板用 Charlotte 案例具体化"原料产地→加工→使用→静物"四段弧，比现有 3 路更落地，不视为冗余；(b) **跨模板共性（运镜/光影/产品植入方式）已在 3 路 commercial ref 内部详细论述**——本文件不重复，只列 cross-ref 锚点；(c) **模板选择决策树的边界 case**——多调性品牌（如"国潮+都市"混搭）需 AI 综合判断，不在决策树覆盖范围；(d) **9 视频 mp4 在 `reference_videos/` 不入仓**——34 MiB 视频文件不入 git（`.gitignore` / `reference_*` 命名约定默认不入仓）；(e) **未独立 bump**——bump 由 hook 在下次 creative-video-suite/ 改动时自动触发，无需本次显式手动 patch 双源（HEAD 已是 41）。
...
#### TODO #111 — hamuna_helper: 跨服务 MCP 工具路由 · agnes-video-25（ADMIN_AGENT_VERSION 24→25）🔄
**触发**：用户报"将 agnes-video-25 mcp 每个工具的使用方法内置到 hamuna_helper/CLAUDE.md 中，这是一个全局 md"——grlling 3 矛盾（职责越界 / 重复内容 / 全局加载成本）→ 4 选项 AskUserQuestion → 用户选「helper 给其他 skill 或者任务使用 agnes-video-25 mcp 提供指导」= A 路由变体。**2 文件改动待 commit**：(1) `bundled-agents/hamuna_helper/CLAUDE.md` 新增「跨服务 MCP 工具路由 · agnes-video-25（multimedia-creator）」段（4 子段：完整创作流水线 / 直接调用 MCP 兜底 / 关键铁律 4 条 / 跨场景路由 5 行 + 「不要做的事」3 条）；(2) `src-tauri/src/commands.rs` bump `ADMIN_AGENT_VERSION` "24"→"25"（line 1039 const + line 1734 测试函数名 + line 1735 单元测试 assert）。**架构核心**：(1) helper 只做"能力路由 + 诊断指引"，**不**dump 7 工具参数表（避免 2000+ tokens 常驻税）；(2) helper **不**直接调 MCP，5 阶段流水线是 creative-video-suite skill 职责，helper 引导加载 skill；(3) 跨场景路由 5 行按用户意图分发；(4) 单一权威 = creative-video-suite skill，helper cross-link 不另起炉灶。**已知遗留**：(a) 「股票服务」段同源模式未升级（同 `bundled-agents/hamuna_helper/CLAUDE.md:97-98` 1 行 MCP 指引），独立 PR 跟进；(b) support skill 未同步引用 multimedia-creator 错误码映射，独立 PR 补 cross-link。**待 commit**——3 文件（helper CLAUDE.md + commands.rs + snapshot.md）。bump-on-commit.mjs hook 会自动 patch HamunaAgent app version（patch +1），但 ADMIN_AGENT_VERSION 不在 hook 范围（CLAUDE.md 红线"修改 hamuna_helper/ 的 CLAUDE.md → MUST bump ADMIN_AGENT_VERSION"是手改要求，已手动 bump）。
