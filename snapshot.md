...
最后更新：**2026-09-18**（任务 #156 bundled-skills/ 作为 SYSTEM_SKILLS 单一权威源（auto-derive） — `generate-system-skills.mjs` 单脚本 emit Rust `include!` + TS re-export，bundled-skills/ = single source of truth；task #155 orphan cleanup 落地 — 见 `git log -1` 与 `git show <hash>`）。
