...
最后更新：**2026-09-19**（任务 #182 desktop launcher 风格 4 方案 HTML mockup v2）。User 要求"换 desktop app launcher 风格，给 HTML 方案"。第一轮 4 个 mockup（A 暗房 / B 白盒 / C 色块拼贴 / D 巨字编排）user 全拒："都不好看，要符合目前整体配色风格的"。**自审 #1 撞 default**：4 个 mockup 都偏离 hamuna 当前 theme — 暗房 = 撞 default 2 (近黑 + 鲜 accent)；白盒 = 撞 default 3 (newspaper broadsheet hairline 规则)；色块 = 撞 default 1 (cream + terracotta + serif editorial accent — **这个最撞** Anthropic Claude 自身的味道)；巨字 = 撞 default 5 (display number + small label + gradient accent)。**重构方向**：先 grep `src/renderer/theme/themes/hamuna-default.css` 抽出完整 token 集（`--paper:#faf6ee` / `--paper-elevated:#fffcf7` / `--paper-inset:#e8dccf` / `--ink:#1c1612` / `--ink-muted:#6f6156` / `--accent-warm:#c26d3a` / `--accent-cool:#2e6f5e` / `--accent-primary:#7b8f6b` / `--line:rgb(28 22 18 / 0.10)` / `--theme-radius-{md:10,lg:14,xl:18}` / `--theme-shadow-{sm,md}` / `--font-{latin,chinese,mono-latin}`），重写 4 个 mockup **全部沿用 token**（只换版式节奏 + 字重 + 几何切分，不换色）。**最终 4 方案**（统一暖纸底 + terracotta accent + ink 文本 + SF Pro / PingFang / mono 三套字）：(a) **A · Hallmark editorial** — 55/45 双栏（左 hero + 右 workspaces），左栏 font-weight 700 56px "Talk to your computer." + slogan + input + provider row + perm seg；(b) **B · Marquee hero** — 单栏顶天 hero（font-weight 700 92px sans 双行 "A desktop that / does the **work**." + 独立 stub-block slogan）+ 中部 input + 底部 workspace rail（4 列 workspace card 4 等宽）；(c) **C · Stacked index** — 38/62 双栏，左栏 "00 / launcher" eyebrow + h1 "Pick where." + 编号索引列表（01-05 每行 num + label + tag，current 用 terracotta active），右栏 detail 视图（file path + actions + glyph + 元数据 table + input + recent sessions 文件感列表）；(d) **D · Card trio** — 三区垂直布局（zone-1 = 一句话 "You write a line. **It does the work.**" + 右上 metadata 4 行；zone-2 = 大圆角输入 + 工具栏含 provider pill；zone-3 = 三张 capability 卡片 Open / Schedule / Connect，hover 暖橙描边 + 1px translateY）。**B 自我冲突 3 轮迭代**：(1) 初版 font-size 168px + italic word accent → 自审撞 default 1 (serif editorial accent，user 已知 reject landing 同样问题)，改 sans 92px 非 italic；(2) 把 stub 放进 `.row-2` flex 子 → 28px baseline 不齐 + inline-block vertical-align:bottom 文字叠 → 改 flex space-between 也叠 → 把 stub 抽出 `.row-2` 独立 `.stub-block` 单行解决；(3) **grllling**: B 撞 default 5 (display 6rem + small label + gradient accent) 红线刻意压字号 168→92 + 不混字体 = 拉开差距。**B 收尾准则**：所有动效/serif italic/word accent 都不要（与 landing v5 一致决策）。**输出**：`pages/launcher-mockups/option-{a,b,c,d}.html` 4 文件 + `launcher-v2-{A,B,C,D}-{1280,375}.png` 8 截图（1280×800 桌面 + 375×812 移动）。**scope-out**：(a) 不动 `src/renderer/pages/Launcher.tsx`（mockup 是视觉对齐工具，未拍板不写代码）；(b) 不抽 CSS helper（4 mockup 单文件 < 3 引用就重抽 = 过度工程）；(c) 不动 theme 文件（验证 token 即用，不改）；(d) 不 commit（user 拍板节奏）；(e) mobile 截图未做 viewport meta 反复迭代（仅一次性 @media (max-width:760px) 默认行为）。**未验证**：(a) playwright console error（第一轮 navigate option-a 报 1 console error，未 dump 内容，下次任务 #183 拍板前先 grep 排查）；(b) 4 mockup 与真实 Launcher.tsx 的 100% 视觉对齐（mockup 是单页 demo，Launcher 是 React component tree + 多状态，差距靠 user 拍板后端到端对齐）。**下一步 user 拍板**：从 4 选 1 → 改 Launcher.tsx 实现；或 4 都不满意 → 重新拍方向。

最后更新：**2026-09-19**（任务 #174 landing v5 升级 — user 反馈 #173 v4 "做的不惊艳"，自审撞 SaaS card kit + serif italic word-accent 两个最常见 AI default；走"杂志感 + 长读节奏"换结构而非加装饰）。**v4 自审（推翻 v4 整套）**：(a) Hero `your own` 单字 Playfair italic + accent 着色 → 撞 default 1「serif editorial accent」；(b) 11 张 caps 同质 card 同 border-radius 同 shadow 同 padding 4-col grid → 撞 default 4「SaaS card kit」(c) CTA `↓` 装饰 + 单按钮 + 右侧 metadata 三列争视觉 → 撞 default 5「`→` appended to CTA」；(d) closer `that's all.` Playfair italic 96px → 撞 default 1+5 复合。**v5 三轮迭代核心改动**：(a) **Hero 拆浮卡 → 两段对比 + 数字锚点** — left half = sans display 800 weight `Run an AI assistant on your own computer.` 全句同色同 weight **不带单字 italic 强调**，right half = `v0.3.184` 72px sans 800 + `~92 MB / Windows 10/11` 13px sans 500 行内堆（**hero 不再独占一整行数字块**，v4 缺陷修）。(b) **Caps 砍 11 卡 → 双栏"大图 + 一句"**（承接 user 后期要求"大家用来做的事 双栏"）— 11 段统一 720×405 (16:9) SVG 占位 + sans caption，10 段 2-col（column-gap 64px / row-gap 80px）；cap.5 self-media = the one memorable thing，全宽 21:9 黑底视频占位（`#0a0604` + terracotta 三角 + 字幕条 + 角标 + 中央 terracotta 聚焦环 + line-strong border + 12/36 ink shadow）。无 card / 无 shadow-at-rest / 无 hover lift / 无 border-radius transition。11 个 SVG 占位手工画（写作 = 文本行 / 办公 = 文件块 / 理财 = K 线 / 营销 = 封面 / 视频 = 黑底播放 / 搜索 = 浏览器搜索栏 / 出图 = 拼贴 / 代码 = terminal / 自动化 = M·W·F / 学习 = 知识图谱同心圆 / 日常 = 邮件+礼物）。(c) **CTA 拆 `↓` + 单 button + metadata 上位** — `download for windows` 24px sans 700 + `free installer` 14px sans 500 行内 baseline，无 `↓` 装饰；dl-meta 5 行 sans 14px 仍保留版本/系统/授权/大小/安全，无 middle-dot。(d) **Closer 拿掉 Playfair italic** — `that's everything.` sans 800 112px max + 18px sans 70% line-height body（无 serif editorial ending 默认）。**新增 token**：无（复用 v4 `--ink-strong / --accent-deep / --accent-glow`）。**删**：v4 `.chat-card`/`.chat-stack`/`.typing`/`@keyframes typing`/`.cap`（同质 11 卡 CSS）/`.cap:hover`（暖橙光晕 hover）/`.cap:focus-within`/`.cap .glyph`/`.closer h2 .lt`（Playfair italic）/所有 chat.* i18n key（8 个）/`.term` 旧 key 早删，整套 v4 11 卡 grid 删光。**Type 升级**：hero h1 font-family Playfair Display → SF Pro Text（**不**用 Playfair，user 反馈 "不惊艳" = 拒 AI 默认衬线 + 字面大小堆）。**承接**：`#173` v4 已被推翻（撞 default 撞太狠 user 退回）→ v4 保留 `--ink-strong` 与 SVG noise + paper grid 纹理，**不算**承接 v4 美学；`#172` v3 配色对齐（暖纸 + terracotta）+ `#171` 删 github + `#170b` caps 11 + `#169` i18n 仍承接。**scope-out**：(a) 不换 brand 色（v3 拍板暖纸保留）；(b) 不引 Playfair / Noto Serif（user 退回 = 拒衬线）；(c) 不抽 CSS helper（landing 单文件 >3 引用才抽象）；(d) 不引 design token via CSS var（landing 独立静态站 = hard-code hex）；(e) 不 commit（user 拍板节奏）；(f) 不改 11 个 capability 文案（v4 已落地翻译，v5 只换排版）。**v5 自审冲突**：user 第一轮反馈"不惊艳" → 第一版 v5 单栏长串节奏 + hero 数字块独占 + closer 太小，**自审第二次自改**（v5b）—— hero 数字锚点改右侧 inline、cap.5 升全宽 + 21:9 视频占位、closer 升 112px。user 第二轮要求"大家用来做的事 双栏" → v5c caps 改 2-col grid，cap.5 仍 `grid-column:1/-1` 全宽 = 双栏节奏里的视觉锚不变。**验证**：Playwright 1440×900 全页截图 `landing-v5-en.png` + `landing-v5-zh.png` 自评：Hero sans 全句大字 + 右侧 inline 数字锚点（无 italic 无 card）/ Caps 2-col 11 段（无 card 无 hover lift）/ cap.5 21:9 黑底视频占位（双栏节奏锚点）/ CTA 单 button + `free installer` 行内 baseline（无 ↓）/ closer 112px sans 收束拍（无 serif）/ i18n 双语 dict 同步（`localStorage.hamuna.lang` 持久化）/ 控制台 0 新增 error（保留 v3/v4 已知的 favicon 404 + R2 CORS 静默 fallback）；删 `.chat-card` + `.cap` 11 卡 + `.term` typewriter 全套死代码清理。**grllling — v5 vs v4 推翻依据**：v4 撞 SaaS card kit + serif italic 复合 default 已 user 反馈"不惊艳"判定；v5 改换结构（card → 大图一句，sans serif 区分，长读节奏 vs 4-col grid）而非加装饰层。**grllling — Playfair 完全拿掉**：v4 hero Playfair Black 900 + closer Playfair italic 双撞 default 1（serif editorial）→ v5 改 sans 800 + 同一 family 整体不混字体（landing 唯一字体家族）。**grllling — "配色大胆"在 v5 怎么落地**：v4 走饱和度+光晕多层（被推翻），v5 走**单一 sans 800 weight + 大字号 + 双栏长读 + 1 个 21:9 全宽视频锚点 + 大字号 closer 收束** = 不用 accent 也能用版式/节奏/字重做"大胆"。**踩坑 — Playwright evaluate `localStorage.setItem` 触发 reload**：v3 试 evaluate 改 localStorage.language 被 auto mode 拦；v4 改用 `page.locator('#lang-toggle').click()` 触发 toggle；v5 还原 evaluate 写法可用（auto mode 升级？或 evaluate setItem 不带 reload 路径放行），但 reload 后必须 `browser_wait_for 1s` 等页面 title 同步。**踩坑 — Bash mv 链失败**：v5 第一轮存 `landing-v5-en.png` 后想重命名为 `landing-v5-en-final.png` 链两次 `mv`，中间态路径被前一步覆盖 → 改用 single-step rename 不再链式。**踩坑 — pkill exit 144**：静默杀 python3 http.server 不返回 0 而非 144 → 不重试，正常进行后续步骤。**追加 — landing 下载地址修正（任务 #175）**：user 反馈 "下载地址不对" → 自查发现 4 个 bug：(a) endpoint `https://download.hamuna.io/update/latest.json` 不存在（NXDOMAIN，整套 `sse_proxy.rs` 测试用的 host 是 dead fixture 不是真地址）；(b) 真 endpoint = `https://pub-2d5b7e0153e94f999bdfea020fb31629.r2.dev/update/windows-x86_64.json`（R2 prod public bucket，user 确认 = prod；`src-tauri/src/updater.rs::get_update_target()` 真返的就是这个 target）；(c) schema 解析 `j.win32_x64` 字段不存在，应解析顶层 `downloads.installer` 或 `downloads.portable`（Tauri updater 读的同 schema）；(d) 硬编 version `0.3.184` 错位（实际最新 = `0.3.185`，HEAD 实测 installer Content-Length = 251445867 bytes ≈ 240 MB，原硬编 ~92 MB 是 v3 老版本尺寸）。**R2 CORS 真相**：R2 prod public bucket `pub-xxx.r2.dev` 不带 `Access-Control-Allow-Origin`，浏览器 fetch runtime probe 永远 `ERR_FAILED`（实测），R2 不开 CORS 是 infra 决策（manifest 公开数据无隐私但 Cloudflare 默认不开避免任意站点跨域）。**user 决策**：硬编 R2 prod URL（不挂 runtime probe），下次发版跑 `./bump_landing_version.sh`（macOS/Linux）或 `./bump_landing_version.ps1`（Windows）自动 bump 三个常量。**bump 脚本设计**：(a) bash + python3（不假设 jq，跨平台）；(b) python heredoc 解析 manifest JSON（避开 bash/awk 模板字符串 quoting 陷阱——之前一版用 awk `sub()` 在 markdown 反引号里错位成 `KNOWN_INSTALLER = \` + url + \`;`，立即 rollback 修）；(c) Cloudflare R2 HEAD 必须设 `User-Agent: hamuna-bump/1.0`，默认 `Python-urllib/3.x` 返 403（实测 403）；(d) 替换 5 处：`KNOWN_VERSION` const + `KNOWN_INSTALLER` const + `KNOWN_SIZE_MB` const + `<span data-i18n="hero.anchor.ver">` + `<span id="dl-ver">` + i18n dict `hero.anchor.ver` × 2；(e) 不 commit、不 push、不写 GitHub release；user 拍板是否挂 publish_release.sh / publish_windows.ps1 钩子。**scope-out**：(a) 不加 runtime probe（user 拍板硬编）；(b) 不引 release script 钩子（publish 流程复杂多变，挂错位置 = 发布坏掉）；(c) 不写 self-test（landing 是单文件，bump 脚本是 shell 一次性工具，跑通即验过）；(d) 不把 R2 base URL 抽到 `src/shared/`（landing 与 app 边界硬隔离 CLAUDE.md 拦）。**验证**：bump script 端到端跑通 fetch manifest → parse → HEAD（UA 已修）→ 251445867 → 240 MB → 检测 "no changes (landing already at v0.3.185, 240 MB)"；HEAD 真实 installer URL 200 / `v0.3.184` 404（user 反馈"地址不对"根因）。**grllling — "sse_proxy.rs 测试用 download.hamuna.io"是 dead fixture**：测试 `request_target_is_loopback("https://download.hamuna.io/update/x.json")` 只是反向断言非 loopback host，host 本身不需要真实可达（NXDOMAIN 也返 false）—— 旧代码误导后续 landing 复制同一个假 host。要做 reachability 集成测试改用真 R2 host。**踩坑 — Python urllib HEAD User-Agent**：bash 跑 python heredoc 默认 UA 被 Cloudflare R2 拒绝，加 `-Headers @{ "User-Agent" = "hamuna-bump/1.0" }` 后通；独立 python 脚本也试过同样 UA，确认是 UA 决定不是网络层。**未验证**：(a) bump_landing_version.ps1 在 Windows 真机跑（沙箱是 macOS，没 ps1 解释器可测，只能对照 sh 端到端逻辑等价）；(b) bump 脚本是否与 bump-on-commit.mjs 的版本 bump 协作（`bump-on-commit.mjs` 改 `package.json` + `src-tauri/Cargo.toml` + `tauri.conf.json`，landing 不在它的 hook 里——两个 bump 解耦，user 跑发布流水线时记得 bump landing）。**§0 narrative 压缩累计**：#168 v1 → v2 cyberpunk → #169 i18n → #170 caps 改场景 → #170b caps 11+ 去技术化 → #171 删 github → #172 v3 配色对齐 → #173 v4 hero 浮卡 + caps 光晕 + SVG noise → #174 v5 杂志感 + 双栏 + sans 替代 Playfair → #175 landing 下载地址修正 + R2 prod 硬编 + bump 脚本 → #176 v5.1 caps 11 段 SVG 内 mini-demo（caret scan / tile highlight / polyline draw / panel alt / video scrub / search input / 5-tile stagger / bullet blink / cron nodes draw / core pulse / breathe，全部 `prefers-reduced-motion: reduce` 降级到首帧静态；IntersectionObserver 仅在 `.in-view` 才 `animation-play-state: running`）10 段全部折叠进本段（详见 git log + `pages/landing/index.html` + `bump_landing_version.sh` + `bump_landing_version.ps1` + `landing-v5-en.png` / `landing-v5-zh.png` / `landing-v5.1-en.png` / `landing-v5.1-cap-frame{3,4}.png` 截图自评）。

§0 last-update 紧接 #172 折叠 — `src/server/nxgd-auth.ts:149` Windows ENOENT bug + 修复（详见 git log `5bae3f1` + commit `e01778f` v0.3.144 + commit `5bae3f1` version bump 0.3.182 → 0.3.183）。

最后更新：**2026-09-18**（任务 #166 wizard step 2 结构化错误路由 + 陈旧数据提示；**user 拍板 A+B**：错误详情结构化 + silent fallback 加陈旧提示）。**result + error class 混用**：stale-cache 不是失败（数据可用，标陈旧）走 `NxgdDiscoveryResult { models, checkedAt? }`；rate-limit 带 retryAfterSeconds + network 未知失败走 throw `NxgdDiscoveryError({ kind:'rate-limit'|'network' })`；caller 兼容 `Array.isArray` 分流（ModelManagementPanel signature `DiscoveredModel[] | NxgdDiscoveryResult`）。**改动 8 文件**：`src/server/nxgd-auth.ts` 新 `getCachedNxgdModelsSnapshotWithCheckedAt()` helper + 删 `getCachedNxgdModelsSnapshot`（无引用）；`src/server/index.ts:693` import 切换 + `/api/nxgd/models` handler 4661-4681 行 429/502 envelope 加 `checkedAt`（让 renderer regex 抽 stale-cache timestamp，不动业务 envelope）；`src/renderer/config/services/nxgdSubscriptionService.ts` 全重写 — `NxgdDiscoveryResult` interface + `NxgdDiscoveryErrorKind` discriminated union + `NxgdDiscoveryError` class + `discoverNxgdModels` 返 `Promise<NxgdDiscoveryResult>` + 429 regex `/retryAfterSeconds["':\s]+(\d+)/` 抽 retryAfterSeconds 抛 rate-limit + 502 regex `/"checkedAt":\s*(\d+)/` 抽 checkedAt 返 stale-cache result；`src/renderer/components/NxgdOnboardingWizard.tsx` 加 `AlertTriangle` + state `candidatesError: string|null → { i18nKey, params? }|null` + `candidatesStale: { ageSeconds }|null` + stale warning banner（无 retry 按钮）+ useEffect 加 `react-hooks/exhaustive-deps` 注释（deps 故意省 `candidates.length/candidatesLoading`，trigger 仅 `expanded/primaryModel/retryCounter`）；`src/renderer/components/ModelManagementPanel.tsx` signature `Promise<DiscoveredModel[] | NxgdDiscoveryResult>` + `Array.isArray` 分流；`src/renderer/i18n/locales/{zh-CN,en-US}/app.json` wizard.step2 删 `errorTitle` 加 `errorRateLimited.{title,body}` + `errorNetwork.{title,body}` + `warningStale.{title,body}`；`src/renderer/config/services/nxgdSubscriptionService.test.ts` NEW 6 case（happy / 429 / 502 stale / 502 parse fail / network / structured Error.message）；`src/renderer/components/NxgdOnboardingWizard.test.tsx` 用 `vi.importActual` 保留真 `NxgdDiscoveryError` class + 9 处 `mockResolvedValue([...]) → { models:[...], checkedAt:undefined }` + 2 新 case（rate-limit 渲染 errorRateLimited.title + retry / stale 渲染 warningStale testid 不渲染 retry）。**grllling — server 502 envelope 加 `checkedAt`**：原 plan 写「checkedAt 怎么传」待 server 现状 grep；实读 `/api/nxgd/models` 502 envelope 后 `checkedAt` 已被剥离 → 必须 server 端加，`getCachedNxgdModelsSnapshotWithCheckedAt()` 透传避免抛错丢 timestamp。**grllling — `Array.isArray` 分流**：discriminated union runtime guard stdlib 内置（无新依赖），比 `[Symbol.toStringTag]` 靠谱。**踩坑 — `vi.mock` 不暴露 `NxgdDiscoveryError`**：初版 mock 完全覆盖导致 `instanceof` 失效 → `vi.importActual` 保留真 class 仅 mock 函数。**#166 scope-out**：不抽 reusable error-state 组件（仅 1 处用 CLAUDE.md 拦）；不动 server `fetchModels` silent fallback 行为（graceful degradation 设计）；不加客户端 UI 限流倒计时（错误条静态文案 + retry 重 fetch 即可，避免撞限流墙）；不 i18n 网络错误原文（保留「Failed to fetch / <原文>」占位避免翻译语义错位）；不改 server error envelope 字段命名（沿用 `models-unavailable` / `rate-limited`，仅补 `checkedAt`）。**验证（全绿）**：`npm run typecheck` 0 errors；`npx vitest run --project unit src/renderer/config/services/nxgdSubscriptionService.test.ts` 6/6；`npx vitest run --project dom src/renderer/components/NxgdOnboardingWizard.test.tsx` 29/29；pre-existing 5 failed tests 不增不减。

---

§0 narrative 之前的 creative-video-suite 11 个 commit 历史（874ad4f / baebe3c / cd6a091 / a79ce1e / dc0bacb / 9207fbd / 4f944b2 / 9207fbd × 2 / darwin-skill v2.1 round 1-6 / agnes-25-mcp v0.1.6）已全部折叠到 §4 git log。**§5.5 输入源铁律按工具拆分** + **§5.6 creative-video-suite promote + SYSTEM_SKILLS_VERSION 39→40** + **§5.7 视频时长边界单源化（双约束 4-12 字符串）** narrative 保留为未来 grep 锚点。**#180 landing polish 7 处最终打磨** — 走 [impeccable/polish] + craft-floor 体检落 7 处：(a) `--dim` #6f6156 → #5d4f44（cream bg 上对比度 4.7:1 → 6.2:1，过 WCAG AA）；(b) `.cap .words` max-width 60ch → 64ch 落 craft-floor 65-75ch 区间低端；(c) IO `rootMargin:0 0 -10% 0` + `threshold:0.15` → `0 0 -5% 0` + `0.05`（屏外提前 5% 触发，scroll 到底部再滚回第一屏 cap 已 in-view 不会卡死）；(d) dl-btn `<button>` → `<a href download>`（button 触发 navigation 是 semantic 不对，anchor + download 属性 = 原生下载语义 + screen reader 念 "link"）；(e) cap.feature ::after focus ring opacity 0.45 → 0.6（更显眼）；(f) `.skip-link:focus { top:16px }` → `top:60px`（跳过 sticky topbar 42px 不被遮）；(g) 560px 下 `.closer p { font-size:16px; max-width:38ch }`（vs 桌面 18px/56ch = 移动端不再 6+ 行溢出）。**Playwright 验证**：1440 桌面 + 375 移动端 + 全图截图 `landing-polish-1440-caps-final.png` / `landing-polish-375-closer.png`；caps.words 569.5px (64ch×8.9px) ✓；dl-btn tagName='A' + href='R2 installer URL' + download attr ✓；cap.feature ::after borderColor rgba(194,109,58,0.6) ✓；scroll 后 2 caps in-view + animation-play-state running ✓；375 viewport closer p 16px / 338px (38ch) / 131.95px ≈ 4 行 ✓。**scope-out**：93 处 SVG 子元素 fill hex 留 polish 不动；closer h2 112px 撞 craft-floor display 6rem 红线但保留（v5 收束节奏是设计意图）；i18n 文本内 `<strong>` dict-inline 改 span.hl 是过度工程（landing 仅 1 处用）保留。**#179 landing audit P1 修复落地** — 走 [impeccable/audit] 体检出 16/20 Good 后落地 4 件事：(a) 加 `<a class="skip-link" href="#caps">Skip to content</a>` + CSS `:focus{top:16px}` 隐藏 slide-down chip（WCAG 2.4.1 Bypass Blocks Level A） + 全局 `a:focus-visible { outline:2px solid var(--accent) }`（WCAG 2.4.7 Level AA）；(b) 包 `<main>` 包 caps/get/closer 三段 + `<div class="label">` × 2 → `<h2 class="label" id="caps-label|get-label">` + `<section aria-labelledby>`（WCAG 1.3.1 Info and Relationships Level A）— heading 层级现在 h1 hero.title → h2 caps-label → h2 get-label → h2 closer 单一有序；(c) SVG 11 段主背景 `fill="#fffcf7"/"#0a0604"/"#1c1612"` × 11 → `fill="var(--bg-elevated)"/var(--ink-strong)`（future dark mode 改 `--bg-elevated` 一个 token 全跟）；(d) responsive adapt：560px 下 cap.5 保留 `aspect-ratio:21/9` + max-width 320px（视觉锚点不丢），dl-btn `flex-wrap:wrap` + `row-gap:4px`（375px 下 `download for windows / direct download · no signup` 两行 wrap 不溢出）。**Playwright 验证**：1440×900 全图 + 375×812 移动端 + 截图 hero/caps/dl-btn/，键盘 Tab 首焦点 = skip-link chip + outline 2px terracotta ✓；main 包 caps/get/closer ✓；caps h2 "things people actually use it for" 13px dim 渲染 ✓；cap.5 frame 320×137px = 21:9 比例保持 ✓；dl-btn 324×83px flex-wrap 后两行（vs 375 viewport 留白 25.5px）✓；console 仅 favicon 404（pre-existing）。**scope-out**：93 处 SVG 子元素 fill（如 `fill="#1c1612"` 的代码行 / `fill="#c26d3a"` 的 accent 文本）保留 hex = 内部子元素 token 化是 polish 范围，不属 P1。**#178 landing clarify 文案体检** — 走 [impeccable/clarify] 体检，按 critical/medium/minor 修 8 处 + HTML fallback 同步 3 处：`meta.desc` 删 macOS/Linux（landing 是 Windows-only，meta 误导 = 搜索跳 PPL），`cap.6.words` 删 "real × 3" 堆砌，`cap.9.words` "it just happens" vague → "It runs on schedule, hands you the result."，`dl.win.sub` "free installer" 与 dl-extra 重复 → "direct download · no signup"（传达差异化事实：零摩擦下载），`dl.extra.intro` 句点切分 → "Free, no account required."，`dl.meta.v.win` en 大写 W 统一，`get.label` "get it" → "download"（与 topbar.get 对齐）。**scope-out**：不动 SVG/CSS/animation、不动 i18n key、不动视觉结构、不动 user 拍板的"自进化 / 投资 / 自媒体"产品定位、不动 hero / caps / CTA 节奏。**#177 landing i18n en 重译自 zh** — 同步 zh 拍板的"自进化 / 投资 / 自动化市场营销 / 自媒体 / 带货视频 / 日常帮手"产品定位：title `"An AI assistant that grows with you."`、cap.1 `Drafts, blogs, essays, weekly reports, work summaries.`、cap.3 `Investing / Turn complex numbers into a clear picture — hand it a thesis and it drafts a strategy, then runs a backtest.`、cap.4 `Marketing / Ad copy, social posts, campaign plans, sales-driving videos.`、cap.5 `Creator channels`、cap.10 `learning / Break it down, quiz you, draw a map of how it all fits.`、cap.11 `The small things add up — and the time comes back.`、closer `Install once — talk to it whenever.`；`meta.title` 沿用 zh 已修改的 `"hamuna — a self-evolving AI assistant"`（不强制改回 — zh 拍板的产品定位）。**scope-out**：不动 SVG/CSS/animation、不动 i18n key、不动 hero / caps / CTA 视觉；仅替换 en 文案值。
...
**image_generate / image_edit / T13 不受约束**：3 类工具无 `seconds` 参数。