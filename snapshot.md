...
最后更新：2026-09-02（**Q&A 续-12：修 stripSchemaDescriptions — desktop app 400 的真根因 + Q&A 续-11：agnes × openai-bridge 真·端到端 SDK loop + 真 MCP 工具执行 PASS**——scripts/agnes-e2e-test.ts 用真 agnes 凭证 + 真 bridge translator（**不是 MIRROR 副本**）驱动 HTTP 请求，三场景 3/3 ✓：(a) chat_completions path（user 当前生产路径）：HTTP 200 + stop_reason=tool_use + tool_use block 1 个；(b) Responses API path（Bug F fix #7/#8 under test）：HTTP 200 + `tools[0].parameters` post-strip 已无 description（`"has description": false` 确认 fix #8 在 wire 上生效）+ stop_reason=tool_use；(d) **chat_completions 多轮 loop closure**：turn 1 HTTP 200 → tool_use → 模拟 SDK 本地执行工具 → 注入 tool_result → turn 2 HTTP 200 → stop_reason=end_turn + 返回 text `{"timezone":"UTC","datetime":"2026-09-02T06:32:11Z"}` —— **loop closed=true**，Claude Agent SDK 整个 loop 链在 agnes 上可跑通。**关键意义**：bridge 翻译层正确 + agnes 真接受 Responses API（含 fix #8）+ tool_call → tool_use → tool_result → text 完整回环 = openai-bridge 可以驱动 Claude Agent SDK 用 agnes 后端。**待 commit**：`scripts/agnes-e2e-test.ts` (new credentialed test, 用真 key 不进默认 CI；env 缺失自抛错；maskKey 永不打日志原文) + `scripts/clean-tauri-dev.sh` (new)。**额外产出**：`scripts/clean-tauri-dev.sh` (cross-project 隔离 via /proc/$pid/cwd + parent chain walking 找 orphan WebKit，4 步清理 T/Z 状态 → SIGTERM 2s grace → orphan WebKit → ports；idempotent 无 `set -e`，避免 Bash tool `nohup &` 模式下的 exit 144)，补了 `npm exec tauri` pattern 后成功杀掉 1 个 stuck 进程。**结论**：openai-bridge + agnes 后端已可生产驱动 Claude Agent SDK。下一步可：(1) 拍板 commit openai-bridge 5 文件 M + 2 文件 A + scripts/agnes-e2e-test.ts + scripts/clean-tauri-dev.sh 这一批；(2) 移除 handler.ts 暂留的 DIAG block（user 验证 fix #8 生效后）；(3) 把 e2e test 进 `npm run test:credentialed` 池（CLAUDE.md 已声明该池不进默认 CI）。
