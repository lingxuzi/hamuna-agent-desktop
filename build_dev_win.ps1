...
Write-ColorOutput "║  🤖 HamunaAgent Windows Dev 构建                         ║" "Cyan"
...
# server-dist.js / plugin-bridge-dist.mjs / cli/hamuna.js 在下面的
...
& npm run build:cli
if ($LASTEXITCODE -ne 0) {
    Write-ColorOutput "✗ hamuna CLI 打包失败" "Red"
    exit 1
}
...
$oldExe = Join-Path $PROJECT_DIR "src-tauri/target/x86_64-pc-windows-msvc/debug/hamuna.exe"
if (Test-Path $oldExe) {
...
$fastConfig = Join-Path $env:TEMP "hamuna-tauri-dev-fast-$PID.json"
$fastConfigJson = @'
{
  "build": {
...
$EXE_PATH = Join-Path $DEBUG_DIR "hamuna.exe"
