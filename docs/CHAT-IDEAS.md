...
- ~~**Discuss launch 丢 initialMessage**：`buildLaunchFields` 只读 `initialMessage`，discuss ctx 的 `content` 没变成消息文本，chat 空开。~~ **已修复** `39ac257`：`content` → `{text}`（tags 保持 thought metadata 不传给 AI），有单测覆盖（`useChatLaunchV2.unit.test.ts`「drops tags」）。
