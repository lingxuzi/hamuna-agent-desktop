...
|----------|----------|-------|------------|-------|
| Chat | WUM12 | `[final] Chat — B: 三栏工作台` | Out of sync | v2 Chat 页面（2026-08-05 晋升）。2026-08-06 落地 `ecc4101`：三栏（左 FileTree+AgentDock / 中 ChatColumn+SplitView / 右 AgentStatusPanel）已按稿渲染，编排复用 v1 Chat 全部能力。SplitViewPanel 用 v1 flow layout 而非稿中 absolute overlay（ratio math 依赖 parentElement width，见 CHAT-IDEAS caveat）；AgentStatusPanel 常驻右栏 + 左栏 AgentDock 紧凑摘要（保留懒加载语义）。未实现：ChatSearchBar 常驻搜索、FileTree 顶部 workspace 切换的 Pencil 细节 |
| Chat | wjhlp | `[deprecated] Chat — B: 三栏工作台（早期版）` | — | 被 fixed 版 WUM12 取代 |
| Chat | Af2bL | `[deprecated] Chat — A: 沉浸式全宽` | — | 落选变体 |
| Launcher | qjQtx | `[final] Launcher — A: 品牌驱动（杂志封面）` | Out of sync | v2 Launcher 页面（2026-08-05 晋升）；2026-08-05 QuickInput 已与代码 1:1（Plus/权限/工具/定时/模型/发送 工具栏 + 工作区/运行时 chips；删假 ⌘K；SendArrow=lucide Send）；整页 v2 布局尚未实现 |
| Launcher | gqlNJ | `[deprecated] Launcher — B: 效率驱动（工作台）` | — | 落选变体；任务直出功能落地时参考 |
| Sidebar/TabBar | u9fa6 | `[final] Sidebar/TabBar — A: 顶部融合（Chrome 风格）` | Synced | v2 全局 chrome 骨架（2026-08-05 晋升）。2026-08-06 落地 `747021e`：`src/render_v2/chrome/` 38px 融合条（LogoMark → NavCluster → TabsGroup → WinControls），token-clean，与设计一致 |
| Sidebar/TabBar | zPYXY | `[deprecated] Sidebar/TabBar — B: 侧边导航（Activity Bar）` | — | 落选变体；Activity Bar 全局导航入口落地时参考 |
| Settings | cTjk2 | `[final] Settings — A: 经典设置（侧边导航）` | Out of sync | v2 Settings 页面（2026-08-05 晋升）；2026-08-05 按整体风格重设计全部 10 页、功能与 SettingsPage 1:1：删 9 个 Hero 改衬线页头（Playfair Display 标题+描述+图标+CTA）、NavSidebar 重构为 4 组导航（模型/连接/数据/通用 + lucide 图标 + per-page `$accent` 活动项高亮）。内容区已按 Bento 方向（t2DE7F）落地：general 主页 BSMXd 替换为 PanelHead + BentoGrid（外观/启动/队列/工作区 4 tile，`$paper-elevated` r18）。10 页：general 主页 + providers(JW6Ha)/工具箱 mcp(KB8c4)/plugins(jz3RU)/skills 含 sub-agents(ZWkb0)/usage-stats(G0RkW)/agent(e0tbs5)/desktop-pet(p4mBrt)/shortcuts(g9b6R)/about(uHhSE)；功能与代码 1:1，视觉超前代码（代码 SettingsSidebar 仍平铺、未实现分组/衬线页头/Bento）。2026-08-05 用户反馈三页卡片大小不一，providers/mcp/plugins 内容区由不等宽 Bento 改为 2×2 等宽规则 grid（584×584，gap 16，spacer 改 `fill_container` 自适应），其余 7 页保持不等宽瓦片。2026-08-05 重建：usage-stats/agent/desktop-pet 三帧（原 dbBZu/pJ97d/w5MeKI）丢失后按代码 1:1 重建为 G0RkW/e0tbs5/p4mBrt，截图验证 `design/qa/e0tbs5.png` + `design/qa/p4mBrt.png` 渲染正确 |
...
| Workspace Config | HivP2 | `[final] Workspace Config — A: 文档编辑器风格（Notion 式）` | Synced | v2 Workspace Config 页面（2026-08-05 晋升）；已按 WorkspaceConfigPanel 1:1（header 3 区 + 通用/系统提示词/简介/技能 tabs + IconBox sliders + 通用 tab 两卡片 + footer hint「按 Esc 关闭 · 配置修改会立即生效」） |
