...
| Force Update Modal — A: Default State | `TUbMk` (lives inside `UIaE6` proposal group) | `[final]` | Synced | 1152×720. Centered 448px-wide modal panel (paper-elevated, cornerRadius 16, line border). Header: 需要更新 (18pt ink). Body: description text (v0.4.0 example, ink-muted 14pt). Footer: pt-3 border-top, justify-end gap-8; secondary "退出 App" button (paper-elevated + line border) + primary "立即更新" button (accent green / on-accent text). Matches `src/renderer/components/ForceUpdateModal/ForceUpdateModal.tsx` Default rendering. |
| Force Update Modal — B: With Release Notes | `M26kkH` (lives inside `UIaE6` proposal group) | `[final]` | Synced | Same as Default + a "更新说明" 12pt label block and a 128px-tall paper-inset notes box (cornerRadius 6) showing multi-line changelog. Rendered when `updateNotes` prop is non-null. |
| Force Update Modal — C: Installing | `rEOSh` (lives inside `UIaE6` proposal group) | `[final]` | Synced | Same as Default but body has an extra row (loader2 icon + "正在安装..." muted text) and footer buttons are 50% opacity with disabled state; primary button label switches to "正在安装...". Rendered when `updating === true`. |
